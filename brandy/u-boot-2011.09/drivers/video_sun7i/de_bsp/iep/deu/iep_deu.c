...
__s32 deu_clk_init (__u32 sel)
{
  ...
  if (!sel) {
    ...
    OSAL_CCMU_SetMclkSrc (h_deumclk0, CLK_FE_SRC);
    ...
    OSAL_CCMU_SetMclkSrc (h_deumclk1, CLK_FE_SRC);
...
__s32 DEU_ALG (__u32 sel)
{
  ...
  */
  ...
  if ( (frameinfo.trd_out_mode == DISP_3D_OUT_MODE_CI_1  || frameinfo.trd_out_mode ==   DISP_3D_OUT_MODE_CI_2  ||
    ...
    if ( ( ( frameinfo.trd_out_mode == DISP_3D_OUT_MODE_FA ) &&
  ...
  scalefact = (frameinfo.out_size.width << 2) / frameinfo.in_size.width;
  
  if (scalefact < 5) {
    filtertype = 0;
  }
  else
    if (scalefact >= 5 && scalefact < 7) {
      filtertype = 1;
    }
    else
      if (scalefact >= 7 && scalefact < 9) {
        filtertype = 2;
      }
      else
        if (scalefact >= 9 && scalefact < 11) {
          filtertype = 3;
        }
        else
          if (scalefact >= 11) {
...
__s32 IEP_Deu_Early_Suspend (__u32 sel);

__s32 iep_deu_suspend (__u32 sel)
{
  __u32 i, reg_val;
  #if defined(__LINUX_OSAL__)
...
__s32 iep_deu_resume (__u32 sel)
{
  __u32 i;
  __u32 reg_val;
...
__s32 IEP_Deu_Late_Resume (__u32 sel);
