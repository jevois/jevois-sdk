  ...
  for (i = 0; i < gdisp.screen[screen_id].max_layers; i++) {
    ...
    for (j = 0; j < gdisp.screen[screen_id].max_layers; j++) {
      ...
      /* no layer use this prio */
        ...
        if (j == gdisp.screen[screen_id].max_layers - 1) {
          return i;
        }
    }
  }
...
__s32 img_sw_para_to_reg (__u8 type, __u8 mode, __u8 value)
{
  ...
  if (type == 0) {
    if (mode == DISP_MOD_NON_MB_PLANAR && value == DISP_FORMAT_YUV411) {
      ...
      if (mode == DISP_MOD_NON_MB_PLANAR && value == DISP_FORMAT_YUV422) {
        return 1;
      }
      else
        if (mode == DISP_MOD_NON_MB_PLANAR && value == DISP_FORMAT_YUV444) {
          return 2;
        }
        else
          if (mode == DISP_MOD_INTERLEAVED && value == DISP_FORMAT_YUV422) {
            return 3;
          }
          else
            if (mode == DISP_MOD_INTERLEAVED && value == DISP_FORMAT_YUV444) {
              return 4;
            }
            else {
  ...
  /* yuv channel pixel sequence */
    ...
    if (type == 1) {
      if (mode == DISP_MOD_NON_MB_PLANAR && value == DISP_SEQ_P3210) {
        ...
        if (mode == DISP_MOD_NON_MB_PLANAR && value == DISP_SEQ_P0123) {
          return 1;
        }
        else
          if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_UYVY) {
            return 0;
          }
          else
            if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_YUYV) {
              return 1;
            }
            else
              if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_VYUY) {
                return 2;
              }
              else
                if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_YVYU) {
                  return 3;
                }
                else
                  if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_AYUV) {
                    return 0;
                  }
                  else
                    if (mode == DISP_MOD_INTERLEAVED && value == DISP_SEQ_VUYA) {
                      return 1;
                    }
                    else {
    ...
    /* image0 pixel sequence */
      ...
      if (type == 3) {
        if (value == DISP_SEQ_ARGB) {
          ...
          if (value == DISP_SEQ_BGRA) {
            return 2;
          }
          else
            if (value == DISP_SEQ_P10) {
              return 0;
            }
            else
              if (value == DISP_SEQ_P01) {
                return 1;
              }
              else
                if (value == DISP_SEQ_P3210) {
                  return 0;
                }
                else
                  if (value == DISP_SEQ_P0123) {
                    return 1;
                  }
                  else
                    if (value == DISP_SEQ_P76543210) {
                      return 0;
                    }
                    else
                      if (value == DISP_SEQ_P67452301) {
                        return 1;
                      }
                      else
                        if (value == DISP_SEQ_P10325476) {
                          return 2;
                        }
                        else
                          if (value == DISP_SEQ_P01234567) {
                            return 3;
                          }
                          else
                            if (value == DISP_SEQ_2BPP_BIG_BIG) {
                              return 0;
                            }
                            else
                              if (value == DISP_SEQ_2BPP_BIG_LITTER) {
                                return 1;
                              }
                              else
                                if (value == DISP_SEQ_2BPP_LITTER_BIG) {
                                  return 2;
                                }
                                else
                                  if (value == DISP_SEQ_2BPP_LITTER_LITTER) {
                                    return 3;
                                  }
                                  else
                                    if (value == DISP_SEQ_1BPP_BIG_BIG) {
                                      return 0;
                                    }
                                    else
                                      if (value == DISP_SEQ_1BPP_BIG_LITTER) {
                                        return 1;
                                      }
                                      else
                                        if (value == DISP_SEQ_1BPP_LITTER_BIG) {
                                          return 2;
                                        }
                                        else
                                          if (value == DISP_SEQ_1BPP_LITTER_LITTER) {
                                            return 3;
                                          }
                                          else  {
...
__s32 bsp_disp_layer_set_framebuffer (__u32 screen_id, __u32 hid, __disp_fb_t * pfb)
{
  __s32           ret;
...
__s32 bsp_disp_layer_set_src_window (__u32 screen_id, __u32 hid, __disp_rect_t * regn)
{
  __u32           cpu_sr;
  __layer_man_t * layer_man;
...
__s32 bsp_disp_layer_get_para (__u32 screen_id, __u32 hid, __disp_layer_info_t * player)
{
  __layer_man_t * layer_man;
