                                   ...
                                   struct device_attribute * attr, char * buf)
{
  ...
  { ret = sprintf (buf, "%d\n", ubi->leb_size); }
    ...
    if (attr == &dev_avail_eraseblocks)
    { ret = sprintf (buf, "%d\n", ubi->avail_pebs); }
    else
      if (attr == &dev_total_eraseblocks)
      { ret = sprintf (buf, "%d\n", ubi->good_peb_count); }
      else
        if (attr == &dev_volumes_count)
        { ret = sprintf (buf, "%d\n", ubi->vol_count - UBI_INT_VOL_COUNT); }
        else
          if (attr == &dev_max_ec)
          { ret = sprintf (buf, "%d\n", ubi->max_ec); }
          else
            if (attr == &dev_reserved_for_bad)
            { ret = sprintf (buf, "%d\n", ubi->beb_rsvd_pebs); }
            else
              if (attr == &dev_bad_peb_count)
              { ret = sprintf (buf, "%d\n", ubi->bad_peb_count); }
              else
                if (attr == &dev_max_vol_count)
                { ret = sprintf (buf, "%d\n", ubi->vtbl_slots); }
                else
                  if (attr == &dev_min_io_size)
                  { ret = sprintf (buf, "%d\n", ubi->min_io_size); }
                  else
                    if (attr == &dev_bgt_enabled)
                    { ret = sprintf (buf, "%d\n", ubi->thread_enabled); }
                    else
                      if (attr == &dev_mtd_num)
                      { ret = sprintf (buf, "%d\n", ubi->mtd->index); }
