...
int axp_probe_startup_cause (void)
{
  ...
  if (buffer_value == PMU_PRE_SYS_MODE) //表示前一次是在系统状态，下一次应该也进入系统
  {
    ...
    if (buffer_value == PMU_PRE_BOOT_MODE) //表示前一次是在boot standby状态，下一次也应该进入boot standby
    {
      ...
      if (status == AXP_VBUS_EXIST)
      {
        ...
        if (status == AXP_DCIN_EXIST)
        {
          return AXP_DCIN_EXIST;
        }
      return AXP_VBUS_DCIN_NOT_EXIST; 
    }
    else
      if (buffer_value == PMU_PRE_FASTBOOT_MODE)
      {
...
int axp_probe_factory_mode (void)
{
  ...
  if (buffer_value == PMU_PRE_FACTORY_MODE)
  {
