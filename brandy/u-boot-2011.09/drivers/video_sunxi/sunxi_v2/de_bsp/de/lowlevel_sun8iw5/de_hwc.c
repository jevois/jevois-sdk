...
__s32 DE_BE_HWC_Set_Src (__u32 sel, de_hwc_src_t * hwc_pat)
{
  ...
  DE_BE_WUINT32 (sel, DE_BE_HWC_FRMBUF_OFF, (tmp & 0xffffffc3) | (x_size << 2) | (y_size << 4) );
  
  tmp = DE_BE_RUINT32 (sel, DE_BE_HWC_FRMBUF_OFF);
  DE_BE_WUINT32 (sel, DE_BE_HWC_FRMBUF_OFF, (tmp & 0xfffffffc) | pixel_fmt);
  
  tmp = DE_BE_RUINT32 (sel, DE_BE_HWC_CRD_CTL_OFF);
  DE_BE_WUINT32 (sel, DE_BE_HWC_CRD_CTL_OFF, (tmp & 0x07ff07ff ) | 0 << 27 | 0 << 11);
  return 0;
}
