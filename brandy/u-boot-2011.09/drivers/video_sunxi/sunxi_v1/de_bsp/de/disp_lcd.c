...
void Lcd_Panel_Parameter_Check (__u32 screen_id)
{
  ...
  { cycle_num = 3; }
    ...
    if (info->lcd_if == 0 && info->lcd_hv_if == 1 && info->lcd_hv_smode == 1)
    { cycle_num = 2; }
    else
      if (info->lcd_if == 1 && info->lcd_cpu_if == 1)
      { cycle_num = 3; }
      else
        if (info->lcd_if == 1 && info->lcd_cpu_if == 2)
        { cycle_num = 2; }
        else
          if (info->lcd_if == 1 && info->lcd_cpu_if == 3)
          { cycle_num = 2; }
          else
            if (info->lcd_if == 1 && info->lcd_cpu_if == 5)
            { cycle_num = 2; }
            else
              if (info->lcd_if == 1 && info->lcd_cpu_if == 6)
              { cycle_num = 3; }
              else
                if (info->lcd_if == 1 && info->lcd_cpu_if == 7)
                { cycle_num = 2; }
                else
                { cycle_num = 1; }
  ...
  if (lcd_clk_div >= 6)
  {
    ...
    if ( (lcd_clk_div == 5) || (lcd_clk_div == 4) || (lcd_clk_div == 2) )
    {
  ...
  if (Lcd_Panel_Err_Flag != 0 || Lcd_Panel_Wrn_Flag != 0)
  {
    ...
    {OSAL_PRINTF ("*** Parallel HV Panel\n");}
      ...
      {OSAL_PRINTF ("*** Serial HV Panel\n");}
        ...
        if (info->lcd_if == 0 && info->lcd_hv_if == 2)
        {OSAL_PRINTF ("*** Serial YUV Panel\n");}
        else
          if (info->lcd_if == 3 && info->lcd_lvds_bitwidth == 0)
          {OSAL_PRINTF ("*** 24Bit LVDS Panel\n");}
          else
            if (info->lcd_if == 3 && info->lcd_lvds_bitwidth == 1)
...
__s32 bsp_disp_lcd_delay_us (__u32 us)
{
  ...
  for (time = 0; time < (us * 700 / 10); time++);
  #endif
  #ifdef __UBOOT_OSAL__
  __usdelay (us);
  #endif
...
__s32 pwm_set_para (__u32 channel, __pwm_info_t * pwm_info)
{
  ...
  tmp = ( (0 << 7) | (1 << 6) | (pwm_info->active_state << 5) | pre_scal_id);
  pwm_write_reg (channel * 0x10, tmp);
...
__s32 LCD_POWER_EN_EXT (__u32 sel, __bool b_en)
{
  ...
  if (1) {
    ...
    /* anx6345 */
      ...
      if ( (gpanel_info[sel].lcd_if == LCD_IF_EDP) && (gpanel_info[sel].lcd_edp_tx_ic == 1) )
      {
        if (b_en) {
  ...
  if (gdisp.screen[screen_id].lcd_cfg.lcd_used) {
    ...
    if (gdisp.screen[screen_id].lcd_cfg.lcd_pwm_used == 1) {
      ...
      else
      {
        ...
        pwm_info.period_ns = 1000000000 / 1000; 
      }
...
__s32 bsp_disp_get_frame_rate (__u32 screen_id)
{
  ...
  if (gdisp.screen[screen_id].output_type & DISP_OUTPUT_TYPE_LCD) {
    ...
    if (gdisp.screen[screen_id].output_type & DISP_OUTPUT_TYPE_TV) {
      ...
      if (gdisp.screen[screen_id].output_type & DISP_OUTPUT_TYPE_HDMI) {
        ...
        if (gdisp.screen[screen_id].output_type & DISP_OUTPUT_TYPE_VGA) {
          frame_rate = 60;
        }
