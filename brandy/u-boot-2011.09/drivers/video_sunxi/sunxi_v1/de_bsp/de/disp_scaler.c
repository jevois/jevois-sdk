...
__s32  Scaler_sw_para_to_reg (__u8 type, __u8 mode, __u8 format, __u8 seq)
{
  ...
  if (type == 0) {
    if (format == DISP_FORMAT_YUV444) {
      ...
      if (format == DISP_FORMAT_YUV420) {
        return DE_SCAL_INYUV420;
      }
      else
        if (format == DISP_FORMAT_YUV422) {
          return DE_SCAL_INYUV422;
        }
        else
          if (format == DISP_FORMAT_YUV411) {
            return DE_SCAL_INYUV411;
          }
          else
            if (format == DISP_FORMAT_CSIRGB) {
              ...
              if (format == DISP_FORMAT_ARGB8888) {
                ...
                if (format == DISP_FORMAT_RGB888) {
                  return DE_SCAL_INRGB888;
                }
                else
                  if (format == DISP_FORMAT_RGB565) {
                    return DE_SCAL_INRGB565;
                  }
                  else
                    if (format == DISP_FORMAT_ARGB4444) {
                      return DE_SCAL_INRGB4444;
                    }
                    else
                      if (format == DISP_FORMAT_ARGB1555) {
      ...
      if (mode == DISP_MOD_INTERLEAVED) {
        ...
        if (mode == DISP_MOD_MB_PLANAR) {
          ...
          if (mode == DISP_MOD_NON_MB_PLANAR) {
            ...
            if (mode == DISP_MOD_NON_MB_UV_COMBINED) {
              return DE_SCAL_UVCOMBINED;
            }
            else
              if (mode == DISP_MOD_MB_UV_COMBINED) {
    ...
    /* scaler input pixel sequence */
      ...
      if (type == 2) {
        if (seq == DISP_SEQ_UYVY) {
          ...
          if (seq == DISP_SEQ_YUYV) {
            return DE_SCAL_YUYV;
          }
          else
            if (seq == DISP_SEQ_VYUY) {
              return DE_SCAL_VYUY;
            }
            else
              if (seq == DISP_SEQ_YVYU) {
                return DE_SCAL_YVYU;
              }
              else
                if (seq == DISP_SEQ_AYUV) {
                  return DE_SCAL_AYUV;
                }
                else
                  if (seq == DISP_SEQ_UVUV) {
                    return DE_SCAL_UVUV;
                  }
                  else
                    if (seq == DISP_SEQ_VUVU) {
                      return DE_SCAL_VUVU;
                    }
                    else
                      if (seq == DISP_SEQ_ARGB) {
                        if (format == DISP_FORMAT_ARGB8888) {
                          return DE_SCAL_ARGB;
                        }
                        else
...
__s32 Scaler_Set_Framebuffer (__u32 scaler_index, __disp_fb_t * pfb)
{
...
__s32 Scaler_Set_Para (__u32 scaler_index, __disp_scaler_t * scl)
{
  ...
  if (get_fb_type (scaler->out_fb.format) == DISP_FB_TYPE_YUV) {
    ...
    if (scaler->out_fb.mode == DISP_MOD_NON_MB_PLANAR
      ...
      if (scaler->out_fb.mode == DISP_MOD_INTERLEAVED
          && scaler->out_fb.format == DISP_FORMAT_ARGB8888 && scaler->out_fb.seq == DISP_SEQ_ARGB) {
        out_type.fmt = DE_SCAL_OUTI1RGB888;
      }
      else
        if (scaler->out_fb.mode == DISP_MOD_INTERLEAVED && scaler->out_fb.format == DISP_FORMAT_ARGB8888
...
__s32 bsp_disp_scaler_start_ex (__u32 handle, __disp_scaler_para_t * para)
{
  ...
  if (get_fb_type (para->output_fb.format) == DISP_FB_TYPE_YUV) {
    ...
    if (para->output_fb.mode == DISP_MOD_NON_MB_PLANAR
      ...
      if (para->output_fb.mode == DISP_MOD_INTERLEAVED && para->output_fb.format == DISP_FORMAT_ARGB8888) {
        out_type.fmt = DE_SCAL_OUTI0RGB888;
      }
      else {
