  ...
  case DISP_TV_MOD_PAL_NC_SVIDEO:
    {
      for (i = 0; i < 4; i++) {
        if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_LUMA) {
          ...
          if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_CHROMA) {
  ...
  case DISP_TV_MOD_1080P_60HZ:
    {
      for (i = 0; i < 4; i++) {
        if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_Y) {
          ...
          if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_PB) {
            TVE_dac_set_source (screen_id, i, DISP_TV_DAC_SRC_PB);
            TVE_dac_enable (screen_id, i);
            TVE_dac_sel (screen_id, i, i);
          }
          else
            if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_PR) {
...
__s32 bsp_disp_tv_get_interface (__u32 screen_id)
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_COMPOSITE && dac[i] == 1) {
      ...
      if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_Y && dac[i] == 1) {
        ret |= DISP_TV_YPBPR;
      }
      else
        if (gdisp.screen[screen_id].dac_source[i] == DISP_TV_DAC_SRC_LUMA && dac[i] == 1) {
          ret |= DISP_TV_SVIDEO;
        }
  }
