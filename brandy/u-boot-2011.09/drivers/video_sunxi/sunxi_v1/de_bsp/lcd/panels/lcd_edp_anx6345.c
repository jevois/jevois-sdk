...
void anx6345_init (__panel_para_t * info)
{
  ...
  if (info->lcd_edp_tx_rate == 1) {
    ...
    if (info->lcd_edp_tx_rate == 2) {
      data_rate = 0x0a;//2.7G
    }
    
  colordepth = (info->lcd_edp_colordepth == 1) ? 0x00 : 0x10;
  ...
  SP_TX_Write_Reg (SP_TX_PORT0_ADDR, SP_TX_HDCP_CTRL, 0x00);
  SP_TX_Write_Reg (SP_TX_PORT0_ADDR, SP_TX_LINK_DEBUG_REG, 0x30);
  
  SP_TX_Write_Reg (SP_TX_PORT0_ADDR, SP_TX_DEBUG_REG1, 0x00);
  ...
  SP_TX_Read_Reg (SP_TX_PORT0_ADDR, SP_TX_AUX_CTRL_REG2, &c);
  SP_TX_Write_Reg (SP_TX_PORT0_ADDR, SP_TX_AUX_CTRL_REG2, c | 0x08);
  
  SP_TX_Write_Reg (SP_TX_PORT2_ADDR, SP_COMMON_INT_MASK1, 0xbf);
  SP_TX_Write_Reg (SP_TX_PORT2_ADDR, SP_COMMON_INT_MASK2, 0xff);
  SP_TX_Write_Reg (SP_TX_PORT2_ADDR, SP_COMMON_INT_MASK3, 0xff);
  SP_TX_Write_Reg (SP_TX_PORT2_ADDR, SP_COMMON_INT_MASK4, 0xff);
  ...
  */
  ...
  SP_TX_Write_Reg (0x70, SP_TX_PLL_CTRL_REG, 0x00);        
  ...
  /* enable ssc function */
  SP_TX_Write_Reg (0x70, 0xA7, 0x00);            
  SP_TX_Write_Reg (0x70, 0xD0, 0x5f);            
  SP_TX_Write_Reg (0x70, 0xD1, 0x00);
  SP_TX_Write_Reg (0x70, 0xD2, 0x75);            
  SP_TX_Read_Reg (0x70, 0xA7, &c);
  SP_TX_Write_Reg (0x70, 0xA7, c | 0x10);        
  SP_TX_Read_Reg (0x72, 0x07, &c);               
  SP_TX_Write_Reg (0x72, 0x07, c | 0x80);
  SP_TX_Write_Reg (0x72, 0x07, c & (~0x80) );
  
  SP_TX_Write_Reg (0x70, SP_TX_LINK_BW_SET_REG, data_rate);  
  ...
  */
  ...
  SP_TX_Write_Reg (0x72, 0x08, 0x83);  
