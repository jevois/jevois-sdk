...
void ipu_dmfc_init (int dmfc_type, int first)
{
  ...
  if (dmfc_type == DMFC_HIGH_RESOLUTION_DC) {
    ...
    if (dmfc_type == DMFC_HIGH_RESOLUTION_DP) {
      /* 1 - segment 0, 1;
       * 5B - segement 2~5;
       * 5F - segement 6,7;
       * 1C, 2C and 6B, 6F unused;
      ...
      if (dmfc_type == DMFC_HIGH_RESOLUTION_ONLY_DP) {
        /* 5B - segement 0~3;
         * 5F - segement 4~7;
    ...
    if (channel == MEM_BG_SYNC) {
      ...
      if (channel == MEM_FG_SYNC) {
        ...
        if (g_dc_di_assignment[dc_chan] == 0)
          ...
          if (g_dc_di_assignment[dc_chan] == 1)
                                   ...
                                   uint8_t alpha)
{
  ...
  { flow = DP_SYNC; }
    ...
    if (channel == MEM_BG_ASYNC0 || channel == MEM_FG_ASYNC0)
    { flow = DP_ASYNC0; }
    else
      if (channel == MEM_BG_ASYNC1 || channel == MEM_FG_ASYNC1)
      { flow = DP_ASYNC1; }
      else
      { return -EINVAL; }
