...
static void flash_get_offsets (ulong base, flash_info_t * info)
{
  ...
  if ( ( (info->flash_id & FLASH_VENDMASK) == FLASH_MAN_SST) ||
    ...
    if ( (info->flash_id & FLASH_TYPEMASK) == FLASH_AMLV128U) {
      for (i = 0; i < info->sector_count; i++) {
        info->start[i] = base + (i * 0x00010000 * 2);
      }
    }
    else
      if ( (info->flash_id & FLASH_TYPEMASK) == FLASH_S29GL128N ) {
        for (i = 0; i < info->sector_count; i++) {
...
static ulong flash_get_size_2 (vu_long * addr, flash_info_t * info)
{
  ...
  case (CONFIG_SYS_FLASH_CHAR_SIZE) AMD_ID_MIRROR:
    if ( (CONFIG_SYS_FLASH_CHAR_SIZE) addr2[0x1c] == (CONFIG_SYS_FLASH_CHAR_SIZE) AMD_ID_LV128U_2
      ...
      if ( (CONFIG_SYS_FLASH_CHAR_SIZE) addr2[0x1c] == (CONFIG_SYS_FLASH_CHAR_SIZE) AMD_ID_GL128N_2
