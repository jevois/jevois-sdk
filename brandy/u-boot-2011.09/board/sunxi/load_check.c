        ...
        for (i = this_bat_cal / (100 / (SUNXI_BAT_BMP_MAX - 1) ); i < SUNXI_BAT_BMP_MAX; i++)
        {
          ...
          if (boot_standby_action & 0x08)   //存在外部电源
          {
            ...
            if (boot_standby_action & 0x02) //短按
            {
              ...
              if (boot_standby_action & 0x01) //长按
              {
                ...
                if (boot_standby_action & 0x10) //拔掉外部电源，没有外部电源
                {
                  status = 10;
  ...
  if (!standby_mode)
  {
    ...
    if (ret < 0)
    {
      ...
      if (!ret)
      {
        ...
        if (ret == 2)          //按键则显示低电图标然后关机，插入火牛则待机
        {
          start_condition = 1;
        }
    ...
    if (ret <= 0)
    {
      ...
      if (ret == AXP_VBUS_DCIN_NOT_EXIST) //当前一次为boot standby状态，但是启动时检查无外部电源，直接关机
      {
        do_shutdown (NULL, 0, 1, NULL);
      }
  }
