...
unsigned int DRAMC_init_auto (boot_dram_para_t * para)
{
  ...
  if ( (para->dram_tpr13 & (0x3 << 3) ) == 0x0)
  {
    if ( (para->dram_tpr13 & (0x1 << 2) ) == 1)
    {
      bus_lock = 1;
...
unsigned int mctl_sys_init (boot_dram_para_t * dram_para)
{
  ...
  reg_val |= ( (0x1 << 0) | (0x1 << 4) );
  reg_val |= ( (dram_para->dram_clk / 24 - 1) << 0x8);
  mctl_write_w (CCM_PLL5_DDR_CTRL, reg_val);
...
unsigned int mctl_channel_init (unsigned int ch_index, boot_dram_para_t * para)
{
  ...
  if ( (para->dram_para2 & (0xF << 12) ) == (0x2 << 12) )
  {
    reg_val = 0x88000000 | (0x1 << 20);
    mctl_write_w (ch_id + SDR_MCMD, reg_val);
  ...
  if (!hold_flag)
  {
    while ( (mctl_read_w (ch_id + SDR_PGSR) & 0x1F) != 0x1F) {};
  }
  else
  {
    standby_serial_putc ('a');
    while ( (mctl_read_w (ch_id + SDR_PGSR) & 0x1F) != 0xB) {};
...
unsigned int mctl_com_init (boot_dram_para_t * para)
{
  ...
  */
  ...
  { reg_val |= 0xa << 8; }
    ...
    if ( ( (para->dram_para1 >> 16) & 0xF) == 4)
    { reg_val |= 0x9 << 8; }
    else
      if ( ( (para->dram_para1 >> 16) & 0xF) == 2)
      { reg_val |= 0x8 << 8; }
      else
        if ( ( (para->dram_para1 >> 16) & 0xF) == 1)
        { reg_val |= 0x7 << 8; }
        else
        { reg_val |= 0x6 << 8; }
