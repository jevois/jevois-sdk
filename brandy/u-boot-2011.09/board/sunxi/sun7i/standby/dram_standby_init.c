...
* 2014-07-22    Liuke   1.2   modify code for 2 ranks support
*********************************************************************************************************
*/
...
__s32 DRAMC_init (boot_dram_para_t * para)
{
  ...
  { reg_val |= 0x0 << 3; }
    ...
    if (para->dram_chip_density == 512)
    { reg_val |= 0x1 << 3; }
    else
      if (para->dram_chip_density == 1024)
      { reg_val |= 0x2 << 3; }
      else
        if (para->dram_chip_density == 2048)
        { reg_val |= 0x3 << 3; }
        else
          if (para->dram_chip_density == 4096)
          { reg_val |= 0x4 << 3; }
          else
            if (para->dram_chip_density == 8192)
            { reg_val |= 0x5 << 3; }
            else
            { reg_val |= 0x0 << 3; }
  ...
  if (para->dram_odt_en)
  {
    reg_val = (0x3u << 30) | (0xcc << 16) | (0x3 << 0);
    reg_val &= ~ ( (para->dram_tpr5 & 0x3) << 30 | (para->dram_tpr5 & 0x3) );
