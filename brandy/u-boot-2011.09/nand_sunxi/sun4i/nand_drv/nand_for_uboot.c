...
int NAND_VersionCheck (void)
{
  ...
  ********************************************************************************/
  nand_version[0] = 0xff;    
  nand_version[1] = 0x00;    
  nand_version[2] = NAND_VERSION_0;    
  nand_version[3] = NAND_VERSION_1;    
  ...
  if (info.chip_cnt == 0x1)      
  {
    ...
    if (info.chip_cnt == 2)   
    {
      if (nand_param->ChipConnectInfo == 0x3)
      {
        ...
        if (nand_param->ChipConnectInfo == 0x5)
        {
          bad_block_num = (bad_block_cnt[0] + bad_block_cnt[2]) << 1;
        }
        else
          if (nand_param->ChipConnectInfo == 0x81)
          {
      ...
      if (info.chip_cnt == 4)   
      {
        if (nand_param->ChipConnectInfo == 0xf)
        {
          ...
          if (nand_param->ChipConnectInfo == 0x55)
          {
