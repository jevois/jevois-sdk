...
__s32 NFC_LSBInit(__u32 read_retry_type)
{
	...
	if((read_retry_type>0)&&((read_retry_mode == 0)||(read_retry_mode == 1)))
	{
		lsb_mode_reg_num = 5;
	...
	else if((read_retry_type>0)&&(read_retry_mode == 2))
	{
		lsb_mode_reg_num = 4;
	...
	else if((read_retry_type>0)&&(read_retry_mode == 3))
	{
		lsb_mode_reg_num = 4;
...
__s32 NFC_LSBExit(__u32 read_retry_type)
{
    ...
    if((read_retry_type>0)&&((read_retry_mode == 2)||(read_retry_mode == 3)))
    {
