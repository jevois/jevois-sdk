...
u32 _get_pll4_periph1_clk (void)
{
  ...
  rval = get_wvalue ( (0x06000000 + 0xC) );
...
int NAND_ClkRequest (__u32 nand_index)
{
  ...
  */
  ...
  if (nand_index == 0) {
    sclk0_reg_adr = (0x06000400 + 0x0);
    sclk1_reg_adr = (0x06000400 + 0x4);
  }
  else
    if (nand_index == 1) {
      sclk0_reg_adr = (0x06000400 + 0x8);
      sclk1_reg_adr = (0x06000400 + 0xC);
    }
    else {
  ...
  { sclk0_pre_ratio_n = 3; }
    ...
    if (sclk0_src > 2 * 16 * sclk0)
    { sclk0_pre_ratio_n = 2; }
    else
      if (sclk0_src > 1 * 16 * sclk0)
      { sclk0_pre_ratio_n = 1; }
      else
      { sclk0_pre_ratio_n = 0; }
  ...
  { sclk1_pre_ratio_n = 3; }
    ...
    if (sclk1_src > 2 * 16 * sclk1)
    { sclk1_pre_ratio_n = 2; }
    else
      if (sclk1_src > 1 * 16 * sclk1)
      { sclk1_pre_ratio_n = 1; }
      else
      { sclk1_pre_ratio_n = 0; }
...
int NAND_SetClk (__u32 nand_index, __u32 nand_clock)
{
  ...
  else
  {
    ...
    if (edo_clk <= 20)
    {
      ...
      if ( (edo_clk > 20) && (edo_clk <= 40) )
      {
        n =  0;
        m = 14;
      }
      else
        if ( (edo_clk > 40) && (edo_clk <= 50) )
        {
          n =  0;
          m = 11;
        }
        else
          if ( (edo_clk > 50) && (edo_clk <= 60) )
          {
            n = 0;
            m = 9;
          }
