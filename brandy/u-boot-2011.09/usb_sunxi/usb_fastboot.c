  ...
  case USB_DT_STRING:
    {
      ...
      if (string_index == 0)
      {
        ...
        if (string_index < SUNXI_USB_FASTBOOT_DEV_MAX)
        {
...
static int __try_to_download (char * download_size, char * response)
{
  ...
  if (0 == trans_data.try_to_recv)
  {
    ...
    if (trans_data.try_to_recv > SUNXI_USB_FASTBOOT_BUFFER_MAX)
    {
...
static int sunxi_fastboot_state_loop (void * buffer)
{
  ...
  case SUNXI_USB_FASTBOOT_SETUP:
    ...
    if (memcmp (sunxi_ubuf->rx_req_buffer, "reboot-bootloader", strlen ("reboot-bootloader") ) == 0)
    {
      ...
      if (memcmp (sunxi_ubuf->rx_req_buffer, "reboot", 6) == 0)
      {
        tick_printf ("reboot\n");
        __fastboot_reboot (0);
      }
      else
        if (memcmp (sunxi_ubuf->rx_req_buffer, "erase:", 6) == 0)
        {
          tick_printf ("erase\n");
          __erase_part ( (char *) (sunxi_ubuf->rx_req_buffer + 6) );
        }
        else
          if (memcmp (sunxi_ubuf->rx_req_buffer, "flash:", 6) == 0)
          {
            tick_printf ("flash\n");
            ...
            if (memcmp (sunxi_ubuf->rx_req_buffer, "download:", 9) == 0)
            {
              ...
              if (memcmp (sunxi_ubuf->rx_req_buffer, "boot", 4) == 0)
              {
                tick_printf ("boot\n");
                __boot();
              }
              else
                if (memcmp (sunxi_ubuf->rx_req_buffer, "getvar:", 7) == 0)
                {
                  tick_printf ("getvar\n");
                  __get_var ( (char *) (sunxi_ubuf->rx_req_buffer + 7) );
                }
                else
                  if (memcmp (sunxi_ubuf->rx_req_buffer, "oem", 3) == 0)
                  {
                    tick_printf ("oem operations\n");
                    __oem_operation ( (char *) (sunxi_ubuf->rx_req_buffer + 4) );
                  }
                  else
                    if (memcmp (sunxi_ubuf->rx_req_buffer, "continue", 8) == 0)
                    {
