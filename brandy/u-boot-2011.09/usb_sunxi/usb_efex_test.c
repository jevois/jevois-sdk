  ...
  case FEX_CMD_fes_up:
    ...
    
    {
      ...
      if ( (trans->type & SUNXI_EFEX_DRAM_MASK) == SUNXI_EFEX_DRAM_MASK) //如果属于内存数据，则执行这里
      {
        ...
        if ( (trans->type & FEX_CMD_BOOT2_GET_VOLTAGE) == FEX_CMD_BOOT2_GET_VOLTAGE )
        {
          ...
          if ( (trans->type & FEX_CMD_BOOT2_GET_FREQ) == FEX_CMD_BOOT2_GET_FREQ)
          {
            ...
            if ( (trans->type & FEX_CMD_BOOT2_GET_CURRENT_LIMIT) == FEX_CMD_BOOT2_GET_CURRENT_LIMIT)
            {
              ...
              *charger_current = axp_probe_vbus_cur_limit();
              ...
              if ( (trans->type & FEX_CMD_BOOT2_GET_LOGO_INFO) == FEX_CMD_BOOT2_GET_LOGO_INFO)
              {
...
static void dram_data_recv_finish (uint data_type)
{
  if (data_type ==  FEX_CMD_BOOT2_DOWNLOAD_SYS_CONFIG)
  {
    ...
    if (data_type == FEX_CMD_BOOT2_DOWNLOAD_LOGO)
    {
