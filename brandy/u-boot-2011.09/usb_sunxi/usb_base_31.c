...
int sunxi_usb_init (int delaytime)
{
  ...
  usb_recv_by_dma.cfg.src_drq_type     = DMAC_CFG_SRC_TYPE_OTG_EP2; 
  ...
  usb_recv_by_dma.cfg.dst_drq_type     = DMAC_CFG_DEST_TYPE_DRAM; 
  ...
  usb_send_by_dma.cfg.src_drq_type     = DMAC_CFG_SRC_TYPE_DRAM; 
  ...
  usb_send_by_dma.cfg.dst_drq_type     = DMAC_CFG_DEST_TYPE_OTG_EP1; 
...
static int ep0_recv_op (void)
{
  ...
  else
  {
    ...
    if (ret == SUNXI_USB_REQ_DATA_HUNGRY)
    {
      ...
      if (ret == SUNXI_USB_REQ_SUCCESSED)
      {
        ep0_stage = 0;
      }
      else
