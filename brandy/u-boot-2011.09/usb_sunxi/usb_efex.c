  ...
  case USB_DT_CONFIG:   //配置描述符
    {
      ...
      config_dscrptr->bmAttributes        = 0x80;  
  ...
  case FEX_CMD_fes_tool_mode:
    ...
    
    {
      ...
      { //如果是升级工具，则直接重启
        ...
        if (fes_work->tool_mode == WORK_MODE_ERASE_KEY)
        {
...
static void dram_data_recv_finish (uint data_type)
{
  if (data_type == SUNXI_EFEX_MBR_TAG)    //传输MBR已经完成
  {
    ...
    if (data_type == SUNXI_EFEX_BOOT1_TAG) //传输BOOT1已经完成
    {
      ...
      if (data_type == SUNXI_EFEX_BOOT0_TAG) //传输BOOT0已经完成
      {
        ...
        if (data_type == SUNXI_EFEX_ERASE_TAG)
        {
          uint erase_flag;
          ...
          if (data_type == SUNXI_EFEX_PMU_SET)
          {
            ...
            if (data_type == SUNXI_EFEX_UNSEQ_MEM_FOR_WRITE)
            {
              int i;
              struct unseq_mem_config * unseq_mem = global_unseq_mem_addr.unseq_mem;
              ...
              if (data_type == SUNXI_EFEX_UNSEQ_MEM_FOR_READ)
              {
                struct unseq_mem_config * unseq_mem = global_unseq_mem_addr.unseq_mem;
                ...
                if (data_type == SUNXI_EFEX_FULLIMG_SIZE_TAG)
                {
...
static int sunxi_efex_state_loop (void * buffer)
{
  ...
  case SUNXI_USB_EFEX_SETUP:   
    ...
    if ( (sunxi_ubuf->rx_req_length == sizeof (struct sunxi_efex_cbw_t) ) )
    {
      ...
      if (sunxi_ubuf->rx_req_length == FES_NEW_CMD_LEN)
      {
    ...
    if (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_IDLE)
    {
      ...
      if (cbw->cmd_package.direction == TL_CMD_RECEIVE) //小机端接收数据
      {
        ...
        {
          sunxi_udc_start_recv_by_dma ( (uint) cmd_buf, sunxi_ubuf->request_size);
        }
        else
        {
      ...
      if ( (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_SEND_DATA) ||     \
           (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA) ) //收到的第二阶段，此时开始解析命令
      {
        ...
        if (cbw->cmd_package.direction == TL_CMD_RECEIVE) //如果要接收数据，则事先启动dma开始接收
        {
          ...
          if (sunxi_ubuf->request_size)
          {
            ...
            sunxi_udc_start_recv_by_dma (trans_data.act_recv_buffer, sunxi_ubuf->request_size);
          }
          else
          {
        ...
        if (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_STATUS)
        {
          ...
          if (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_EXIT)
          {
  ...
  case SUNXI_USB_EFEX_RECEIVE_DATA:
    ...
    if (sunxi_usb_efex_write_enable == 1)   //数据部分接收完毕
    {
      ...
      if (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_CMD)
      {
        ...
        if (sunxi_usb_efex_app_step == SUNXI_USB_EFEX_APPS_RECEIVE_DATA) //来自于命令阶段，要求接收数据，下一阶段只能是发送状态(status_t)
        {
  ...
  case SUNXI_USB_EFEX_SETUP_NEW:     
    {
      ...
      if (trans_data.app_next_status == SUNXI_USB_EFEX_APPS_SEND_DATA)
      {
        ...
        if (trans_data.app_next_status == SUNXI_USB_EFEX_APPS_RECEIVE_DATA)
        {
          ...
          if (trans_data.app_next_status == SUNXI_USB_EFEX_APPS_STATUS)
          {
            sunxi_usb_efex_status = SUNXI_USB_EFEX_STATUS;
          }
          else
            if (trans_data.app_next_status == SUNXI_USB_EFEX_APPS_EXIT)
            {
              sunxi_usb_efex_status = SUNXI_USB_EFEX_EXIT;
            }
            else
  ...
  case SUNXI_USB_EFEX_EXIT:
    ...
    if (sunxi_usb_efex_status_enable == 1)
    {
      ...
      if (sunxi_usb_efex_status_enable >= 2)
      {
