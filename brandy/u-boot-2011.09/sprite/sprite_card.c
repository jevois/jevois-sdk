  ...
  for (part_info = dl_map->one_part_info, i = 0; i < dl_map->download_count; i++, part_info++)
  {
    ...
    if (!strncmp ("UDISK", (char *) part_info->name, strlen ("UDISK") ) )
    {
      ...
      if (!strncmp ("sysrecovery", (char *) part_info->name, strlen ("sysrecovery") ) )
      {
        ...
        if (!strncmp ("private", (char *) part_info->name, strlen ("private") ) )
        {
          if (1)
          {
...
int card_erase (int erase, void * mbr_buffer)
{
  ...
  for (i = 1; i < mbr->PartCount; i++)
  {
    ...
    if (mbr->array[i].lenlo > CARD_ERASE_BLOCK_SECTORS * 2) 
    {
      ...
      if (mbr->array[i].lenlo > CARD_ERASE_BLOCK_SECTORS)
      {
        erase_head_sectors = CARD_ERASE_BLOCK_SECTORS;
        erase_head_addr = mbr->array[i].addrlo;
        erase_tail_sectors = 2 * 1024 * 1024 / 512;
  ...
  */
  ...
  for (part_info = dl_map->one_part_info, i = 0; i < dl_map->download_count; i++, part_info++)
  {
    ...
    if (!strcmp ("env", (const char *) part_info->name) )
    {
      ...
      if (!strcmp ("sysrecovery", (const char *) part_info->name) )
      {
        ...
        if (!strcmp ("UDISK", (const char *) part_info->name) )
        {
          ...
          if (!strcmp ("private", (const char *) part_info->name) )
          {
