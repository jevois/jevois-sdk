...
static int smack_sb_copy_data (char * orig, char * smackopts)
{
  ...
  for (cp = orig, commap = orig; commap != NULL; cp = commap + 1) {
    ...
    { dp = smackopts; }
      ...
      if (strstr (cp, SMK_FSFLOOR) == cp)
      { dp = smackopts; }
      else
        if (strstr (cp, SMK_FSHAT) == cp)
        { dp = smackopts; }
        else
          if (strstr (cp, SMK_FSROOT) == cp)
          { dp = smackopts; }
          else
          { dp = otheropts; }
...
static int smack_sb_kern_mount (struct super_block * sb, int flags, void * data)
{
  ...
  for (op = data; op != NULL; op = commap) {
    ...
    if (strncmp (op, SMK_FSHAT, strlen (SMK_FSHAT) ) == 0) {
      ...
      if (strncmp (op, SMK_FSFLOOR, strlen (SMK_FSFLOOR) ) == 0) {
        ...
        if (strncmp (op, SMK_FSDEFAULT,
          ...
          if (strncmp (op, SMK_FSROOT, strlen (SMK_FSROOT) ) == 0) {
                                       ...
                                       const void * value, size_t size, int flags)
{
  ...
  if (strcmp (name, XATTR_NAME_SMACK) == 0) {
    ...
    if (strcmp (name, XATTR_NAME_SMACKEXEC) == 0) {
      nsp = smk_import (value, size);
      if (nsp != NULL)
      { isp->smk_task = nsp; }
      else
      ...
      if (strcmp (name, XATTR_NAME_SMACKMMAP) == 0) {
        nsp = smk_import (value, size);
        if (nsp != NULL)
        { isp->smk_mmap = nsp; }
        else
                                    ...
                                    bool alloc)
{
  ...
  { isp = ssp->smk_in; }
    ...
    if (strcmp (name, XATTR_SMACK_IPOUT) == 0)
    { isp = ssp->smk_out; }
    else
    { return -EOPNOTSUPP; }
    ...
    int __user * optlen, unsigned len)
{
  ...
  { rc = -ERANGE; }
    ...
    if (copy_to_user (optval, rcp, slen) != 0)
    { rc = -EFAULT; }
