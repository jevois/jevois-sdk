...
enum sched_prio string_to_prio (const char * str)
{
  ...
  { return  SCHED_HIGH; }
    ...
    if (strncasecmp ("default", str, strlen (str) ) == 0)
    { return SCHED_DEFAULT; }
    else
      if (strncasecmp ("low", str, strlen (str) ) == 0)
      { return SCHED_LOW; }
      else
      { return SCHED_ERR; }
}
...
int prepare_config (const char * path, struct config * config)
{
  ...
  while (getline (&line, &len, configfile) != -1) {
    ...
    { sscanf (val, "%li", &config->sleep); }
      ...
      if (strncmp ("load", opt, strlen (opt) ) == 0)
      { sscanf (val, "%li", &config->load); }
      
      else
        if (strncmp ("load_step", opt, strlen (opt) ) == 0)
        { sscanf (val, "%li", &config->load_step); }
        
        else
          if (strncmp ("sleep_step", opt, strlen (opt) ) == 0)
          { sscanf (val, "%li", &config->sleep_step); }
          
          else
            if (strncmp ("cycles", opt, strlen (opt) ) == 0)
            { sscanf (val, "%u", &config->cycles); }
            
            else
              if (strncmp ("rounds", opt, strlen (opt) ) == 0)
              { sscanf (val, "%u", &config->rounds); }
              
              else
                if (strncmp ("verbose", opt, strlen (opt) ) == 0)
                { sscanf (val, "%u", &config->verbose); }
                
                else
                  if (strncmp ("output", opt, strlen (opt) ) == 0)
                  { config->output = prepare_output (val); }
                  
                  else
                    if (strncmp ("cpu", opt, strlen (opt) ) == 0)
                    { sscanf (val, "%u", &config->cpu); }
                    
                    else
                      if (strncmp ("governor", opt, 14) == 0)
                      { strncpy (config->governor, val, 14); }
                      
                      else
                        if (strncmp ("priority", opt, strlen (opt) ) == 0) {
