...
static void update_shadow_stats (struct perf_evsel * counter, u64 * count)
{
  ...
  { update_stats (&runtime_nsecs_stats[0], count[0]); }
    ...
    { update_stats (&runtime_cycles_stats[0], count[0]); }
      ...
      if (perf_evsel__match (counter, HARDWARE, HW_STALLED_CYCLES_FRONTEND) )
      { update_stats (&runtime_stalled_cycles_front_stats[0], count[0]); }
        ...
        { update_stats (&runtime_stalled_cycles_back_stats[0], count[0]); }
          ...
          { update_stats (&runtime_branches_stats[0], count[0]); }
            ...
            if (perf_evsel__match (counter, HARDWARE, HW_CACHE_REFERENCES) )
            { update_stats (&runtime_cacherefs_stats[0], count[0]); }
            else
              if (perf_evsel__match (counter, HW_CACHE, HW_CACHE_L1D) )
              { update_stats (&runtime_l1_dcache_stats[0], count[0]); }
              else
                if (perf_evsel__match (counter, HW_CACHE, HW_CACHE_L1I) )
                { update_stats (&runtime_l1_icache_stats[0], count[0]); }
                else
                  if (perf_evsel__match (counter, HW_CACHE, HW_CACHE_LL) )
                  { update_stats (&runtime_ll_cache_stats[0], count[0]); }
                  else
                    if (perf_evsel__match (counter, HW_CACHE, HW_CACHE_DTLB) )
                    { update_stats (&runtime_dtlb_cache_stats[0], count[0]); }
                    else
                      if (perf_evsel__match (counter, HW_CACHE, HW_CACHE_ITLB) )
...
static const char * get_ratio_color (enum grc_type type, double ratio)
{
  ...
  { color = PERF_COLOR_RED; }
    ...
    if (ratio > grc_table[type][1])
    { color = PERF_COLOR_MAGENTA; }
    else
      if (ratio > grc_table[type][2])
          ...
          if (
            ...
            if (
              ...
              if (
                ...
                if (perf_evsel__match (evsel, HARDWARE, HW_CACHE_MISSES) &&
                  ...
                  if (perf_evsel__match (evsel, HARDWARE, HW_STALLED_CYCLES_FRONTEND) ) {
                    ...
                    if (perf_evsel__match (evsel, HARDWARE, HW_STALLED_CYCLES_BACKEND) ) {
                      ...
                      if (perf_evsel__match (evsel, HARDWARE, HW_CPU_CYCLES) ) {
                        ...
                        if (runtime_nsecs_stats[cpu].n != 0) {
                          char unit = 'M';
...
int cmd_stat (int argc, const char ** argv, const char * prefix __used)
{
  ...
  if (csv_output) {
    ...
    if (big_num_opt == 0) /* User passed --no-big-num */
    { big_num = false; }
