                          ...
                          unsigned int freq_in, unsigned int freq_out)
{
  ...
  if ( (freq_in < 128000) || (freq_in > 24576000) ) {
    ...
    if ( (freq_in == 24576000) || (freq_in == 22579200) ) {
      switch (pll_id) {
      case AC100_MCLK1:
...
static void earphone_switch_work (struct work_struct * work)
{
  ...
  if ( (tmp >= 0xb) && (ac100->mode == FOUR_HEADPHONE_PLUGIN) ) {
    ...
    if ( (tmp < 0xb && tmp >= 0x2) && (ac100->mode == FOUR_HEADPHONE_PLUGIN) ) {
      /*read HMIC_DATA */
      tmp = snd_soc_read (codec, HMIC_STS);
      tmp = (tmp >> HMIC_DATA);
      tmp &= 0x1f;
      ...
      while (irq_flag == 0) {
        ...
        if (ac100->check_count_sum <= HEADSET_CHECKCOUNT_SUM) {
          ...
          if (temp_value[ac100->check_count - 2] >= 0xb) {
            ...
            if (temp_value[ac100->check_count - 2] >= 0x1 && temp_value[ac100->check_count - 2] < 0xb) {
