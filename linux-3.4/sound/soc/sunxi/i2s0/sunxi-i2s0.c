...
static int sunxi_i2s0_set_clkdiv (struct snd_soc_dai * cpu_dai, int div_id, int sample_rate)
{
  ...
  { reg_val |= SUNXI_I2S0FAT0_WSS_16BCLK; }
    ...
    if (sunxi_i2s0.ws_size == 20)
    { reg_val |= SUNXI_I2S0FAT0_WSS_20BCLK; }
    else
      if (sunxi_i2s0.ws_size == 24)
  ...
  { reg_val |= 0x4 << 12; }
    ...
    if (sunxi_i2s0.pcm_sync_period == 128)
    { reg_val |= 0x3 << 12; }
    else
      if (sunxi_i2s0.pcm_sync_period == 64)
      { reg_val |= 0x2 << 12; }
      else
        if (sunxi_i2s0.pcm_sync_period == 32)
        { reg_val |= 0x1 << 12; }
