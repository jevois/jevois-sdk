    ...
    struct snd_pcm_hw_params * hw_params)
{
  ...
  if (params_periods (hw_params) == CS46XX_FRAGS) {
    ...
    if (cpcm->pcm_channel_id == DSP_PCM_MAIN_CHANNEL) {
      ...
      if (cpcm->pcm_channel_id == DSP_PCM_REAR_CHANNEL) {
        substream->ops = &snd_cs46xx_playback_rear_ops;
      }
      else
        if (cpcm->pcm_channel_id == DSP_PCM_CENTER_LFE_CHANNEL) {
          substream->ops = &snd_cs46xx_playback_clfe_ops;
        }
        else
          if (cpcm->pcm_channel_id == DSP_IEC958_CHANNEL) {
                                  ...
                                  struct snd_ctl_elem_value * ucontrol)
{
  ...
  case CS46XX_MIXER_SPDIF_OUTPUT_ELEMENT:
    ...
    { cs46xx_dsp_enable_spdif_out (chip); }
      ...
      if (change && !ucontrol->value.integer.value[0])
...
static void snd_cs46xx_codec_reset (struct snd_ac97 * ac97)
{
  ...
  if (ac97->num == CS46XX_PRIMARY_CODEC_INDEX) {
    ...
    if (ac97->num == CS46XX_SECONDARY_CODEC_INDEX) {
