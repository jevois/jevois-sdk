  ...
  if (asihpi->support_mrx) {
    ...
    for (idx = -1; idx < 100; idx++) {
      if (idx == -1) {
        ...
        if (hpi_sample_clock_query_local_rate (h_control,
                                               idx, &sample_rate) ) {
          break;
        }
                             ...
                             char * name)
{
  ...
  { dir = ""; } /* clock is neither capture nor playback */
    ...
    { dir = "Capture "; }  /* On or towards a PCM capture destination*/
      ...
      if ( (hpi_ctl->src_node_type + HPI_SOURCENODE_NONE != HPI_SOURCENODE_OSTREAM) &&
           (!hpi_ctl->dst_node_type) )
      { dir = "Capture "; } /* On a source node that is not PCM playback */
      else
        if (hpi_ctl->src_node_type &&
