...
do_digit_io_switch (struct ct_atc * atc, int state)
{
  ...
  { atc->select_line_in (atc); }
    ...
    if (get_switch_state (mixer, MIXER_MIC_C_S) )
    { atc->select_mic_in (atc); }
...
static void do_switch (struct ct_atc * atc, enum CTALSA_MIXER_CTL type, int state)
{
  ...
  if (!cap.dedicated_mic &&
    ...
    if (cap.dedicated_mic && (MIXER_LINEIN_C_S == type) )
    { atc->line_in_unmute (atc, state); }
    else
      if (cap.dedicated_mic && (MIXER_MIC_C_S == type) )
      { atc->mic_unmute (atc, state); }
      else
        if (MIXER_SPDIFI_C_S == type)
        { atc->spdif_in_unmute (atc, state); }
        else
          if (MIXER_WAVEF_P_S == type)
          { atc->line_front_unmute (atc, state); }
          else
            if (MIXER_WAVES_P_S == type)
            { atc->line_surround_unmute (atc, state); }
            else
              if (MIXER_WAVEC_P_S == type)
              { atc->line_clfe_unmute (atc, state); }
              else
                if (MIXER_WAVER_P_S == type)
                { atc->line_rear_unmute (atc, state); }
                else
                  if (MIXER_SPDIFO_P_S == type)
                  { atc->spdif_out_unmute (atc, state); }
                  else
                    if (MIXER_DIGITAL_IO_S == type)
