...
static enum VIA_HDA_CODEC get_codec_type (struct hda_codec * codec)
{
  ...
  { codec_type = UNKNOWN; }
    ...
    if (dev_id >= 0x1708 && dev_id <= 0x170b)
    { codec_type = VT1708; }
    else
      if (dev_id >= 0xe710 && dev_id <= 0xe713)
      { codec_type = VT1709_10CH; }
      else
        if (dev_id >= 0xe714 && dev_id <= 0xe717)
        { codec_type = VT1709_6CH; }
        else
          if (dev_id >= 0xe720 && dev_id <= 0xe723) {
            codec_type = VT1708B_8CH;
            if (snd_hda_param_read (codec, 0x16, AC_PAR_CONNLIST_LEN) == 0x7)
            { codec_type = VT1708BCE; }
          }
          else
            if (dev_id >= 0xe724 && dev_id <= 0xe727)
            { codec_type = VT1708B_4CH; }
            else
              if ( (dev_id & 0xfff) == 0x397
                   && (dev_id >> 12) < 8)
              { codec_type = VT1708S; }
              else
                if ( (dev_id & 0xfff) == 0x398
                     && (dev_id >> 12) < 8)
                { codec_type = VT1702; }
                else
                  if ( (dev_id & 0xfff) == 0x428
                       && (dev_id >> 12) < 8)
                  { codec_type = VT1718S; }
                  else
                    if (dev_id == 0x0433 || dev_id == 0xa721)
                    { codec_type = VT1716S; }
                    else
                      if (dev_id == 0x0441 || dev_id == 0x4441)
                      { codec_type = VT1718S; }
                      else
                        if (dev_id == 0x0438 || dev_id == 0x4438)
                        { codec_type = VT2002P; }
                        else
                          if (dev_id == 0x0448)
                          { codec_type = VT1812; }
...
static void vt1708_update_hp_work (struct via_spec * spec)
{
  ...
  if (spec->vt1708_jack_detect &&
    ...
    if (!hp_detect_with_aa (spec->codec) )
                           ...
                           int chs, bool check_dac, struct nid_path * path)
{
  ...
  { nid = dac; }
    ...
    if (check_amp_caps (codec, pin, HDA_OUTPUT, AC_AMPCAP_NUM_STEPS) )
    { nid = pin; }
    else
      if (check_amp_caps (codec, sel, HDA_OUTPUT, AC_AMPCAP_NUM_STEPS) )
      { nid = sel; }
      else
      { nid = 0; }
  if (nid) {
  ...
  { nid = dac; }
    ...
    if (check_amp_caps (codec, pin, HDA_OUTPUT, AC_AMPCAP_MUTE) )
    { nid = pin; }
    else
      if (check_amp_caps (codec, sel, HDA_OUTPUT, AC_AMPCAP_MUTE) )
      { nid = sel; }
      else
      { nid = 0; }
  if (nid) {
...
static void set_widgets_power_state_vt1708B (struct hda_codec * codec)
{
  ...
  if (is_8ch) {
    ...
    if (codec->vendor_id == 0x11064397 && spec->hp_independent_mode)
    { update_power_state (codec, 0x25, parm); }
}
