...
static int alc_auto_create_shared_input (struct hda_codec * codec)
{
  ...
  { nid = cfg->hp_pins[0]; } /* OK, we have a single HP-out */
    ...
    if (cfg->line_outs == 1 && cfg->line_out_type == AUTO_PIN_HP_OUT)
  ...
  for (i = 0; i < num_outs; i++) {
    ...
    if (!dac) {
      ...
      if (dac) {
        ...
        if (alc_auto_is_dac_reachable (codec, pin,
    ...
    hda_nid_t pin, hda_nid_t dac)
{
  ...
  { return pin; }
    ...
    if (mix && nid_has_mute (codec, mix, HDA_INPUT) )
    { return mix; }
    else
    ...
    hda_nid_t pin, hda_nid_t dac)
{
  ...
  { return dac; }
    ...
    if (nid_has_volume (codec, mix, HDA_OUTPUT) )
    { return mix; }
    else
      if (nid_has_volume (codec, pin, HDA_OUTPUT) )
      { return pin; }
  return 0;
}
  ...
  for (n = 0; n < spec->num_adc_nids; n++) {
    ...
    for (i = 0; i < imux->num_items; i++) {
      ...
      if (pin) {
        ...
        if (num_conns <= imux->items[i].index)
        { break; }
    }
    if (i >= imux->num_items) {
                                     ...
                                     int idx)
{
  if (get_wcaps_type (get_wcaps (codec, cap) ) == AC_WID_AUD_MIX) {
    ...
    if (snd_hda_get_conn_list (codec, cap, NULL) > 1) {
