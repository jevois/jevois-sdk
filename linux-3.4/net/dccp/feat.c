                                 ...
                                 const bool is_local, dccp_feat_val const * fval)
{
  ...
  if (fval == NULL) {
    ...
    if (dccp_feat_table[idx].reconciliation == FEAT_SP) {
      if (fval->sp.vec == NULL) {
        /*
                                 ...
                                 u8 feat, u8 * val, u8 len, const bool server)
{
  ...
  if (entry == NULL) {
    ...
    if (len > 1 && server) {
      ...
      if (!dccp_feat_is_valid_sp_val (feat, fval.sp.vec[0]) ) {
        kfree (fval.sp.vec);
        goto unknown_feature_or_value;
      }
    ...
    if (entry->state == FEAT_UNSTABLE) { /* 6.6.2 */
      return 0;
    }
  ...
  if (dccp_feat_reconcile (&entry->val, val, len, server, true) ) {
    ...
    if (is_mandatory) {
      ...
      if (entry->state == FEAT_INITIALISING) {
        /*
