...
static unsigned int get_conntrack_index (const struct tcphdr * tcph)
{
  if (tcph->rst) { return TCP_RST_SET; }
    ...
    if (tcph->syn) { return (tcph->ack ? TCP_SYNACK_SET : TCP_SYN_SET); }
    else
      if (tcph->fin) { return TCP_FIN_SET; }
      else
        if (tcph->ack) { return TCP_ACK_SET; }
                           ...
                           u_int8_t pf)
{
  ...
  if (sender->td_maxwin == 0) {
    ...
    if ( ( (state->state == TCP_CONNTRACK_SYN_SENT
  ...
  if (! (tcph->ack) ) {
    ...
    if ( ( (tcp_flag_word (tcph) & (TCP_FLAG_ACK | TCP_FLAG_RST) ) ==
