...
adbhid_input_keycode (int id, int scancode, int repeat)
{
  ...
  case ADB_KEY_POWER:
    ...
    if (ahid->flags & FLAG_FN_KEY_PRESSED) {
      ...
      if (ahid->flags & FLAG_POWER_FROM_FN) {
        keycode = ADB_KEY_CMD;
        ahid->flags &= ~FLAG_POWER_FROM_FN;
      }
    break;
...
adbhid_probe (void)
{
  ...
  for (i = 0; i < mouse_ids.nids; i++) {
    ...
    if (adb_try_handler_change (id, 4) ) {
      ...
      if (adb_try_handler_change (id, 0x2F) ) {
        ...
        if (adb_try_handler_change (id, 0x42) ) {
          ...
          if (adb_try_handler_change (id, 0x66) ) {
            ...
            if (adb_try_handler_change (id, 0x5F) ) {
              ...
              if (adb_try_handler_change (id, 3) ) {
                ...
                if (adb_try_handler_change (id, 2) ) {
    ...
    if ( (mouse_kind == ADBMOUSE_TRACKBALLPRO)
      ...
      if (mouse_kind == ADBMOUSE_MS_A3) {
        init_ms_a3 (id);
      }
      else
        if (mouse_kind ==  ADBMOUSE_EXTENDED) {
          /*
