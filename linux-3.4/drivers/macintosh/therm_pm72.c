...
static void do_monitor_cpu_combined (void)
{
  ...
  if (temp_combi >= ( (state0->mpu.tmax + 8) << 16) ) {
    ...
    if (temp_combi > (state0->mpu.tmax << 16) ) {
      state0->overtemp++;
...
static int create_control_loops (void)
{
  ...
  { cpu_pid_type = CPU_PID_TYPE_RACKMAC; }
    ...
    if (of_machine_is_compatible ("PowerMac7,3")
        && (cpu_count > 1)
...
static int therm_pm72_attach (struct i2c_adapter * adapter)
{
  ...
  if (u3_0 == NULL && !strcmp (adapter->name, "u3 0") ) {
    ...
    if (u3_1 == NULL && !strcmp (adapter->name, "u3 1") ) {
      ...
      if (k2 == NULL && !strcmp (adapter->name, "mac-io 0") ) {
        k2 = adapter;
        DBG ("Found K2\n");
        if (u3_0 && rackmac)
          if (create_control_loops() )
