...
static int cpu_check_overtemp (s32 temp)
{
  ...
  if (new_state) {
    ...
    if ( (failure_state & FAILURE_LOW_OVERTEMP) &&
...
static void pm112_new_control (struct wf_control * ct)
{
  ...
  if (i >= NR_CPU_FANS) {
    ...
    if (!strcmp (ct->name, "backside-fan") ) {
      ...
      if (!strcmp (ct->name, "slots-fan") ) {
        if (slots_fan == NULL && wf_get_control (ct) == 0)
        { slots_fan = ct; }
      }
      else
        if (!strcmp (ct->name, "drive-bay-fan") ) {
...
static void pm112_new_sensor (struct wf_sensor * sr)
{
  ...
  if (!strncmp (sr->name, "cpu-temp-", 9) ) {
    ...
    if (!strncmp (sr->name, "cpu-power-", 10) ) {
      i = sr->name[10] - '0';
      ...
      if (!strcmp (sr->name, "hd-temp") ) {
        if (hd_temp == NULL && wf_get_sensor (sr) == 0)
        { hd_temp = sr; }
      }
      else
        if (!strcmp (sr->name, "slots-power") ) {
          if (slots_power == NULL && wf_get_sensor (sr) == 0)
          { slots_power = sr; }
        }
        else
          if (!strcmp (sr->name, "backside-temp") ) {
