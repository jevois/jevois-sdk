...
#define ANNOTATE_BUFFER_SIZE     (64*1024)
#define SCHED_TRACE_BUFFER_SIZE  (128*1024)
#define GPU_TRACE_BUFFER_SIZE    (64*1024)
#define COUNTER2_BUFFER_SIZE     (64*1024)
#define WFI_BUFFER_SIZE          (32*1024)
...
static void gator_buffer_header (int cpu, int buftype)
{
  ...
  { frame = FRAME_BACKTRACE; }
    ...
    if (buftype == COUNTER_BUF)
    { frame = FRAME_COUNTER; }
    else
      if (buftype == ANNOTATE_BUF)
      { frame = FRAME_ANNOTATE; }
      else
        if (buftype == SCHED_TRACE_BUF)
        { frame = FRAME_SCHED_TRACE; }
        else
          if (buftype == GPU_TRACE_BUF)
          { frame = FRAME_GPU_TRACE; }
          else
            if (buftype == COUNTER2_BUF)
