...
static unsigned long __ddrfreq_get (void)
{
  ...
  if (value == 0x3) {
    ...
    if ( (readl (SUNXI_DRAMPHY0_VBASE + 0x04) >> 17) & 0x1) 
...
static unsigned long __ddrfreq_get (void)
{
  ...
  if (mdfs_in_cfs == 1) {
    if (readl (SUNXI_CCM_VBASE + 0xF4) & 0xF)
                              ...
                              unsigned int freq)
{
  ...
  if (para->dram_tpr9 != 0) {
    if ( ( (para->dram_clk % freq) == 0 ) && ( (para->dram_tpr9 % freq) == 0) ) {
      ...
      if ( (para->dram_clk % freq) == 0) {
        source = 1;
        div = para->dram_clk / freq;
      }
      else
        if ( (para->dram_tpr9 % freq) == 0) {
          source = 0;
          div = para->dram_tpr9 / freq ;
        }
        else {
