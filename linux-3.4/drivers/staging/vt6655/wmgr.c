...
)
{
  ...
  if (pMgmt->eCurrentPHYMode == PHY_TYPE_11G) {
    ...
    if (pMgmt->eCurrentPHYMode == PHY_TYPE_11B) {
  ...
  if ( (pMgmt->eAuthenMode == WMAC_AUTH_WPAPSK) ||         
    ...
    if (pCurr->bWPAValid == true)  { 
      ...
      if (pCurr->abyPKType[0] == WPA_TKIP) {
        ...
        if (pCurr->abyPKType[0] == WPA_AESCCMP) {
          pDevice->eEncryptionStatus = Ndis802_11Encryption3Enabled;   
          PRINT_K ("Encyption_Rebuild--->ssid reset config to [WPAPSK-AES]\n");
        }
    }
    else
      if (pCurr->bWPA2Valid == true) {
        pMgmt->eAuthenMode = WMAC_AUTH_WPA2PSK;
        if (pCurr->abyCSSPK[0] == WLAN_11i_CSS_TKIP) {
          ...
          if (pCurr->abyCSSPK[0] == WLAN_11i_CSS_CCMP) {
            pDevice->eEncryptionStatus = Ndis802_11Encryption3Enabled;   
...
)
{
  ...
  if ( (WLAN_GET_CAP_INFO_PRIVACY (pBSSNode->wCapInfo) != 0) &&
    ...
    if ( (pBSSNode->byCSSGK == WLAN_11i_CSS_WEP40) ||
      ...
      if (pBSSNode->byCSSGK == WLAN_11i_CSS_TKIP) {
    ...
    for (i = 0; i < pBSSNode->wCSSPKCount; i++) {
      if ( (pBSSNode->abyCSSPK[i] == WLAN_11i_CSS_WEP40) ||
        ...
        if (pBSSNode->abyCSSPK[i] == WLAN_11i_CSS_TKIP) {
          byCipherMask |= 0x02;
        }
        else
          if (pBSSNode->abyCSSPK[i] == WLAN_11i_CSS_CCMP) {
            byCipherMask |= 0x04;
          }
          else
    ...
    if ( (WLAN_GET_CAP_INFO_PRIVACY (pBSSNode->wCapInfo) != 0) &&
      ...
      if ( (pBSSNode->byGKType == WPA_WEP40) ||
        ...
        if (pBSSNode->byGKType == WPA_TKIP) {
          byMulticastCipher = KEY_CTL_TKIP;
        }
        else
          if (pBSSNode->byGKType == WPA_AESCCMP) {
            byMulticastCipher = KEY_CTL_CCMP;
          }
          else {
            byMulticastCipher = KEY_CTL_INVALID;
      ...
      for (i = 0; i < pBSSNode->wPKCount; i++) {
        if (pBSSNode->abyPKType[i] == WPA_TKIP) {
          ...
          if (pBSSNode->abyPKType[i] == WPA_AESCCMP) {
            byCipherMask |= 0x04;
          }
          else
            if (pBSSNode->abyPKType[i] == WPA_NONE) {
              byCipherMask = 0;
              i = pBSSNode->wPKCount;
            }
      }
  ...
  if (EncStatus == Ndis802_11Encryption1Enabled) {
    ...
    if (EncStatus == Ndis802_11Encryption2Enabled) {
      ...
      if (EncStatus == Ndis802_11Encryption3Enabled) {
