...
int private_ioctl (PSDevice pDevice, struct ifreq * rq)
{
  ...
  case WLAN_CMD_ZONETYPE_SET:
    ...
    if (sZoneTypeCmd.bWrite == true) {
      ...
      if (sZoneTypeCmd.ZoneType == ZoneType_USA) {
        ...
        if (sZoneTypeCmd.ZoneType == ZoneType_Japan) {
          /* set to Japan */
          printk ("set_ZoneType:Japan\n");
        }
        else
          if (sZoneTypeCmd.ZoneType == ZoneType_Europe) {
            /* set to Europe */
      ...
      if (zonetype == 0x00) {   /* USA */
        ...
        if (zonetype == 0x01) {  /* Japan */
          sZoneTypeCmd.ZoneType = ZoneType_Japan;
        }
        else
          if (zonetype == 0x02) {  /* Europe */
  ...
  case WLAN_CMD_AP_START:
    ...
    { pMgmt->byAPBBType = PHY_TYPE_11G; }
      ...
      if (sStartAPCmd.wBBPType == PHY80211a)
    ...
    if (sStartAPCmd.byBasicRate & BIT3) {
      ...
      if (sStartAPCmd.byBasicRate & BIT2) {
        pMgmt->abyIBSSSuppRates[2] |= BIT7;
        pMgmt->abyIBSSSuppRates[3] |= BIT7;
        pMgmt->abyIBSSSuppRates[4] |= BIT7;
      }
      else
        if (sStartAPCmd.byBasicRate & BIT1) {
          pMgmt->abyIBSSSuppRates[2] |= BIT7;
          pMgmt->abyIBSSSuppRates[3] |= BIT7;
        }
        else
