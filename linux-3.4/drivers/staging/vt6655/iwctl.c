  ...
  if ( (wrq->flags & IW_ENCODE_DISABLED) == 0) {
    ...
    if (wrq->length > 0) {
      ...
      if (wrq->length ==  WLAN_WEP232_KEYLEN) {
        ...
        if (wrq->length ==  WLAN_WEP104_KEYLEN) {
          DBG_PRT (MSG_LEVEL_DEBUG, KERN_INFO "Set 104 bit wep key\n");
        }
        else
          if (wrq->length == WLAN_WEP40_KEYLEN) {
                    ...
                    char * extra)
{
  ...
  if ( (wrq->flags & IW_POWER_TYPE) == IW_POWER_TIMEOUT) {
    ...
    if ( (wrq->flags & IW_POWER_TYPE) == IW_POWER_PERIOD) {
      pDevice->ePSMode = WMAC_POWER_FAST;
      PSvEnablePowerSaving ( (void *) pDevice, pMgmt->wListenInterval);
    }
                   ...
                   char * extra)
{
  ...
  case IW_AUTH_WPA_VERSION:
    ...
    if (wrq->value == IW_AUTH_WPA_VERSION_DISABLED) {
      ...
      if (wrq->value == IW_AUTH_WPA_VERSION_WPA) {
  ...
  case IW_AUTH_CIPHER_PAIRWISE:
    ...
    if (pairwise == IW_AUTH_CIPHER_CCMP) {
      ...
      if (pairwise == IW_AUTH_CIPHER_TKIP) {
        pDevice->eEncryptionStatus = Ndis802_11Encryption2Enabled;
      }
      else
        if (pairwise == IW_AUTH_CIPHER_WEP40 || pairwise == IW_AUTH_CIPHER_WEP104) {
          pDevice->eEncryptionStatus = Ndis802_11Encryption1Enabled;
        }
        else
  ...
  case IW_AUTH_KEY_MGMT:
    ...
    if (wpa_version == IW_AUTH_WPA_VERSION_WPA2) {
      ...
      if (wpa_version == IW_AUTH_WPA_VERSION_WPA) {
        if (wrq->value == 0) {
  ...
  case IW_AUTH_80211_AUTH_ALG:
    if (wrq->value == IW_AUTH_ALG_OPEN_SYSTEM) {
      ...
      if (wrq->value == IW_AUTH_ALG_SHARED_KEY) {
        pMgmt->bShareKeyAlgorithm = true;
      }
    break;
...
static const iw_handler   iwctl_handler[] =
{
  ...
  (iw_handler) iwctl_giwname,    
  (iw_handler) NULL,       
  (iw_handler) NULL,       
  (iw_handler) iwctl_siwfreq,  
  (iw_handler) iwctl_giwfreq,  
  (iw_handler) iwctl_siwmode,  
  (iw_handler) iwctl_giwmode,  
  (iw_handler) NULL,           
  (iw_handler) iwctl_giwsens,          
  (iw_handler) NULL,           
  ...
  (iw_handler) NULL,           
  (iw_handler) NULL,           
  (iw_handler) iwctl_siwap,      
  (iw_handler) iwctl_giwap,      
  (iw_handler) NULL,           
  (iw_handler) iwctl_giwaplist,      
  (iw_handler) iwctl_siwscan,        
  (iw_handler) iwctl_giwscan,        
  (iw_handler) iwctl_siwessid,   
  (iw_handler) iwctl_giwessid,   
  (iw_handler) NULL,   
  (iw_handler) NULL,   
  (iw_handler) NULL,           
  (iw_handler) NULL,           
  (iw_handler) iwctl_siwrate,  
  (iw_handler) iwctl_giwrate,  
  (iw_handler) iwctl_siwrts,   
  (iw_handler) iwctl_giwrts,   
  (iw_handler) iwctl_siwfrag,  
  (iw_handler) iwctl_giwfrag,  
  (iw_handler) NULL,   
  (iw_handler) NULL,   
  (iw_handler) iwctl_siwretry,   
  (iw_handler) iwctl_giwretry,   
  (iw_handler) iwctl_siwencode,  
  (iw_handler) iwctl_giwencode,  
  (iw_handler) iwctl_siwpower,   
  (iw_handler) iwctl_giwpower,   
  (iw_handler) NULL,     
  (iw_handler) NULL,     
  (iw_handler) iwctl_siwgenie,   
  (iw_handler) iwctl_giwgenie,   
  (iw_handler) iwctl_siwauth,  
  (iw_handler) iwctl_giwauth,  
  (iw_handler) iwctl_siwencodeext,   
  (iw_handler) iwctl_giwencodeext,   
  (iw_handler) NULL,       
  (iw_handler) NULL,       
