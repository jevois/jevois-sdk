  ...
  if (events == 0) {
    ...
    { room = PCI230P2_DAC_FIFOROOM_FULL; }
      ...
      { room = PCI230P2_DAC_FIFOROOM_HALFTOFULL; }
        ...
        if ( (dacstat & PCI230P2_DAC_FIFO_EMPTY) != 0)
                              ...
                              struct comedi_subdevice * s)
{
  ...
  if (devpriv->ai_continuous) {
    ...
    if (devpriv->ai_scan_count == 0) {
      todo = 0;
    }
    else
      if ( (devpriv->ai_scan_count > PCI230_ADC_FIFOLEVEL_HALFFULL)
           || (scanlen > PCI230_ADC_FIFOLEVEL_HALFFULL) ) {
        todo = PCI230_ADC_FIFOLEVEL_HALFFULL;
      }
      else {
  ...
  for (i = 0; i < todo; i++) {
    if (fifoamount == 0) {
      ...
      if ( (status_fifo & PCI230_ADC_FIFO_FULL_LATCHED) != 0) {
        ...
        if ( (status_fifo & PCI230_ADC_FIFO_EMPTY) != 0) {
          /* FIFO empty. */
          break;
        }
        else
          if ( (status_fifo & PCI230_ADC_FIFO_HALF) != 0) {
            /* FIFO half full. */
