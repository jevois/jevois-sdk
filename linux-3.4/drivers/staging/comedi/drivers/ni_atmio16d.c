                            ...
                            struct comedi_subdevice * s)
{
  ...
  if (cmd->convert_arg < 65536000) {
    ...
    if (cmd->convert_arg < 655360000) {
      base_clock = CLOCK_100_KHZ;
      timer = cmd->convert_arg / 10000;
    }
    else
      if (cmd->convert_arg <= 0xffffffff /* 6553600000 */) {
        base_clock = CLOCK_10_KHZ;
        timer = cmd->convert_arg / 100000;
      }
      else
        if (cmd->convert_arg <= 0xffffffff /* 65536000000 */) {
  ...
  if (cmd->chanlist_len > 1) {
    if (cmd->scan_begin_arg < 65536000) {
      ...
      if (cmd->scan_begin_arg < 655360000) {
        base_clock = CLOCK_100_KHZ;
        timer = cmd->scan_begin_arg / 10000;
      }
      else
        if (cmd->scan_begin_arg < 0xffffffff /* 6553600000 */) {
          base_clock = CLOCK_10_KHZ;
          timer = cmd->scan_begin_arg / 100000;
        }
        else
          if (cmd->scan_begin_arg < 0xffffffff /* 65536000000 */) {
            base_clock = CLOCK_1_KHZ;
