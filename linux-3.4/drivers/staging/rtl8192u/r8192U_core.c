  ...
  
  {
    ...
    if (queue_index != BEACON_QUEUE) {
      ...
      if ( (skb_queue_len (&priv->ieee80211->skb_waitQ[queue_index]) != 0) && \
        ...
        if ( (skb_queue_len (&priv->ieee80211->skb_drv_aggQ[queue_index]) != 0) && \
             (! (priv->ieee80211->queue_stop) ) ) {
...
void rtl8192_update_ratr_table (struct net_device * dev)
{
  ...
  if (ieee->pHTInfo->bCurTxBW40MHz && ieee->pHTInfo->bCurShortGI40MHz) {
    ...
    if (!ieee->pHTInfo->bCurTxBW40MHz && ieee->pHTInfo->bCurShortGI20MHz) {
      ratr_value |= 0x80000000;
    }
...
void rtl8192_SetWirelessMode (struct net_device * dev, u8 wireless_mode)
{
  ...
  if ( (wireless_mode == WIRELESS_MODE_AUTO) || ( (wireless_mode & bSupportMode) == 0) )
  {
    if (bSupportMode & WIRELESS_MODE_N_24G)
    {
      ...
      if (bSupportMode & WIRELESS_MODE_N_5G)
      {
        wireless_mode = WIRELESS_MODE_N_5G;
      }
      else
        if ( (bSupportMode & WIRELESS_MODE_A) )
        {
          wireless_mode = WIRELESS_MODE_A;
        }
        else
          if ( (bSupportMode & WIRELESS_MODE_G) )
          {
            wireless_mode = WIRELESS_MODE_G;
          }
          else
            if ( (bSupportMode & WIRELESS_MODE_B) )
            {
...
bool rtl8192_adapter_start (struct net_device * dev)
{
  ...
  { dwRegRead |= CPU_GEN_SYSTEM_RESET; }
    ...
    if (priv->pFirmware->firmware_status == FW_STATUS_5_READY)
  ...
  { dwRegRead = ( (dwRegRead & CPU_GEN_NO_LOOPBACK_MSK) | CPU_GEN_NO_LOOPBACK_SET); }
    ...
    if (priv->LoopbackMode == RTL819xU_MAC_LOOPBACK)
...
HalRxCheckStuck819xUsb (struct net_device * dev)
{
  ...
  if (priv->undecorated_smoothed_pwdb >= (RateAdaptiveTH_High + 5) )
  {
          ...
          (priv->CurrentChannelBW == HT_CHANNEL_WIDTH_20 && priv->undecorated_smoothed_pwdb >= RateAdaptiveTH_Low_20M) ) )
    {
      ...
      if ( ( (priv->CurrentChannelBW != HT_CHANNEL_WIDTH_20 && priv->undecorated_smoothed_pwdb < RateAdaptiveTH_Low_40M) ||
             (priv->CurrentChannelBW == HT_CHANNEL_WIDTH_20 && priv->undecorated_smoothed_pwdb < RateAdaptiveTH_Low_20M) ) &&
...
CamRestoreAllEntry ( struct net_device * dev)
{
       ...
       (priv->ieee80211->pairwise_key_type == KEY_TYPE_WEP104) )
  {
    ...
    if (priv->ieee80211->pairwise_key_type == KEY_TYPE_TKIP)
    {
      ...
      if (priv->ieee80211->pairwise_key_type == KEY_TYPE_CCMP)
      {
  ...
  if (priv->ieee80211->group_key_type == KEY_TYPE_TKIP)
  {
    ...
    if (priv->ieee80211->group_key_type == KEY_TYPE_CCMP)
    {
      MacAddr = CAM_CONST_BROAD;
      for (EntryId = 1; EntryId < 4 ; EntryId++)
      {
...
RESET_START:
    ...
    if (ieee->state == IEEE80211_LINKED && ieee->iw_mode == IW_MODE_INFRA)
    {
      ...
      if (ieee->state == IEEE80211_LINKED && ieee->iw_mode == IW_MODE_ADHOC)
      {
        ieee->set_chan (ieee->dev, ieee->current_network.channel);
      ...
      if (ipw->u.crypt.set_tx)
      {
        ...
        { ieee->pairwise_key_type = KEY_TYPE_CCMP; }
          ...
          { ieee->pairwise_key_type = KEY_TYPE_TKIP; }
            ...
            if (strcmp (ipw->u.crypt.alg, "WEP") == 0)
            {
...
void EnableHWSecurityConfig8192 (struct net_device * dev)
{
  ...
  if ( ( (KEY_TYPE_WEP40 == ieee->pairwise_key_type) || (KEY_TYPE_WEP104 == ieee->pairwise_key_type) ) && (priv->ieee80211->auth_mode != 2) )
  {
    ...
    if ( (ieee->iw_mode == IW_MODE_ADHOC) && (ieee->pairwise_key_type & (KEY_TYPE_CCMP | KEY_TYPE_TKIP) ) )
    {
      SECR_value |= SCR_RxUseDK;
      SECR_value |= SCR_TxUseDK;
    }
