  ...
  { retValue = false; }
    ...
    { retValue = false; }
      ...
      { retValue = false; }
        ...
        if ( ( (PHT_CAPABILITY_ELE) (pHTInfo->PeerHTCapBuf) )->ChlWidth)
        { retValue = true; }
        else
        { retValue = false; }
 ...
 *  output:  none
 *  return:  tx rate
 *  notice:  quite unsure about how to use this function
 * *****************************************************************************************************************/
u16  TxCountToDataRate ( struct ieee80211_device * ieee, u8 nDataRate)
{
  ...
  else
  {
    if (nDataRate >= 0x10 && nDataRate <= 0x1f)
    {
      ...
      if (nDataRate >= 0x20  && nDataRate <= 0x2f )
      {
        is40MHz = 1;
        isShortGI = 0;
        ...
        if (nDataRate >= 0x30  && nDataRate <= 0x3f )
        {
          is40MHz = 0;
          isShortGI = 1;
          ...
          if (nDataRate >= 0x40  && nDataRate <= 0x4f )
          {
            is40MHz = 1;
            isShortGI = 1;
...
bool IsHTHalfNmodeAPs (struct ieee80211_device * ieee)
{
  ...
  { retValue = true; }
    ...
    if ( (memcmp (net->bssid, UNKNOWN_BORADCOM, 3) == 0) ||
    ...
    { pHTInfo->IOTPeer = HT_IOT_PEER_BROADCOM; }
      ...
      { pHTInfo->IOTPeer = HT_IOT_PEER_BROADCOM; }
        ...
        { pHTInfo->IOTPeer = HT_IOT_PEER_RALINK; }
          ...
          if (net->atheros_cap_exist)
          { pHTInfo->IOTPeer = HT_IOT_PEER_ATHEROS; }
          else
            if (memcmp (net->bssid, CISCO_BROADCOM, 3) == 0)
 ...
 *   input:  struct ieee80211_network *network  
 *  output:  none
...
void HTSetConnectBwModeCallback (struct ieee80211_device * ieee)
{
  ...
  if (pHTInfo->bCurBW40MHz)
  {
    ...
    { ieee->set_chan (ieee->dev, ieee->current_network.channel + 2); }
      ...
      if (pHTInfo->CurSTAExtChnlOffset == HT_EXTCHNL_OFFSET_LOWER)
      { ieee->set_chan (ieee->dev, ieee->current_network.channel - 2); }
      else
