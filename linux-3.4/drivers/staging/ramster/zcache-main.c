...
found_unbuddied:
  ...
  if (zh0->size != 0) { /* buddy0 in use, buddy1 is vacant */
    ...
    if (zh1->size != 0) { /* buddy1 in use, buddy0 is vacant */
...
static struct kobj_attribute zcache_zv_page_count_policy_percent_attr = {
  .attr = {
    .name = "zv_page_count_policy_percent",
    .mode = 0644
  },
...
int zcache_pampd_replace_in_obj (void * new_pampd, struct tmem_obj * obj)
{
  ...
  if (new_pampd != NULL) {
     ...
     * in the same node! */
      ...
      if (pampd_remote_node (new_pampd) !=
                     ...
                     void * extra)
{
  ...
  if (pampd == NULL) {
    ...
    if (unlikely (!pampd_is_remote (pampd) ) ) {
    ...
    if (node_num == -1UL) {
      ...
      if (node_num >= MANUAL_NODES) {
        ...
        if (!ramster_nodes_manual_up[node_num]) {
          ...
          if (r2net_remote_target_node_set ( (int) node_num) >= 0) {
