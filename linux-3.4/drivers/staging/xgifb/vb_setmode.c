      ...
      if (pVBInfo->VBInfo & SetCRT2ToHiVision) {
        ...
        if ( (pVBInfo->VBType & VB_SIS301LV) &&
          ...
          { VCLKIndex = XGI_YPbPr750pVCLK; }
            ...
            if (pVBInfo->VBExtInfo == YPbPr525p)
                             ...
                             struct vb_device_info * pVBInfo)
{
  ...
  if (pVBInfo->VBInfo & (SetCRT2ToLCD | XGI_SetCRT2ToLCDA) ) {
    ...
    if (pVBInfo->LCDResInfo == Panel_1024x768) {
      ...
      if (pVBInfo->LCDResInfo == Panel_1024x768x75) {
        tempax = 1024;
        tempbx = 768;
      }
      else
        if (pVBInfo->LCDResInfo == Panel_1280x1024) {
          ...
          if (pVBInfo->LCDResInfo == Panel_1280x1024x75) {
            tempax = 1280;
            tempbx = 1024;
          }
          else
...
static unsigned char XGI_IsLCDON (struct vb_device_info * pVBInfo)
{
  ...
  { return 0; }
    ...
    if (tempax & (DisableCRT2Display | SwitchCRT2 | SetSimuScanMode) )
    { return 1; }
