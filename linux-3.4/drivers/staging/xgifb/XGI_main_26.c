  ...
  case XGIFB_DISP_TV:
    ...
    case 720:
      if (xgifb_info->TV_type == TVMODE_NTSC) {
        ...
        if (xgifb_info->TV_type == TVMODE_PAL) {
      ...
      if (XGIfb_vrate[i].refresh == rate) {
        ...
        if (XGIfb_vrate[i].refresh > rate) {
          if ( (XGIfb_vrate[i].refresh - rate) <= 3) {
            ...
            if ( ( (rate - XGIfb_vrate[i - 1].refresh)
                   <= 2) && (XGIfb_vrate[i].idx
                             != 1) ) {
          ...
          if ( (rate - XGIfb_vrate[i].refresh) <= 2) {
            DPRINTK ("XGIfb: Adjusting rate from %d down to %d\n",
                     rate, XGIfb_vrate[i].refresh);
            xgifb_info->rate_idx = XGIfb_vrate[i].idx;
            break;
  ...
  case XGIFB_DISP_TV:
    if (xgifb_info->TV_type == TVMODE_HIVISION)
      ...
      if (xgifb_info->TV_plug == TVPLUG_SVIDEO)
        ...
        if (xgifb_info->TV_plug == TVPLUG_COMPOSITE)
          cr30 = (SIS_VB_OUTPUT_COMPOSITE
                  | SIS_SIMULTANEOUS_VIEW_ENABLE);
        else
          if (xgifb_info->TV_plug == TVPLUG_SCART)
  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    ...
    if (!strncmp (this_opt, "mode:", 5) ) {
      ...
      if (!strncmp (this_opt, "vesa:", 5) ) {
        ...
        if (!strncmp (this_opt, "vrate:", 6) ) {
          refresh_rate = xgifb_optval (this_opt, 6);
        }
        else
          if (!strncmp (this_opt, "rate:", 5) ) {
            refresh_rate = xgifb_optval (this_opt, 5);
          }
          else
            if (!strncmp (this_opt, "crt1off", 7) ) {
              XGIfb_crt1off = 1;
            }
            else
              if (!strncmp (this_opt, "filter:", 7) ) {
                filter = xgifb_optval (this_opt, 7);
              }
              else
                if (!strncmp (this_opt, "forcecrt2type:", 14) ) {
                  XGIfb_search_crt2type (this_opt + 14);
                }
                else
                  if (!strncmp (this_opt, "forcecrt1:", 10) ) {
                    XGIfb_forcecrt1 = xgifb_optval (this_opt, 10);
                  }
                  else
                    if (!strncmp (this_opt, "tvmode:", 7) ) {
                      XGIfb_search_tvstd (this_opt + 7);
                    }
                    else
                      if (!strncmp (this_opt, "tvstandard:", 11) ) {
                        XGIfb_search_tvstd (this_opt + 7);
                      }
                      else
                        if (!strncmp (this_opt, "dstn", 4) ) {
                          enable_dstn = 1;
                          /* TW: DSTN overrules forcecrt2type */
                          XGIfb_crt2type = XGIFB_DISP_LCD;
                        }
                        else
                          if (!strncmp (this_opt, "noypan", 6) ) {
                            XGIfb_ypan = 0;
                          }
                          else {
                            mode = this_opt;
