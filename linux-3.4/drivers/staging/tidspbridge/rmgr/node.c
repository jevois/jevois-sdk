                          ...
                          u8 ** pbuffer)
{
  ...
  { status = -EFAULT; }
    ...
    if (node_get_type (pnode) == NODE_DEVICE)
    { status = -EPERM; }
...
int node_change_priority (struct node_object * hnode, s32 prio)
{
  ...
  if (!hnode || !hnode->node_mgr) {
    ...
    { status = -EPERM; }
      ...
      if (prio < hnode_mgr->min_pri || prio > hnode_mgr->max_pri)
      { status = -EDOM; }
  }
  if (status)
  { goto func_end; }
