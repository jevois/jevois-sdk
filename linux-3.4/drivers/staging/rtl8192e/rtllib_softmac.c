  ...
  if (time) {
    if (ieee->bAwakePktSent == true) {
      ...
      { MaxPeriod = 1; }
        ...
        if (pPSC->RegMaxLPSAwakeIntvl == 0xFF)
                                      ...
                                      u8 is_mesh)
{
  ...
  if (ops == NULL && strcmp (param->u.crypt.alg, "R-WEP") == 0) {
    ...
    if (ops == NULL && strcmp (param->u.crypt.alg, "R-TKIP") == 0) {
      request_module ("rtllib_crypt_tkip");
      ops = lib80211_get_crypto_ops (param->u.crypt.alg);
    }
    else
      if (ops == NULL && strcmp (param->u.crypt.alg, "R-CCMP") == 0) {
...
skip_host_crypt:
  ...
  if (param->u.crypt.alg != NULL) {
    ...
    if (strcmp (param->u.crypt.alg, "R-WEP") == 0) {
      ...
      if (strcmp (param->u.crypt.alg, "R-TKIP") == 0) {
        sec.flags |= SEC_LEVEL;
        sec.level = SEC_LEVEL_2;
      }
      else
        if (strcmp (param->u.crypt.alg, "R-CCMP") == 0) {
          sec.flags |= SEC_LEVEL;
