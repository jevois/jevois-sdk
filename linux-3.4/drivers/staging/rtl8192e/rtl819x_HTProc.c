  ...
  { retValue = false; }
    ...
    { retValue = false; }
      ...
      { retValue = false; }
        ...
        if ( ( (struct ht_capab_ele *) (pHTInfo->PeerHTCapBuf) )->ChlWidth)
        { retValue = true; }
        else
        { retValue = false; }
...
bool IsHTHalfNmodeAPs (struct rtllib_device * ieee)
{
  ...
  { retValue = true; }
    ...
    if (!memcmp (net->bssid, UNKNOWN_BORADCOM, 3) ||
        ...
        { pHTInfo->IOTPeer = HT_IOT_PEER_RALINK; }
          ...
          { pHTInfo->IOTPeer = HT_IOT_PEER_ATHEROS; }
            ...
            { pHTInfo->IOTPeer = HT_IOT_PEER_CISCO; }
              ...
              if ( (memcmp (net->bssid, LINKSYS_MARVELL_4400N, 3) == 0) ||
                   net->marvell_cap_exist)
              { pHTInfo->IOTPeer = HT_IOT_PEER_MARVELL; }
              else
                if (net->airgo_cap_exist)
...
static void HTSetConnectBwModeCallback (struct rtllib_device * ieee)
{
  ...
  if (pHTInfo->bCurBW40MHz) {
    if (pHTInfo->CurSTAExtChnlOffset == HT_EXTCHNL_OFFSET_UPPER)
      ...
      if (pHTInfo->CurSTAExtChnlOffset ==
          HT_EXTCHNL_OFFSET_LOWER)
        ieee->set_chan (ieee->dev,
                        ieee->current_network.channel - 2);
      else
