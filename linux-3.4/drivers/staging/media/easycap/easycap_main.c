  ...
  if (0 <= input && INPUT_MANY > input) {
    ...
    if (0 <= rc) {
      ...
      { peasycap->lost[input] = 0; }
        ...
        if ( (2 * VIDEO_LOST_TOLERATE) < peasycap->lost[input])
                              ...
                              const struct usb_device_id * id)
{
  ...
  if (0 == bInterfaceNumber) {
    ...
    for (i = 0; 0xFFFFFFFF != easycap_control[i].id; i++) {
      ...
      if (V4L2_CID_BRIGHTNESS == easycap_control[i].id) {
        ...
        if (V4L2_CID_CONTRAST == easycap_control[i].id) {
          m++;
          for (k = 0; k < INPUT_MANY; k++)
          { inputset[k].contrast = value; }
        }
        else
          if (V4L2_CID_SATURATION == easycap_control[i].id) {
            m++;
            for (k = 0; k < INPUT_MANY; k++)
            { inputset[k].saturation = value; }
          }
          else
            if (V4L2_CID_HUE == easycap_control[i].id) {
              m++;
              for (k = 0; k < INPUT_MANY; k++)
  ...
  if ( (USB_CLASS_VIDEO == bInterfaceClass) ||
    ...
    if ( (USB_CLASS_AUDIO == bInterfaceClass) &&
  ...
  case 2: {
      ...
      if (9 == peasycap->audio_isoc_maxframesize) {
        ...
        if (256 == peasycap->audio_isoc_maxframesize) {
          peasycap->ilk &= ~0x02;
