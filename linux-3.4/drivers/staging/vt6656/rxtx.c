...
s_vSaveTxPktInfo (PSDevice pDevice, BYTE byPktNum, PBYTE pbyDestAddr, WORD wPktLength, WORD wFIFOCtl)
{
  ...
  { pStatistic->abyTxPktInfo[byPktNum].byBroadMultiUni = TX_PKT_BROAD; }
    ...
    if (is_multicast_ether_addr (pbyDestAddr) )
...
)
{
  ...
  case RTSDUR_BA_F0:
    ...
    if ( (byFBOption == AUTO_FB_0) && (wRate >= RATE_18M) && (wRate <= RATE_54M) ) {
      ...
      if ( (byFBOption == AUTO_FB_1) && (wRate >= RATE_18M) && (wRate <= RATE_54M) ) {
  ...
  if (byPktType == PK_TYPE_11GB || byPktType == PK_TYPE_11GA) {
    ...
    if (byPktType == PK_TYPE_11A) {
      ...
      if (pvRTS != NULL) {//RTS_need, non PCF mode
        ...
        if (pvRTS == NULL) {//RTS_needless, non PCF mode
          if (pvRrvTime) {
            PSRrvTime_ab pBuf = (PSRrvTime_ab) pvRrvTime;
...
)
{
  ...
  if (wCurrentRate >= RATE_18M) {
    if (pDevice->byAutoFBCtrl == AUTO_FB_0) {
      ...
      if (pDevice->byAutoFBCtrl == AUTO_FB_1) {
  ...
  if ( (bNeedEncryption == TRUE) && (pTransmitKey != NULL) && (pTransmitKey->byCipherSuite == KEY_CTL_TKIP) ) {
    ...
    if (pDevice->sMgmtObj.eAuthenMode == WMAC_AUTH_WPANONE) {
      ...
      if ( (pTransmitKey->dwKeyIndex & AUTHENTICATOR_KEY) != 0) {
        dwMICKey0 = * (PDWORD) (&pTransmitKey->abyKey[16]);
        dwMICKey1 = * (PDWORD) (&pTransmitKey->abyKey[20]);
      }
      else {
  ...
  if (bSoftWEP == TRUE) {
    ...
    if (  ( (pDevice->eEncryptionStatus == Ndis802_11Encryption1Enabled) && (bNeedEncryption == TRUE) )  ||
...
)
{
  ...
  if (byPktType == PK_TYPE_11A) {//0000 0000 0000 0000
    ...
    if (byPktType == PK_TYPE_11B) {//0000 0001 0000 0000
      pTxBufHead->wFIFOCtl |= FIFOCTL_11B;
    }
    else
      if (byPktType == PK_TYPE_11GB) {//0000 0010 0000 0000
        pTxBufHead->wFIFOCtl |= FIFOCTL_11GB;
      }
      else
        if (byPktType == PK_TYPE_11GA) {//0000 0011 0000 0000
    ...
    if (pDevice->sTxEthHeader.wType == cpu_to_be16 (ETH_P_PAE) ) {
      ...
      if ( ( (Protocol_Version == 1) || (Protocol_Version == 2) ) &&
        ...
        if (! (Key_info & BIT3) &&
          ...
          if ( (Key_info & BIT3) && (Descriptor_type == 2) &&
               (Key_info & BIT8) && (Key_info & BIT9) ) {
            pDevice->fWPA_Authened = TRUE;
