  ...
  if ( (pRSN->len >= 6)
    ...
    if (pRSN->len >= 10)
    {
      ...
      { pBSSList->byGKType = WPA_WEP40; }
        ...
        if ( !memcmp (pRSN->abyMulticast, abyOUI02, 4) )
        { pBSSList->byGKType = WPA_TKIP; }
        else
          if ( !memcmp (pRSN->abyMulticast, abyOUI03, 4) )
          { pBSSList->byGKType = WPA_AESWRAP; }
          else
            if ( !memcmp (pRSN->abyMulticast, abyOUI04, 4) )
            { pBSSList->byGKType = WPA_AESCCMP; }
            else
              if ( !memcmp (pRSN->abyMulticast, abyOUI05, 4) )
      ...
      for (i = 0; (i < pRSN->wPKCount) &&
        ...
        if (pRSN->len >= 12 + i * 4 + 4) {
          ...
          { pBSSList->abyPKType[j++] = WPA_NONE; }
            ...
            if ( !memcmp (pRSN->PKSList[i].abyOUI, abyOUI02, 4) )
            { pBSSList->abyPKType[j++] = WPA_TKIP; }
            else
              if ( !memcmp (pRSN->PKSList[i].abyOUI, abyOUI03, 4) )
              { pBSSList->abyPKType[j++] = WPA_AESWRAP; }
              else
                if ( !memcmp (pRSN->PKSList[i].abyOUI, abyOUI04, 4) )
      ...
      for (i = 0; (i < pIE_RSN_Auth->wAuthCount) &&
        ...
        if (pRSN->len >= 14 + 4 + (m + i) * 4) {
          ...
          { pBSSList->abyAuthType[j++] = WPA_AUTH_IEEE802_1X; }
            ...
            if ( !memcmp (pIE_RSN_Auth->AuthKSList[i].abyOUI, abyOUI02, 4) )
    ...
    if (pBSSList->wPKCount > 0) {
      for (ii = 0; ii < pBSSList->wPKCount; ii ++) {
        ...
        { byPKType = WPA_AESCCMP; }
          ...
          if ( (pBSSList->abyPKType[ii] == WPA_TKIP) && (byPKType != WPA_AESCCMP) )
          { byPKType = WPA_TKIP; }
          else
            if ( (pBSSList->abyPKType[ii] == WPA_WEP40) && (byPKType != WPA_AESCCMP) && (byPKType != WPA_TKIP) )
            { byPKType = WPA_WEP40; }
            else
              if ( (pBSSList->abyPKType[ii] == WPA_WEP104) && (byPKType != WPA_AESCCMP) && (byPKType != WPA_TKIP) )
              { byPKType = WPA_WEP104; }
      }
