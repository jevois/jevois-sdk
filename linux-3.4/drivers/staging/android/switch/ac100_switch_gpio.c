...
static irqreturn_t hook_volume_irq (int irq, void * data)
{
  ...
  if ( (reg_val >= 0x19) && switch_data->state == 1) {
    ...
    if ( (reg_val == 0x15 || reg_val == 0x14) && switch_data->state == 1) {
      ...
      if ( (reg_val == 0x18 || reg_val == 0x17) && switch_data->state == 1) {
        ...
        if (reg_val < 0xb && switch_data->state == 1) {
          if (KEY_HOOK_FLAG == 1) {
