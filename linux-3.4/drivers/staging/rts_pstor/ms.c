                             ...
                             u8 cfg, int mode_2k, int use_sg, void * buf, int buf_len)
{
  ...
  if (trans_mode == MS_TM_AUTO_READ) {
    ...
    if (trans_mode == MS_TM_AUTO_WRITE) {
      dir = DMA_TO_DEVICE;
...
static int ms_set_init_para (struct rtsx_chip * chip)
{
  ...
  if (CHK_HG8BIT (ms_card) ) {
    ...
    if (CHK_MSPRO (ms_card) || CHK_MS4BIT (ms_card) ) {
      if (chip->asic_code) {
...
static int reset_ms_pro (struct rtsx_chip * chip)
{
  ...
  { change_power_class = 2; }
    ...
    if (chip->ms_power_class_en & 0x01)
...
RE_SEARCH:
  ...
  if (ms_card->block_shift == 4)  { /* 4MB or 8MB */
    ...
    if (j < 5)  { /* Effective block for 16MB: 0x3E0 */
      ...
      if (j < 0xA) { /* Effective block for 32MB: 0x7C0 */
        ms_card->capacity = 0xF7C0;
      }
      else
        if (j < 0x11) { /* Effective block for 64MB: 0xF80 */
...
void ms_cleanup_work (struct rtsx_chip * chip)
{
  ...
  if (CHK_MSPRO (ms_card) ) {
    ...
    if ( (!CHK_MSPRO (ms_card) ) && ms_card->delay_write.delay_write_flag) {
