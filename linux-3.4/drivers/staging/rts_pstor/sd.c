    ...
    if (CHK_SD_SDR104 (sd_card) || CHK_SD_SDR50 (sd_card) ) {
      ...
      if (CHK_SD_DDR50 (sd_card) || CHK_MMC_DDR52 (sd_card) ) {
        ...
        if ( (chip->sd_ctl & SD_SAMPLE_POINT_CTL_MASK) == SD_SAMPLE_POINT_AUTO) {
          ...
          if ( (chip->sd_ctl & SD_SAMPLE_POINT_CTL_MASK) == SD_SAMPLE_POINT_DELAY) {
            val = SD20_RX_14_DELAY;
          }
          else {
            val = SD20_RX_POS_EDGE;
    ...
    if (CHK_SD_DDR50 (sd_card) ) {
      ...
      if (CHK_SD_SDR50 (sd_card) ) {
        ...
        if (CHK_SD_HS (sd_card) ) {
          ...
          if (CHK_MMC_52M (sd_card) || CHK_MMC_DDR52 (sd_card) ) {
            if (chip->asic_code) {
...
static int sd_set_clock_divider (struct rtsx_chip * chip, u8 clk_div)
{
  ...
  if (CHECK_PID (chip, 0x5209) ) {
    ...
    if (clk_div == SD_CLK_DIVIDE_0) {
      ...
      if (clk_div == SD_CLK_DIVIDE_128) {
        val = 0x40;
      }
      else
        if (clk_div == SD_CLK_DIVIDE_256) {
          val = 0x20;
        }
  }
  ...
  if (func_to_switch) {
    ...
    if (retval != STATUS_SUCCESS) {
      if (func_to_switch == SDR104_SUPPORT) {
        ...
        if (func_to_switch == DDR50_SUPPORT) {
...
int sd_rw (struct scsi_cmnd * srb, struct rtsx_chip * chip, u32 start_sector, u16 sector_cnt)
{
  ...
  if (CHK_MMC_8BIT (sd_card) ) {
    ...
    if (CHK_MMC_4BIT (sd_card) || CHK_SD (sd_card) ) {
...
RW_FAIL:
  ...
  if (sd_check_err_code (chip, SD_CRC_ERR) ) {
    ...
    if (sd_check_err_code (chip, SD_TO_ERR | SD_STS_ERR) ) {
      retval = reset_sd_card (chip);
      if (retval != STATUS_SUCCESS) {
        chip->card_ready &= ~SD_CARD;
        chip->card_fail |= SD_CARD;
...
int sd_execute_no_data (struct scsi_cmnd * srb, struct rtsx_chip * chip)
{
  ...
  if ( (sd_card->sd_lock_status & SD_LOCK_1BIT_MODE) == 0) {
    if (CHK_MMC_8BIT (sd_card) ) {
      ...
      if (CHK_SD (sd_card) || CHK_MMC_4BIT (sd_card) ) {
...
int sd_get_cmd_rsp (struct scsi_cmnd * srb, struct rtsx_chip * chip)
{
  ...
  if (sd_card->last_rsp_type == SD_RSP_TYPE_R0) {
    ...
    if (sd_card->last_rsp_type == SD_RSP_TYPE_R2) {
