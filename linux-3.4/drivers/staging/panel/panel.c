...
static inline int handle_lcd_special_code (void)
{
  ...
  if (oldflags != lcd_flags) {
    ...
    /* check whether one of F,N flags was changed */
      ...
      if ( (oldflags ^ lcd_flags) & (LCD_FLAG_F | LCD_FLAG_N) )
        lcd_write_cmd (0x30
                       | ( (lcd_flags & LCD_FLAG_F) ? 4 : 0)
                       | ( (lcd_flags & LCD_FLAG_N) ? 8 : 0) );
      /* check wether L flag was changed */
      else
        if ( (oldflags ^ lcd_flags) & (LCD_FLAG_L) ) {
...
void lcd_init (void)
{
  ...
  if (lcd_proto == LCD_PROTO_SERIAL) {  /* SERIAL */
    ...
    if (lcd_proto == LCD_PROTO_PARALLEL) { /* PARALLEL */
...
static inline void input_state_falling (struct logical_input * input)
{
  ...
  if ( (phys_curr & input->mask) == input->value) {
    ...
    if (input->fall_timer >= input->fall_time) {
      /* call release event */
      if (input->type == INPUT_TYPE_STD) {
