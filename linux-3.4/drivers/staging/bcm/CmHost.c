...
static VOID CopyToAdapter (register PMINI_ADAPTER Adapter, /* <Pointer to the Adapter structure */
  ...
  if (psfLocalSet->u32MaxSustainedTrafficRate == 0) {
    ...
    if (ntohl (psfLocalSet->u32MaxSustainedTrafficRate) > WIMAX_MAX_ALLOWED_RATE) {
  ...
  case DSA_ACK:
    {
      ...
      if ( (uiSearchRuleIndex < NO_OF_QUEUES) ) {
        ...
        if (!psfLocalSet) {
          ...
          if (psfLocalSet->bValid && (pstAddIndication->u8CC == 0) ) {
      ...
      if ( (uiSearchRuleIndex < NO_OF_QUEUES) ) {
        ...
        if (pstChangeIndication->sfActiveSet.bValid == FALSE) {
          ...
          { psfLocalSet = &pstChangeIndication->sfAdmittedSet; }
            ...
            if (pstChangeIndication->sfAuthorizedSet.bValid)
