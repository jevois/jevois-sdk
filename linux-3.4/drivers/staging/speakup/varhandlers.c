...
int chartab_get_value (char * keyword)
{
  ...
  { value = ALPHA; }
    ...
    if (!strcmp (keyword, "B_CTL") )
    { value = B_CTL; }
    else
      if (!strcmp (keyword, "WDLM") )
      { value = WDLM; }
      else
        if (!strcmp (keyword, "A_PUNC") )
        { value = A_PUNC; }
        else
          ...
          { value = PUNC; }
            ...
            { value = NUM; }
              ...
              if (!strcmp (keyword, "A_CAP") )
              { value = A_CAP; }
              else
                if (!strcmp (keyword, "B_CAPSYM") )
                { value = B_CAPSYM; }
                else
                  if (!strcmp (keyword, "B_SYM") )
                  { value = B_SYM; }
  return value;
}
