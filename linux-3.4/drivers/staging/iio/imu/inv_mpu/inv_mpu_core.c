                                ...
                                const struct i2c_device_id * id)
{
  ...
  { st->chip_type = INV_ITG3500; }
    ...
    if (!strcmp (id->name, "mpu3050") )
    { st->chip_type = INV_MPU3050; }
    else
      if (!strcmp (id->name, "mpu6050") )
      { st->chip_type = INV_MPU6050; }
      else
        if (!strcmp (id->name, "mpu9150") )
        { st->chip_type = INV_MPU6050; }
        else
          if (!strcmp (id->name, "mpu6500") )
          { st->chip_type = INV_MPU6500; }
          else
            if (!strcmp (id->name, "mpu6515") )
            { st->chip_type = INV_MPU6500; }
            else
              if (!strcmp (id->name, "mpu9250") )
              { st->chip_type = INV_MPU6500; }
              else
...
static int invsens_detect (struct i2c_client * client, struct i2c_board_info * info)
{
  ...
  if (sensor_info.twi_id == adapter->nr) {
    while ( i < 2) {
      ...
      if ( ( (ret & 0x00FF) == MPU6500_ID) || ( (ret & 0xff) == 0x7d) ) {
        printk ( "%s: MPU6500(i2c addr:0x%x) is detected!\n ", __func__, client->addr);
        addr = client->addr;
