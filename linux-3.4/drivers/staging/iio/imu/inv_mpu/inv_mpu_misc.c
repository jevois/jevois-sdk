...
#define DEF_SELFTEST_ACCL_FULL_SCALE         (0)//(0)//+-2g
...
int inv_get_silicon_rev_mpu6050 (struct inv_mpu_iio_s * st)
{
  ...
  if (sw_rev == 0) {
    ...
    if (sw_rev < ARRAY_SIZE (sw_rev_map) ) {
      p_rev = (struct prod_rev_map_t *) &sw_rev_map[sw_rev];
    }
    else {
...
#define INV_DO_TEST_DEBUG   0
  ...
  /* setup parameters */
  result = inv_i2c_single_write (st, REG_6500_ACCEL_CONFIG2, 2); 
  if (result)
  { return result; }
...
#define ACCEL_BIAS_THRESHOLD_SMT      500
#define GYRO_BIAS_THRESHOLD_SMT      20
...
static int inv_gyro_dmp_cal (struct inv_mpu_iio_s * st)
{
  ...
  { regs[0] = tmpD; }
    ...
    if ( (inv_gyro_orient & 3) == 1)
    { regs[0] = tmpE; }
    else
      if ( (inv_gyro_orient & 3) == 2)
      { regs[0] = tmpF; }
  if ( (inv_gyro_orient & 0x18) == 0)
  { regs[1] = tmpD; }
  else
    if ( (inv_gyro_orient & 0x18) == 0x8)
    { regs[1] = tmpE; }
    else
      if ( (inv_gyro_orient & 0x18) == 0x10)
      { regs[1] = tmpF; }
  if ( (inv_gyro_orient & 0xc0) == 0)
  { regs[2] = tmpD; }
  else
    if ( (inv_gyro_orient & 0xc0) == 0x40)
    { regs[2] = tmpE; }
    else
      if ( (inv_gyro_orient & 0xc0) == 0x80)
      { regs[2] = tmpF; }
