...
s32 r8712_signal_scale_mapping (s32 cur_sig)
{
  ...
  { ret_sig = 100; }
    ...
    if (cur_sig >= 41 && cur_sig <= 50)
    { ret_sig = 80 + ( (cur_sig - 40) * 2); }
    else
      if (cur_sig >= 31 && cur_sig <= 40)
      { ret_sig = 66 + (cur_sig - 30); }
      else
        if (cur_sig >= 21 && cur_sig <= 30)
        { ret_sig = 54 + (cur_sig - 20); }
        else
          if (cur_sig >= 10 && cur_sig <= 20)
          { ret_sig = 42 + ( ( (cur_sig - 10) * 2) / 3); }
          else
            if (cur_sig >= 5 && cur_sig <= 9)
            { ret_sig = 22 + ( ( (cur_sig - 5) * 3) / 2); }
            else
              if (cur_sig >= 1 && cur_sig <= 4)
                                 ...
                                 union recv_frame * prframe)
{
  ...
  if (bcck_rate) {
    ...
    { pwdb_all -= 2; }
      ...
      if (pwdb_all > 26 && pwdb_all <= 34)
      { pwdb_all -= 6; }
      else
        if (pwdb_all > 14 && pwdb_all <= 26)
        { pwdb_all -= 8; }
        else
          if (pwdb_all > 4 && pwdb_all <= 14)
          { pwdb_all -= 4; }
    /*
