...
static int sd_check_csd (struct rts51x_chip * chip, char check_wp)
{
  ...
  if ( (trans_speed & 0x07) == 0x02) { /* 10Mbits/s */
    if ( (trans_speed & 0xf8) >= 0x30) { /* >25Mbits/s */
      ...
      if ( (trans_speed & 0xf8) == 0x28) { /* 20Mbits/s */
        if (chip->asic_code)
        { sd_card->sd_clock = 39; }
        else
        ...
        if ( (trans_speed & 0xf8) == 0x20) { /* 15Mbits/s */
          if (chip->asic_code)
          { sd_card->sd_clock = 29; }
          else
          ...
          if ( (trans_speed & 0xf8) >= 0x10) { /* 12Mbits/s */
            if (chip->asic_code)
            { sd_card->sd_clock = 23; }
            else
            ...
            if ( (trans_speed & 0x08) >= 0x08) { /* 10Mbits/s */
              if (chip->asic_code)
              { sd_card->sd_clock = 19; }
              else
              { sd_card->sd_clock = CLK_20; }
...
static int mmc_switch_timing_bus (struct rts51x_chip * chip)
{
  ...
  if (mmc_test_switch_bus (chip, MMC_8BIT_BUS) == STATUS_SUCCESS) {
    ...
    if (mmc_test_switch_bus (chip, MMC_4BIT_BUS) == STATUS_SUCCESS) {
...
static inline int sd_auto_tune_clock (struct rts51x_chip * chip)
{
  ...
  if (chip->asic_code) {
    ...
    { sd_card->sd_clock = CLK_80; }
      ...
      if (sd_card->sd_clock == CLK_80)
      { sd_card->sd_clock = CLK_60; }
      else
        if (sd_card->sd_clock == CLK_60)
        { sd_card->sd_clock = CLK_50; }
  }
