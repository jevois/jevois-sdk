                              ...
                              struct attribute * attr, char * data)
{
  ...
  if (strcmp (attr->name, "packets_started") == 0) {
    ...
    if (strcmp (attr->name, "packets_finished") == 0) {
      n = sprintf (data, "%lu\n", pd->stats.pkt_ended);
      
    }
    else
      if (strcmp (attr->name, "kb_written") == 0) {
        n = sprintf (data, "%lu\n", pd->stats.secs_w >> 1);
        
      }
      else
        if (strcmp (attr->name, "kb_read") == 0) {
          n = sprintf (data, "%lu\n", pd->stats.secs_r >> 1);
          
        }
        else
          if (strcmp (attr->name, "kb_read_gather") == 0) {
            n = sprintf (data, "%lu\n", pd->stats.secs_rg >> 1);
            ...
            if (strcmp (attr->name, "size") == 0) {
              ...
              if (strcmp (attr->name, "congestion_off") == 0) {
                spin_lock (&pd->lock);
                ...
                if (strcmp (attr->name, "congestion_on") == 0) {
                  spin_lock (&pd->lock);
...
static int pkt_set_segment_merging (struct pktcdvd_device * pd, struct request_queue * q)
{
  if ( (pd->settings.size << 9) / CD_FRAMESIZE
    ...
    if ( (pd->settings.size << 9) / PAGE_SIZE
         <= queue_max_segments (q) ) {
      /*
...
static noinline_for_stack int pkt_set_write_settings (struct pktcdvd_device * pd)
{
  ...
  if (wp->data_block_type == PACKET_BLOCK_MODE1) {
    ...
    if (wp->data_block_type == PACKET_BLOCK_MODE2) {
...
static void pkt_make_request (struct request_queue * q, struct bio * bio)
{
  ...
  if (was_empty) {
    ...
    if (!list_empty (&pd->cdrw.pkt_free_list) && !blocked_bio) {
      /*
...
static int pkt_seq_show (struct seq_file * m, void * p)
{
  ...
  { msg = "Mode 1"; }
    ...
    if (pd->settings.block_mode == PACKET_BLOCK_MODE2)
    { msg = "Mode 2"; }
    else
    { msg = "Unknown"; }
