...
is_valid_state (struct drbd_conf * mdev, union drbd_state ns)
{
  ...
  if (rv <= 0)
    ...
    if (ns.role == R_SECONDARY && mdev->open_cnt)
    { rv = SS_DEVICE_IN_USE; }
    
    else
      if (ns.role == R_PRIMARY && ns.conn < C_CONNECTED && ns.disk < D_UP_TO_DATE)
      { rv = SS_NO_UP_TO_DATE_DISK; }
        ...
        { rv = SS_PRIMARY_NOP; }
          ...
          if (ns.role == R_PRIMARY && ns.disk <= D_INCONSISTENT && ns.pdsk <= D_INCONSISTENT)
          { rv = SS_NO_UP_TO_DATE_DISK; }
          
          else
            if (ns.conn > C_CONNECTED && ns.disk < D_INCONSISTENT)
            { rv = SS_NO_LOCAL_DISK; }
            
            else
              if (ns.conn > C_CONNECTED && ns.pdsk < D_INCONSISTENT)
              { rv = SS_NO_REMOTE_DISK; }
              
              else
                if (ns.conn > C_CONNECTED && ns.disk < D_UP_TO_DATE && ns.pdsk < D_UP_TO_DATE)
                { rv = SS_NO_UP_TO_DATE_DISK; }
                  ...
                  { rv = SS_CONNECTED_OUTDATES; }
                    ...
                    { rv = SS_NO_VERIFY_ALG; }
                      ...
                      { rv = SS_NOT_SUPPORTED; }
                        ...
                        if (ns.conn >= C_CONNECTED && ns.pdsk == D_UNKNOWN)
                        { rv = SS_CONNECTED_OUTDATES; }
