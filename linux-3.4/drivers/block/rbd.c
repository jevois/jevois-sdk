...
static int parse_rbd_opts_token (char * c, void * private)
{
  ...
  if (token < Opt_last_int) {
    ...
    if (token > Opt_last_int && token < Opt_last_string) {
      dout ("got string token %d val %s\n", token,
            argstr[0].from);
    }
    else
      if (token > Opt_last_string && token < Opt_last_bool) {
...
static void rbd_req_cb (struct ceph_osd_request * req, struct ceph_msg * msg)
{
  ...
  if (rc == -ENOENT && read_op) {
    ...
    if (rc == 0 && read_op && bytes < req_data->len) {
                               ...
                               size_t options_size)
{
  ...
  *mon_addrs = buf;
  ...
  if (!len)
    ...
    if (len >= sizeof (rbd_dev->snap_name) )
    { return -EINVAL; }
