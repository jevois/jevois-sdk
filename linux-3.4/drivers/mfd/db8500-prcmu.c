...
int db8500_prcmu_request_clock (u8 clock, bool enable)
{
  ...
  { return request_sga_clock (clock, enable); }
    ...
    { return request_clock (clock, enable); }
      ...
      if (clock == PRCMU_TIMCLK)
      { return request_timclk (enable); }
      else
        if ( (clock == PRCMU_DSI0CLK) || (clock == PRCMU_DSI1CLK) )
        { return request_dsiclk ( (clock - PRCMU_DSI0CLK), enable); }
        else
          if ( (PRCMU_DSI0ESCCLK <= clock) && (clock <= PRCMU_DSI2ESCCLK) )
          { return request_dsiescclk ( (clock - PRCMU_DSI0ESCCLK), enable); }
          else
            if (clock == PRCMU_PLLDSI)
            { return request_plldsi (enable); }
            else
              if (clock == PRCMU_SYSCLK)
              { return request_sysclk (enable); }
              else
                if ( (clock == PRCMU_PLLSOC0) || (clock == PRCMU_PLLSOC1) )
...
static unsigned long clock_rate (u8 clock)
{
  ...
  { rate = pll_rate (PRCM_PLLSOC0_FREQ, rate, clk_mgt[clock].branch); }
    ...
    { rate = pll_rate (PRCM_PLLSOC1_FREQ, rate, clk_mgt[clock].branch); }
      ...
      if (pllsw == PRCM_CLK_MGT_CLKPLLSW_DDR)
...
static unsigned long clock_source_rate (u32 clk_mgt_val, int branch)
{
  ...
  { return pll_rate (PRCM_PLLSOC0_FREQ, ROOT_CLOCK_RATE, branch); }
    ...
    if (clk_mgt_val == PRCM_CLK_MGT_CLKPLLSW_SOC1)
    { return pll_rate (PRCM_PLLSOC1_FREQ, ROOT_CLOCK_RATE, branch); }
    else
      if (clk_mgt_val == PRCM_CLK_MGT_CLKPLLSW_DDR)
