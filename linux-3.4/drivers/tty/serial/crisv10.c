  ...
  *info->iclrintradr =
  ...
  if (rstat & SER_ERROR_MASK) {
    ...
    { add_char_and_flag (info, data, TTY_PARITY); }
      ...
      if (rstat & SER_OVERRUN_MASK)
      { add_char_and_flag (info, data, TTY_OVERRUN); }
      else
        if (rstat & SER_FRAMING_ERR_MASK)
    ...
    if ( ( (data_read & IO_MASK (R_SERIAL0_READ, data_in) ) == 0) &&
      ...
      if (info->break_detected_cnt) {
        ...
        if (data_read & IO_MASK (R_SERIAL0_READ, par_err) ) {
          ...
          if (data_read & IO_MASK (R_SERIAL0_READ, overrun) ) {
            info->icount.overrun++;
            flag = TTY_OVERRUN;
          }
          else
  ...
  if ( (info->flags & ASYNC_SPD_MASK) == ASYNC_SPD_CUST) {
    ...
    if (info->baud_base == SERIAL_PRESCALE_BASE) {
      ...
      *R_SERIAL_PRESCALE = divisor;
      ...
      if ( (info->baud_base == CONFIG_ETRAX_EXTERN_PB6CLK_FREQ / 8 &&
  ...
  if (info->count == 1) {
    ...
    if (info->dma_in_enabled) {
      if (request_irq (info->dma_in_irq_nbr,
        ...
        if (cris_request_dma (info->dma_in_nbr,
                              info->dma_in_irq_description,
    ...
    if (info->dma_out_enabled) {
      if (request_irq (info->dma_out_irq_nbr,
        ...
        if (cris_request_dma (info->dma_out_nbr,
                              info->dma_out_irq_description,
                              DMA_VERBOSE_ON_ERROR,
                              info->dma_owner) ) {
          free_irq (info->dma_out_irq_nbr, info);
