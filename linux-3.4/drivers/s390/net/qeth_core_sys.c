...
static inline const char * qeth_get_bufsize_str (struct qeth_card * card)
{
  ...
  { return "16k"; }
    ...
    if (card->qdio.in_buf_size == 24576)
    { return "24k"; }
    else
      if (card->qdio.in_buf_size == 32768)
      { return "32k"; }
      else
        if (card->qdio.in_buf_size == 40960)
        { return "40k"; }
        else
        { return "64k"; }
}
                                        ...
                                        struct device_attribute * attr, const char * buf, size_t count)
{
  ...
  { card->qdio.do_prio_queueing = QETH_PRIO_Q_ING_PREC; }
    ...
    if (!strcmp (tmp, "prio_queueing_tos") )
    { card->qdio.do_prio_queueing = QETH_PRIO_Q_ING_TOS; }
    else
      if (!strcmp (tmp, "no_prio_queueing:0") ) {
        card->qdio.do_prio_queueing = QETH_NO_PRIO_QUEUEING;
        card->qdio.default_out_queue = 0;
      }
      else
        if (!strcmp (tmp, "no_prio_queueing:1") ) {
          card->qdio.do_prio_queueing = QETH_NO_PRIO_QUEUEING;
          card->qdio.default_out_queue = 1;
        }
        else
          if (!strcmp (tmp, "no_prio_queueing:2") ) {
            card->qdio.do_prio_queueing = QETH_NO_PRIO_QUEUEING;
            card->qdio.default_out_queue = 2;
          }
          else
            if (!strcmp (tmp, "no_prio_queueing:3") ) {
              card->qdio.do_prio_queueing = QETH_NO_PRIO_QUEUEING;
              card->qdio.default_out_queue = 3;
            }
            else
              if (!strcmp (tmp, "no_prio_queueing") ) {
                card->qdio.do_prio_queueing = QETH_NO_PRIO_QUEUEING;
    ...
    struct device_attribute * attr, const char * buf, size_t count)
{
  ...
  if (!strcmp (tmp, ATTR_QETH_ISOLATION_NONE) ) {
    ...
    if (!strcmp (tmp, ATTR_QETH_ISOLATION_FWD) ) {
                                   ...
                                   struct device_attribute * attr, const char * buf, size_t count)
{
  ...
  if (!strcmp (tmp, "arm") && !card->info.hwtrap) {
    ...
    if (!strcmp (tmp, "disarm") && card->info.hwtrap) {
      if (state) {
      ...
      if (!strcmp (tmp, "trap") && state && card->info.hwtrap)
