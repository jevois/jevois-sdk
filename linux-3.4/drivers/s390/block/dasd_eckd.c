...
static int dasd_eckd_identify_conf_parts (struct dasd_eckd_private * private)
{
  ...
  for (i = 0; i < count; ++i) {
    ...
    { private->sneq = sneq; }
      ...
      if (sneq->flags.identifier == 1 && sneq->format == 4)
      { private->vdsneq = (struct vd_sneq *) sneq; }
      else
        if (sneq->flags.identifier == 2)
        { private->gneq = (struct dasd_gneq *) sneq; }
        else
          if (sneq->flags.identifier == 3 && sneq->res1 == 1)
...
static int dasd_eckd_analysis_evaluation (struct dasd_ccw_req * init_cqr)
{
  ...
  { return INIT_CQR_OK; }
    ...
    if (init_cqr->status == DASD_CQR_NEED_ERP ||
    ...
    struct request * req)
{
  ...
  if (cdlspecial || dasd_page_cache) {
    ...
    if ( (data_size <= private->fcx_max_data)
