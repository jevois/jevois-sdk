...
static int __init intelfb_setup (char * options)
{
  ...
  while ( (this_opt = strsep (&options, ",") ) ) {
    ...
    if (get_opt_bool (this_opt, "accel", &accel) )
      ...
      if (get_opt_int (this_opt, "vram", &vram) )
        ;
      else
        if (get_opt_bool (this_opt, "hwcursor", &hwcursor) )
          ;
        else
          if (get_opt_bool (this_opt, "mtrr", &mtrr) )
            ;
          else
            if (get_opt_bool (this_opt, "fixed", &fixed) )
              ;
            else
              if (get_opt_bool (this_opt, "init", &noinit) )
              { noinit = !noinit; }
              else
                if (OPT_EQUAL (this_opt, "mode=") )
