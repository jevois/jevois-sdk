    ...
    struct fb_info * info)
{
  ...
  if ( (task->t.regs.ebx & 0x3) == 3) {
    ...
    if ( (task->t.regs.ebx & 0x3) == 2) {
      printk (KERN_INFO "uvesafb: VBIOS/hardware supports DDC2 "
              "transfers\n");
    }
    else
      if ( (task->t.regs.ebx & 0x3) == 1) {
...
static int __devinit uvesafb_setup (char * options)
{
  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    ...
    { ypan = 0; }
      ...
      if (!strcmp (this_opt, "ypan") )
      { ypan = 1; }
      else
        if (!strcmp (this_opt, "ywrap") )
        { ypan = 2; }
        else
          if (!strcmp (this_opt, "vgapal") )
          { pmi_setpal = 0; }
          else
            if (!strcmp (this_opt, "pmipal") )
            { pmi_setpal = 1; }
            else
              if (!strncmp (this_opt, "mtrr:", 5) )
              { mtrr = simple_strtoul (this_opt + 5, NULL, 0); }
              else
                if (!strcmp (this_opt, "nomtrr") )
                { mtrr = 0; }
                else
                  if (!strcmp (this_opt, "nocrtc") )
                  { nocrtc = 1; }
                  else
                    if (!strcmp (this_opt, "noedid") )
                    { noedid = 1; }
                    else
                      if (!strcmp (this_opt, "noblank") )
                      { blank = 0; }
                      else
                        if (!strncmp (this_opt, "vtotal:", 7) )
                        { vram_total = simple_strtoul (this_opt + 7, NULL, 0); }
                        else
                          if (!strncmp (this_opt, "vremap:", 7) )
                          { vram_remap = simple_strtoul (this_opt + 7, NULL, 0); }
                          else
                            if (!strncmp (this_opt, "maxhf:", 6) )
                            { maxhf = simple_strtoul (this_opt + 6, NULL, 0); }
                            else
                              if (!strncmp (this_opt, "maxvf:", 6) )
                              { maxvf = simple_strtoul (this_opt + 6, NULL, 0); }
                              else
                                if (!strncmp (this_opt, "maxclk:", 7) )
                                { maxclk = simple_strtoul (this_opt + 7, NULL, 0); }
                                else
                                  if (!strncmp (this_opt, "vbemode:", 8) )
