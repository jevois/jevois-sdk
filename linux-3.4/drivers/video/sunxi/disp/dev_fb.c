...
static u32 Fb_reset_disp_rsl (disp_init_para * init_para)
{
  ...
  if (DISP_OUTPUT_TYPE_HDMI == type) {
    ...
    if (DISP_OUTPUT_TYPE_TV == type) {
      valid &= ( ( (DISP_TV_MOD_PAL == mode) || (DISP_TV_MOD_NTSC == mode) ) ? 1 : 0);
    }
    else
      if (DISP_OUTPUT_TYPE_VGA == type) {
        valid = 0;
      }
      else {
        valid = 0;
...
static int Fb_map_boot_logo (__u32 sel, struct fb_info * info)
{
  ...
  src_phy_addr = bsp_disp_layer_get_addr (2, 0);
  ...
  if (var->nonstd == 0)
  {
    ...
    case 32:
      ...
      { fb->format = DISP_FORMAT_ARGB_8888; }
        ...
        if (var->blue.offset == 24 && var->green.offset == 16 && var->red.offset == 8)
        { fb->format = DISP_FORMAT_BGRA_8888; }
        else
          if (var->blue.offset == 16 && var->green.offset == 8 && var->red.offset == 0)
          { fb->format = DISP_FORMAT_ABGR_8888; }
          else
            if (var->red.offset == 24 && var->green.offset == 16 && var->blue.offset == 8)
    ...
    case 24:
      if (var->red.offset == 16 && var->green.offset == 8 && var->blue.offset == 0) {
        ...
        if (var->blue.offset == 16 && var->green.offset == 8 && var->red.offset == 0) {//bgr
    ...
    for (i = 0; i < fb_num; i++) {
      ...
      if (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_SCREEN0) {
        ...
        if (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_SCREEN1) {
          ...
          if (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN) {
            if (i == 0) {
              fb_para.fb_mode = FB_MODE_SCREEN0;
            }
            else {
            ...
            if (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_TWO_SAME_SCREEN) {
              fb_para.fb_mode = FB_MODE_DUAL_SAME_SCREEN_TB;
              fb_para.height *= 2;
              fb_para.output_height *= 2;
            }
            else
              if (g_fbi.disp_init.disp_mode == DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS) {
