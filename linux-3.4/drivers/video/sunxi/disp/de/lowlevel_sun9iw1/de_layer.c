...
static s32 DE_BE_Layer_Set_Addr(u32 sel, u8 layidx,u32 addr)  
{
	u32 tmp_l,tmp_h,tmp;
	tmp_l = addr<<3;
...
static s32 DE_BE_Layer_Set_Line_Width(u32 sel, u8 layidx,u32 width)   
{
...
s32 DE_BE_YUV_CH_Set_Src(u32 sel, de_yuv_ch_src_t * in_src)
{
    ...
    if(in_mode == 0x00)   
    {
        ...
        de_scal_ch1_offset = (image_w>>w_shift) * (offset_y>>h_shift) + (offset_x>>w_shift);
        de_scal_ch2_offset = (image_w>>w_shift) * (offset_y>>h_shift) + (offset_x>>w_shift);

        ch0_addr = ch0_base + de_scal_ch0_offset;
        ch1_addr = ch1_base + de_scal_ch1_offset;
        ch2_addr = ch2_base + de_scal_ch2_offset;
    }
    else if(in_mode == 0x01)
    {
