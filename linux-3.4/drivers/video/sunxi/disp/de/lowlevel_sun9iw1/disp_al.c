...
static void * maf_flag_mem[2][2];
static interlace_para_t g_interlace[2];
static u32 wb_buffer = 0;
#endif
...
s32 disp_al_edp_int (__edp_irq_id_t edp_irq)
{
  ...
  { interrupt = LINE0; }
    ...
    if (edp_irq == EDP_IRQ_LINE1)
    { interrupt = LINE1; }
    else
      if (edp_irq == EDP_IRQ_EMPTY)
...
s32 disp_al_manager_clk_init (u32 clk_id)
{
  ...
  for (i = 0; i < num_screens; i++) {
    ...
    if (al_priv->lcd_info.edp_rate == 2700) {
      ...
      if (al_priv->lcd_info.edp_rate == 1620) {
        de_freq_level = 0;
        break;
      }
      else
        if (al_priv->hdmi_info.support_4k == 1) {
          de_freq_level = 1;
        }
  }
...
static s32 scaler_release (u32 scaler_id, bool b_layer)
{
  ...
  if (screen_index != 0xff) {
    __disp_al_private_data * al_priv_tmp;
...
s32 disp_al_set_interlace_info (u32 scaler_id, disp_scaler_info * scaler)
{
  ...
  if (false != g_interlace[scaler_id].enable)
  {
    if (g_interlace[scaler_id].cur_frame_addr_luma != scaler->in_fb.addr[0])
    {
      #if defined(__LINUX_PLAT__)
