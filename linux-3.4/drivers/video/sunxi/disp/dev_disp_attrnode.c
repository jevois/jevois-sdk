                              ...
                              struct device_attribute * attr, char * buf)
{
  ...
  for (screen_id = 0; screen_id < num_screens; screen_id ++) {
    ...
    if (bsp_disp_get_output_type (screen_id) == DISP_OUTPUT_TYPE_LCD) {
      ...
      if (bsp_disp_get_output_type (screen_id) == DISP_OUTPUT_TYPE_HDMI) {
        ...
        if (bsp_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_720P_50HZ) {
          ...
          if (BSP_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_720P_60HZ) {
            ...
            if (BSP_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_1080P_60HZ) {
              count += sprintf (buf + count, "\t%16s", "1080p60hz");
            }
            else
              if (BSP_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_1080P_50HZ) {
                count += sprintf (buf + count, "\t%16s", "1080p50hz");
              }
              else
                if (BSP_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_1080I_50HZ) {
                  count += sprintf (buf + count, "\t%16s", "1080i50hz");
                }
                else
                  if (BSP_disp_hdmi_get_mode (screen_id) == DISP_TV_MOD_1080I_60HZ) {
