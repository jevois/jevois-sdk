...
static void lcd_panel_parameter_check (u32 disp, struct disp_device * lcd)
{
  ...
  { cycle_num = 3; }
    ...
    if (info->lcd_if == 0 && info->lcd_hv_if == 10)
    { cycle_num = 3; }
    else
      if (info->lcd_if == 0 && info->lcd_hv_if == 11)
      { cycle_num = 4; }
      else
        if (info->lcd_if == 0 && info->lcd_hv_if == 12)
        { cycle_num = 4; }
        else
          if (info->lcd_if == 1 && info->lcd_cpu_if == 2)
          { cycle_num = 3; }
          else
            if (info->lcd_if == 1 && info->lcd_cpu_if == 4)
            { cycle_num = 2; }
            else
              if (info->lcd_if == 1 && info->lcd_cpu_if == 6)
              { cycle_num = 2; }
              else
                if (info->lcd_if == 1 && info->lcd_cpu_if == 10)
                { cycle_num = 2; }
                else
                  if (info->lcd_if == 1 && info->lcd_cpu_if == 12)
                  { cycle_num = 3; }
                  else
                    if (info->lcd_if == 1 && info->lcd_cpu_if == 14)
                    { cycle_num = 2; }
                    else
                    { cycle_num = 1; }
       ...
       || (info->lcd_if == 1 && info->lcd_cpu_if == 12) || (info->lcd_if == 3 && info->lcd_lvds_colordepth == 1) )
  {
    ...
    if (info->lcd_if == 1 && ( (info->lcd_cpu_if == 2) || (info->lcd_cpu_if == 4) || (info->lcd_cpu_if == 6)
  ...
  if (Lcd_Panel_Err_Flag != 0 || Lcd_Panel_Wrn_Flag != 0)
  {
        ...
        {OSAL_PRINTF ("*** Dummy RGB HV Panel\n");}
          ...
          {OSAL_PRINTF ("*** RGB Dummy HV Panel\n");}
            ...
            {OSAL_PRINTF ("*** Serial YUV Panel\n");}
              ...
              if (info->lcd_if == 3 && info->lcd_lvds_colordepth == 0)
              {OSAL_PRINTF ("*** 24Bit LVDS Panel\n");}
              else
                if (info->lcd_if == 3 && info->lcd_lvds_colordepth == 1)
                {OSAL_PRINTF ("*** 18Bit LVDS Panel\n");}
                else
                  if ( (info->lcd_if == 1) && (info->lcd_cpu_if == 0 || info->lcd_cpu_if == 10 || info->lcd_cpu_if == 12) )
                  {OSAL_PRINTF ("*** 18Bit CPU Panel\n");}
                  else
                    if ( (info->lcd_if == 1) && (info->lcd_cpu_if == 2 || info->lcd_cpu_if == 4 ||
    ...
    if (Lcd_Panel_Err_Flag != 0)
    {
      ...
      sys_put_wvalue (image_base_addr + 0x800, reg_value & 0xfffff0ff);
...
static s32 lcd_clk_init (struct disp_device * lcd)
{
  ...
  if (LCD_IF_LVDS == lcdp->panel_info.lcd_if) {
    ...
    if (LCD_IF_DSI == lcdp->panel_info.lcd_if) {
      ...
      if (LCD_IF_EDP == lcdp->panel_info.lcd_if) {
...
static s32 disp_lcd_power_enable (struct disp_device * lcd, u32 power_id)
{
  ...
  if (disp_lcd_is_used (lcd) ) {
    if (lcdp->lcd_cfg.lcd_power_type[power_id] == 1) {
      ...
      if (lcdp->lcd_cfg.lcd_power_type[power_id] == 2) {
        /* regulator type */
...
#define SUNXI_CCM_VBASE SUNXI_CCM_MOD_VBASE
  ...
  #if !defined(CONFIG_ARCH_SUN9IW1)
  writel (readl (SUNXI_CCM_VBASE) | 0x10, SUNXI_CCM_VBASE + 0x2c4);
  writel (readl (SUNXI_CCM_VBASE) | 0x1000, SUNXI_CCM_VBASE + 0x2c4);
  #endif
...
static s32 disp_lcd_sw_enable (struct disp_device * lcd)
{
  ...
  for (i = 0; i < LCD_POWER_NUM; i++) {
    if (lcdp->lcd_cfg.lcd_power_type[i] == 1) {
      ...
      if (lcdp->lcd_cfg.lcd_power_type[i] == 2) {
        /* regulator type */
