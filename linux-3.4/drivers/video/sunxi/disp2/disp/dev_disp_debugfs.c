...
static void dispdbg_process (void)
{
  ...
  if (!strncmp (dispdbg_priv.name, "layer", 5) ) {
    ...
    if (!strncmp (dispdbg_priv.command, "enable", 6) ) {
      ...
      if (!strncmp (dispdbg_priv.command, "disable", 7) ) {
      }
      else
    ...
    if (!strncmp (dispdbg_priv.name, "lcd", 3) ) {
      ...
      if (!strncmp (dispdbg_priv.command, "enable", 6) ) {
        ...
        if (!strncmp (dispdbg_priv.command, "disable", 7) ) {
          ...
          if (!strncmp (dispdbg_priv.command, "setbl", 6) ) {
            ...
            if (!strncmp (dispdbg_priv.command, "getbl", 5) ) {
              int bl;
              if (lcd->get_bright) {
      ...
      if (!strncmp (dispdbg_priv.name, "disp", 4) ) {
        ...
        if (!strncmp (dispdbg_priv.command, "getinfo", 7) ) {
          ...
          if (!strncmp (dispdbg_priv.command, "switch", 6) ) {
            u32 type, mode;
            ...
            if (!strncmp (dispdbg_priv.command, "blank", 5) ) {
              u32 level;
              ...
              if (!strncmp (dispdbg_priv.command, "getxres", 7) ) {
                u32 width, height;
                struct disp_device * dispdev = mgr->device;
                ...
                if (!strncmp (dispdbg_priv.command, "getyres", 7) ) {
                  u32 width, height;
                  struct disp_device * dispdev = mgr->device;
                  ...
                  if (!strncmp (dispdbg_priv.command, "getfps", 6) ) {
                    ...
                    if (!strncmp (dispdbg_priv.command, "suspend", 7) ) {
                      if (mgr->device) {
                      ...
                      if (!strncmp (dispdbg_priv.command, "resume", 6) ) {
                        if (mgr->device) {
                          if ( (DISP_OUTPUT_TYPE_TV == mgr->device->type) ) {
        ...
        if (!strncmp (dispdbg_priv.name, "enhance", 7) ) {
          ...
          if (!strncmp (dispdbg_priv.name, "smbl", 4) ) {
            char * p = dispdbg_priv.name + 4;
            int disp;
            struct disp_manager * mgr = NULL;
            ...
            if (!strncmp (dispdbg_priv.name, "hdmi", 4) ) {
              char * p = dispdbg_priv.name + 4;
              int disp;
              unsigned int mode;
...
static ssize_t dispdbg_command_read (struct file * file, char __user * buf, size_t count, loff_t * ppos)
{
  ...
  if (len) {
    ...
    if (count > (len - *ppos) )
    { count = (len - *ppos); }
    copy_to_user ( (void __user *) buf, (const void *) dispdbg_priv.tmpbuf, (unsigned long) len);
    *ppos += count;
  }
  else
  { count = 0; }
...
static ssize_t dispdbg_command_write (struct file * file, const char __user * buf, size_t count, loff_t * ppos)
{
  ...
  { return 0; }
  copy_from_user (dispdbg_priv.command, buf, count);
...
static ssize_t dispdbg_name_write (struct file * file, const char __user * buf, size_t count, loff_t * ppos)
{
  ...
  { return 0; }
  copy_from_user (dispdbg_priv.name, buf, count);
...
static ssize_t dispdbg_param_write (struct file * file, const char __user * buf, size_t count, loff_t * ppos)
{
  ...
  { return 0; }
  copy_from_user (dispdbg_priv.param, buf, count);
...
static ssize_t dispdbg_start_write (struct file * file, const char __user * buf, size_t count, loff_t * ppos)
{
  ...
  { return 0; }
  copy_from_user (dispdbg_priv.start, buf, count);
...
static ssize_t dispdbg_info_write (struct file * file, const char __user * buf, size_t count, loff_t * ppos)
{
  ...
  { return 0; }
  copy_from_user (dispdbg_priv.info, buf, count);
