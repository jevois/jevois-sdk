                            ...
                            struct fb_info * info)
{
  ...
  { return -EINVAL; }
    ...
    { depth = 24; }
      ...
      if (depth == 15 && viafb_dual_fb && ppar->iga_path == IGA1)
      { depth = 15; }
      else
        if (depth == 30)
        { depth = 30; }
...
static int __init parse_active_dev (void)
{
  ...
  if (!viafb_active_dev) {
    ...
    if (!strcmp (viafb_active_dev, "CRT+DVI") ) {
      ...
      if (!strcmp (viafb_active_dev, "DVI+CRT") ) {
        /* DVI+CRT */
        viafb_CRT_ON = STATE_ON;
        viafb_DVI_ON = STATE_ON;
        ...
        if (!strcmp (viafb_active_dev, "CRT+LCD") ) {
          /* CRT+LCD */
          viafb_CRT_ON = STATE_ON;
          viafb_LCD_ON = STATE_ON;
          viafb_primary_dev = CRT_Device;
          ...
          if (!strcmp (viafb_active_dev, "LCD+CRT") ) {
            /* LCD+CRT */
            viafb_CRT_ON = STATE_ON;
            viafb_LCD_ON = STATE_ON;
            ...
            if (!strcmp (viafb_active_dev, "DVI+LCD") ) {
              /* DVI+LCD */
              viafb_DVI_ON = STATE_ON;
              viafb_LCD_ON = STATE_ON;
              viafb_primary_dev = DVI_Device;
              ...
              if (!strcmp (viafb_active_dev, "LCD+DVI") ) {
                /* LCD+DVI */
                viafb_DVI_ON = STATE_ON;
                viafb_LCD_ON = STATE_ON;
                viafb_primary_dev = LCD_Device;
                ...
                if (!strcmp (viafb_active_dev, "LCD+LCD2") ) {
                  viafb_LCD_ON = STATE_ON;
                  viafb_LCD2_ON = STATE_ON;
                  viafb_primary_dev = LCD_Device;
                }
                else
                  if (!strcmp (viafb_active_dev, "LCD2+LCD") ) {
                    viafb_LCD_ON = STATE_ON;
                    viafb_LCD2_ON = STATE_ON;
                    viafb_primary_dev = LCD2_Device;
                  }
                  else
                    if (!strcmp (viafb_active_dev, "CRT") ) {
                      /* CRT only */
                      viafb_CRT_ON = STATE_ON;
                      viafb_SAMM_ON = STATE_OFF;
                    }
                    else
                      if (!strcmp (viafb_active_dev, "DVI") ) {
                        /* DVI only */
                        viafb_DVI_ON = STATE_ON;
                        viafb_SAMM_ON = STATE_OFF;
                      }
                      else
                        if (!strcmp (viafb_active_dev, "LCD") ) {
                          /* LCD only */
                          viafb_LCD_ON = STATE_ON;
                          viafb_SAMM_ON = STATE_OFF;
                        }
...
static int __devinit parse_port (char * opt_str, int * output_interface)
{
  ...
  { *output_interface = INTERFACE_DVP0; }
    ...
    if (!strncmp (opt_str, "DVP1", 4) )
    { *output_interface = INTERFACE_DVP1; }
    else
      if (!strncmp (opt_str, "DFP_HIGHLOW", 11) )
      { *output_interface = INTERFACE_DFP; }
      else
        if (!strncmp (opt_str, "DFP_HIGH", 8) )
        { *output_interface = INTERFACE_DFP_HIGH; }
        else
          if (!strncmp (opt_str, "DFP_LOW", 7) )
  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    ...
    if (!strncmp (this_opt, "viafb_mode1=", 12) ) {
      ...
      if (!strncmp (this_opt, "viafb_mode=", 11) ) {
        ...
        if (!strncmp (this_opt, "viafb_bpp1=", 11) ) {
          if (kstrtouint (this_opt + 11, 0, &viafb_bpp1) < 0)
          { return -EINVAL; }
        }
        else
          if (!strncmp (this_opt, "viafb_bpp=", 10) ) {
            if (kstrtouint (this_opt + 10, 0, &viafb_bpp) < 0)
            { return -EINVAL; }
          }
          else
            if (!strncmp (this_opt, "viafb_refresh1=", 15) ) {
              if (kstrtoint (this_opt + 15, 0, &viafb_refresh1) < 0)
              { return -EINVAL; }
            }
            else
              if (!strncmp (this_opt, "viafb_refresh=", 14) ) {
                if (kstrtoint (this_opt + 14, 0, &viafb_refresh) < 0)
                { return -EINVAL; }
              }
              else
                if (!strncmp (this_opt, "viafb_lcd_dsp_method=", 21) ) {
                  if (kstrtoint (this_opt + 21, 0,
                                 &viafb_lcd_dsp_method) < 0)
                  { return -EINVAL; }
                }
                else
                  if (!strncmp (this_opt, "viafb_lcd_panel_id=", 19) ) {
                    ...
                    if (!strncmp (this_opt, "viafb_accel=", 12) ) {
                      ...
                      if (!strncmp (this_opt, "viafb_SAMM_ON=", 14) ) {
                        if (kstrtoint (this_opt + 14, 0, &viafb_SAMM_ON) < 0)
                        { return -EINVAL; }
                      }
                      else
                        if (!strncmp (this_opt, "viafb_active_dev=", 17) ) {
                          ...
                          if (!strncmp (this_opt,
                            ...
                            if (!strncmp (this_opt, "viafb_second_size=", 18) ) {
                              ...
                              if (!strncmp (this_opt,
                                ...
                                if (!strncmp (this_opt,
                                  ...
                                  if (!strncmp (this_opt, "viafb_bus_width=", 16) ) {
                                    if (kstrtoint (this_opt + 16, 0, &viafb_bus_width) < 0)
                                    { return -EINVAL; }
                                  }
                                  else
                                    if (!strncmp (this_opt, "viafb_lcd_mode=", 15) ) {
                                      if (kstrtoint (this_opt + 15, 0, &viafb_lcd_mode) < 0)
                                      { return -EINVAL; }
                                    }
                                    else
                                      if (!strncmp (this_opt, "viafb_lcd_port=", 15) ) {
                                        viafb_lcd_port = kstrdup (this_opt + 15, GFP_KERNEL);
                                      }
                                      else
                                        if (!strncmp (this_opt, "viafb_dvi_port=", 15) ) {
