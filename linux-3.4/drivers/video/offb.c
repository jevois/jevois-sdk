                                     ...
                                     const char * name, unsigned long address)
{
  ...
  if (dp && !strncmp (name, "ATY,Rage128", 11) ) {
    ...
    if (dp && (!strncmp (name, "ATY,RageM3pA", 12)
      ...
      if (dp && !strncmp (name, "ATY,RageM3pB", 12) ) {
        par->cmap_adr = offb_map_reg (dp, 2, 0, 0x1fff);
        if (par->cmap_adr)
        { par->cmap_type = cmap_M3B; }
      }
      else
        if (dp && !strncmp (name, "ATY,Rage6", 9) ) {
          par->cmap_adr = offb_map_reg (dp, 1, 0, 0x1fff);
          if (par->cmap_adr)
          { par->cmap_type = cmap_radeon; }
        }
        else
          if (!strncmp (name, "ATY,", 4) ) {
            ...
            if (dp && (of_device_is_compatible (dp, "pci1014,b7") ||
              ...
              if (dp && !strncmp (name, "vga,Display-", 12) ) {
                ...
                if (dp && of_device_is_compatible (dp, "qemu,std-vga") ) {
