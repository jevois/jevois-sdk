...
static u32 fb_cvt_aspect_ratio (struct fb_cvt_data * cvt)
{
  ...
  { aspect = 0; }
    ...
    if (xres == (yres * 16) / 9 && ! ( (yres * 16) % 9) )
    { aspect = 1; }
    else
      if (xres == (yres * 16) / 10 && ! ( (yres * 16) % 10) )
      { aspect = 2; }
      else
        if (xres == (yres * 5) / 4 && ! ( (yres * 5) % 4) )
        { aspect = 3; }
        else
          if (xres == (yres * 15) / 9 && ! ( (yres * 15) % 9) )
          { aspect = 4; }
          else {
...
static void fb_cvt_print_name (struct fb_cvt_data * cvt)
{
  ...
  if (cvt->status)
    ...
    { read = snprintf (buf + offset, cnt, "3"); }
      ...
      if (cvt->aspect_ratio == 3)
      { read = snprintf (buf + offset, cnt, "4"); }
      else
        if (cvt->aspect_ratio == 1 || cvt->aspect_ratio == 4)
        { read = snprintf (buf + offset, cnt, "9"); }
        else
          if (cvt->aspect_ratio == 2)
