    ...
    unsigned long * hdmi_rate, unsigned long * parent_rate)
{
  ...
  *hdmi_rate = clk_round_rate (hdmi->hdmi_clk, target);
  ...
  { rate_error = pdata->clk_optimize_parent (target, hdmi_rate, parent_rate); }
    ...
    if (clk_get_parent (hdmi->hdmi_clk) )
                              ...
                              unsigned long * parent_rate)
{
  ...
  for (i = 0, mode = hdmi->monspec.modedb;
    ...
    if (scanning) {
      ...
      { scanning = false; }
        ...
        if (found && found_rate_error <= rate_error)
          /*
  ...
  { hdmi->preprogrammed_vic = 1; }
    ...
    if (found->xres == 720 && found->yres == 480 && found->refresh == 60)
    { hdmi->preprogrammed_vic = 2; }
    else
      if (found->xres == 720 && found->yres == 576 && found->refresh == 50)
      { hdmi->preprogrammed_vic = 17; }
      else
        if (found->xres == 1280 && found->yres == 720 && found->refresh == 60)
        { hdmi->preprogrammed_vic = 4; }
        else
          if (found->xres == 1920 && found->yres == 1080 && found->refresh == 24)
          { hdmi->preprogrammed_vic = 32; }
          else
            if (found->xres == 1920 && found->yres == 1080 && found->refresh == 50)
            { hdmi->preprogrammed_vic = 31; }
            else
              if (found->xres == 1920 && found->yres == 1080 && found->refresh == 60)
