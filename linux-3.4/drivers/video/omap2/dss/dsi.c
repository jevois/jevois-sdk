                                      ...
                                      int channel)
{
  ...
  while (REG_GET (dsidev, DSI_VC_CTRL (channel), 20, 20) ) {
    ...
    if (dt == MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT) {
      ...
      if (dt == MIPI_DSI_RX_DCS_SHORT_READ_RESPONSE_1BYTE) {
        DSSERR ("\tDCS short response, 1 byte: %#x\n",
                FLD_GET (val, 23, 8) );
      }
      else
        if (dt == MIPI_DSI_RX_DCS_SHORT_READ_RESPONSE_2BYTE) {
          DSSERR ("\tDCS short response, 2 byte: %#x\n",
                  FLD_GET (val, 23, 8) );
        }
        else
          if (dt == MIPI_DSI_RX_DCS_LONG_READ_RESPONSE) {
                                ...
                                u8 * buf, int buflen, enum dss_dsi_content_type type)
{
  ...
  if (dt == MIPI_DSI_RX_ACKNOWLEDGE_AND_ERROR_REPORT) {
    ...
    if (dt == (type == DSS_DSI_CONTENT_GENERIC ?
