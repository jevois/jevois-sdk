...
static u32 __devinit nvidia_get_chipset (struct fb_info * info)
{
  ...
  if ( (id & 0xfff0) == 0x00f0 ||
    ...
    { id = 0x10DE0000 | (id >> 16); }
      ...
      if ( (id & 0xffff0000) == 0xDE100000) /* wrong endian */
...
static int __devinit nvidiafb_setup (char * options)
{
  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    if (!strncmp (this_opt, "forceCRTC", 9) ) {
      ...
      if (!strncmp (this_opt, "flatpanel", 9) ) {
        flatpanel = 1;
      }
      else
        if (!strncmp (this_opt, "hwcur", 5) ) {
          hwcur = 1;
        }
        else
          if (!strncmp (this_opt, "noaccel", 6) ) {
            noaccel = 1;
          }
          else
            if (!strncmp (this_opt, "noscale", 7) ) {
              noscale = 1;
            }
            else
              if (!strncmp (this_opt, "reverse_i2c", 11) ) {
                reverse_i2c = 1;
              }
              else
                if (!strncmp (this_opt, "paneltweak:", 11) ) {
                  ...
                  if (!strncmp (this_opt, "vram:", 5) ) {
                    ...
                    if (!strncmp (this_opt, "backlight:", 10) ) {
                      ...
                      if (!strncmp (this_opt, "nomtrr", 6) ) {
                        ...
                        if (!strncmp (this_opt, "fpdither:", 9) ) {
