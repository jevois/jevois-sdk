  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    ...
    { mtrr = 1; }
      ...
      { accel = 1; }
        ...
        if (!strncmp (this_opt, "extvga", 6) )
        { extvga = 1; }
        else
          if (!strncmp (this_opt, "sync", 4) )
          { sync = 1; }
          else
            if (!strncmp (this_opt, "vram:", 5) )
            { vram = (simple_strtoul (this_opt + 5, NULL, 0) ); }
            else
              if (!strncmp (this_opt, "voffset:", 8) )
