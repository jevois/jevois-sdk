...
static int  __devinit sstfb_setup (char * options)
{
  ...
  while ( (this_opt = strsep (&options, ",") ) != NULL) {
    ...
    { vgapass = 0; }
      ...
      if (!strcmp (this_opt, "vgapass") )
      { vgapass = 1; }
      else
        if (!strcmp (this_opt, "clipping") )
        { clipping = 1; }
        else
          if (!strcmp (this_opt, "noclipping") )
          { clipping = 0; }
          else
            if (!strcmp (this_opt, "fastpci") )
            { slowpci = 0; }
            else
              if (!strcmp (this_opt, "slowpci") )
              { slowpci = 1; }
              else
                if (!strncmp (this_opt, "mem:", 4) )
                { mem = simple_strtoul (this_opt + 4, NULL, 0); }
                else
                  if (!strncmp (this_opt, "gfxclk:", 7) )
