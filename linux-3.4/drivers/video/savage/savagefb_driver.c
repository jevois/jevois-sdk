                                ...
                                struct savage_reg     *     reg)
{
  ...
  { reg->CR50 |= 0x40; }
    ...
    if (var->xres_virtual == 800)
    { reg->CR50 |= 0x80; }
    else
      if (var->xres_virtual == 1024)
      { reg->CR50 |= 0x00; }
      else
        if (var->xres_virtual == 1152)
        { reg->CR50 |= 0x01; }
        else
          if (var->xres_virtual == 1280)
          { reg->CR50 |= 0xc0; }
          else
            if (var->xres_virtual == 1600)
            { reg->CR50 |= 0x81; }
            else
  ...
  if (par->dacSpeedBpp <= 0) {
    ...
    { par->dacSpeedBpp = par->clock[3]; }
      ...
      { par->dacSpeedBpp = par->clock[2]; }
        ...
        if ( (var->bits_per_pixel > 8) && (var->bits_per_pixel < 24) )
...
static int savage_init_hw (struct savagefb_par * par)
{
  ...
  { par->display_type = DISP_LCD; }
    ...
    if (dvi || (par->chip == S3_SAVAGE4 && par->dvi) )
    { par->display_type = DISP_DFP; }
    else
                                     ...
                                     const struct pci_device_id * id)
{
  ...
  if (par->SavagePanelWidth) {
    ...
    { printk (KERN_WARNING "No CVT mode found for panel\n"); }
      ...
      if (fb_find_mode (&info->var, info, NULL, NULL, 0,
                        &cvt_mode, 0) != 3)
      { info->var = savagefb_var800x600x8; }
  }
