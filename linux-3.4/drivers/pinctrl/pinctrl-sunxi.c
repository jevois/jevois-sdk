...
static void pin_reset_bias (unsigned int * pin_bias, unsigned int pin)
{
  ...
  if (pin < SUNXI_PL_BASE) {
    *pin_bias = pin;
    ...
    if ( (pin >= SUNXI_PL_BASE) && (pin < SUNXI_PO_BASE) ) {
      *pin_bias = pin - SUNXI_PL_BASE;
    }
    else
      if (pin >= SUNXI_PO_BASE) {
        *pin_bias = pin - 3 * 32;
      }
  #else
  if (pin < SUNXI_PL_BASE) {
    ...
    char * device_name, char * state_name)
{
  ...
  if (cfg->mul_sel == SUNXI_PIN_INPUT_FUNC) {
    ...
    if (cfg->mul_sel == SUNXI_PIN_OUTPUT_FUNC) {
      /* pin mux : output */
      function_name = "gpio_out";
    }
    else
      if (cfg->mul_sel == eint_mux) {
        /* pin mux : eint */
        function_name = "eint";
      }
      else
        if (cfg->mul_sel == SUNXI_PIN_IO_DISABLE) {
