...
static void clktest_reg_dump (void)
{
  ...
  for (i = 0; i <= sunxi_clk_maxreg;)
  {
    if (i + 12 <= sunxi_clk_maxreg)
      ...
      if (i + 8 <= sunxi_clk_maxreg)
        sprintf (dumpline, "[Reg %08x] %08x %08x %08x ", i
                 , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_base + i) ) )
                 , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_base + i + 4) ) )
                 , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_base + i + 8) ) ) );
      else
        if (i + 4 <= sunxi_clk_maxreg)
  ...
  if (cpus_clk_maxreg)
  {
    ...
    for (i = 0; i <= cpus_clk_maxreg;)
    {
      if (i + 12 <= cpus_clk_maxreg)
        ...
        if (i + 8 <= cpus_clk_maxreg)
          sprintf (dumpline, "[Reg %08x] %08x %08x %08x ", i
                   , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_cpus_base + i) ) )
                   , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_cpus_base + i + 4) ) )
                   , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_cpus_base + i + 8) ) ) );
        else
          if (i + 4 <= cpus_clk_maxreg)
            sprintf (dumpline, "[Reg %08x] %08x %08x ", i
                     , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_cpus_base + i) ) )
                     , * ( (unsigned int *) ( (unsigned int __force) (sunxi_clk_cpus_base + i + 4) ) ) );
          else
...
static void clktest_process (void)
{
  ...
  if (start == 1)
  {
    ...
    { command = 1; }
      ...
      if (!strcmp (testclk_priv.command, "getparent") )
      { command = 2; }
      else
        if (!strcmp (testclk_priv.command, "setparent") )
        { command = 3; }
        else
          if (!strcmp (testclk_priv.command, "getrate") )
          { command = 4; }
          else
            if (!strcmp (testclk_priv.command, "setrate") )
            { command = 5; }
            else
              if (!strcmp (testclk_priv.command, "is_enabled") )
              { command = 6; }
              else
                if (!strcmp (testclk_priv.command, "disable") )
                { command = 7; }
                else
                  if (!strcmp (testclk_priv.command, "enable") )
                  { command = 8; }
                  else
                    if (!strcmp (testclk_priv.command, "dumpreg") )
                    { command = 9; }
                    else
                      if (!strcmp (testclk_priv.command, "assert") )
                      { command = 10; }
                      else
                        if (!strcmp (testclk_priv.command, "deassert") )
                        { command = 11; }
                        else
                        {
