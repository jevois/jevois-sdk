...
__u32 mixer_in_fmtseq_set (__u32 format, __u32 pixel_seq)
{
  ...
  case G2D_FMT_1BPP_PALETTE:
    ...
    { val = 0x3A; }
      ...
      if (pixel_seq == G2D_SEQ_1BPP_BIG_LITTER)
      { val = 0x1A; }
      else
        if (pixel_seq == G2D_SEQ_1BPP_LITTER_BIG)
        { val = 0x2A; }
        else
        { val = 0xA; }
    break;
  ...
  case G2D_FMT_2BPP_PALETTE:
    ...
    { val = 0x39; }
      ...
      if (pixel_seq == G2D_SEQ_2BPP_BIG_LITTER)
      { val = 0x19; }
      else
        if (pixel_seq == G2D_SEQ_2BPP_LITTER_BIG)
        { val = 0x29; }
        else
        { val = 0x9; }
    break;
  ...
  case G2D_FMT_4BPP_PALETTE:
    ...
    { val = 0x38; }
      ...
      if (pixel_seq == G2D_SEQ_P67452301)
      { val = 0x18; }
      else
        if (pixel_seq == G2D_SEQ_P10325476)
        { val = 0x28; }
        else
        { val = 0x8; }
    break;
...
__u64 mixer_get_addr (__u32 buffer_addr, __u32 format, __u32 stride, __u32 x, __u32 y) {
  ...
  addr = (__u64) (buffer_addr) * 8 + (__u64) ( (stride * y + x) * bpp);
...
__u32 mixer_set_rotate_reg (__u32 flag) {
  ...
  if     (flag & G2D_BLT_FLIP_HORIZONTAL  ) { rot = 0x10; }
    ...
    if (flag & G2D_BLT_FLIP_VERTICAL  ) { rot = 0x20; }
    else
      if (flag & G2D_BLT_ROTATE90     ) { rot = 0x50; }
      else
        if (flag & G2D_BLT_ROTATE180    ) { rot = 0x30; }
        else
          if (flag & G2D_BLT_ROTATE270    ) { rot = 0x60; }
          else
            if (flag & G2D_BLT_MIRROR45     ) { rot = 0x70; }
            else
              if (flag & G2D_BLT_MIRROR135    ) { rot = 0x40; }
              else { rot = 0; }
...
__s32 mixer_blt (g2d_blt * para) {
  ...
  if ( (para->src_image.format > 0x16) && (para->src_image.format < 0x1A) )
  {
    ...
    { addr_val = (__u64) para->src_image.addr[1] * 8 + (__u64) ( (para->src_image.w * (para->src_rect.y / 2) + para->src_rect.x) * bppnum); }
    else { addr_val = (__u64) para->src_image.addr[1] * 8 + (__u64) ( (para->src_image.w * para->src_rect.y + para->src_rect.x) * bppnum); }
  ...
  if ( (para->flag & G2D_BLT_PIXEL_ALPHA) || (para->flag & G2D_BLT_PLANE_ALPHA) || (para->flag & G2D_BLT_MULTI_ALPHA) || (para->flag & G2D_BLT_SRC_COLORKEY) || (para->flag & G2D_BLT_DST_COLORKEY) )
  {
    ...
    if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1A) )
    {
      ...
      { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * (para->dst_y / 2) + para->dst_x) * bppnum); }
      else { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * para->dst_y + para->dst_x) * bppnum); }
      reg_val = read_wvalue (G2D_DMA_HADDR_REG);
  ...
  if (para->flag & G2D_BLT_SRC_COLORKEY)
  {
    ...
    if (para->flag & G2D_BLT_DST_COLORKEY)
    {
      reg_val = 0x5;
    }
    else
      if ( (para->flag & G2D_BLT_PIXEL_ALPHA) || (para->flag & G2D_BLT_PLANE_ALPHA) || (para->flag & G2D_BLT_MULTI_ALPHA) )
      {
        reg_val = 0x1;
      }
      else {reg_val = 0x0;}
  ...
  if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1D) )
  {
    if ( (para->dst_image.format == G2D_FMT_PYUV411UVC) || (para->dst_image.format == G2D_FMT_PYUV422) || (para->dst_image.format == G2D_FMT_PYUV420) ) { bppnum = 4; }
      ...
      if (para->dst_image.format == G2D_FMT_PYUV411) { bppnum = 2; }
      else { bppnum = 8; }
    ...
    { addr_val = (__u64) para->dst_image.addr[2] * 8 + (__u64) ( (para->dst_image.w * (para->dst_y / 2) + para->dst_x) * bppnum); }
    else
      if ( (para->dst_image.format == G2D_FMT_PYUV422) || (para->dst_image.format == G2D_FMT_PYUV411) )
      { addr_val = (__u64) para->dst_image.addr[2] * 8 + (__u64) ( (para->dst_image.w * para->dst_y + para->dst_x) * bppnum); }
      else { addr_val = 0; }
    reg_val = read_wvalue (G2D_OUTPUT_HADDR_REG);
    reg_val |= ( (addr_val >> 32) & 0xF) << 16; /* high addr in bits */
    write_wvalue (G2D_OUTPUT_HADDR_REG, reg_val);
  ...
  if (para->dst_rect.w > 0x400)
  {
    ...
    for (i = 0; i < cnt; i++)
    {
      ...
      if ( (para->src_image.format > 0x16) && (para->src_image.format < 0x1A) )
      {
        ...
        if (para->src_image.format == G2D_FMT_PYUV420UVC)
        { addr_val = (__u64) para->src_image.addr[1] * 8 + (__u64) ( (para->src_image.w * (para->src_rect.y / 2) + scaler_inx) * bppnum); }
        else { addr_val = (__u64) para->src_image.addr[1] * 8 + (__u64) ( (para->src_image.w * para->src_rect.y + scaler_inx) * bppnum); }
        reg_val = read_wvalue (G2D_DMA_HADDR_REG);
        reg_val |= ( (addr_val >> 32) & 0xF) << 8; /* high addr in bits */
        write_wvalue (G2D_DMA_HADDR_REG, reg_val);
        reg_val = addr_val & 0xFFFFFFFF; /* low addr in bits */
      ...
      if ( (para->flag & G2D_BLT_PIXEL_ALPHA) || (para->flag & G2D_BLT_PLANE_ALPHA) || (para->flag & G2D_BLT_MULTI_ALPHA) || (para->flag & G2D_BLT_SRC_COLORKEY) || (para->flag & G2D_BLT_DST_COLORKEY) )
      {
        ...
        if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1A) )
        {
          ...
          if (para->dst_image.format == G2D_FMT_PYUV420UVC)
          { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * (para->dst_rect.y / 2) + scaler_outx) * bppnum); }
          else { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * para->dst_rect.y + scaler_outx) * bppnum); }
          reg_val = read_wvalue (G2D_DMA_HADDR_REG);
          reg_val |= ( (addr_val >> 32) & 0xF) << 16; /* high addr in bits */
          write_wvalue (G2D_DMA_HADDR_REG, reg_val);
          reg_val = addr_val & 0xFFFFFFFF; /* low addr in bits */
      ...
      if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1A) )
      {
        ...
        if (para->dst_image.format == G2D_FMT_PYUV420UVC)
        { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * (scaler_outy / 2) + scaler_outx) * bppnum); }
        else { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * scaler_outy + scaler_outx) * bppnum); }
        reg_val = read_wvalue (G2D_OUTPUT_HADDR_REG);
        reg_val |= ( (addr_val >> 32) & 0xF) << 8; /* high addr in bits */
        write_wvalue (G2D_OUTPUT_HADDR_REG, reg_val);
        reg_val = addr_val & 0xFFFFFFFF; /* low addr in bits */
  ...
  else
  {
    ...
    if ( (para->flag & G2D_BLT_PIXEL_ALPHA) || (para->flag & G2D_BLT_PLANE_ALPHA) || (para->flag & G2D_BLT_MULTI_ALPHA) || (para->flag & G2D_BLT_SRC_COLORKEY) || (para->flag & G2D_BLT_DST_COLORKEY) )
    {
      ...
      if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1A) )
      {
        ...
        if (para->dst_image.format == G2D_FMT_PYUV420UVC)
        { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * (para->dst_rect.y / 2) + para->dst_rect.x) * bppnum); }
        else { addr_val = (__u64) para->dst_image.addr[1] * 8 + (__u64) ( (para->dst_image.w * para->dst_rect.y + para->dst_rect.x) * bppnum); }
        reg_val = read_wvalue (G2D_DMA_HADDR_REG);
        reg_val |= ( (addr_val >> 32) & 0xF) << 16; /* high addr in bits */
        write_wvalue (G2D_DMA_HADDR_REG, reg_val);
        reg_val = addr_val & 0xFFFFFFFF; /* low addr in bits */
    ...
    if ( (para->dst_image.format > 0x16) && (para->dst_image.format < 0x1D) )
    {
      ...
      write_wvalue (G2D_OUTPUT1_STRIDE_REG, para->dst_image.w * bppnum);
      if (para->dst_image.format == G2D_FMT_PYUV420)
      { addr_val = (__u64) para->dst_image.addr[2] * 8 + (__u64) ( (para->dst_image.w * (para->dst_rect.y / 2) + para->dst_rect.x) * bppnum); }
      else
        if ( (para->dst_image.format == G2D_FMT_PYUV422) || (para->dst_image.format == G2D_FMT_PYUV411) )
        { addr_val = (__u64) para->dst_image.addr[2] * 8 + (__u64) ( (para->dst_image.w * para->dst_rect.y + para->dst_rect.x) * bppnum); }
        else { addr_val = 0; }
      reg_val = read_wvalue (G2D_OUTPUT_HADDR_REG);
      reg_val |= ( (addr_val >> 32) & 0xF) << 16; /* high addr in bits */
      write_wvalue (G2D_OUTPUT_HADDR_REG, reg_val);
...
__u32 mixer_premultiply_set (__u32 flag)
{
  ...
  if (flag & G2D_BLT_SRC_PREMULTIPLY)
  {
    ...
    if (flag & G2D_BLT_DST_PREMULTIPLY)
    {
      reg_val |= (0x7 << 5);
      write_wvalue (G2D_CK_CONTROL_REG, reg_val);
...
__u32 mixer_micro_block_set (g2d_blt * para)
{
  ...
  if (para->src_image.format > 0x20)
  {
    ...
    reg_val |= mixer_set_rotate_reg (para->flag);
    write_wvalue (G2D_DMA1_CONTROL_REG, reg_val);
    if    (para->src_image.format > 0x23 && para->src_image.format < 0x27) { reg_val = 0x29 << 17; }
    else
      if (para->src_image.format > 0x26 && para->src_image.format < 0x2A) { reg_val = 0x32 << 17; }
      else
        if (para->src_image.format > 0x29 && para->src_image.format < 0x2D) { reg_val = 0x3b << 17; }
        else { reg_val = 0x20 << 17; }
    write_wvalue (G2D_DMA0_CONTROL_REG, (reg_val | (read_wvalue (G2D_DMA0_CONTROL_REG) & 0xff81ffff) ) );
    write_wvalue (G2D_DMA1_CONTROL_REG, (reg_val | (read_wvalue (G2D_DMA1_CONTROL_REG) & 0xff81ffff) ) );
