...
__u32 mixer_micro_block_set(g2d_blt *para)
{
	...
	if(para->src_image.format>0x20)
	{
		...
		else if	(para->src_image.format>0x26 && para->src_image.format<0x2A)reg_val = 0x32<<17;
		else if	(para->src_image.format>0x29 && para->src_image.format<0x2D)reg_val = 0x3b<<17;
