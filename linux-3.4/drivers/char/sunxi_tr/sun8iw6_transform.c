...
int de_tr_set_cfg (tr_info * info)
{
  ...
  if (info->mode <= TR_ROT_270)
  {
    ...
    if ( (info->mode == TR_HFLIP) || (info->mode == TR_HFLIP_ROT_90) )
    {
      hflip_en = 0x1;
    }
    else
      if ( (info->mode == TR_VFLIP) || (info->mode == TR_VFLIP_ROT_90) )
      {
        vflip_en = 0x1;
      }
  ...
  if       (fmt <= TR_FORMAT_BGRX_8888)     {ycnt = 4;}
    ...
    if  (fmt <= TR_FORMAT_BGR_888)       {ycnt = 3;}
    else
      if  (fmt <= TR_FORMAT_BGRA_5551)     {ycnt = 2;}
      else
        if  (fmt <= TR_FORMAT_YUV422_I_VYUY) {ycnt = 2; ucnt = 0;}
        else
          if  (fmt == TR_FORMAT_YUV422_P)      {ycnt = 1; ucnt = 1; x1 = x0 / 2; y1 = y0;}
          else
            if  (fmt == TR_FORMAT_YUV420_P)      {ycnt = 1; ucnt = 1; x1 = x0 / 2; y1 = y0 / 2;}
            else
              if  (fmt == TR_FORMAT_YUV411_P)      {ycnt = 1; ucnt = 1; x1 = x0 / 4; y1 = y0;}
              else
                if  (fmt <= TR_FORMAT_YUV422_SP_VUVU) {ycnt = 1; ucnt = 2; x1 = x0 / 2; y1 = y0;}
                else
                  if  (fmt <= TR_FORMAT_YUV420_SP_VUVU) {ycnt = 1; ucnt = 2; x1 = x0 / 2; y1 = y0 / 2;}
                  else
                    if  (fmt <= TR_FORMAT_YUV411_SP_VUVU) {ycnt = 1; ucnt = 2; x1 = x0 / 4; y1 = y0;}
                    else                                    {ycnt = 4;}
  ...
  if    (TR_FORMAT_YUV422_I_VYUY  == info->src_frame.fmt) {fmt = 0x20;}
    ...
    if (TR_FORMAT_YUV422_I_YVYU  == info->src_frame.fmt) {fmt = 0x21;}
    else
      if (TR_FORMAT_YUV422_I_UYVY  == info->src_frame.fmt) {fmt = 0x22;}
      else
        if (TR_FORMAT_YUV422_I_YUYV  == info->src_frame.fmt) {fmt = 0x23;}
        else
          if (TR_FORMAT_YUV422_SP_UVUV == info->src_frame.fmt) {fmt = 0x24;}
          else
            if (TR_FORMAT_YUV422_SP_VUVU == info->src_frame.fmt) {fmt = 0x25;}
            else
              if (TR_FORMAT_YUV422_P       == info->src_frame.fmt) {fmt = 0x26;}
              else
                if (TR_FORMAT_YUV420_SP_UVUV == info->src_frame.fmt) {fmt = 0x28;}
                else
                  if (TR_FORMAT_YUV420_SP_VUVU == info->src_frame.fmt) {fmt = 0x29;}
                  else
                    if (TR_FORMAT_YUV420_P       == info->src_frame.fmt) {fmt = 0x2a;}
                    else
                      if (TR_FORMAT_YUV411_SP_UVUV == info->src_frame.fmt) {fmt = 0x2c;}
                      else
                        if (TR_FORMAT_YUV411_SP_VUVU == info->src_frame.fmt) {fmt = 0x2d;}
                        else
                          if (TR_FORMAT_YUV411_P       == info->src_frame.fmt) {fmt = 0x2e;}
