  ...
  int iscsi_task_attr)
{
  ...
  { sam_task_attr = MSG_SIMPLE_TAG; }
    ...
    if (iscsi_task_attr == ISCSI_ATTR_ORDERED)
    { sam_task_attr = MSG_ORDERED_TAG; }
    else
      if (iscsi_task_attr == ISCSI_ATTR_HEAD_OF_QUEUE)
      { sam_task_attr = MSG_HEAD_TAG; }
      else
        if (iscsi_task_attr == ISCSI_ATTR_ACA)
...
static inline int iscsit_check_received_cmdsn (struct iscsi_session * sess, u32 cmdsn)
{
  ...
  if (iscsi_sna_gt (cmdsn, sess->max_cmd_sn) ) {
    ...
    if (cmdsn == sess->exp_cmd_sn) {
      ...
      if (iscsi_sna_gt (cmdsn, sess->exp_cmd_sn) ) {
  ...
  u8 status_detail)
{
  ...
  { ls->accepts++; }
    ...
    if (status_class == ISCSI_STATUS_CLS_REDIRECT) {
      ls->redirects++;
      ls->last_fail_type = ISCSI_LOGIN_FAIL_REDIRECT;
    }
    else
      if ( (status_class == ISCSI_STATUS_CLS_INITIATOR_ERR)  &&
