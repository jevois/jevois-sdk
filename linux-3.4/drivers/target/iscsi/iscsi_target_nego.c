  ...
  { return 1; }
    ...
    if (strstr ("SRP", authtype) )
      ...
      if (strstr ("CHAP", authtype) )
        ...
        if (strstr ("SPKM1", authtype) )
        { return 2; }
        else
          if (strstr ("SPKM2", authtype) )
          { return 2; }
          else
            if (strstr ("KRB5", authtype) )
            { return 2; }
            else
            { return 2; }
}
  ...
  struct iscsi_login * login)
{
  ...
  while (start < end) {
    ...
    { i_buf = value; }
      ...
      if (!strncmp (key, "SessionType", 11) )
      { s_buf = value; }
      else
        if (!strncmp (key, "TargetName", 10) )
        { t_buf = value; }
