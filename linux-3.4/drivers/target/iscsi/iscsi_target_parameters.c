  ...
  struct iscsi_param_list * param_list)
{
  ...
  list_for_each_entry (param, &param_list->param_list, p_list) {
    ...
    if (!strcmp (param->name, AUTHMETHOD) ) {
      ...
      if (!strcmp (param->name, HEADERDIGEST) ) {
        SET_PSTATE_NEGOTIATE (param);
      }
      else
        if (!strcmp (param->name, DATADIGEST) ) {
          SET_PSTATE_NEGOTIATE (param);
        }
        else
          if (!strcmp (param->name, MAXCONNECTIONS) ) {
            ...
            if (!strcmp (param->name, TARGETNAME) ) {
              continue;
            }
            else
              if (!strcmp (param->name, INITIATORNAME) ) {
                ...
                if (!strcmp (param->name, TARGETALIAS) ) {
                  if (param->value)
                  { SET_PSTATE_NEGOTIATE (param); }
                }
                else
                  if (!strcmp (param->name, INITIATORALIAS) ) {
                    continue;
                  }
                  else
                    if (!strcmp (param->name, TARGETPORTALGROUPTAG) ) {
                      ...
                      if (!strcmp (param->name, INITIALR2T) ) {
                        SET_PSTATE_NEGOTIATE (param);
                      }
                      else
    ...
    if (IS_TYPE_BOOL_AND (param) || IS_TYPE_BOOL_OR (param) ) {
      ...
      if (IS_TYPE_NUMBER (param) ) {
        ...
        if (IS_TYPE_NUMBER_RANGE (param) ) {
          ...
          if (IS_TYPE_STRING (param) || IS_TYPE_VALUE_LIST (param) ) {
