  ...
  u32 dataout_timeout)
{
  ...
  if (dataout_timeout > NA_DATAOUT_TIMEOUT_MAX) {
    ...
    if (dataout_timeout < NA_DATAOUT_TIMEOUT_MIX) {
  ...
  u32 dataout_timeout_retries)
{
  ...
  if (dataout_timeout_retries > NA_DATAOUT_TIMEOUT_RETRIES_MAX) {
    ...
    if (dataout_timeout_retries < NA_DATAOUT_TIMEOUT_RETRIES_MIN) {
  ...
  u32 nopin_timeout)
{
  ...
  if (nopin_timeout > NA_NOPIN_TIMEOUT_MAX) {
    ...
    if ( (nopin_timeout < NA_NOPIN_TIMEOUT_MIN) &&
         (nopin_timeout != 0) ) {
  ...
  u32 nopin_response_timeout)
{
  ...
  if (nopin_response_timeout > NA_NOPIN_RESPONSE_TIMEOUT_MAX) {
    ...
    if (nopin_response_timeout < NA_NOPIN_RESPONSE_TIMEOUT_MIN) {
