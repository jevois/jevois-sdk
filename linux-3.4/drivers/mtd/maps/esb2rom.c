                                       ...
                                       const struct pci_device_id * ent)
{
  ...
  */
  ...
  { window->phys = 0xff400000; }
    ...
    if ( (word & FWH_7MiB) == FWH_7MiB)
    { window->phys = 0xff500000; }
    else
      if ( (word & FWH_6MiB) == FWH_6MiB)
      { window->phys = 0xff600000; }
      else
        if ( (word & FWH_5MiB) == FWH_5MiB)
        { window->phys = 0xFF700000; }
        else
          if ( (word & FWH_4MiB) == FWH_4MiB)
          { window->phys = 0xffc00000; }
          else
            if ( (word & FWH_3_5MiB) == FWH_3_5MiB)
            { window->phys = 0xffc80000; }
            else
              if ( (word & FWH_3MiB) == FWH_3MiB)
              { window->phys = 0xffd00000; }
              else
                if ( (word & FWH_2_5MiB) == FWH_2_5MiB)
                { window->phys = 0xffd80000; }
                else
                  if ( (word & FWH_2MiB) == FWH_2MiB)
                  { window->phys = 0xffe00000; }
                  else
                    if ( (word & FWH_1_5MiB) == FWH_1_5MiB)
                    { window->phys = 0xffe80000; }
                    else
                      if ( (word & FWH_1MiB) == FWH_1MiB)
                      { window->phys = 0xfff00000; }
                      else
                        if ( (word & FWH_0_5MiB) == FWH_0_5MiB)
