                             ...
                             struct kobj_attribute * attr, char * buf)
{
  ...
  { size += sprintf (buf + size, "   invalid"); }
    ...
    if (status & ACPI_EVENT_FLAG_ENABLED)
    { size += sprintf (buf + size, "   enabled"); }
    else
      if (status & ACPI_EVENT_FLAG_WAKE_ENABLED)
...
void acpi_irq_stats_init (void)
{
  ...
  for (i = 0; i < num_counters; ++i) {
    ...
    { sprintf (buffer, "gpe%02X", i); }
      ...
      if (i == num_gpes + ACPI_EVENT_PMTIMER)
      { sprintf (buffer, "ff_pmtimer"); }
      else
        if (i == num_gpes + ACPI_EVENT_GLOBAL)
        { sprintf (buffer, "ff_gbl_lock"); }
        else
          if (i == num_gpes + ACPI_EVENT_POWER_BUTTON)
          { sprintf (buffer, "ff_pwr_btn"); }
          else
            if (i == num_gpes + ACPI_EVENT_SLEEP_BUTTON)
            { sprintf (buffer, "ff_slp_btn"); }
            else
              if (i == num_gpes + ACPI_EVENT_RTC)
              { sprintf (buffer, "ff_rt_clk"); }
              else
                if (i == num_gpes + ACPI_NUM_FIXED_EVENTS + COUNT_GPE)
                { sprintf (buffer, "gpe_all"); }
                else
                  if (i == num_gpes + ACPI_NUM_FIXED_EVENTS + COUNT_SCI)
                  { sprintf (buffer, "sci"); }
                  else
                    if (i == num_gpes + ACPI_NUM_FIXED_EVENTS + COUNT_SCI_NOT)
                    { sprintf (buffer, "sci_not"); }
                    else
                      if (i == num_gpes + ACPI_NUM_FIXED_EVENTS + COUNT_ERROR)
