    ...
    struct device_attribute * attr, const char * buf, size_t count)
{
  ...
  if (cnt >= 3 && strncmp ("lcd", buf, 3) == 0) {
    ...
    if (cnt >= 10 && strncmp ("bootloader", buf, 10) == 0) {
      if (! (data->status & PICOLCD_BOOTLOADER) )
