...
static int philips_tdm1316l_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  if (tuner_frequency < 87000000) { return -EINVAL; }
    ...
    if (tuner_frequency < 130000000) { cp = 3; }
    else
      if (tuner_frequency < 160000000) { cp = 5; }
      else
        if (tuner_frequency < 200000000) { cp = 6; }
        else
          if (tuner_frequency < 290000000) { cp = 3; }
          else
            if (tuner_frequency < 420000000) { cp = 5; }
            else
              if (tuner_frequency < 480000000) { cp = 6; }
              else
                if (tuner_frequency < 620000000) { cp = 3; }
                else
                  if (tuner_frequency < 830000000) { cp = 5; }
                  else
                    if (tuner_frequency < 895000000) { cp = 7; }
                    else { return -EINVAL; }
                    
  if (p->frequency < 49000000)
  { return -EINVAL; }
  else
    if (p->frequency < 159000000)
    { band = 1; }
    else
      if (p->frequency < 444000000)
      { band = 2; }
      else
        if (p->frequency < 861000000)
        { band = 4; }
        else { return -EINVAL; }
...
static int dvbc_philips_tdm1316l_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  if      (tuner_frequency <  87000000) {return -EINVAL;}
    ...
    if (tuner_frequency < 130000000) {cp = 3; band = 1;}
    else
      if (tuner_frequency < 160000000) {cp = 5; band = 1;}
      else
        if (tuner_frequency < 200000000) {cp = 6; band = 1;}
        else
          if (tuner_frequency < 290000000) {cp = 3; band = 2;}
          else
            if (tuner_frequency < 420000000) {cp = 5; band = 2;}
            else
              if (tuner_frequency < 480000000) {cp = 6; band = 2;}
              else
                if (tuner_frequency < 620000000) {cp = 3; band = 4;}
                else
                  if (tuner_frequency < 830000000) {cp = 5; band = 4;}
                  else
                    if (tuner_frequency < 895000000) {cp = 7; band = 4;}
                    else {return -EINVAL;}
