...
static void dib0090_gain_apply (struct dib0090_state * state, s16 gain_delta, s16 top_delta, u8 force)
{
  ...
  for (i = 0; i < 7; i++) { /* Go over all amplifiers => 5RF amps + 2 BB amps = 7 amps */
    ...
    { v = 0; }  /* force the gain to write for the current amp to be null */
      ...
      if (ref >= g[1]) /* Gain to set is higher than the high working point of this amp */
    ...
    { gain_reg[0] = v; }
      ...
      { gain_reg[0] |= v << 7; }
        ...
        { gain_reg[1] = v; }
          ...
          if (i == 3)  /* LNA 4 reg mapping */
          { gain_reg[1] |= v << 7; }
          else
            if (i == 4)  /* CBAND LNA reg mapping */
            { gain_reg[2] = v | state->rf_lt_def; }
            else
              if (i == 5)  /* BB gain 1 reg mapping */
              { gain_reg[3] = v << 3; }
              else
                if (i == 6)  /* BB gain 2 reg mapping */
                { gain_reg[3] |= v << 8; }
...
static void dib0090_set_bandwidth (struct dib0090_state * state)
{
  ...
  { tmp = (3 << 14); }
    ...
    if (state->fe->dtv_property_cache.bandwidth_hz / 1000 <= 6000)
    { tmp = (2 << 14); }
    else
    ...
    if (*tune_state == CT_TUNER_STEP_0) {
      ...
      *tune_state = CT_TUNER_STEP_1;
      ...
      if (*tune_state == CT_TUNER_STEP_1) {
        ...
        if (*tune_state == CT_TUNER_STEP_2) {  /* this step is only used by krosus < P1G */
...
static int dib0090_tune (struct dvb_frontend * fe)
{
  ...
  if (*tune_state == CT_TUNER_START) {
    ...
    if (*tune_state == CT_TUNER_STEP_0) {  /* Warning : because of captrim cal, if you change this step, change it also in _cal.c file because it is the step following captrim cal state machine */
      const struct dib0090_wbd_slope * wbd = state->current_wbd_table;
