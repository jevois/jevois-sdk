                                     ...
                                     struct dtv_frontend_properties * fe_params)
{
  ...
  if (symbol_rate < (u32) (SACLK * 0.0137) ) {
    ...
    if (symbol_rate < (u32) (SACLK * 0.0208) ) {
      dfn = 4;
      afs = 0;
    }
    else
      if (symbol_rate < (u32) (SACLK * 0.0270) ) {
        dfn = 3;
        afs = 1;
      }
      else
        if (symbol_rate < (u32) (SACLK * 0.0416) ) {
          dfn = 3;
          afs = 0;
        }
        else
          if (symbol_rate < (u32) (SACLK * 0.0550) ) {
            dfn = 2;
            afs = 1;
          }
          else
            if (symbol_rate < (u32) (SACLK * 0.0833) ) {
              dfn = 2;
              afs = 0;
            }
            else
              if (symbol_rate < (u32) (SACLK * 0.1100) ) {
                dfn = 1;
                afs = 1;
              }
              else
                if (symbol_rate < (u32) (SACLK * 0.1666) ) {
                  dfn = 1;
                  afs = 0;
                }
                else
                  if (symbol_rate < (u32) (SACLK * 0.2200) ) {
                    dfn = 0;
                    afs = 1;
                  }
                  else
                    if (symbol_rate < (u32) (SACLK * 0.3333) ) {
                      dfn = 0;
                      afs = 0;
                    }
                    else {
