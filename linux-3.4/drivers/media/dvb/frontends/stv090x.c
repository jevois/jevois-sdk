...
static void stv090x_get_lock_tmg (struct stv090x_state * state)
{
  ...
  case STV090x_BLIND_SEARCH:
    ...
    if (state->srate <= 1500000) {  /*10Msps< SR <=15Msps*/
      ...
      if (state->srate <= 5000000) {  /*10Msps< SR <=15Msps*/
  ...
  default:
    ...
    if (state->srate <= 1000000) {  /*SR <=1Msps*/
      ...
      if (state->srate <= 2000000) { /*1Msps < SR <= 2Msps */
        ...
        if (state->srate <= 5000000) { /*2Msps < SR <= 5Msps */
          state->DemodTimeout = 1000;
          state->FecTimeout = 550;
        }
        else
          if (state->srate <= 10000000) { /*5Msps < SR <= 10Msps */
            state->DemodTimeout = 700;
            state->FecTimeout = 250;
          }
          else
            if (state->srate <= 20000000) { /*10Msps < SR <= 20Msps */
...
static int stv090x_dvbs_track_crl (struct stv090x_state * state)
{
  if (state->internal->dev_ver >= 0x30) {
    ...
    if (state->srate >= 15000000) {
      ...
      if ( (state->srate >= 7000000) && (15000000 > state->srate) ) {
...
static enum stv090x_signal_state stv090x_get_sig_params (struct stv090x_state * state)
{
  ...
  if ( (state->algo == STV090x_BLIND_SEARCH) || (state->srate < 10000000) ) {
    ...
    { return STV090x_RANGEOK; }
      ...
      if (abs (offst_freq) <= (stv090x_car_width (state->srate, state->rolloff) / 2000) )
      { return STV090x_RANGEOK; }
      else
  ...
  case STV090x_DVBS2:
    ...
    if (state->frame_len == STV090x_LONG_FRAME) {
      ...
      if (state->modulation == STV090x_QPSK) {
        ...
        if (state->modulation == STV090x_8PSK) {
          if (STV090x_WRITE_DEMOD (state, ACLC2S2Q, 0x2a) < 0)
          { goto err; }
          if (STV090x_WRITE_DEMOD (state, ACLC2S28, aclc) < 0)
          { goto err; }
          ...
          if (state->modulation == STV090x_16APSK) {
            if (STV090x_WRITE_DEMOD (state, ACLC2S2Q, 0x2a) < 0)
            { goto err; }
            if (STV090x_WRITE_DEMOD (state, ACLC2S216A, aclc) < 0)
            { goto err; }
            ...
            if (state->modulation == STV090x_32APSK)  {
              if (STV090x_WRITE_DEMOD (state, ACLC2S2Q, 0x2a) < 0)
              { goto err; }
              if (STV090x_WRITE_DEMOD (state, ACLC2S232A, aclc) < 0)
              { goto err; }
...
static int stv090x_read_signal_strength (struct dvb_frontend * fe, u16 * strength)
{
  ...
  { str = 0; }
    ...
    if (agc < stv090x_rf_tab[ARRAY_SIZE (stv090x_rf_tab) - 1].read)
    { str = -100; }
