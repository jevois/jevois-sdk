...
static void stb0899_dvbs2_init_calc (struct stb0899_state * state)
{
  ...
  { step_size = (1 << 17) / 5; }
    ...
    if (internal->srate / 1000000 >= 10)
    { step_size = (1 << 17) / 7; }
    else
      if (internal->srate / 1000000 >= 5)
      { step_size = (1 << 17) / 10; }
      else
      { step_size = (1 << 17) / 4; }
