                                 ...
                                 s32 * pSignalToNoise)
{
  ...
  { iMER = 0; }
    ...
    if ( (EqRegTdSqrErrI + EqRegTdSqrErrQ) == 0) {
  ...
  { *pLockStatus = MPEG_LOCK; }
    ...
    { *pLockStatus = FEC_LOCK; }
      ...
      { *pLockStatus = DEMOD_LOCK; }
        ...
        if (ScRaRamLock & OFDM_SC_RA_RAM_LOCK_NODVBT__M)
...
static int QAMSetSymbolrate (struct drxk_state * state)
{
  ...
  { ratesel = 3; }
    ...
    if (state->props.symbol_rate <= 2377500)
    { ratesel = 2; }
    else
      if (state->props.symbol_rate <= 4755000)
      { ratesel = 1; }
...
static int GetQAMLockStatus (struct drxk_state * state, u32 * pLockStatus)
{
  ...
  *pLockStatus = NOT_LOCKED;
  ...
  if (Result[1] < SCU_RAM_QAM_LOCKED_LOCKED_DEMOD_LOCKED) {
    ...
    if (Result[1] < SCU_RAM_QAM_LOCKED_LOCKED_LOCKED) {
      /* 0x4000 DEMOD LOCKED */
      *pLockStatus = DEMOD_LOCK;
    }
    else
      if (Result[1] < SCU_RAM_QAM_LOCKED_LOCKED_NEVER_LOCK) {
