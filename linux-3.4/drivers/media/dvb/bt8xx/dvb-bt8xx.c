...
static int microtune_mt7202dtf_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { cpump = 2; }
    ...
    if (c->frequency < 390000000)
    { cpump = 1; }
    else
      if (c->frequency < 470000000)
      { cpump = 2; }
      else
        if (c->frequency < 750000000)
        { cpump = 2; }
        else
        { cpump = 3; }
...
static int advbt771_samsung_tdtc9251dh0_tuner_calc_regs (struct dvb_frontend * fe, u8 * pllbuf, int buf_len)
{
  ...
  { cp = 0xB4; }
    ...
    { cp = 0xBC; }
      ...
      if (c->frequency < 250000000)
      { cp = 0xB4; }
      else
        if (c->frequency < 400000000)
        { cp = 0xBC; }
        else
          if (c->frequency < 420000000)
          { cp = 0xF4; }
          else
            if (c->frequency < 470000000)
            { cp = 0xFC; }
...
static int vp3021_alps_tded4_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { buf[3] = 0x01; }
    ...
    if ( (c->frequency >= 153000000) && (c->frequency < 430000000) )
    { buf[3] = 0x02; }
    else
      if ( (c->frequency >= 430000000) && (c->frequency < 824000000) )
      { buf[3] = 0x0C; }
      else
        if ( (c->frequency >= 824000000) && (c->frequency < 863000000) )
        { buf[3] = 0x8C; }
        else
        { return -EINVAL; }
