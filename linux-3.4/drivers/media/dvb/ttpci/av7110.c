...
static void init_av7110_av (struct av7110 * av7110)
{
  ...
  if (i2c_writereg (av7110, 0x20, 0x00, 0x00) == 1) {
    ...
    if (0 == av7110_init_analog_module (av7110) ) {
      /* done. */
    }
    else
...
static int alps_bsrv2_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { pwr = 3; }
    ...
    if (p->frequency > 1800000)
    { pwr = 2; }
    else
      if (p->frequency > 1600000)
      { pwr = 1; }
      else
        if (p->frequency > 1200000)
        { pwr = 0; }
        else
          if (p->frequency >= 1100000)
          { pwr = 1; }
          else
          { pwr = 2; }
...
static int nexusca_stv0297_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { return -EINVAL; }
    ...
    if (p->frequency < 137000000)
    { data[3] = 0x01; }
    else
      if (p->frequency < 403000000)
      { data[3] = 0x02; }
      else
        if (p->frequency < 860000000)
        { data[3] = 0x04; }
        else
        { return -EINVAL; }
...
static int grundig_29504_401_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { cpump = 2; }
    ...
    if (p->frequency < 390000000)
    { cpump = 1; }
    else
      if (p->frequency < 470000000)
      { cpump = 2; }
      else
        if (p->frequency < 750000000)
        { cpump = 1; }
        else
        { cpump = 3; }
...
static int frontend_init (struct av7110 * av7110)
{
  ...
  if (av7110->dev->pci->subsystem_vendor == 0x110a) {
    ...
    if (av7110->dev->pci->subsystem_vendor == 0x13c2) {
      switch (av7110->dev->pci->subsystem_device) {
      case 0x0000:
      case 0x0003:
      case 0x1002:
