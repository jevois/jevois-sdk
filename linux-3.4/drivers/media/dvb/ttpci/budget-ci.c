...
static int philips_su1278_tt_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { buf[3] |= 0; }
    ...
    if (p->frequency < 1550000)
    { buf[3] |= 0x40; }
    else
      if (p->frequency < 2050000)
      { buf[3] |= 0x80; }
      else
        if (p->frequency < 2150000)
        { buf[3] |= 0xC0; }
...
static int philips_tdm1316l_tuner_set_params (struct dvb_frontend * fe)
{
  ...
  { return -EINVAL; }
    ...
    if (tuner_frequency < 130000000)
    { cp = 3; }
    else
      if (tuner_frequency < 160000000)
      { cp = 5; }
      else
        if (tuner_frequency < 200000000)
        { cp = 6; }
        else
          if (tuner_frequency < 290000000)
          { cp = 3; }
          else
            if (tuner_frequency < 420000000)
            { cp = 5; }
            else
              if (tuner_frequency < 480000000)
              { cp = 6; }
              else
                if (tuner_frequency < 620000000)
                { cp = 3; }
                else
                  if (tuner_frequency < 830000000)
                  { cp = 5; }
                  else
                    if (tuner_frequency < 895000000)
                    { cp = 7; }
                    else
                    { return -EINVAL; }
