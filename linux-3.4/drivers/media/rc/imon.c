                              ...
                              unsigned char * buf, int len)
{
  ...
  if (ictx->product != 0xffdc && (buf[0] & 0x01) && len == 5) {
    ...
    if (ictx->product == 0xffdc && (buf[0] & 0x40) &&
      ...
      if (ictx->product == 0xffdc && (buf[0] == 0x68) ) {
        right_shift = 2;
        /* ch+/- buttons, which we use for an emulated scroll wheel */
      }
      else
        if (ictx->kc == KEY_CHANNELUP && (buf[2] & 0x40) != 0x40) {
          dir = 1;
        }
        else
          if (ictx->kc == KEY_CHANNELDOWN && (buf[2] & 0x40) != 0x40) {
            dir = -1;
          }
          else
          { mouse_input = false; }
                                  ...
                                  unsigned char * buf, u8 ktype)
{
  ...
  /* mouse button release on (some) 0xffdc devices */
    ...
    if (ictx->kc == KEY_RESERVED && buf[0] == 0x68 && buf[1] == 0x82 &&
        buf[2] == 0x81 && buf[3] == 0xb7)
    { ictx->kc = ictx->last_keycode; }
    
    /* mouse button release on (some other) 0xffdc devices */
    else
      if (ictx->kc == KEY_RESERVED && buf[0] == 0x01 && buf[1] == 0x00 &&
          buf[2] == 0x81 && buf[3] == 0xb7)
      { ictx->kc = ictx->last_keycode; }
      ...
      /* mce-specific button handling, no keyup events */
        ...
        if (ktype == IMON_KEY_MCE) {
          ...
          if (ictx->kc == KEY_RESERVED)
          { press_type = -EINVAL; }
          
          /* key release of 0xXXXXXXb7 key */
          else
            if (ictx->release_code)
            { press_type = 0; }
                                 ...
                                 struct usb_host_interface * iface_desc)
{
  ...
  for (i = 0; i < num_endpts && ! (ir_ep_found && display_ep_found); ++i) {
    ...
    if (!ir_ep_found && ep_dir == USB_DIR_IN &&
      ...
      if (!display_ep_found && ep_dir == USB_DIR_OUT &&
          ep_type == USB_ENDPOINT_XFER_INT) {
        tx_endpoint = ep;
  ...
  if (ifnum == 0) {
    ...
    if (ictx->display_supported) {
      ...
      { usb_deregister_dev (interface, &imon_lcd_class); }
        ...
        if (ictx->display_type == IMON_DISPLAY_TYPE_VFD)
