...
static int ati_remote_compute_accel (struct ati_remote * ati_remote)
{
  ...
  if (time_after (now, ati_remote->old_jiffies + msecs_to_jiffies (250) ) ) {
    ...
    if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (125) ) )
    { acc = accel[0]; }
    else
      if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (250) ) )
      { acc = accel[1]; }
      else
        if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (500) ) )
        { acc = accel[2]; }
        else
          if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (1000) ) )
          { acc = accel[3]; }
          else
            if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (1500) ) )
            { acc = accel[4]; }
            else
              if (time_before (now, ati_remote->acc_jiffies + msecs_to_jiffies (2000) ) )
              { acc = accel[5]; }
              else
              { acc = accel[6]; }
