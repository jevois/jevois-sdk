                                 ...
                                 unsigned int count)
{
  ...
  if (debugifc_match_keyword (wptr, wlen, "reset") ) {
    ...
    if (debugifc_match_keyword (wptr, wlen, "cpu") ) {
      ...
      if (debugifc_match_keyword (wptr, wlen, "bus") ) {
        pvr2_hdw_device_reset (hdw);
      }
      else
        if (debugifc_match_keyword (wptr, wlen, "soft") ) {
          return pvr2_hdw_cmd_powerup (hdw);
        }
        else
          if (debugifc_match_keyword (wptr, wlen, "deep") ) {
            return pvr2_hdw_cmd_deep_reset (hdw);
          }
          else
            if (debugifc_match_keyword (wptr, wlen, "firmware") ) {
              return pvr2_upload_firmware2 (hdw);
            }
            else
              if (debugifc_match_keyword (wptr, wlen, "decoder") ) {
                return pvr2_hdw_cmd_decoder_reset (hdw);
              }
              else
                if (debugifc_match_keyword (wptr, wlen, "worker") ) {
                  return pvr2_hdw_untrip (hdw);
                }
                else
                  if (debugifc_match_keyword (wptr, wlen, "usbstats") ) {
    ...
    if (debugifc_match_keyword (wptr, wlen, "cpufw") ) {
      scnt = debugifc_isolate_word (buf, count, &wptr, &wlen);
      if (!scnt) { return -EINVAL; }
      count -= scnt; buf += scnt;
      if (!wptr) { return -EINVAL; }
      if (debugifc_match_keyword (wptr, wlen, "fetch") ) {
        ...
        if (debugifc_match_keyword (wptr, wlen, "done") ) {
          pvr2_hdw_cpufw_set_enabled (hdw, 0, 0);
          return 0;
        }
        else {
      ...
      if (debugifc_match_keyword (wptr, wlen, "gpio") ) {
        int dir_fl = 0;
        int ret;
        u32 msk, val;
        scnt = debugifc_isolate_word (buf, count, &wptr, &wlen);
        ...
        if (debugifc_match_keyword (wptr, wlen, "dir") ) {
          ...
          if (!debugifc_match_keyword (wptr, wlen, "out") ) {
            return -EINVAL;
          }
        scnt = debugifc_isolate_word (buf, count, &wptr, &wlen);
        if (!scnt) { return -EINVAL; }
