                                   ...
                                   struct v4l2_format * f)
{
  ...
  if (is_ntsc) {
    ...
    { f->fmt.pix.width = LINE_SZ_4CIFS_NTSC; }
      ...
      if (f->fmt.pix.width >= LINE_SZ_2CIFS_NTSC)
      { f->fmt.pix.width = LINE_SZ_2CIFS_NTSC; }
      else
        if (f->fmt.pix.width >= LINE_SZ_1CIFS_NTSC)
    ...
    if (f->fmt.pix.width >= LINE_SZ_4CIFS_PAL) {
      ...
      if (f->fmt.pix.width >= LINE_SZ_2CIFS_PAL) {
        f->fmt.pix.width = LINE_SZ_2CIFS_PAL;
        field = V4L2_FIELD_TOP;
      }
      else
        if (f->fmt.pix.width >= LINE_SZ_1CIFS_PAL) {
                          ...
                          struct v4l2_streamparm * sp)
{
  ...
  { sp->parm.capture.timeperframe.numerator = def_num; }
    ...
    { sp->parm.capture.timeperframe.numerator = def_num; }
      ...
      if (sp->parm.capture.timeperframe.numerator <= (def_num * 2) ) {
        sp->parm.capture.timeperframe.numerator = def_num * 2;
        fdec = FDEC_2;
      }
      else
        if (sp->parm.capture.timeperframe.numerator <= (def_num * 3) ) {
          sp->parm.capture.timeperframe.numerator = def_num * 3;
          fdec = FDEC_3;
        }
        else {
...
static int s2255_open (struct file * file)
{
  ...
  case S2255_FW_LOADED_DSPWAIT:
    /* give S2255_LOAD_TIMEOUT time for firmware to load in case    driver loaded and then device immediately opened */
