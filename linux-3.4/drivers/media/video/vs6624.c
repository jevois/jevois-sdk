                              ...
                              struct v4l2_mbus_framefmt * fmt)
{
  ...
  { vs6624_write (sd, VS6624_IMAGE_SIZE0, 0x2); }
    ...
    if ( (fmt->width == QVGA_WIDTH) && (fmt->height == QVGA_HEIGHT) )
    { vs6624_write (sd, VS6624_IMAGE_SIZE0, 0x4); }
    else
      if ( (fmt->width == QQVGA_WIDTH) && (fmt->height == QQVGA_HEIGHT) )
      { vs6624_write (sd, VS6624_IMAGE_SIZE0, 0x6); }
      else
        if ( (fmt->width == CIF_WIDTH) && (fmt->height == CIF_HEIGHT) )
        { vs6624_write (sd, VS6624_IMAGE_SIZE0, 0x3); }
        else
          if ( (fmt->width == QCIF_WIDTH) && (fmt->height == QCIF_HEIGHT) )
          { vs6624_write (sd, VS6624_IMAGE_SIZE0, 0x5); }
          else
            if ( (fmt->width == QQCIF_WIDTH) && (fmt->height == QQCIF_HEIGHT) )
