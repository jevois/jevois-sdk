...
u32 cx231xx_Get_Colibri_CarrierOffset (u32 mode, u32 standerd)
{
  ...
  if (mode == TUNER_MODE_FM_RADIO) {
    ...
    if (standerd & (V4L2_STD_MN | V4L2_STD_NTSC_M_JP) ) {
      colibri_carrier_offset = 4832000;  /*4.83MHz  */
    }
    else
      if (standerd & (V4L2_STD_PAL_B | V4L2_STD_PAL_G) ) {
        colibri_carrier_offset = 2700000;  /*2.70MHz       */
      }
      else
        if (standerd & (V4L2_STD_PAL_D | V4L2_STD_PAL_I
                        | V4L2_STD_SECAM) ) {
        ...
        if (standard & V4L2_STD_PAL_M) {
          ...
          if (standard & (V4L2_STD_PAL_N | V4L2_STD_PAL_Nc) ) {
            ...
            if (standard &
              ...
              if (standard & (V4L2_STD_SECAM_L | V4L2_STD_SECAM_LC) ) {
                /* Is it SECAM_L1? */
                status = cx231xx_reg_mask_write (dev, VID_BLK_I2C_ADDRESS, 32,
                                                 DIF_PLL_CTRL, 0, 31, 0x6503bc0c);
                status = cx231xx_reg_mask_write (dev, VID_BLK_I2C_ADDRESS, 32,
