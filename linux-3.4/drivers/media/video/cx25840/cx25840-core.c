...
static int set_v4lstd (struct i2c_client * client)
{
  ...
  if (state->std == V4L2_STD_NTSC_M_JP) {
    ...
    if (state->std == V4L2_STD_NTSC_443) {
      fmt = 0x3;
    }
    else
      if (state->std == V4L2_STD_PAL_M) {
        ...
        if (state->std == V4L2_STD_PAL_N) {
          fmt = 0x6;
        }
        else
          if (state->std == V4L2_STD_PAL_Nc) {
            fmt = 0x7;
          }
          else
            if (state->std == V4L2_STD_PAL_60) {
              fmt = 0x8;
            }
            else {
...
static u32 get_cx2388x_ident (struct i2c_client * client)
{
  ...
  if (cx25840_read4 (client, 0x204) & 0xffff) {
    ...
    if (cx25840_read4 (client, 0x300) & 0x0fffffff) {
  ...
  if ( (device_id & 0xff00) == 0x8300) {
    ...
    if ( (device_id & 0xff00) == 0x8400) {
      ...
      if (device_id == 0x0000) {
        ...
        if ( (device_id & 0xfff0) == 0x5A30) {
          /* The CX23100 (0x5A3C = 23100) doesn't have an A/V decoder */
          id = V4L2_IDENT_CX2310X_AV;
        }
        else
          if ( (device_id & 0xff) == (device_id >> 8) ) {
            v4l_err (client,
