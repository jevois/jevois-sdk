...
int cci_read_a8_d8_no_restart (struct v4l2_subdev * sd, unsigned char addr, unsigned char * value)
{
  unsigned char data;
  struct i2c_msg msg;
  int ret;
  ...
  if (ret < 0)
  {
    cci_err ("%s error sending register address! slave = 0x%x, addr = 0x%2x\n ", __func__, client->addr, addr);
    return ret;
  }
  ...
  if (ret >= 0) {
    *value = data;
    ...
    cci_err ("%s error reading register value! slave = 0x%x, addr = 0x%2x, value = 0x%2x\n ", __func__, client->addr, addr, *value);
  }
  return ret;
}
EXPORT_SYMBOL_GPL (cci_read_a8_d8_no_restart);
...
int cci_write_a8_d8_no_restart (struct v4l2_subdev * sd, unsigned char addr,
                                unsigned char value)
{
  #ifdef USE_SPECIFIC_CCI
  struct cci_driver * cci_drv;
  ...
  ret = i2c_transfer (client->adapter, &msg, 1);
  if (ret >= 0) { ret = 0; }
  else { cci_err ("%s error! slave = 0x%x, addr = 0x%2x, value = 0x%2x\n ", __func__, client->addr, addr, value); }
...
EXPORT_SYMBOL_GPL (cci_write_a8_d8_no_restart);
  ...
  if (8 == addr_width && 8 == data_width)
  {
    ...
    if (8 == addr_width && 16 == data_width)
    {
      ...
      if (16 == addr_width && 8 == data_width)
      {
        ...
        if (16 == addr_width && 16 == data_width)
        {
          return cci_write_a16_d16 (sd, addr, value);
        }
        else
          if (0 == addr_width && 16 == data_width)
          {
