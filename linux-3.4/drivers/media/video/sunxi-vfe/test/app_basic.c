...
static int read_frame (void)
{
	...
	if(count == (read_num - 10))
    {
	        ...
	        if(buf_size_1 != 0)
	        {
			    ...
			    fwrite(buffers[buf.index].start, buf_size_1*2/3, 1, file_fd);
    			    file_fd = fopen("/mnt/sdcard/fb1_u.bin","w");    
			    fwrite(buffers[buf.index].start + buf_size_1*2/3, buf_size_1/6, 1, file_fd);
    			    file_fd = fopen("/mnt/sdcard/fb1_v.bin","w");    
			    fwrite(buffers[buf.index].start + buf_size_1*2/3 + buf_size_1/6,  buf_size_1/6, 1, file_fd);
		}
	        if(buf_size_2 != 0)
	        {
			    file_fd = fopen("/mnt/sdcard/fb2_y.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1), buf_size_2*2/3, 1, file_fd);
   			    file_fd = fopen("/mnt/sdcard/fb2_u.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1) + buf_size_2*2/3, buf_size_2/6, 1, file_fd);
   			    file_fd = fopen("/mnt/sdcard/fb2_v.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1)+ buf_size_2*2/3+ buf_size_2/6, buf_size_2/6, 1, file_fd);

		 }
	        if(buf_size_3 != 0)
	        {
			    file_fd = fopen("/mnt/sdcard/fb3_y.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1+buf_size_2), buf_size_3*2/3, 1, file_fd);
    			    file_fd = fopen("/mnt/sdcard/fb3_u.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1+buf_size_2) + buf_size_3*2/3, buf_size_3/6, 1, file_fd);
  			    file_fd = fopen("/mnt/sdcard/fb3_v.bin","w");
			    fwrite(buffers[buf.index].start+ALIGN_4K(buf_size_1+buf_size_2) + buf_size_3*2/3 + buf_size_3/6, buf_size_3/6, 1, file_fd);
		 }
	}
