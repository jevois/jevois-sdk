...
static void vfe_dump_csi_regs (struct vfe_dev * dev)
{
  ...
  if (vfe_dump & DUMP_CSI)
  {
    if (5 == frame_cnt % 10)
    {
      ...
      {
        val = vfe_reg_readl (dev->regs.csi_regs + i);
        printk ("%08x: %08x ++++----++++----++++----++++----\n", i, val);
        printk ("                   ");
        for (j = 31; j >= 0; --j) if (val & (1 << j) ) { printk ("1"); }
          else { printk ("0"); }
        printk ("\n");
      }
    }
...
static int isp_set_capture_flash (struct vfe_dev * dev)
{
      ...
      dev->isp_gen_set_pt->isp_ini_cfg.isp_tunning_settings.flash_delay_frame)
  {
    ...
    if (ev_cumul >= 100)
    {
      ...
      if (ev_cumul >= dev->isp_gen_set_pt->isp_ini_cfg.isp_tunning_settings.flash_gain * 100 / 256 && ev_cumul < 100)
      {
        dev->isp_gen_set_pt->exp_settings.tbl_cnt = CLIP (dev->isp_gen_set_pt->exp_settings.expect_tbl_cnt, 1,
            dev->isp_gen_set_pt->exp_settings.tbl_max_ind);
      }
      else
        if (ev_cumul >= -25 && ev_cumul < dev->isp_gen_set_pt->isp_ini_cfg.isp_tunning_settings.flash_gain * 100 / 256)
        {
...
static int buffer_setup (struct videobuf_queue * vq, unsigned int * count, unsigned int * size)
{
  ...
  *size = dev->buf_byte_size;
  ...
  vfe_dbg (1, "%s, buffer count=%d, size=%d\n", __func__, *count, *size);
                           ...
                           enum v4l2_field field)
{
  ...
  if (dev->width  < MIN_WIDTH || dev->width  > MAX_WIDTH ||
    ...
    vfe_err ("buffer_prepare size problem\n");
    return -EINVAL;
  }
  ...
  if (VIDEOBUF_NEEDS_INIT == buf->vb.state) {
    ...
    if (rc < 0) {
      vfe_err ("buffer_prepare init problem\n");
      goto fail;
    }
  }
    ...
    if (pix_fmt_type == YUV422_INTLVD) {
      ...
      if (pix_fmt_type == BAYER_RGB) {
        ...
        if (pix_fmt_type == RGB565) {
          ...
          if (pix_fmt_type == RGB888 || pix_fmt_type == PRGB888) {
                                 ...
                                 struct v4l2_format * f)
{
  ...
  if (mbus_cfg.type == V4L2_MBUS_CSI2) {
    ...
    { dev->mipi_para.lane_num = 4; }
      ...
      if (IS_FLAG (mbus_cfg.flags, V4L2_MBUS_CSI2_3_LANE) )
      { dev->mipi_para.lane_num = 3; }
      else
        if (IS_FLAG (mbus_cfg.flags, V4L2_MBUS_CSI2_2_LANE) )
    ...
    dev->total_rx_ch = dev->total_bus_ch;
    ...
    if (mbus_cfg.type == V4L2_MBUS_PARALLEL) {
  ...
  if (mbus_cfg.type == V4L2_MBUS_CSI2) {
    ...
    dev->mipi_para.auto_check_bps = 0; 
    dev->mipi_para.dphy_freq = DPHY_CLK;
  ...
  if (dev->is_isp_used) {
    ...
    if (f->fmt.pix.subchannel) {
      if (f->fmt.pix.subchannel->rot_angle == 0) {
        ...
        if (f->fmt.pix.subchannel->rot_angle == 90) {
          ...
          if (f->fmt.pix.subchannel->rot_angle == 180) {
            isp_size[ROT_CH].width = f->fmt.pix.subchannel->width;
            isp_size[ROT_CH].height = f->fmt.pix.subchannel->height;
            bsp_isp_set_rot (SUB_CH, ANGLE_180);
          }
          else
            if (f->fmt.pix.subchannel->rot_angle == 270) {
              isp_size[ROT_CH].height = f->fmt.pix.subchannel->width;
              isp_size[ROT_CH].width = f->fmt.pix.subchannel->height;
    ...
    ob_black_size.width = win_cfg.width_input + 2 * win_cfg.hoffset;
    ob_black_size.height = win_cfg.height_input + 2 * win_cfg.voffset;
    ob_valid_size.width = win_cfg.width_input;
    ob_valid_size.height = win_cfg.height_input;
    ob_start.hor =  win_cfg.hoffset; 
    ob_start.ver =  win_cfg.voffset; 
    if ( (f->fmt.pix.rot_angle == 0 && f->fmt.pix.subchannel == NULL) ||
         (f->fmt.pix.rot_angle == 0 && f->fmt.pix.subchannel != NULL && f->fmt.pix.subchannel->rot_angle == 0) )
    {
  ...
  if (dev->is_isp_used == 1 )
  {
    ...
    if (dev->is_bayer_raw == 1)
    {
      ...
      if (V4L2_MODE_IMAGE == dev->capture_mode)
      {
        ...
        if (V4L2_MODE_VIDEO == dev->capture_mode)
        {
...
static int vidioc_querymenu (struct file * file, void * fh, struct v4l2_querymenu * a)
{
  struct vfe_dev * dev = video_drvdata (file);
  int ret;
  
  ret = v4l2_subdev_call (dev->sd, core, querymenu, a);
  if (ret < 0) { vfe_warn ("v4l2 sub device querymenu failed\n"); }
...
static int vfe_mmap (struct file * file, struct vm_area_struct * vma)
{
  ...
  if (turbo == 1) { }
    ...
    if (turbo == 2) { vma->vm_page_prot = pgprot_noncached (vma->vm_page_prot); }
    else { vma->vm_page_prot = pgprot_dmacoherent (vma->vm_page_prot); }
...
static int vfe_resource_request (struct platform_device * pdev , struct vfe_dev * dev)
{
  ...
  for (i = 0, j = 0; i < pdev->num_resources; i++)
  {
    ...
    if (!strcmp (vfe_res_name[j], vfe_res_csi) ) {
      ...
      if (!strcmp (vfe_res_name[j], vfe_res_isp) ) {
    ...
    struct ccm_config * ccm_cfg, struct i2c_board_info * sensor_i2c_board)
{
  int ret;
  ccm_cfg->sd = NULL;
  ccm_cfg->sd = cci_bus_match (ccm_cfg->ccm, dev->id, sensor_i2c_board->addr);
  if (ccm_cfg->sd)
  {
...
{
  ccm_cfg->sd_act = NULL;
  ccm_cfg->sd_act = cci_bus_match (ccm_cfg->act_name, dev->id, act_i2c_board->addr);
  if (!ccm_cfg->sd_act) {
...
static int vfe_actuator_subdev_register (struct vfe_dev * dev, struct ccm_config * ccm_cfg, struct i2c_board_info * act_i2c_board)
{
  ...
  struct i2c_adapter * i2c_adap_act = i2c_get_adapter (ccm_cfg->twi_id);
...
static int cpy_ccm_sub_device_cfg (struct ccm_config * ccm_cfg, int n)
{
  ...
  if (ccm_cfg->sensor_cfg_ini->camera_inst[n].sensor_type == SENSOR_RAW)
  {
    ...
    if (ccm_cfg->sensor_cfg_ini->camera_inst[n].sensor_type == SENSOR_YUV)
    {
