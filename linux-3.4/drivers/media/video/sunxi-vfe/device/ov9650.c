...
static int sensor_g_autoexp (struct v4l2_subdev * sd, __s32 * value)
{
  ...
  SENSOR_READ (REG_COM8);
  if (val & COM8_AEC) { *value = V4L2_EXPOSURE_AUTO; }
  else { *value = V4L2_EXPOSURE_MANUAL; }
  info->autoexp = *value;
  return 0;
}
...
static int sensor_s_exposure_absolute (struct v4l2_subdev * sd, int value)
{
  ...
  if (exposure < 1) { exposure = 1; }
    ...
    if (exposure > info->exp_max_lines) { exposure = info->exp_max_lines; }
...
static int sensor_g_autowb (struct v4l2_subdev * sd, int * value)
{
  ...
  SENSOR_READ (REG_COM8);
  if (val & COM8_AWB) { *value = 1; info->autowb = 1; }
  else { *value = 0; info->autowb = 0; }
  return 0;
}
...
static int sensor_s_sharpness (struct v4l2_subdev * sd, int value)
{
  ...
  if (v) { val |= COM14_EDGE_EN; }
  else { val &= ~COM14_EDGE_EN; }
  
  v -= 1; if (v > 0x0f) { val |= COM14_EEF_X2; v >>= 1; }
  else { val &= ~COM14_EEF_X2; }
...
static int sensor_s_parm (struct v4l2_subdev * sd, struct v4l2_streamparm * parms)
{
  ...
  if (ratex100 > 1200 * 100) { return -EINVAL; }
    ...
    if (ratex100 > 60 * 100) { basex100 = 12000; if (prescaler < 8) { return -EINVAL; } else { prescaler >>= 3; } }
    else
      if (ratex100 > 30 * 100) { basex100 = 6000; if (prescaler < 4) { return -EINVAL; } else { prescaler >>= 2; } }
      else
        if (ratex100 > 15 * 100) { basex100 = 3000; if (prescaler < 2) { return -EINVAL; } else { prescaler >>= 1; } }
        else
          if (ratex100 > 750)    { basex100 = 1500; if (prescaler < 1) { return -EINVAL; } }
          else
            if (ratex100 > 375)    { basex100 = 750; prescaler <<= 1; }
            else
              if (ratex100 >= 10)    { basex100 = 375; prescaler <<= 2; }
              else { return -EINVAL; }
