...
int bsp_csi_set_fmt (unsigned int sel, struct bus_info * bus_info, struct frame_info * frame_info)
{
  ...
  {
  case PARALLEL: if_cfg.interface = CSI_IF_INTLV; break;
  case BT656:    if_cfg.interface = CSI_IF_CCIR656_1CH; break;
  case CSI2:     if_cfg.interface = CSI_IF_MIPI; break;
  default:       printk ("JEVOIS: bad bus\n"); return -1;
  }
  ...
  for (ch = 0; ch < bus_info->ch_total_num; ++ch)
  {
    ...
    if (if_cfg.interface != CSI_IF_MIPI) { if_cfg.data_width = bus_width[ch]; }
    
    switch (frame_info->ch_field[ch])
    {
    case FIELD_ANY:
    case FIELD_NONE: if_cfg.src_type = CSI_PROGRESSIVE; fmt_cfg[ch].field_sel = CSI_EITHER; break;
    
    case FIELD_TOP: if_cfg.src_type = CSI_INTERLACE; fmt_cfg[ch].field_sel = CSI_ODD; break;
    
    case FIELD_BOTTOM: if_cfg.src_type = CSI_INTERLACE; fmt_cfg[ch].field_sel = CSI_EVEN; break;
    
    case FIELD_INTERLACED:
    case FIELD_INTERLACED_TB:
      if_cfg.src_type = CSI_INTERLACE; tmg_cfg.field = CSI_FIELD_TF; fmt_cfg[ch].field_sel = CSI_EITHER;
      break;
      
    case FIELD_INTERLACED_BT:
      if_cfg.src_type = CSI_INTERLACE; tmg_cfg.field = CSI_FIELD_BF; fmt_cfg[ch].field_sel = CSI_EITHER;
      break;
      
    default: printk ("JEVOIS field problem\n"); return -1;
    }
    ...
    case PIX_FMT_RGB565:
      if (bus_pix_type[ch] == BUS_FMT_RGB565) {
        ...
        printk ("JEVOIS bad format RGB565\n");
        return -1;
      }
      break;
    ...
    case PIX_FMT_RGB888:
      if (bus_pix_type[ch] == BUS_FMT_RGB888) {
        ...
        printk ("JEVOIS bad format RGB888\n");
        return -1;
      }
      break;
    ...
    case PIX_FMT_PRGB888:
      if (bus_pix_type[ch] == BUS_FMT_RGB888) {
        ...
        printk ("JEVOIS bad format PRGB888\n");
        return -1;
      }
      break;
    ...
    case PIX_FMT_YUYV:
      if (bus_pix_type[ch] == BUS_FMT_YUYV) {
        ...
        printk ("JEVOIS bad format YUYV 0x%x BUS_FMT_YUYV=0x%x\n", bus_pix_type[ch], BUS_FMT_YUYV);
        return -1;
      }
      break;
    ...
    case PIX_FMT_YVU420MB_8:
      if (bus_pix_type[ch] == BUS_FMT_YUYV || bus_pix_type[ch] == BUS_FMT_YVYU || bus_pix_type[ch] == BUS_FMT_UYVY ||
          bus_pix_type[ch] == BUS_FMT_VYUY)
      { fmt_cfg[ch].input_fmt = CSI_YUV422; }
      else
        if (bus_pix_type[ch] == BUS_FMT_YY_YUYV || bus_pix_type[ch] == BUS_FMT_YY_YVYU ||
            bus_pix_type[ch] == BUS_FMT_YY_UYVY || bus_pix_type[ch] == BUS_FMT_YY_VYUY)
        { fmt_cfg[ch].input_fmt = CSI_YUV420; }
        else
          if (bus_pix_type[ch] == BUS_FMT_SBGGR || bus_pix_type[ch] == BUS_FMT_SGBRG ||
              bus_pix_type[ch] == BUS_FMT_SRGGB || bus_pix_type[ch] == BUS_FMT_SGRBG)
          { fmt_cfg[ch].input_fmt = CSI_RAW; } 
          
      if (fmt_cfg[ch].input_fmt == CSI_YUV422 || fmt_cfg[ch].input_fmt == CSI_YUV420)
      {
        ...
        { fmt_cfg[ch].output_fmt = (is_buf_itl[ch] == 1) ? CSI_FRAME_PLANAR_YUV420 : CSI_FIELD_PLANAR_YUV420; }
          ...
          if (frame_info->pix_ch_fmt[ch] == PIX_FMT_YUV420SP_8 || frame_info->pix_ch_fmt[ch] == PIX_FMT_YVU420SP_8)
          { fmt_cfg[ch].output_fmt = (is_buf_itl[ch] == 1) ? CSI_FRAME_UV_CB_YUV420 : CSI_FIELD_UV_CB_YUV420; }
          else
            if (frame_info->pix_ch_fmt[ch] == PIX_FMT_YUV420MB_8 || frame_info->pix_ch_fmt[ch] == PIX_FMT_YVU420MB_8)
    ...
    case PIX_FMT_YVU422MB_8:
      if (bus_pix_type[ch] == BUS_FMT_YUYV || bus_pix_type[ch] == BUS_FMT_YVYU ||
          bus_pix_type[ch] == BUS_FMT_UYVY || bus_pix_type[ch] == BUS_FMT_VYUY)
      { fmt_cfg[ch].input_fmt = CSI_YUV422; }
      else
        if (bus_pix_type[ch] == BUS_FMT_SBGGR || bus_pix_type[ch] == BUS_FMT_SGBRG ||
            bus_pix_type[ch] == BUS_FMT_SRGGB || bus_pix_type[ch] == BUS_FMT_SGRBG)
      ...
      if (fmt_cfg[ch].input_fmt == CSI_YUV422)
      {
        ...
        { fmt_cfg[ch].output_fmt = (is_buf_itl[ch] == 1) ? CSI_FRAME_PLANAR_YUV422 : CSI_FIELD_PLANAR_YUV422; }
          ...
          if (frame_info->pix_ch_fmt[ch] == PIX_FMT_YUV422SP_8 || frame_info->pix_ch_fmt[ch] == PIX_FMT_YVU422SP_8)
          { fmt_cfg[ch].output_fmt = (is_buf_itl[ch] == 1) ? CSI_FRAME_UV_CB_YUV422 : CSI_FIELD_UV_CB_YUV422; }
          else
            if (frame_info->pix_ch_fmt[ch] == PIX_FMT_YUV422MB_8 || frame_info->pix_ch_fmt[ch] == PIX_FMT_YVU422MB_8)
    ...
    case PIX_FMT_SRGGB_12:
      ...
      break;
    default:
      printk ("JEVOIS bad format UNKNOWN\n");
      return -1;
    }
