...
#define SDRAM_REGS_pBASE    (REGS_pBASE + 0x01000)   
...
static struct csi_fmt formats[] = {
  ...
  {
    .name         = "planar YUV 420",
    .ccm_fmt      = V4L2_MBUS_FMT_YUYV8_2X8, 
  ...
  {
    .name         = "RAW Bayer",
    .ccm_fmt      = V4L2_MBUS_FMT_SBGGR8_1X8,
...
static inline void csi_set_addr (struct csi_dev * dev, struct csi_buffer * buffer)
{
  ...
  if (dev->fmt->input_fmt == CSI_RAW) {
    ...
    if (dev->fmt->input_fmt == CSI_BAYER) {
      dev->csi_buf_addr.cb = addr_org;//for G channel
      dev->csi_buf_addr.y  = addr_org + dev->width * dev->height * 1 / 2;
      dev->csi_buf_addr.cr = addr_org + dev->width * dev->height * 3 / 4;
      
    }
    else
      if (dev->fmt->input_fmt == CSI_CCIR656) {
        
      }
      else
        if (dev->fmt->input_fmt == CSI_YUV422) {
...
{
  clk_prepare_enable (dev->csi_module_clk);
}

static void csi_reset_disable (struct csi_dev * dev)
{
  clk_disable_unprepare (dev->csi_module_clk);
}
...
static int fetch_config (struct csi_dev * dev)
{
  ...
  if (dev->dev_qty > 0)
  {
    ...
    if ( (dev->ccm_cfg[0]->i2c_addr == 0xff) && (ret == 0) )
    {
  ...
  if (dev->dev_qty > 1)
  {
    ...
    if ( (dev->ccm_cfg[1]->i2c_addr == 0xff) && (ret == 0) )
    {
...
reg_sd:
  ...
  *vfd = csi_template;
  ...
  videobuf_queue_dma_contig_init (&dev->vb_vidq, &csi_video_qops,
                                  ...
                                  sizeof (struct csi_buffer), dev, NULL);
