  ...
  while (1) {
    ...
    if (scratch_len (usbvision) ) {
      if (frame->scanstate == scan_state_scanning) {
        ...
        if (frame->scanstate == scan_state_lines) {
                          ...
                          int height)
{
  ...
  { frame_drop = frame_rate * 32 / 25 - 1; }
    ...
    if (usbvision->tvnorm_id & V4L2_STD_525_60)
...
int usbvision_set_input (struct usb_usbvision * usbvision)
{
  ...
  if (usbvision_device_data[usbvision->dev_model].vin_reg1_override) {
    ...
    if (usbvision_device_data[usbvision->dev_model].codec == CODEC_SAA7113) {
  ...
  if (usbvision->tvnorm_id & V4L2_STD_PAL) {
    ...
    if (usbvision->tvnorm_id & V4L2_STD_SECAM) {
      value[0] = 0xC0;
      value[1] = 0x02;  /* 0x02C0 -> 704 Input video line length */
      value[2] = 0x20;
      value[3] = 0x01;  /* 0x0120 -> 288 Input video n. of lines */
