...
static int sd_init (struct gspca_dev * gspca_dev)
{
  ...
  if (init_ov_sensor (sd, OV7xx0_SID) >= 0) {
    ...
    if (init_ov_sensor (sd, OV6xx0_SID) >= 0) {
      ov6xx0_configure (sd);
      
      /* Test for 8xx0 */
    }
    else
      if (init_ov_sensor (sd, OV8xx0_SID) >= 0) {
        ov8xx0_configure (sd);
        
        /* Test for 3xxx / 2xxx */
      }
      else
        if (init_ov_sensor (sd, OV_HIRES_SID) >= 0) {
