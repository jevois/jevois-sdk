...
s32 gup_enter_update_mode (struct i2c_client * client)
{
  ...
  {
    ret = gtp_i2c_write (client, inbuf, 2); 
    if (ret > 0)
    {
...
static s16 gup_check_version (u16 sw_ver)
{
  if ( (sw_ver & 0xff) < TPD_CHIP_VERSION_C_FIRMWARE_BASE )
  {
    ...
    if ( (sw_ver & 0xff) < TPD_CHIP_VERSION_D1_FIRMWARE_BASE )
    {
      return TPD_GT818_VERSION_C;
    }
    else
      if ( (sw_ver & 0xff) < TPD_CHIP_VERSION_E_FIRMWARE_BASE)
      {
        return TPD_GT818_VERSION_D1;
      }
      else
        if ( (sw_ver & 0xff) < TPD_CHIP_VERSION_D2_FIRMWARE_BASE)
        {
