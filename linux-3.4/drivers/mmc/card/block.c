                                   ...
                                   struct device_attribute * attr, char * buf)
{
  ...
  { locked = 2; }
    ...
    if (card->ext_csd.boot_ro_lock & EXT_CSD_BOOT_WP_B_PWR_WP_EN)
    { locked = 1; }
                                    ...
                                    struct mmc_ioc_erase_cmd __user * ic_ptr)
{
  ...
  if (erase_cmd.flags == 0xa) {
    ...
    if ( (erase_cmd.start_sec < (offset >> 9) ) || (erase_cmd.size_sec > (size >> 9) ) ) {
                               ...
                               unsigned int nr)
{
  ...
  if (card->ext_csd.rev <= 5) /* ver 4.41 or older */
  {
    if (mmc_can_secure_erase_trim (card) && mmc_can_trim (card) )
    {
      ...
      if (mmc_can_secure_erase_trim (card) && mmc_can_erase (card) )
      {
    ...
    if (card->ext_csd.rev <= 7) /* v4.5 or newer */
    {
      if (!mmc_can_sanitize (card)
    ...
    struct mmc_ioc_erase_cmd __user * ic_ptr)
{
  ...
  if (erase_cmd.flags == 0xA) {
    ...
    if ( (erase_cmd.start_sec >= (offset >> 9) )
...
static int mmc_blk_issue_rq (struct mmc_queue * mq, struct request * req)
{
  ...
  if (req && req->cmd_flags & REQ_DISCARD) {
    ...
    if (req && req->cmd_flags & REQ_FLUSH) {
