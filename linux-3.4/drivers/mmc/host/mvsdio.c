                            ...
                            u32 err_status)
{
  ...
  if (err_status & MVSD_ERR_CMD_TIMEOUT) {
    ...
    if (err_status & (MVSD_ERR_CMD_CRC | MVSD_ERR_CMD_ENDBIT |
                             ...
                             u32 err_status)
{
  ...
  { data->error = -ETIMEDOUT; }
    ...
    if (err_status & (MVSD_ERR_DATA_CRC | MVSD_ERR_DATA_ENDBIT) )
    { data->error = -EILSEQ; }
    else
      if (err_status & MVSD_ERR_XFER_SIZE)
      { data->error = -EBADE; }
  ...
  if (data->stop) {
    ...
    if (err_status & MVSD_ERR_AUTOCMD12) {
      ...
      { data->stop->error = -ENOEXEC; }
        ...
        if (err_cmd12 & MVSD_AUTOCMD12_ERR_TIMEOUT)
  ...
  if (host->pio_size &&
    ...
    if (s <= 32) {
      ...
      if (s == 0) {
        ...
        if (host->intr_en & MVSD_NOR_RX_FIFO_8W) {
