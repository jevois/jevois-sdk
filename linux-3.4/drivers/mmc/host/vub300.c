  ...
  if (urb->status) {
    ...
    if (vub300->command_res_urb->actual_length == 0) {
      ...
      if (!vub300->data) {
        ...
        if (vub300->resp.common.header_type != 0x02) {
          /*
  ...
  if (vub300->app_spec) {
    ...
    case 6:
      ...
      { vub300->bus_width = 1; }
        ...
        if (0x00000002 == (0x00000003 & cmd->arg) )
        { vub300->bus_width = 4; }
        else
                                 ...
                                 struct mmc_command * cmd, struct mmc_data * data)
{
  ...
  if (linear_length < 64) {
    ...
    if ( (!vub300->large_usb_packets && (0 < modulo_64_length) ) ||
  ...
  if (respretval == 0) { /* TIMED OUT */
    ...
    if (respretval < 0) {
      ...
      if (cmd->error) {
        ...
        if (vub300->command_out_urb->status) {
          ...
          if (vub300->command_res_urb->status) {
            ...
            if (vub300->resp.common.header_type == 0x00) {
              /*
              ...
              if (vub300->resp.common.header_type == RESPONSE_ERROR) {
                cmd->error =
                  vub300_response_error (vub300->resp.error.error_code);
                if (vub300->data)
                { usb_sg_cancel (&vub300->sg_request); }
                ...
                if (vub300->resp.common.header_type == RESPONSE_PIGGYBACKED) {
                  ...
                  if (vub300->resp.common.header_type == RESPONSE_PIG_DISABLED) {
                    int offloaded_data_length =
                      vub300->resp.common.header_size -
                      sizeof (struct sd_register_header);
                    int register_count = offloaded_data_length >> 3;
                    ...
                    if (vub300->resp.common.header_type == RESPONSE_PIG_ENABLED) {
                      int offloaded_data_length =
                        vub300->resp.common.header_size -
                        sizeof (struct sd_register_header);
                      int register_count = offloaded_data_length >> 3;
...
static void vub300_cmndwork_thread (struct work_struct * work)
{
  ...
  if (!vub300->interface) {
    ...
    if (likely (vub300->vub_name[0]) || !vub300->mmc->card ||
      ...
      if (0 == vub300->mmc->card->sdio_funcs) {
    ...
    struct mmc_command * cmd)
{
  ...
  { return 0; }
    ...
    if (vub300->fn[func].offload_count == 0)
    { return 0; }
    else
