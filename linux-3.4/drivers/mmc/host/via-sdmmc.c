...
static void via_sdc_set_ios (struct mmc_host * mmc, struct mmc_ios * ios)
{
  ...
  { clock = PCI_CLK_48M; }
    ...
    if (ios->clock >= 33000000)
    { clock = PCI_CLK_33M; }
    else
      if (ios->clock >= 24000000)
      { clock = PCI_CLK_24M; }
      else
        if (ios->clock >= 16000000)
        { clock = PCI_CLK_16M; }
        else
          if (ios->clock >= 12000000)
          { clock = PCI_CLK_12M; }
          else
            if (ios->clock >=  8000000)
            { clock = PCI_CLK_8M; }
            else
            { clock = PCI_CLK_375K; }
...
static void via_sdc_cmd_isr (struct via_crdr_mmc_host * host, u16 intmask)
{
  ...
  { host->cmd->error = -ETIMEDOUT; }
    ...
    if (intmask & VIA_CRDR_SDSTS_SC)
    { host->cmd->error = -EILSEQ; }
    
  if (host->cmd->error)
  { tasklet_schedule (&host->finish_tasklet); }
  else
    if (intmask & VIA_CRDR_SDSTS_CRD)
...
static void via_sdc_data_isr (struct via_crdr_mmc_host * host, u16 intmask)
{
  ...
  { host->data->error = -ETIMEDOUT; }
    ...
    if (intmask & (VIA_CRDR_SDSTS_RC | VIA_CRDR_SDSTS_WC) )
