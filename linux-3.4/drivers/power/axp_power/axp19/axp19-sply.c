                                      ...
                                      union power_supply_propval * val)
{
  if (charger->bat_det) {
    ...
    { val->intval = POWER_SUPPLY_STATUS_CHARGING; }
      ...
      if ( charger->rest_vol == 100 && charger->ext_valid)
                                      ...
                                      union power_supply_propval * val)
{
  ...
  { val->intval = POWER_SUPPLY_HEALTH_DEAD; }
    ...
    if (charger->fault & AXP19_FAULT_LOG_OVER_TEMP)
    { val->intval = POWER_SUPPLY_HEALTH_OVERHEAT; }
    else
      if (charger->fault & AXP19_FAULT_LOG_COLD)
...
{
  if (Bat_Ocv_Vol > FUELGUAGE_TOP_VOL)       
  {
    return FUELGUAGE_TOP_LEVEL;
  }
  else
    if (Bat_Ocv_Vol < FUELGUAGE_LOW_VOL)  
    {
      return FUELGUAGE_LOW_LEVEL;
    }
    else
      if (Bat_Ocv_Vol < FUELGUAGE_VOL1)     
      {
...
static int axp_main_task (void * arg)
{
  ...
  while (1) {
    ...
    Bat_Ocv_Vol = (uint16_t) axp_bat_vol (charger->ext_valid && charger->bat_current_direction, \
                                          (int) Bat_Vol, charger->ibat, Bat_Rdc);
    ...
    if ( (j == AXP19_VOL_MAX) && (charger->bat_det == 1) ) {
      ...
      if ( ( (ocv_rest_vol > Pre_ocv_rest_cap) && (rt_charge_status < 0x04) )
           || (ocv_rest_vol < (Pre_ocv_rest_cap - 2) ) ) {
        ocv_rest_vol = (int) Pre_ocv_rest_cap;
      }
      else
        if ( ( (ocv_rest_vol < Pre_ocv_rest_cap) && (rt_charge_status > 0x03) )
             || (ocv_rest_vol > (Pre_ocv_rest_cap + 2) ) ) {
          ocv_rest_vol = (int) Pre_ocv_rest_cap;
        }
      ...
      if (cou_rest_vol > 100) {
        if (Real_Cou_Flag) {
          charger->rest_vol = ocv_rest_vol  + (3 * 100);  
        }
        else {
      ...
      if ( ( (charger->rest_vol > pre_rest_vol) && (rt_charge_status < 0x04) ) ) {
        charger->rest_vol = pre_rest_vol;
      }
      else
        if ( (charger->rest_vol < pre_rest_vol) && (rt_charge_status > 0x03) ) {
          charger->rest_vol = pre_rest_vol;
        }
