...
void axp81x_power_off (int power_start)
{
  ...
  if (axp81x_config.pmu_pwroff_vol >= 2600 && axp81x_config.pmu_pwroff_vol <= 3300) {
    if (axp81x_config.pmu_pwroff_vol > 3200) {
      ...
      if (axp81x_config.pmu_pwroff_vol > 3100) {
        val = 0x6;
      }
      else
        if (axp81x_config.pmu_pwroff_vol > 3000) {
          val = 0x5;
        }
        else
          if (axp81x_config.pmu_pwroff_vol > 2900) {
            val = 0x4;
          }
          else
            if (axp81x_config.pmu_pwroff_vol > 2800) {
              val = 0x3;
            }
            else
              if (axp81x_config.pmu_pwroff_vol > 2700) {
                val = 0x2;
              }
              else
                if (axp81x_config.pmu_pwroff_vol > 2600) {
                  val = 0x1;
                }
                else
                { val = 0x0; }
...
static void axp_set_charge (struct axp_charger * charger)
{
  ...
  if (charger->chgvol < AXP81X_CHARGE_VOLTAGE_LEVEL1) {
    ...
    if (charger->chgvol < AXP81X_CHARGE_VOLTAGE_LEVEL2) {
      val &= ~ (3 << 5);
      val |= 1 << 5;
    }
    else
      if (charger->chgvol < AXP81X_CHARGE_VOLTAGE_LEVEL3) {
        val &= ~ (3 << 5);
        val |= 1 << 6;
      }
      else
...
int axp81x_chg_current_limit (int current_limit)
{
  ...
  if (current_limit >= AXP81X_CHARGE_CURRENT_MIN && current_limit <= AXP81X_CHARGE_CURRENT_MAX) {
    ...
    if (current_limit < AXP81X_CHARGE_CURRENT_MIN) {
...
int axp81x_init (struct axp_charger * charger)
{
  ...
  { val = 0x00; }
    ...
    if (axp81x_config.pmu_pekon_time < 2000) {
      val = 0x40;
    }
    else
      if (axp81x_config.pmu_pekon_time < 3000) {
        val = 0x80;
      }
      else {
        val = 0xc0;
  ...
  { val = 0x00; }
    ...
    if (20 > axp81x_config.pmu_pekoff_delay_time)
    { val = 0x01; }
    else
      if (30 > axp81x_config.pmu_pekoff_delay_time)
      { val = 0x02; }
      else
        if (40 > axp81x_config.pmu_pekoff_delay_time)
        { val = 0x03; }
        else
          if (50 > axp81x_config.pmu_pekoff_delay_time)
          { val = 0x04; }
          else
            if (60 > axp81x_config.pmu_pekoff_delay_time)
            { val = 0x05; }
            else
              if (70 > axp81x_config.pmu_pekoff_delay_time)
              { val = 0x06; }
              else
              { val = 0x07; }
  ...
  if ( (axp81x_config.pmu_battery_cap) && (! (val & 0x80) ) ) {
    ...
    if (!axp81x_config.pmu_battery_cap) {
