...
static int axp22x_chg_current_temp_limit (int current_limit, int temp_limit)
{
  ...
  { arisc_adjust_pmu_chgcur (2550000, (unsigned int) temp_limit); }
    ...
    if (current_limit > 0 && current_limit < 300000)
...
static void axp_set_charge (struct axp_charger * charger)
{
  ...
  if (charger->chgvol < 4200000) {
    ...
    if (charger->chgvol < 4240000) {
      val &= ~ (3 << 5);
      val |= 1 << 5;
    }
    else
      if (charger->chgvol < 4350000) {
        val &= ~ (3 << 5);
        val |= 1 << 6;
      }
      else
                                      ...
                                      union power_supply_propval * val)
{
  ...
  { val->intval = POWER_SUPPLY_HEALTH_DEAD; }
    ...
    if (charger->fault & AXP22_FAULT_LOG_OVER_TEMP)
    { val->intval = POWER_SUPPLY_HEALTH_OVERHEAT; }
    else
      if (charger->fault & AXP22_FAULT_LOG_COLD)
...
static void axp_usb (struct work_struct * work)
{
  ...
  if ( (val & 0x10) == 0x00) { /*usb or usb adapter can not be used*/
    ...
    if (CHARGE_USB_20 == axp_usbcurflag) {
      if (axp_debug)
      ...
      if (CHARGE_USB_30 == axp_usbcurflag) {
        axp_clr_bits (charger->master, AXP22_CHARGE_VBUS, 0x03);
      }
      else {
        if (axp_debug)
  ...
  if (CHARGE_USB_20 == axp_usbvolflag) {
    ...
    if (CHARGE_USB_30 == axp_usbvolflag) {
      axp_read (charger->master, AXP22_CHARGE_VBUS, &val);
      val &= 0xC7;
      val |= 7 << 3;
      axp_write (charger->master, AXP22_CHARGE_VBUS, val);
...
static int axp_battery_probe (struct platform_device * pdev)
{
  ...
  { val &= 0x3f; }
    ...
    if (axp22_config.pmu_pekon_time < 2000) {
      val &= 0x3f;
      val |= 0x40;
    }
    else
      if (axp22_config.pmu_pekon_time < 3000) {
        val &= 0x3f;
        val |= 0x80;
      }
      else {
  ...
  { axp_set_bits (charger->master, AXP22_POK_SET, 0x04); }
  else
  { axp_clr_bits (charger->master, AXP22_POK_SET, 0x04); }
  
  #ifdef CONFIG_AXP809
  if (10 > axp22_config.pmu_pekoff_delay_time)
  { val = 0x00; }
  else
    if (20 > axp22_config.pmu_pekoff_delay_time)
    { val = 0x01; }
    else
      if (30 > axp22_config.pmu_pekoff_delay_time)
      { val = 0x02; }
      else
        if (40 > axp22_config.pmu_pekoff_delay_time)
        { val = 0x03; }
        else
          if (50 > axp22_config.pmu_pekoff_delay_time)
          { val = 0x04; }
          else
            if (60 > axp22_config.pmu_pekoff_delay_time)
            { val = 0x05; }
            else
              if (70 > axp22_config.pmu_pekoff_delay_time)
              { val = 0x06; }
              else
              { val = 0x07; }
  ...
  if ( (axp22_config.pmu_battery_cap) && (! (val2 & 0x80) ) ) {
    ...
    if (!axp22_config.pmu_battery_cap) {
