...
static void axp_power_off (void)
{
  ...
  if (axp22_config.pmu_pwroff_vol >= 2600 && axp22_config.pmu_pwroff_vol <= 3300) {
    if (axp22_config.pmu_pwroff_vol > 3200) {
      ...
      if (axp22_config.pmu_pwroff_vol > 3100) {
        val = 0x6;
      }
      else
        if (axp22_config.pmu_pwroff_vol > 3000) {
          val = 0x5;
        }
        else
          if (axp22_config.pmu_pwroff_vol > 2900) {
            val = 0x4;
          }
          else
            if (axp22_config.pmu_pwroff_vol > 2800) {
              val = 0x3;
            }
            else
              if (axp22_config.pmu_pwroff_vol > 2700) {
                val = 0x2;
              }
              else
                if (axp22_config.pmu_pwroff_vol > 2600) {
                  val = 0x1;
                }
                else
                { val = 0x0; }
