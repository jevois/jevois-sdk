...
static void axp_set_startup_sequence (struct axp_charger * charger)
{
  ...
  axp_write (charger->master, 0x03, 0x00);
  axp_write (charger->master, 0x04, 0x00);
                                      ...
                                      union power_supply_propval * val)
{
  ...
  { val->intval = POWER_SUPPLY_HEALTH_DEAD; }
    ...
    if (charger->fault & AXP20_FAULT_LOG_OVER_TEMP)
    { val->intval = POWER_SUPPLY_HEALTH_OVERHEAT; }
    else
      if (charger->fault & AXP20_FAULT_LOG_COLD)
...
static void axp_charging_monitor (struct work_struct * work)
{
  ...
  if (flag_state_change) {
    ...
    if (charger->bat_current_direction && (rest_vol < pre_rest_vol) ) {
      ...
      if (!charger->ext_valid && (rest_vol > pre_rest_vol) ) {
        rest_vol = pre_rest_vol;
      }
    if (axp_debug) {
  ...
  if (pmu_used2) {
    ...
    if (early_suspend_flag) {
      if (pmu_earlysuspend_chgcur == 0) {
        ...
        if (pmu_earlysuspend_chgcur >= 300000 && pmu_init_chgcur <= 1800000) {
          axp_set_bits (axp_charger->master, AXP20_CHARGE_CONTROL1, 0x80);
          tmp = (pmu_earlysuspend_chgcur - 200001) / 100000;
          charger->chgcur = tmp * 100000 + 300000;
...
static int axp_battery_probe (struct platform_device * pdev)
{
  ...
  { axp20_config.pmu_pwrnoe_time = 128; }
    ...
    if (axp20_config.pmu_pwrnoe_time > 3000)
  ...
  if (axp20_config.pmu_pekon_time < 1000) {
    ...
    if (axp20_config.pmu_pekon_time < 2000) {
      val &= 0x3f;
      val |= 0x80;
    }
    else
      if (axp20_config.pmu_pekon_time < 3000) {
        val &= 0x3f;
        val |= 0xc0;
      }
      else {
...
static int axp20_resume (struct platform_device * dev)
{
  ...
  if ( (val1 >> 7) ) {
    ...
    if (charger->ext_valid && charger->charge_on) {
      rt_rest_vol = (int) (val & 0x7F);
