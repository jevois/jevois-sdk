...
void axp20_power_off (int power_start)
{
  ...
  if (axp20_config.pmu_pwroff_vol >= 2600 && axp20_config.pmu_pwroff_vol <= 3300) {
    if (axp20_config.pmu_pwroff_vol > 3200) {
      ...
      if (axp20_config.pmu_pwroff_vol > 3100) {
        val = 0x6;
      }
      else
        if (axp20_config.pmu_pwroff_vol > 3000) {
          val = 0x5;
        }
        else
          if (axp20_config.pmu_pwroff_vol > 2900) {
            val = 0x4;
          }
          else
            if (axp20_config.pmu_pwroff_vol > 2800) {
              val = 0x3;
            }
            else
              if (axp20_config.pmu_pwroff_vol > 2700) {
                val = 0x2;
              }
              else
