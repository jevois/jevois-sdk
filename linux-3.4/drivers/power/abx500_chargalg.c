...
static int abx500_chargalg_check_charger_connection (struct abx500_chargalg * di)
{
  if (di->chg_info.conn_chg != di->chg_info.prev_conn_chg ||
    ...
    if ( (di->chg_info.conn_chg & AC_CHG) &&
      ...
      if ( (di->chg_info.conn_chg & USB_CHG) &&
...
static int abx500_chargalg_kick_watchdog (struct abx500_chargalg * di)
{
  ...
  { return di->ac_chg->ops.kick_wd (di->ac_chg); }
    ...
    if (di->usb_chg && di->usb_chg->ops.kick_wd &&
    ...
    int iset)
{
  ...
  if (di->ac_chg && di->ac_chg->ops.update_curr &&
    ...
    if (di->usb_chg && di->usb_chg->ops.update_curr &&
        di->chg_info.charger_type & USB_CHG) {
      /*
       * Select maximum of what both the charger
       * and the battery supports
...
static void abx500_chargalg_algorithm (struct abx500_chargalg * di)
{
  ...
  /* If suspended, we should not continue checking the flags */
    ...
    if (di->charge_state == STATE_SUSPENDED_INIT ||
    ...
    /* Safety timer expiration */
      ...
      if (di->events.safety_timer_expired) {
      ...
      /* Battery removed */
        ...
        /* Main or USB charger not ok. */
          ...
          if (di->events.mainextchnotok || di->events.usbchargernotok) {
            /*
