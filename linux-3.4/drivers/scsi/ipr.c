                                ...
                                struct ipr_config_table_entry_wrapper * cfgtew)
{
  ...
  if (ioa_cfg->sis64) {
    ...
    if (res->type == IPR_RES_TYPE_GENERIC_SCSI) {
      ...
      if (res->type == IPR_RES_TYPE_IOAFP) {
        res->bus = IPR_IOAFP_VIRTUAL_BUS;
        res->target = 0;
      }
      else
        if (res->type == IPR_RES_TYPE_ARRAY) {
          ...
          if (res->type == IPR_RES_TYPE_VOLUME_SET) {
...
static void ipr_clear_res_target (struct ipr_resource_entry * res)
{
  ...
  { clear_bit (res->target, ioa_cfg->array_ids); }
    ...
    if (res->bus == IPR_VSET_VIRTUAL_BUS)
    { clear_bit (res->target, ioa_cfg->vset_ids); }
    else
      if (res->bus == 0 && res->type == IPR_RES_TYPE_GENERIC_SCSI) {
        list_for_each_entry (gscsi_res, &ioa_cfg->used_res_q, queue)
  ...
  for (i = 0; i < ARRAY_SIZE (path_active_desc); i++) {
    ...
    for (j = 0; j < ARRAY_SIZE (path_state_desc); j++) {
      ...
      if (fabric->cascaded_expander == 0xff && fabric->phy == 0xff) {
        ...
        if (fabric->cascaded_expander == 0xff) {
    ...
    u32 int_reg)
{
  ...
  if (int_reg & IPR_PCII_IOA_TRANS_TO_OPER) {
    ...
    if ( (int_reg & IPR_PCII_HRRQ_UPDATED) == int_reg) {
...
static irqreturn_t ipr_isr (int irq, void * devp)
{
  ...
  while (1) {
    ...
    if (ipr_cmd != NULL) {
      ...
      if (rc == IRQ_NONE && irq_none == 0) {
        int_reg = readl (ioa_cfg->regs.sense_interrupt_reg32);
        irq_none++;
      }
      else
        if (num_hrrq == IPR_MAX_HRRQ_RETRIES &&
                            ...
                            struct ipr_cmnd * ipr_cmd, struct ipr_resource_entry * res)
{
  ...
  { data_len = sizeof (struct ipr_ioasa64); }
    ...
    if (!ioa_cfg->sis64 && sizeof (struct ipr_ioasa) < data_len)
...
static int ipr_init_res_table (struct ipr_cmnd * ipr_cmd)
{
  ...
  for (i = 0; i < entries; i++) {
    ...
    if (!found) {
      ...
      if (res->sdev && (ipr_is_vset_device (res) || ipr_is_scsi_disk (res) ) )
...
static int ipr_reset_next_stage (struct ipr_cmnd * ipr_cmd)
{
  ...
  { stage_time = IPR_IPL_INIT_DEFAULT_STAGE_TIME; }
    ...
    { stage_time = IPR_IPL_INIT_MIN_STAGE_TIME; }
      ...
      if (stage_time > IPR_LONG_OPERATIONAL_TIMEOUT)
  ...
  if (stage == IPR_IPL_INIT_STAGE_UNKNOWN) {
    ...
    if (stage == IPR_IPL_INIT_STAGE_TRANSOP) {
      int_reg = readl (ioa_cfg->regs.sense_interrupt_reg32);
      if (int_reg & IPR_PCII_IOA_TRANS_TO_OPER) {
        ipr_cmd->job_step = ipr_ioafp_identify_hrrq;
...
static int ipr_reset_shutdown_ioa (struct ipr_cmnd * ipr_cmd)
{
  ...
  if (shutdown_type != IPR_SHUTDOWN_NONE && !ioa_cfg->ioa_is_dead) {
    ...
    { timeout = IPR_SHUTDOWN_TIMEOUT; }
      ...
      if (shutdown_type == IPR_SHUTDOWN_PREPARE_FOR_NORMAL)
                                    ...
                                    const struct pci_device_id * dev_id)
{
  ...
  { ioa_cfg->transop_timeout = ipr_transop_timeout; }
    ...
    if (dev_id->driver_data & IPR_USE_LONG_TRANSOP_TIMEOUT)
