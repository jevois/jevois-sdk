...
void bnx2i_ep_ofld_timer (unsigned long data)
{
  ...
  if (ep->state == EP_STATE_OFLD_START) {
    ...
    if (ep->state == EP_STATE_DISCONN_START) {
      printk (KERN_ALERT "ofld_timer: CONN_DISCON timeout\n");
      ep->state = EP_STATE_DISCONN_TIMEDOUT;
    }
    else
      if (ep->state == EP_STATE_CLEANUP_START) {
                                 ...
                                 struct cqe * cqe)
{
  ...
  if (resp_cqe->status == SAM_STAT_CHECK_CONDITION) {
    ...
    if (datalen > BNX2I_RQ_WQE_SIZE) {
      ...
      if (datalen > ISCSI_DEF_MAX_RECV_SEG_LEN) {
        iscsi_conn_printk (KERN_ERR, conn,
                                 ...
                                 u32 num_cqe)
{
  ...
  while (i < num_cqe) {
    ...
    { bnx2i_fastpath_notification (hba, ikcqe); }
      ...
      if (ikcqe->op_code == ISCSI_KCQE_OPCODE_OFFLOAD_CONN)
      { bnx2i_process_ofld_cmpl (hba, ikcqe); }
      else
        if (ikcqe->op_code == ISCSI_KCQE_OPCODE_UPDATE_CONN)
        { bnx2i_process_update_conn_cmpl (hba, ikcqe); }
        else
          if (ikcqe->op_code == ISCSI_KCQE_OPCODE_INIT) {
            ...
            if (ikcqe->op_code == ISCSI_KCQE_OPCODE_DESTROY_CONN)
            { bnx2i_process_conn_destroy_cmpl (hba, ikcqe); }
            else
              if (ikcqe->op_code == ISCSI_KCQE_OPCODE_ISCSI_ERROR)
              { bnx2i_process_iscsi_error (hba, ikcqe); }
              else
                if (ikcqe->op_code == ISCSI_KCQE_OPCODE_TCP_ERROR)
...
static void bnx2i_cm_connect_cmpl (struct cnic_sock * cm_sk)
{
  ...
  { ep->state = EP_STATE_CONNECT_FAILED; }
    ...
    if (test_bit (SK_F_OFFLD_COMPLETE, &cm_sk->flags) )
