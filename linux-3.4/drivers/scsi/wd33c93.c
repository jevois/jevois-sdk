...
set_clk_freq (int freq, int * mhz)
{
  ...
  { freq = 8; }
    ...
    if (WD33C93_FS_12_15 == freq)
    { freq = 12; }
    else
      if (WD33C93_FS_16_20 == freq)
      { freq = 16; }
      else
        if (freq > 7 && freq < 11)
        { x = WD33C93_FS_8_10; }
      ...
      if (!strncmp (bp, "debug:", 6) ) {
        ...
        if (!strncmp (bp, "disconnect:", 11) ) {
          ...
          if (!strncmp (bp, "period:", 7) ) {
            ...
            if (!strncmp (bp, "resync:", 7) ) {
              set_resync (hd, (int) simple_strtoul (bp + 7, &bp, 0) );
            }
            else
              if (!strncmp (bp, "proc:", 5) ) {
                hd->proc = simple_strtoul (bp + 5, &bp, 0);
              }
              else
                if (!strncmp (bp, "nodma:", 6) ) {
                  hd->no_dma = simple_strtoul (bp + 6, &bp, 0);
                }
                else
                  if (!strncmp (bp, "level2:", 7) ) {
                    hd->level2 = simple_strtoul (bp + 7, &bp, 0);
                  }
                  else
                    if (!strncmp (bp, "burst:", 6) ) {
                      hd->dma_mode =
                        simple_strtol (bp + 6, &bp, 0) ? CTRL_BURST : CTRL_DMA;
                    }
                    else
                      if (!strncmp (bp, "fast:", 5) ) {
