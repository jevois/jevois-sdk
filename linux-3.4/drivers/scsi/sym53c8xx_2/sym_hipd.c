    ...
    if (np->features & FE_DIFF) {
      if (SYM_SETUP_SCSI_DIFF == 1) {
        if (np->sv_scntl3) {
          ...
          if (nvram->type == SYM_SYMBIOS_NVRAM) {
            if (! (INB (np, nc_gpreg) & 0x08) )
            { np->scsi_mode = SMODE_HVD; }
          }
      }
      else
        if (SYM_SETUP_SCSI_DIFF == 2)
        { np->scsi_mode = SMODE_HVD; }
    }
...
static int sym_prepare_setting (struct Scsi_Host * shost, struct sym_hcb * np, struct sym_nvram * nvram)
{
  ...
  if  (period <= 250)   { np->minsync = 10; }
    ...
    if (period <= 303)   { np->minsync = 11; }
    else
      if (period <= 500)   { np->minsync = 12; }
...
static int sym_prepare_nego (struct sym_hcb * np, struct sym_ccb * cp, u_char * msgptr)
{
  ...
  if (goal->renego == NS_PPR || (goal->offset &&
    ...
    if (goal->renego == NS_WIDE || goal->width) {
      nego = NS_WIDE;
    }
    else
      if (goal->renego == NS_SYNC || goal->offset) {
        nego = NS_SYNC;
      }
      else {
        goal->check_nego = 0;
...
void sym_start_up (struct Scsi_Host * shost, int reason)
{
  ...
  { OUTB (np, nc_ctest0, (1 << 5) ); }
    ...
    if (pdev->device == PCI_DEVICE_ID_NCR_53C896)
    { np->rv_ccntl0 |= DPR; }
...
static void sym_int_par (struct sym_hcb * np, u_short sist)
{
  ...
  if (phase == 1 || phase == 5) {
    ...
    if (phase == 7)  /* We definitely cannot handle parity errors */
...
unexpected_phase:
  ...
  case 6: /* MSG OUT phase */
    ...
    if  (dsp == SCRIPTA_BA (np, send_ident) ) {
      ...
      if (dsp == SCRIPTB_BA (np, send_wdtr) ||
...
irqreturn_t sym_interrupt (struct Scsi_Host * shost)
{
  ...
  if (! (sist  & (STO | GEN | HTH | SGE | UDC | SBMC | RST) ) &&
    ...
    if  (sist & PAR)  { sym_int_par (np, sist); }
      ...
      if (sist & MA) { sym_int_ma (np); }
      else
        if (dstat & SIR) { sym_int_sir (np); }
        else
          if (dstat & SSI) { OUTONB_STD(); }
...
static int sym_evaluate_dp (struct sym_hcb * np, struct sym_ccb * cp, u32 scr, int * ofs)
{
  ...
  { pm = &cp->phys.pm0; }
    ...
    if (dp_scr == SCRIPTA_BA (np, pm1_data) )
    { pm = &cp->phys.pm1; }
    else
    { pm = NULL; }
