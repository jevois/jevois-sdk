...
static int sym_xerr_cam_status (int cam_status, int x_status)
{
  if (x_status) {
    ...
    { cam_status = DID_PARITY; }
      ...
      if (x_status & (XE_EXTRA_DATA | XE_SODL_UNRUN | XE_SWIDE_OVRUN) )
...
void sym_set_cam_result_error (struct sym_hcb * np, struct sym_ccb * cp, int resid)
{
  ...
  if (cp->host_flags & HF_SENSE) {
    ...
    if (cp->host_status == HS_COMPLETE)  /* Bad SCSI status */
    { cam_status = DID_OK; }
    else
      if (cp->host_status == HS_SEL_TIMEOUT) /* Selection timeout */
      { cam_status = DID_NO_CONNECT; }
      else
        if (cp->host_status == HS_UNEXPECTED)  /* Unexpected BUS FREE*/
        { cam_status = DID_ERROR; }
        else {            /* Extended error */
          if (sym_verbose) {
...
static int sym_user_command (struct Scsi_Host * shost, char * buffer, int length)
{
  ...
  { uc->cmd = UC_SETSYNC; }
    ...
    if ( (arg_len = is_keyword (ptr, len, "settags") ) != 0)
    { uc->cmd = UC_SETTAGS; }
    else
      if ( (arg_len = is_keyword (ptr, len, "setverbose") ) != 0)
      { uc->cmd = UC_SETVERBOSE; }
      else
        if ( (arg_len = is_keyword (ptr, len, "setwide") ) != 0)
        { uc->cmd = UC_SETWIDE; }
  #ifdef SYM_LINUX_DEBUG_CONTROL_SUPPORT
        else
          if ( (arg_len = is_keyword (ptr, len, "setdebug") ) != 0)
          { uc->cmd = UC_SETDEBUG; }
  #endif
          else
            if ( (arg_len = is_keyword (ptr, len, "setflag") ) != 0)
            { uc->cmd = UC_SETFLAG; }
            else
              if ( (arg_len = is_keyword (ptr, len, "resetdev") ) != 0)
              { uc->cmd = UC_RESETDEV; }
              else
                if ( (arg_len = is_keyword (ptr, len, "cleardev") ) != 0)
                { uc->cmd = UC_CLEARDEV; }
                else
                { arg_len = 0; }
  ...
  case UC_SETDEBUG:
    while (len > 0) {
      ...
      { uc->data |= DEBUG_ALLOC; }
        ...
        if ( (arg_len = is_keyword (ptr, len, "phase") ) )
        { uc->data |= DEBUG_PHASE; }
        else
          if ( (arg_len = is_keyword (ptr, len, "queue") ) )
          { uc->data |= DEBUG_QUEUE; }
          else
            if ( (arg_len = is_keyword (ptr, len, "result") ) )
            { uc->data |= DEBUG_RESULT; }
            else
              if ( (arg_len = is_keyword (ptr, len, "scatter") ) )
              { uc->data |= DEBUG_SCATTER; }
              else
                if ( (arg_len = is_keyword (ptr, len, "script") ) )
                { uc->data |= DEBUG_SCRIPT; }
                else
                  if ( (arg_len = is_keyword (ptr, len, "tiny") ) )
                  { uc->data |= DEBUG_TINY; }
                  else
                    if ( (arg_len = is_keyword (ptr, len, "timing") ) )
                    { uc->data |= DEBUG_TIMING; }
                    else
                      if ( (arg_len = is_keyword (ptr, len, "nego") ) )
                      { uc->data |= DEBUG_NEGO; }
                      else
                        if ( (arg_len = is_keyword (ptr, len, "tags") ) )
                        { uc->data |= DEBUG_TAGS; }
                        else
                          if ( (arg_len = is_keyword (ptr, len, "pointer") ) )
