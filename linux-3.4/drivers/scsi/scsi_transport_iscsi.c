    ...
    struct attribute * attr, int i)
{
  ...
  { param = ISCSI_NET_PARAM_IFACE_ENABLE; }
    ...
    { param = ISCSI_NET_PARAM_VLAN_ID; }
      ...
      if (attr == &dev_attr_iface_vlan_priority.attr)
      { param = ISCSI_NET_PARAM_VLAN_PRIORITY; }
      else
        if (attr == &dev_attr_iface_vlan_enabled.attr)
        { param = ISCSI_NET_PARAM_VLAN_ENABLED; }
        else
          if (attr == &dev_attr_iface_mtu.attr)
          { param = ISCSI_NET_PARAM_MTU; }
          else
            if (attr == &dev_attr_iface_port.attr)
            { param = ISCSI_NET_PARAM_PORT; }
              ...
              if (iface->iface_type == ISCSI_IFACE_TYPE_IPV4) {
                ...
                { param = ISCSI_NET_PARAM_IPV4_ADDR; }
                  ...
                  if (attr == &dev_attr_ipv4_iface_gateway.attr)
                  { param = ISCSI_NET_PARAM_IPV4_GW; }
                  else
                    if (attr == &dev_attr_ipv4_iface_subnet.attr)
                    { param = ISCSI_NET_PARAM_IPV4_SUBNET; }
                    else
                      if (attr == &dev_attr_ipv4_iface_bootproto.attr)
                ...
                if (iface->iface_type == ISCSI_IFACE_TYPE_IPV6) {
                  ...
                  { param = ISCSI_NET_PARAM_IPV6_ADDR; }
                    ...
                    if (attr == &dev_attr_ipv6_iface_link_local_addr.attr)
                    { param = ISCSI_NET_PARAM_IPV6_LINKLOCAL; }
                    else
                      if (attr == &dev_attr_ipv6_iface_router_addr.attr)
                      { param = ISCSI_NET_PARAM_IPV6_ROUTER; }
                      else
                        if (attr == &dev_attr_ipv6_iface_ipaddr_autocfg.attr)
                        { param = ISCSI_NET_PARAM_IPV6_ADDR_AUTOCFG; }
                        else
                          if (attr == &dev_attr_ipv6_iface_link_local_autocfg.attr)
    ...
    struct attribute * attr, int i)
{
  ...
  { param = ISCSI_PARAM_MAX_RECV_DLENGTH; }
    ...
    if (attr == &dev_attr_conn_max_xmit_dlength.attr)
    { param = ISCSI_PARAM_MAX_XMIT_DLENGTH; }
    else
      if (attr == &dev_attr_conn_header_digest.attr)
      { param = ISCSI_PARAM_HDRDGST_EN; }
      else
        if (attr == &dev_attr_conn_data_digest.attr)
        { param = ISCSI_PARAM_DATADGST_EN; }
        else
          if (attr == &dev_attr_conn_ifmarker.attr)
          { param = ISCSI_PARAM_IFMARKER_EN; }
          else
            if (attr == &dev_attr_conn_ofmarker.attr)
            { param = ISCSI_PARAM_OFMARKER_EN; }
            else
              if (attr == &dev_attr_conn_address.attr)
              { param = ISCSI_PARAM_CONN_ADDRESS; }
              else
                if (attr == &dev_attr_conn_port.attr)
                { param = ISCSI_PARAM_CONN_PORT; }
                else
                  if (attr == &dev_attr_conn_exp_statsn.attr)
                  { param = ISCSI_PARAM_EXP_STATSN; }
                  else
                    if (attr == &dev_attr_conn_persistent_address.attr)
                    { param = ISCSI_PARAM_PERSISTENT_ADDRESS; }
                    else
                      if (attr == &dev_attr_conn_persistent_port.attr)
                      { param = ISCSI_PARAM_PERSISTENT_PORT; }
                      else
                        if (attr == &dev_attr_conn_ping_tmo.attr)
                        { param = ISCSI_PARAM_PING_TMO; }
                        else
                          if (attr == &dev_attr_conn_recv_tmo.attr)
    ...
    struct attribute * attr, int i)
{
  ...
  { param = ISCSI_PARAM_INITIAL_R2T_EN; }
    ...
    if (attr == &dev_attr_sess_max_outstanding_r2t.attr)
    { param = ISCSI_PARAM_MAX_R2T; }
    else
      if (attr == &dev_attr_sess_immediate_data.attr)
      { param = ISCSI_PARAM_IMM_DATA_EN; }
      else
        if (attr == &dev_attr_sess_first_burst_len.attr)
        { param = ISCSI_PARAM_FIRST_BURST; }
        else
          if (attr == &dev_attr_sess_max_burst_len.attr)
          { param = ISCSI_PARAM_MAX_BURST; }
          else
            if (attr == &dev_attr_sess_data_pdu_in_order.attr)
            { param = ISCSI_PARAM_PDU_INORDER_EN; }
            else
              if (attr == &dev_attr_sess_data_seq_in_order.attr)
              { param = ISCSI_PARAM_DATASEQ_INORDER_EN; }
              else
                if (attr == &dev_attr_sess_erl.attr)
                { param = ISCSI_PARAM_ERL; }
                else
                  if (attr == &dev_attr_sess_targetname.attr)
                  { param = ISCSI_PARAM_TARGET_NAME; }
                  else
                    if (attr == &dev_attr_sess_tpgt.attr)
                    { param = ISCSI_PARAM_TPGT; }
                    else
                      if (attr == &dev_attr_sess_chap_in_idx.attr)
                      { param = ISCSI_PARAM_CHAP_IN_IDX; }
                      else
                        if (attr == &dev_attr_sess_chap_out_idx.attr)
                        { param = ISCSI_PARAM_CHAP_OUT_IDX; }
                        else
                          if (attr == &dev_attr_sess_password.attr)
                          { param = ISCSI_PARAM_USERNAME; }
                          else
                            if (attr == &dev_attr_sess_password_in.attr)
                            { param = ISCSI_PARAM_USERNAME_IN; }
                            else
                              if (attr == &dev_attr_sess_username.attr)
                              { param = ISCSI_PARAM_PASSWORD; }
                              else
                                if (attr == &dev_attr_sess_username_in.attr)
                                { param = ISCSI_PARAM_PASSWORD_IN; }
                                else
                                  if (attr == &dev_attr_sess_fast_abort.attr)
                                  { param = ISCSI_PARAM_FAST_ABORT; }
                                  else
                                    if (attr == &dev_attr_sess_abort_tmo.attr)
                                    { param = ISCSI_PARAM_ABORT_TMO; }
                                    else
                                      if (attr == &dev_attr_sess_lu_reset_tmo.attr)
                                      { param = ISCSI_PARAM_LU_RESET_TMO; }
                                      else
                                        if (attr == &dev_attr_sess_tgt_reset_tmo.attr)
                                        { param = ISCSI_PARAM_TGT_RESET_TMO; }
                                        else
                                          if (attr == &dev_attr_sess_ifacename.attr)
                                          { param = ISCSI_PARAM_IFACE_NAME; }
                                          else
                                            if (attr == &dev_attr_sess_initiatorname.attr)
                                            { param = ISCSI_PARAM_INITIATOR_NAME; }
                                            else
                                              if (attr == &dev_attr_sess_targetalias.attr)
                                              { param = ISCSI_PARAM_TARGET_ALIAS; }
                                              else
                                                if (attr == &dev_attr_priv_sess_recovery_tmo.attr)
                                                { return S_IRUGO | S_IWUSR; }
                                                else
                                                  if (attr == &dev_attr_priv_sess_state.attr)
                                                  { return S_IRUGO; }
                                                  else
                                                    if (attr == &dev_attr_priv_sess_creator.attr)
                                                    { return S_IRUGO; }
                                                    else {
    ...
    struct attribute * attr, int i)
{
  ...
  { param = ISCSI_HOST_PARAM_NETDEV_NAME; }
    ...
    if (attr == &dev_attr_host_hwaddress.attr)
    { param = ISCSI_HOST_PARAM_HWADDRESS; }
    else
      if (attr == &dev_attr_host_ipaddress.attr)
      { param = ISCSI_HOST_PARAM_IPADDRESS; }
      else
        if (attr == &dev_attr_host_initiatorname.attr)
        { param = ISCSI_HOST_PARAM_INITIATOR_NAME; }
        else
          if (attr == &dev_attr_host_port_state.attr)
          { param = ISCSI_HOST_PARAM_PORT_STATE; }
          else
            if (attr == &dev_attr_host_port_speed.attr)
