                                        ...
                                        uint32_t mbox_status)
{
  ...
  if ( (mbox_status == MBOX_STS_BUSY) ||
    ...
    if (mbox_status >> 12 == MBOX_ASYNC_EVENT_STATUS) {
      ...
      case MBOX_ASTS_IP_ADDR_STATE_CHANGED:
        ...
        { set_bit (DPC_GET_DHCP_IP_ADDR, &ha->dpc_flags); }
          ...
          if ( (mbox_sts[3] == ACB_STATE_ACQUIRING) &&
            ...
            if ( (mbox_sts[3] == ACB_STATE_UNCONFIGURED) )
...
void qla4xxx_free_irqs (struct scsi_qla_host * ha)
{
  ...
  { qla4_8xxx_disable_msix (ha); }
    ...
    if (test_and_clear_bit (AF_MSI_ENABLED, &ha->flags) ) {
      ...
      if (test_and_clear_bit (AF_INTx_ENABLED, &ha->flags) )
      { free_irq (ha->pdev->irq, ha); }
}
