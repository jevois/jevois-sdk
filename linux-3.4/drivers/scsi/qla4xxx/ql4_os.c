                              ...
                              uint32_t pid, struct sockaddr * dst_addr)
{
  ...
  if ( (iface_type == ISCSI_IFACE_TYPE_IPV4) &&
    ...
    if ( (iface_type == ISCSI_IFACE_TYPE_IPV6) &&
                              ...
                              struct addr_ctrl_blk * init_fw_cb)
{
  ...
  case ISCSI_NET_PARAM_IPV6_ADDR_AUTOCFG:
    ...
    if (iface_param->value[0] == ISCSI_IPV6_AUTOCFG_DISABLE)
      ...
      if (iface_param->value[0] == ISCSI_IPV6_AUTOCFG_ND_ENABLE)
        init_fw_cb->ipv6_addtl_opts |=
          cpu_to_le16 (
            IPV6_ADDOPT_NEIGHBOR_DISCOVERY_ADDR_ENABLE);
      else
                              ...
                              struct addr_ctrl_blk * init_fw_cb)
{
  ...
  case ISCSI_NET_PARAM_IPV4_BOOTPROTO:
    if (iface_param->value[0] == ISCSI_BOOTPROTO_DHCP)
      ...
      if (iface_param->value[0] == ISCSI_BOOTPROTO_STATIC)
        init_fw_cb->ipv4_tcp_opts &=
          cpu_to_le16 (~TCPOPT_DHCP_ENABLE);
      else
...
static int qla4_8xxx_check_temp (struct scsi_qla_host * ha)
{
  ...
  if (temp_state == QLA82XX_TEMP_PANIC) {
    ...
    if (temp_state == QLA82XX_TEMP_WARN) {
