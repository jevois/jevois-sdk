                              ...
                              struct ibmvfc_host * vhost, unsigned long timeout)
{
  ...
  *evt->xfer_iu = evt->iu;
  ...
  { evt->xfer_iu->cmd.tag = (u64) evt; }
    ...
    if (evt->crq.format == IBMVFC_MAD_FORMAT)
...
static void ibmvfc_tgt_implicit_logout_done (struct ibmvfc_event * evt)
{
  ...
  { ibmvfc_init_tgt (tgt, ibmvfc_tgt_send_plogi); }
    ...
    if (vhost->action == IBMVFC_HOST_ACTION_QUERY_TGTS &&
...
static void ibmvfc_tgt_add_rport (struct ibmvfc_target * tgt)
{
  ...
  if (rport && tgt->action == IBMVFC_TGT_ACTION_DEL_RPORT) {
    ...
    if (rport && tgt->action == IBMVFC_TGT_ACTION_DELETED_RPORT) {
      spin_unlock_irqrestore (vhost->host->host_lock, flags);
      return;
    }
