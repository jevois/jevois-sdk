...
static void asd_dl_tasklet_handler (unsigned long data)
{
  ...
  while (1) {
    ...
    if (unlikely (!ascb) ) {
      ...
      if (ascb->scb->header.opcode == EMPTY_SCB) {
        goto out;
      }
      else
        if (!ascb->uldd_timer && !del_timer (&ascb->timer) ) {
          goto next_1;
        }
        ...
        { asd_printk ("data parity for %s\n", pci_name (asd_ha->pcidev) ); }
          ...
          if (pcix_status & RCV_SCE) {
            ...
            if (pcix_status & UNEXP_SC) {
              ...
              if (pcix_status & SC_DISCARD)
                asd_printk ("split completion discarded for %s\n",
                            pci_name (asd_ha->pcidev) );
              else
                if (ecc_status & UNCOR_ECCERR)
