...
static int pmcraid_slave_alloc (struct scsi_device * scsi_dev)
{
  ...
  list_for_each_entry (temp, &pinstance->used_res_q, queue) {
    ...
    if (RES_IS_VSET (temp->cfg_entry) ) {
      ...
      if (RES_IS_GSCSI (temp->cfg_entry) ) {
                                    ...
                                    struct pmcraid_config_table_entry * cfgte)
{
  ...
  if (cfgte->resource_type == RES_TYPE_VSET) {
    ...
    if (cfgte->resource_type == RES_TYPE_GSCSI)
...
static void pmcraid_handle_config_change (struct pmcraid_instance * pinstance)
{
  ...
  if (pinstance->ccn.hcam->notification_type ==
    ...
    if (!pmcraid_expose_resource (fw_version, cfg_entry) ) {
      goto out_notify_apps;
    }
...
static void pmcraid_fail_outstanding_cmds (struct pmcraid_instance * pinstance)
{
  ...
  list_for_each_entry_safe (cmd, temp, &pinstance->pending_cmd_pool,
    ...
    if (cmd->scsi_cmd) {
      ...
      if (cmd->cmd_done == pmcraid_internal_done ||
...
static int pmcraid_error_handler (struct pmcraid_cmd * cmd)
{
  ...
  { atomic_inc (&res->read_failures); }
    ...
    if (SCSI_CMD_TYPE (scsi_cmd->cmnd[0]) == SCSI_WRITE_CMD)
...
out_handle_response:
   ...
   */
    ...
    if (direction == DMA_FROM_DEVICE && request_size > 0) {
      rc = pmcraid_copy_sglist (cmd->sglist,
                                request_buffer,
                                request_size,
                                direction);
