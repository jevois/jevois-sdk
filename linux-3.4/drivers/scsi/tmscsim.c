...
DC390_Interrupt (void * dev_id)
{
  ...
  if (istatus &  RESELECTED)
  {
    ...
    if (istatus & (SELECTED | SEL_ATTENTION) )
    {
...
dc390_Disconnect ( struct dc390_acb * pACB )
{
  ...
  { pSRB->SRBState = 0; }
    ...
    if ( pSRB->SRBState & SRB_ABORT_SENT )
    {
      pDCB->TagMask = 0;
      pDCB->DCBFlag = 0;
    ...
    else
    {
      ...
      { /* Selection time out */
        ...
        if (! (pSRB->SRBState & SRB_DISCONNECT) && (pSRB->SRBState & SRB_COMPLETED) )
        {
disc1:
          dc390_freetag (pDCB, pSRB);
...
dc390_SRBdone ( struct dc390_acb * pACB, struct dc390_dcb * pDCB, struct dc390_srb * pSRB )
{
  ...
  { /*  Target status == 0 */
    ...
    if (status == H_OVER_UNDER_RUN)
    {
      ...
      if (status == H_SEL_TIMEOUT)
      {
        pcmd->result = MK_RES (0, DID_NO_CONNECT, 0, 0);
        /* Devices are removed below ... */
      }
      else
        if ( pSRB->SRBStatus & PARITY_ERROR)
        {
