...
irqreturn_t fas216_intr (FAS216_Info * info)
{
  ...
  if (stat & STAT_INT) {
    if (inst & INST_BUSRESET) {
      ...
      if (inst & INST_ILLEGALCMD) {
        fas216_log (info, LOG_ERROR, "illegal command given\n");
        fas216_dumpstate (info);
        print_debug_list();
      }
      else
        if (inst & INST_DISCONNECT)
        { fas216_disconnect_intr (info); }
        else
          if (inst & INST_RESELECTED)  /* reselected     */
          { fas216_reselected_intr (info); }
          else
            if (inst & INST_BUSSERVICE)  /* bus service request    */
            { fas216_busservice_intr (info, stat, is); }
            else
              if (inst & INST_FUNCDONE)    /* function done    */
    ...
    struct scsi_cmnd * SCpnt)
{
  ...
  if (queue_remove_cmd (&info->queues.issue, SCpnt) ) {
    ...
    if (queue_remove_cmd (&info->queues.disconnected, SCpnt) ) {
      /*
