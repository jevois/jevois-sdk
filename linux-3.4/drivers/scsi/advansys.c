  ...
  for (i = 0; i <= ADV_MAX_TID; i++) {
    ...
    if ( (lramword & 0x1F) == 0) { /* Check for REQ/ACK Offset 0. */
      ...
      if ( (lramword & 0x1F00) == 0x1100) { /* 80 Mhz */
        ...
        if ( (lramword & 0x1F00) == 0x1000) { /* 40 Mhz */
          len =
...
static int AdvInitAsc3550Driver (ADV_DVC_VAR * asc_dvc)
{
  ...
  for (tid = 0; tid <= ADV_MAX_TID; tid++) {
    ...
    if (tid == 3) { /* Check if done with sdtr_speed1. */
      ...
      if (tid == 7) {  /* Check if done with sdtr_speed2. */
        AdvWriteWordLram (iop_base, ASC_MC_SDTR_SPEED2, word);
        word = 0;
      }
      else
        if (tid == 11) { /* Check if done with sdtr_speed3. */
          AdvWriteWordLram (iop_base, ASC_MC_SDTR_SPEED3, word);
          word = 0;
        }
        else
          if (tid == 15) { /* Check if done with sdtr_speed4. */
...
static int AscIsrChipHalted (ASC_DVC_VAR * asc_dvc)
{
  ...
  if (int_halt_code == ASC_HALT_DISABLE_ASYN_USE_SYN_FIX) {
    ...
    if (int_halt_code == ASC_HALT_ENABLE_ASYN_USE_SYN_FIX) {
      if (asc_dvc->pci_fix_asyn_xfer & target_id) {
      ...
      if (int_halt_code == ASC_HALT_EXTMSG_IN) {
        ...
        if (int_halt_code == ASC_HALT_CHK_CONDITION) {
          ...
          if (int_halt_code == ASC_HALT_SDTR_REJECTED) {
            ...
            if (int_halt_code == ASC_HALT_SS_QUEUE_FULL) {
              ...
              if (int_halt_code == ASC_HALT_HOST_COPY_SG_LIST_TO_RISC) {
                uchar q_no;
                ushort q_addr;
                uchar sg_wk_q_no;
...
static ushort __devinit AscInitAscDvcVar (ASC_DVC_VAR * asc_dvc)
{
  ...
  if ( (asc_dvc->bus_type & ASC_IS_PCI) &&
    ...
    if (chip_version == ASC_CHIP_VER_PCI_ULTRA_3150) {
      ...
      if (chip_version >= ASC_CHIP_VER_PCI_ULTRA_3050) {
        AscSetExtraControl (iop_base,
                            (SEC_ACTIVE_NEGATE |
                             SEC_ENABLE_FILTER) );
      }
...
static int __devinit AdvInitFrom3550EEP (ADV_DVC_VAR * asc_dvc)
{
  ...
  if (eep_config.max_dvc_qng > ASC_DEF_MAX_DVC_QNG) {
    ...
    if (eep_config.max_dvc_qng < ASC_DEF_MIN_DVC_QNG) {
      /* If the value is zero, assume it is uninitialized. */
  ...
  if (eep_config.termination == 0) {
    ...
    if (eep_config.termination == 1) {
      ...
      if (eep_config.termination == 2) {
        asc_dvc->cfg->termination = TERM_CTL_SEL | TERM_CTL_H;
        
        /* Enable manual control with low on / high on. */
      }
      else
        if (eep_config.termination == 3) {
...
static int __devinit AdvInitFrom38C0800EEP (ADV_DVC_VAR * asc_dvc)
{
  ...
  if (eep_config.termination_se == 0) {
    ...
    if (eep_config.termination_se == 1) {
      ...
      if (eep_config.termination_se == 2) {
        termination = TERM_SE_HI;
        
        /* Enable manual control with low on / high on. */
      }
      else
        if (eep_config.termination_se == 3) {
          termination = TERM_SE;
        }
        else {
          /*
  ...
  if (eep_config.termination_lvd == 0) {
    ...
    if (eep_config.termination_lvd == 1) {
      ...
      if (eep_config.termination_lvd == 2) {
        asc_dvc->cfg->termination = termination | TERM_LVD_HI;
        
        /* Enable manual control with low on / high on. */
      }
      else
        if (eep_config.termination_lvd == 3) {
...
static int __devinit advansys_wide_init_chip (struct Scsi_Host * shost)
{
  ...
  if (adv_dvc->chip_type == ADV_CHIP_ASC3550) {
    ...
    if (adv_dvc->chip_type == ADV_CHIP_ASC38C0800) {
    ...
    unsigned int iop, int bus_type)
{
  ...
  if (ASC_NARROW_BOARD (boardp) ) {
    ...
    if (pdev->device == PCI_DEVICE_ID_ASP_ABP940UW) {
      ...
      if (pdev->device == PCI_DEVICE_ID_38C0800_REV1) {
