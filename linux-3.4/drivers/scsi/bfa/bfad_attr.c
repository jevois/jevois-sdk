...
bfad_im_vport_create (struct fc_vport * fc_vport, bool disable)
{
  ...
  if (rc == BFA_STATUS_OK) {
    ...
    if (rc == BFA_STATUS_INVALID_WWN)
    { return VPCERR_BAD_WWN; }
    else
      if (rc == BFA_STATUS_VPORT_EXISTS)
      { return VPCERR_BAD_WWN; }
      else
        if (rc == BFA_STATUS_VPORT_MAX)
        { return VPCERR_NO_FABRIC_SUPP; }
        else
          if (rc == BFA_STATUS_VPORT_WWN_BP)
          { return VPCERR_BAD_WWN; }
          else
                                ...
                                if (strstr (model, "Brocade-1860") ) {
                                  if (nports == 1 && bfa_ioc_is_cna (&bfad->bfa.ioc) )
                                    ...
                                    if (nports == 1 && !bfa_ioc_is_cna (&bfad->bfa.ioc) )
                                      ...
                                      if (nports == 2 && bfa_ioc_is_cna (&bfad->bfa.ioc) )
                                        snprintf (model_descr, BFA_ADAPTER_MODEL_DESCR_LEN,
                                                  "Brocade 10Gbps dual port CNA");
                                      else
                                        if (nports == 2 && !bfa_ioc_is_cna (&bfad->bfa.ioc) )
