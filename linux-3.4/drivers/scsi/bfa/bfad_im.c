                  ...
                  int sns_len, u8 * sns_info, s32 residue)
{
  ...
  if (itnim_data) {
    ...
    if (!cmnd->result && itnim &&
      ...
      if (cmnd->result == SAM_STAT_TASK_SET_FULL && itnim) {
        /* qfull handling */
...
bfad_im_supported_speeds (struct bfa_s * bfa)
{
  ...
  if (ioc_attr->adapter_attr.max_speed == BFA_PORT_SPEED_16GBPS)
    ...
    if (ioc_attr->adapter_attr.max_speed == BFA_PORT_SPEED_8GBPS) {
      ...
      if (ioc_attr->adapter_attr.max_speed == BFA_PORT_SPEED_4GBPS) {
        supported_speed |=  FC_PORTSPEED_4GBIT | FC_PORTSPEED_2GBIT |
                            FC_PORTSPEED_1GBIT;
      }
      else
        if (ioc_attr->adapter_attr.max_speed == BFA_PORT_SPEED_10GBPS) {
