                ...
                (*compare_fn) (struct scsi_device *, u8 *, u8 *, int) )
{
  ...
  for (;;) {
    ...
    if (i->f->set_iu && spi_iu (starget) ) {
      ...
      if (i->f->set_qas && spi_qas (starget) ) {
    ...
    struct attribute * attr, int i)
{
  ...
  { return TARGET_ATTRIBUTE_HELPER (period); }
    ...
    if (attr == &dev_attr_min_period.attr &&
        spi_support_sync (starget) )
    { return TARGET_ATTRIBUTE_HELPER (period); }
    else
      if (attr == &dev_attr_offset.attr &&
          spi_support_sync (starget) )
      { return TARGET_ATTRIBUTE_HELPER (offset); }
      else
        if (attr == &dev_attr_max_offset.attr &&
            spi_support_sync (starget) )
        { return TARGET_ATTRIBUTE_HELPER (offset); }
        else
          if (attr == &dev_attr_width.attr &&
              spi_support_wide (starget) )
          { return TARGET_ATTRIBUTE_HELPER (width); }
          else
            if (attr == &dev_attr_max_width.attr &&
                spi_support_wide (starget) )
            { return TARGET_ATTRIBUTE_HELPER (width); }
            else
              if (attr == &dev_attr_iu.attr &&
                  spi_support_ius (starget) )
              { return TARGET_ATTRIBUTE_HELPER (iu); }
              else
                if (attr == &dev_attr_max_iu.attr &&
                    spi_support_ius (starget) )
                { return TARGET_ATTRIBUTE_HELPER (iu); }
                else
                  if (attr == &dev_attr_dt.attr &&
                      spi_support_dt (starget) )
                  { return TARGET_ATTRIBUTE_HELPER (dt); }
                  else
                    if (attr == &dev_attr_qas.attr &&
                        spi_support_qas (starget) )
                    { return TARGET_ATTRIBUTE_HELPER (qas); }
                    else
                      if (attr == &dev_attr_max_qas.attr &&
                          spi_support_qas (starget) )
                      { return TARGET_ATTRIBUTE_HELPER (qas); }
                      else
                        if (attr == &dev_attr_wr_flow.attr &&
                            spi_support_ius (starget) )
                        { return TARGET_ATTRIBUTE_HELPER (wr_flow); }
                        else
                          if (attr == &dev_attr_rd_strm.attr &&
                              spi_support_ius (starget) )
                          { return TARGET_ATTRIBUTE_HELPER (rd_strm); }
                          else
                            if (attr == &dev_attr_rti.attr &&
                                spi_support_ius (starget) )
                            { return TARGET_ATTRIBUTE_HELPER (rti); }
                            else
                              if (attr == &dev_attr_pcomp_en.attr &&
                                  spi_support_ius (starget) )
                              { return TARGET_ATTRIBUTE_HELPER (pcomp_en); }
                              else
                                if (attr == &dev_attr_hold_mcs.attr &&
                                    spi_support_ius (starget) )
                                { return TARGET_ATTRIBUTE_HELPER (hold_mcs); }
                                else
                                  if (attr == &dev_attr_revalidate.attr)
                                  { return S_IWUSR; }
