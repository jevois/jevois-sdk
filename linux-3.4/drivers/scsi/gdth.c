                                    ...
                                    gdth_ha_str * ha)
{
  ...
  if (ha->pdev->device <= PCI_DEVICE_ID_VORTEX_GDT6000B) {  /* GDT6000/B */
    ...
    if (ha->pdev->device <= PCI_DEVICE_ID_VORTEX_GDT6555) { /* GDT6110, ... */
  ...
  for (nscp = pscp = ha->req_first; nscp; nscp = (Scsi_Cmnd *) nscp->SCp.ptr) {
    ...
    if (nscp_cmndinfo->OpCode != -1) {
      if ( (nscp_cmndinfo->phase & 0xff) == CACHESERVICE) {
        ...
        if ( (nscp_cmndinfo->phase & 0xff) == SCSIRAWSERVICE) {
      ...
      if (gdth_cmnd_priv (nscp)->internal_command) {
        ...
        if (b != ha->virt_bus) {
          ...
          if (t >= MAX_HDRIVES || !ha->hdr[t].present || l != 0) {
            ...
            case SERVICE_ACTION_IN:
              ...
              if (ha->hdr[t].media_changed && nscp->cmnd[0] != INQUIRY) {
                ...
                if (gdth_internal_cache_cmd (ha, nscp) )
                { gdth_scsi_done (nscp); }
              break;
      ...
      { cmdp->OpCode = GDT_RELEASE_DRV; }
        ...
        if (scp->cmnd[0] == ALLOW_MEDIUM_REMOVAL) {
          ...
          { cmdp->OpCode = GDT_MOUNT; }
            ...
            if (scp->cmnd[3] & 1)              /* removable drive ? */
  ...
  *cmdp = *cmndinfo->internal_cmd_str;
  ...
  if (cmdp->OpCode == GDT_IOCTL) {
    ...
    if (cmdp->Service == CACHESERVICE) {
      ...
      if (cmdp->Service == SCSIRAWSERVICE) {
      ...
      if (cmndinfo->OpCode != -1) {
        ...
        if (cmndinfo->OpCode == GDT_CLUST_INFO) {
          ...
          if (cmndinfo->OpCode == GDT_MOUNT) {
            ...
            if (cmndinfo->OpCode == GDT_UNMOUNT) {
              ha->hdr[t].cluster_type &= ~CLUSTER_MOUNTED;
              ha->hdr[t].media_changed = TRUE;
            }
          cmndinfo->OpCode = -1;
...
static void __init internal_setup (char * str, int * ints)
{
  ...
  while (argv && (cur_str = strchr (argv, ':') ) ) {
    ...
    { disable = val; }
      ...
      if (!strncmp (argv, "reserve_mode:", 13) )
      { reserve_mode = val; }
      else
        if (!strncmp (argv, "reverse_scan:", 13) )
        { reverse_scan = val; }
        else
          if (!strncmp (argv, "hdr_channel:", 12) )
          { hdr_channel = val; }
          else
            if (!strncmp (argv, "max_ids:", 8) )
            { max_ids = val; }
            else
              if (!strncmp (argv, "rescan:", 7) )
              { rescan = val; }
              else
                if (!strncmp (argv, "shared_access:", 14) )
                { shared_access = val; }
                else
                  if (!strncmp (argv, "probe_eisa_isa:", 15) )
                  { probe_eisa_isa = val; }
                  else
                    if (!strncmp (argv, "reserve_list:", 13) ) {
                      reserve_list[0] = val;
...
static int ioc_event (void __user * arg)
{
  ...
  if (evt.erase == 0xff) {
    ...
    { evt.event.event_data.size = sizeof (evt.event.event_data.eu.test); }
      ...
      if (evt.event.event_source == ES_DRIVER)
      { evt.event.event_data.size = sizeof (evt.event.event_data.eu.driver); }
      else
        if (evt.event.event_source == ES_SYNC)
...
static int ioc_general (void __user * arg, char * cmnd)
{
  ...
  if (gen.data_len + gen.sense_len != 0) {
    ...
    if (gen.command.OpCode == GDT_IOCTL) {
      ...
      if (gen.command.Service == CACHESERVICE) {
        if (ha->cache_feat & GDT_64BIT) {
        ...
        if (gen.command.Service == SCSIRAWSERVICE) {
          if (ha->raw_feat & GDT_64BIT) {
            /* copy elements from 32-bit IOCTL structure */
            char cmd[16];
