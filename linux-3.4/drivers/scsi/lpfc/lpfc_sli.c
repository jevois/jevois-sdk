...
lpfc_handle_rrq_active (struct lpfc_hba * phba)
{
  ...
  list_for_each_entry_safe (rrq, nextrrq,
    ...
    { list_move (&rrq->list, &send_rrq); }
      ...
      if (time_before (rrq->rrq_stop_time, next_time) )
                          ...
                          struct lpfc_iocbq * piocb, uint32_t flag)
{
  ...
  if (unlikely (phba->link_state == LPFC_LINK_DOWN) ) {
    ...
    if (unlikely (pring->ringno == phba->sli.fcp_ring &&
                  ! (phba->sli.sli_flag & LPFC_PROCESS_LA) ) ) {
      goto iocb_busy;
    }
                   ...
                   struct lpfc_sglq * sglq)
{
  ...
  if (icmd->un.genreq64.bdl.bdeFlags == BUFF_TYPE_BLP_64) {
    ...
    if (icmd->un.genreq64.bdl.bdeFlags == BUFF_TYPE_BDE_64) {
                    ...
                    union lpfc_wqe * wqe)
{
  ...
  { command_type = FCP_COMMAND; }
    ...
    if (fip && (iocbq->iocb_flag & LPFC_FIP_ELS_ID_MASK) )
                                 ...
                                 int cnt)
{
  ...
  list_for_each_entry (rsrc_blk, &phba->sli4_hba.lpfc_xri_blk_list,
    ...
    { continue; }
      ...
      if (rsrc_blk->rsrc_used >= rsrc_blk->rsrc_size)
      { continue; }
      else
...
lpfc_drain_txq (struct lpfc_hba * phba)
{
  ...
  while (pring->txq_cnt) {
    ...
    { fail_msg = "to convert bpl to sgl"; }
      ...
      if (lpfc_sli4_iocb2wqe (phba, piocbq, &wqe) )
