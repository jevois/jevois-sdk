                       ...
                       const char * buf, size_t count)
{
  ...
  { status = phba->lpfc_hba_init_link (phba, MBX_NOWAIT); }
    ...
    if ( (strncmp (buf, "down", sizeof ("down") - 1) == 0) &&
...
lpfc_sli4_pdev_reg_request (struct lpfc_hba * phba, uint32_t opcode)
{
  ...
  { reg_val |= LPFC_FW_DUMP_REQUEST; }
    ...
    if (opcode == LPFC_FW_RESET)
    { reg_val |= LPFC_CTL_PDEV_CTL_FRST; }
    else
      if (opcode == LPFC_DV_RESET)
                      ...
                      char * buf)
{
  ...
  { state = "error"; }
    ...
    if (phba->link_state == LPFC_WARM_START)
    { state = "warm start"; }
    else
      if (phba->link_state == LPFC_INIT_START)
      { state = "offline"; }
      else
      { state = "online"; }
                       ...
                       const char * buf, size_t count)
{
  ...
  if (strncmp (buf, "online", sizeof ("online") - 1) == 0) {
    ...
    if (strncmp (buf, "offline", sizeof ("offline") - 1) == 0)
    { status = lpfc_do_offline (phba, LPFC_EVT_OFFLINE); }
    else
      if (strncmp (buf, "warm", sizeof ("warm") - 1) == 0)
        if (phba->sli_rev == LPFC_SLI_REV4)
        { status = -EINVAL; }
        else
        { status = lpfc_do_offline (phba, LPFC_EVT_WARM_START); }
      else
        if (strncmp (buf, "error", sizeof ("error") - 1) == 0)
          if (phba->sli_rev == LPFC_SLI_REV4)
          { status = -EINVAL; }
          else
          { status = lpfc_do_offline (phba, LPFC_EVT_KILL); }
        else
          if (strncmp (buf, "dump", sizeof ("dump") - 1) == 0)
          { status = lpfc_sli4_pdev_reg_request (phba, LPFC_FW_DUMP); }
          else
            if (strncmp (buf, "fw_reset", sizeof ("fw_reset") - 1) == 0)
            { status = lpfc_sli4_pdev_reg_request (phba, LPFC_FW_RESET); }
            else
              if (strncmp (buf, "dv_reset", sizeof ("dv_reset") - 1) == 0)
...
#define LPFC_MAX_DATA_CTRL_LEN 1024
  ...
  if (!strncmp (buf, "setbucket", strlen ("setbucket") ) ) {
    ...
    { bucket_type = LPFC_LINEAR_BUCKET; }
      ...
      if (!strncmp (bucket_type_str, "power2", strlen ("power2") ) )
