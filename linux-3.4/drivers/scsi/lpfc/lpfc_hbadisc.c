                        ...
                        struct lpfc_work_evt * evtp)
{
  ...
  if (evt_category == FC_REG_FABRIC_EVENT) {
    if (evt_sub_category == LPFC_EVENT_FCPRDCHKERR) {
      ...
      if ( (evt_sub_category == LPFC_EVENT_FABRIC_BUSY) ||
    ...
    if (evt_category == FC_REG_SCSI_EVENT) {
                                ...
                                struct fcf_record * new_fcf_record)
{
  ...
  list_for_each_entry_safe (fcf_pri, next_fcf_pri,
    ...
    if (new_fcf_pri->fcf_rec.priority <=
      ...
      if (fcf_pri->list.next == &phba->fcf.fcf_pri_list
          || new_fcf_pri->fcf_rec.priority <
...
lpfc_mbx_cmpl_read_topology (struct lpfc_hba * phba, LPFC_MBOXQ_t * pmb)
{
  ...
  if ( (bf_get (lpfc_mbx_read_top_att_type, la) == LPFC_ATT_LINK_UP) &&
    ...
    if (bf_get (lpfc_mbx_read_top_att_type, la) ==
        LPFC_ATT_LINK_DOWN) {
                     ...
                     struct lpfc_nodelist * ndlp)
{
  ...
  if (pring->ringno == LPFC_ELS_RING) {
    ...
    if (pring->ringno == psli->extra_ring) {
      ...
      if (pring->ringno == psli->fcp_ring) {
        ...
        if (pring->ringno == psli->next_ring) {
