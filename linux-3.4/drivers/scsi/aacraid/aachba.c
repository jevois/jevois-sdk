  ...
  if (fsa_dev_ptr) {
    ...
    if ( (le32_to_cpu (dresp->status) == ST_OK) &&
      ...
      { fsa_dev_ptr->sense_data.sense_key = NOT_READY; }
        ...
        if (fsa_dev_ptr->sense_data.sense_key == NOT_READY)
...
int aac_get_adapter_info (struct aac_dev * dev)
{
  ...
  if (dev->dac_support != 0) {
    if (!pci_set_dma_mask (dev->pdev, DMA_BIT_MASK (64) ) &&
      ...
      if (!pci_set_dma_mask (dev->pdev, DMA_BIT_MASK (32) ) &&
...
static int aac_write (struct scsi_cmnd * scsicmd)
{
  ...
  if (scsicmd->cmnd[0] == WRITE_6)  /* 6 byte command */
  {
    ...
    if (scsicmd->cmnd[0] == WRITE_16) { /* 16 byte command */
      ...
      if (scsicmd->cmnd[0] == WRITE_12) { /* 12 byte command */
...
int aac_scsi_cmd (struct scsi_cmnd * scsicmd)
{
   ...
   */
    ...
    if ( (scsicmd->cmnd[0] != INQUIRY) && /* only INQUIRY & TUR cmnd supported for controller */
