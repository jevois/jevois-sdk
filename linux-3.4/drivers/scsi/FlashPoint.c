...
static int FlashPoint_ProbeHostAdapter (struct sccb_mgr_info * pCardInfo)
{
  ...
  if (pCardInfo->si_card_model[1] == '3') {
    ...
    if (pCardInfo->si_card_model[2] == '0') {
    ...
    *pCardInfo)
{
  ...
  for (thisCard = 0; thisCard <= MAX_CARDS; thisCard++) {
    ...
    if (FPT_BL_Card[thisCard].ioPort == ioport) {
      ...
      if (FPT_BL_Card[thisCard].ioPort == 0x00) {
      ...
      if (hp_int & ICMD_COMP) {
        ...
        if (hp_int & ITAR_DISC) {
          ...
          if (hp_int & RSEL) {
            ...
            if ( (hp_int & IDO_STRT) && (! (hp_int & BUS_FREE) ) ) {
              ...
              if ( (hp_int & IUNKWN) || (hp_int & PROG_HLT) ) {
                WRW_HARPOON ( (ioport + hp_intstat),
...
static void FPT_ssel (unsigned long port, unsigned char p_card)
{
  ...
  if (currSCCB->OperationCode == RESET_COMMAND) {
    ...
    if (currSCCB->Sccb_scsistat == ABORT_ST) {
      ...
      if (! (currTar_Info->TarStatus & WIDE_NEGOCIATED) ) {
        auto_loaded = FPT_siwidn (port, p_card);
        currSCCB->Sccb_scsistat = SELECT_WN_ST;
      }
      
      else
        if (! ( (currTar_Info->TarStatus & TAR_SYNC_MASK)
                == SYNC_SUPPORTED) ) {
                       ...
                       unsigned char p_card)
{
  ...
  if (message == SMREST_DATA_PTR) {
    ...
    if (message == SMCMD_COMP) {
      ...
      if ( (message == SMNO_OP) || (message >= SMIDENT)
...
static void FPT_stsyncn (unsigned long port, unsigned char p_card)
{
  ...
  { our_sync_msg = 12; }  /* Setup our Message to 20mb/s */
    ...
    if ( (currTar_Info->TarEEValue & EE_SYNC_MASK) == EE_SYNC_10MB)
    
    { our_sync_msg = 25; }  /* Setup our Message to 10mb/s */
    
    else
      if ( (currTar_Info->TarEEValue & EE_SYNC_MASK) == EE_SYNC_5MB)
  ...
  if (currSCCB != NULL) {
    ...
    if (message == SMDEV_RESET) {
      ...
      if (currSCCB->Sccb_scsistat == ABORT_ST) {
        ...
        if (currSCCB->Sccb_scsistat < COMMAND_ST) {
  ...
  for (byte_cnt = 0; byte_cnt < ID_STRING_LENGTH; byte_cnt++) {
    ...
    for (bit_cnt = 0x80; bit_cnt != 0; bit_cnt >>= 1) {
      ...
      { ret_data = FPT_scxferc (p_port, 00); }
        ...
        if (p_id_string[byte_cnt] & bit_cnt)
...
static void FPT_utilUpdateResidual (struct sccb * p_SCCB)
{
  ...
  if (p_SCCB->Sccb_XferState & F_ALL_XFERRED) {
    ...
    if (p_SCCB->Sccb_XferState & F_SG_XFER) {
