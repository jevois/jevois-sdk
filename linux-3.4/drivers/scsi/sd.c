                            ...
                            const char * buf, size_t count)
{
  ...
  { sd_config_discard (sdkp, SD_LBP_UNMAP); }
    ...
    if (!strncmp (buf, lbp_mode[SD_LBP_WS16], 20) )
    { sd_config_discard (sdkp, SD_LBP_WS16); }
    else
      if (!strncmp (buf, lbp_mode[SD_LBP_WS10], 20) )
      { sd_config_discard (sdkp, SD_LBP_WS10); }
      else
        if (!strncmp (buf, lbp_mode[SD_LBP_ZERO], 20) )
        { sd_config_discard (sdkp, SD_LBP_ZERO); }
        else
          if (!strncmp (buf, lbp_mode[SD_LBP_DISABLE], 20) )
