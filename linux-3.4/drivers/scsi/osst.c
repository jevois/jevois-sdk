  ...
  while (1) {
    ...
    if ( osst_initiate_read (STp, aSRpnt)
      ...
      { position = 0xbb8; }
        ...
        if (position > STp->eod_frame_ppos || ++bad == 10) {
  ...
  for (flag = 1, new_frame = frame, p = buffer, i = 0; i < nframes + pending; ) {
    ...
    if (flag) {
      if (STp->write_type == OS_WRITE_HEADER) {
        ...
        if (new_frame < 2990 && new_frame + skip + nframes + pending >= 2990)
        { new_frame = 3000 - i; }
        else
        { new_frame += skip; }
      #if DEBUG
...
os_bypass:
  ...
  if (!ioctl_result) {        /* success */
    ...
    { STps->eof = ST_EOD; }
      ...
      if ( (cmd_in == MTFSFM || cmd_in == MTBSF) && STps->eof == ST_FM_HIT) {
    ...
    if (cmd_in == MTBSF || cmd_in == MTBSFM ) {
      ...
      if (cmd_in == MTFSF || cmd_in == MTFSFM) {
        ...
        if (cmd_in == MTBSR || cmd_in == MTFSR || cmd_in == MTWEOF || cmd_in == MTEOM) {
          STps->drv_file = STps->drv_block = (-1);
          STps->eof = ST_NOEOF;
          STp->header_ok = 0;
        }
          ...
          if (cmd_in == MTERASE) {
            ...
            if (SRpnt) {  /* SCSI command was not completely successful. */
