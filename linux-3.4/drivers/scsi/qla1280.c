  ...
  */
  ...
  case CS_INCOMPLETE:
    ...
    { host_status = DID_NO_CONNECT; }
      ...
      if (! (state_flags & SF_GOT_TARGET) )
      { host_status = DID_BAD_TARGET; }
      else
        if (! (state_flags & SF_SENT_CDB) )
        { host_status = DID_ERROR; }
        else
          if (! (state_flags & SF_TRANSFERRED_DATA) )
          { host_status = DID_ERROR; }
          else
            if (! (state_flags & SF_GOT_STATUS) )
            { host_status = DID_ERROR; }
            else
              if (! (state_flags & SF_GOT_SENSE) )
              { host_status = DID_ERROR; }
    break;
                     ...
                     struct list_head * done_q)
{
  ...
  if (sp) {
    ...
    if (pkt->entry_status & (BIT_3 + BIT_2) ) {
      ...
      if (pkt->entry_status & BIT_1) { /* FULL flag */
    ...
    if (pkt->entry_type == COMMAND_A64_TYPE) {
