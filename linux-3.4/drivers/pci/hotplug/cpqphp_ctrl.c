...
static u32 remove_board (struct pci_func * func, u32 replace_flag, struct controller * ctrl)
{
  ...
  { rc = cpqhp_save_base_addr_length (ctrl, func); }
    ...
    if (!func->bus_head && !func->mem_head &&
    ...
    for (loop = 0; loop < 10; loop++) {
      ...
      if (ctrl->event_queue[loop].event_type != 0) {
        ...
        if (ctrl->event_queue[loop].event_type == INT_BUTTON_PRESS) {
          ...
          if (ctrl->event_queue[loop].event_type ==
              INT_BUTTON_CANCEL) {
            dbg ("button cancel\n");
          ...
          /*** button Released (No action on press...) */
            ...
            if (ctrl->event_queue[loop].event_type == INT_BUTTON_RELEASE) {
              dbg ("button release\n");
            ...
            /***********POWER FAULT */
              ...
              if (ctrl->event_queue[loop].event_type == INT_POWER_FAULT) {
                dbg ("power fault\n");
              }
              else {
                                   ...
                                   struct resource_lists * resources)
{
  ...
  if ( (temp_byte & 0x7F) == PCI_HEADER_TYPE_BRIDGE) {
    ...
    if ( (temp_byte & 0x7F) == PCI_HEADER_TYPE_NORMAL) {
      /* Standard device */
      rc = pci_bus_read_config_byte (pci_bus, devfn, 0x0B, &class_code);
