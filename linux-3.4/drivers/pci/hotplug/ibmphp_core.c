    ...
    case HPC_SLOT_SPEED_66:
      if (SLOT_PCIX (slot_cur->ext_status) ) {
        ...
        { cmd = HPC_BUS_66PCIXMODE; }
          ...
          if (!SLOT_BUS_MODE (slot_cur->ext_status) )
...
static inline void print_card_capability (struct slot * slot_cur)
{
  ...
  { info ("   133 MHz PCI-X\n"); }
    ...
    if ( (slot_cur->ext_status & CARD_INFO) == PCIX66)
    { info ("    66 MHz PCI-X\n"); }
    else
      if ( (slot_cur->ext_status & CARD_INFO) == PCI66)
      { info ("    66 MHz PCI\n"); }
      else
      { info ("    33 MHz PCI\n"); }
...
static int enable_slot (struct hotplug_slot * hs)
{
  ...
  if (rc) {
    ...
    { err ("power fault occurred trying to power up\n"); }
      ...
      if (SLOT_BUS_SPEED (slot_cur->status) ) {
