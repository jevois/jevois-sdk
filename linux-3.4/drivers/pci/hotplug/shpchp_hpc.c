...
static int shpc_get_max_bus_speed (struct controller * ctrl)
{
  ...
  if (pi == 2) {
    ...
    { bus_speed = PCI_SPEED_133MHz_PCIX_533; }
      ...
      if (slot_avail2 & SLOT_100MHZ_PCIX_533)
      { bus_speed = PCI_SPEED_100MHz_PCIX_533; }
      else
        if (slot_avail2 & SLOT_66MHZ_PCIX_533)
        { bus_speed = PCI_SPEED_66MHz_PCIX_533; }
        else
          if (slot_avail2 & SLOT_133MHZ_PCIX_266)
          { bus_speed = PCI_SPEED_133MHz_PCIX_266; }
          else
            if (slot_avail2 & SLOT_100MHZ_PCIX_266)
            { bus_speed = PCI_SPEED_100MHz_PCIX_266; }
            else
              if (slot_avail2 & SLOT_66MHZ_PCIX_266)
              { bus_speed = PCI_SPEED_66MHz_PCIX_266; }
  }
  ...
  if (bus_speed == PCI_SPEED_UNKNOWN) {
    ...
    { bus_speed = PCI_SPEED_133MHz_PCIX; }
      ...
      if (slot_avail1 & SLOT_100MHZ_PCIX)
      { bus_speed = PCI_SPEED_100MHz_PCIX; }
      else
        if (slot_avail1 & SLOT_66MHZ_PCIX)
        { bus_speed = PCI_SPEED_66MHz_PCIX; }
        else
          if (slot_avail2 & SLOT_66MHZ)
          { bus_speed = PCI_SPEED_66MHz; }
          else
            if (slot_avail1 & SLOT_33MHZ)
            { bus_speed = PCI_SPEED_33MHz; }
            else
            { retval = -ENODEV; }
  }
