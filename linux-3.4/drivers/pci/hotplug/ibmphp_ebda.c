...
static int __init ebda_rio_table (void)
{
  ...
  for (i = 0; i < rio_table_ptr->riodev_count; i++) {
    ...
    { list_add (&rio_detail_ptr->rio_detail_list, &rio_vg_head); }
      ...
      if (rio_detail_ptr->rio_type == 6 || rio_detail_ptr->rio_type == 7)
  ...
  for (ctlr = 0; ctlr < hpc_list_ptr->num_ctlrs; ctlr++) {
    ...
    for (index = 0; index < hpc_ptr->slot_count; index++) {
      ...
      { tmp_slot->supported_speed =  3; }
        ...
        if ( (hpc_ptr->slots[index].slot_cap & EBDA_SLOT_100_MAX) == EBDA_SLOT_100_MAX)
        { tmp_slot->supported_speed =  2; }
        else
          if ( (hpc_ptr->slots[index].slot_cap & EBDA_SLOT_66_MAX) == EBDA_SLOT_66_MAX)
...
void ibmphp_free_ebda_hpc_queue (void)
{
  ...
  list_for_each_safe (list, next, &ebda_hpc_head) {
    ...
    { release_region (controller->u.isa_ctlr.io_start, (controller->u.isa_ctlr.io_end - controller->u.isa_ctlr.io_start + 1) ); }
      ...
      if ( (controller->ctlr_type == 1) && (!pci_flag) ) {
        ++pci_flag;
