    ...
    case 1:
      if (i == src_cnt - 2 || (order == -1
        ...
        if (cur_addr == old_addr + len * order) {
          ...
          if (cur_addr == old_addr + 2 * len) {
            state = 2;
            if (i == src_cnt - 1)
            { addr_count++; }
          }
          else
            if (cur_addr == old_addr + 3 * len) {
              state = 2;
              if (i == src_cnt - 1)
              { addr_count++; }
            }
...
static void __ppc440spe_adma_slot_cleanup (struct ppc440spe_adma_chan * chan)
{
  ...
  list_for_each_entry_safe (iter, _iter, &chan->chain,
    ...
    if (slots_per_op != 0 && slot_cnt == 0) {
      ...
      if (slots_per_op) /* wait for group completion */
      { continue; }
    ...
    if (src_cnt > 1 &&
      ...
      if (src_cnt != 2) {
        ...
        if ( (src[1] + len) == src[2]) {
          ...
          if ( (src[1] + len * 2) == src[2]) {
            /* do RXOR R1 R2 R4 */
            set_bit (PPC440SPE_DESC_RXOR124, &op);
          }
          else
            if ( (src[1] + len * 3) == src[2]) {
              /* do RXOR R1 R2 R5 */
    ...
    if (test_bit (PPC440SPE_DESC_RXOR, &sw_desc->flags) ) {
      ...
      if (index == 0) {
        ...
        if (test_bit (PPC440SPE_DESC_RXOR12,
          ...
          if (test_bit (PPC440SPE_DESC_RXOR123,
                        &sw_desc->flags) )
            haddr = DMA_RXOR123 <<
                    DMA_CUED_REGION_OFF;
          else
            if (test_bit (PPC440SPE_DESC_RXOR124,
                          &sw_desc->flags) )
              haddr = DMA_RXOR124 <<
                      DMA_CUED_REGION_OFF;
            else
              if (test_bit (PPC440SPE_DESC_RXOR125,
                            &sw_desc->flags) )
                haddr = DMA_RXOR125 <<
                        DMA_CUED_REGION_OFF;
              else
  ...
  struct ppc440spe_rxor * cursor, int index, int src_cnt)
{
  ...
  if (index == src_cnt - 1) {
    ...
    if (cursor->addr_count == XOR_MAX_OPS) {
      ppc440spe_desc_set_xor_src_cnt (desc, cursor->addr_count);
      cursor->addr_count = 0;
      cursor->desc_count++;
    }
  ...
  int src_cnt, u32 addr)
{
  ...
  case 0:
    if (addr == cursor->addrl + cursor->len) {
      ...
      if (cursor->addrl == addr + cursor->len) {
        /* reverse RXOR */
        cursor->state = 1;
        cursor->xor_count++;
  ...
  case 1:
    ...
    if (index == src_cnt - 2 || (sign == -1
      ...
      if (addr == cursor->addrl + 2 * sign * cursor->len) {
        cursor->state = 2;
        cursor->xor_count = 0;
        ppc440spe_rxor_set_region (desc,
                                   cursor->addr_count,
        ...
        if (addr == cursor->addrl + 3 * cursor->len) {
          cursor->state = 2;
          cursor->xor_count = 0;
          ppc440spe_rxor_set_region (desc,
                                     cursor->addr_count,
          ...
          if (addr == cursor->addrl + 4 * cursor->len) {
            cursor->state = 2;
            cursor->xor_count = 0;
            ppc440spe_rxor_set_region (desc,
                                       cursor->addr_count,
