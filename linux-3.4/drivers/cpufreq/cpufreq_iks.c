...
static void dbs_check_cpu (struct cpu_dbs_info_s * this_dbs_info)
{
  ...
  for_each_cpu (cpu, policy->cpus) {
    ...
    { heavy_load_core_cnt ++; }
      ...
      if (j_dbs_info->is_little && loading < dbs_tuners_ins.core_down_threshold) {
        light_load_core_cnt ++;
...
__process_power_change:
    ...
    if (!cpumask_empty (&tmp_core_up_mask) ) {
      ...
      if (!cpumask_empty (&tmp_core_down_mask) ) {
...
static int cpufreq_governor_dbs (struct cpufreq_policy * policy, unsigned int event)
{
  ...
  case CPUFREQ_GOV_LIMITS:
    ...
    if (policy->max < this_dbs_info->cur_policy->cur)
      ...
      if (policy->min > this_dbs_info->cur_policy->cur)
        __cpufreq_driver_target (this_dbs_info->cur_policy,
