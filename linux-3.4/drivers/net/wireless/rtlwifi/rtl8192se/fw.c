...
void rtl92s_set_fw_pwrmode_cmd (struct ieee80211_hw * hw, u8 Mode)
{
  ...
  { max_wakeup_period = mac->vif->bss_conf.beacon_int; }
    ...
    if (pwrmode.mode == FW_PS_MAX_MODE)
      max_wakeup_period = mac->vif->bss_conf.beacon_int *
  ...
  { pwrmode.bcn_pass_cnt = 1; }
    ...
    if ( (max_wakeup_period >= 300) && (max_wakeup_period < 500) )
    { pwrmode.bcn_pass_cnt = 2; }
    else
      if ( (max_wakeup_period >= 200) && (max_wakeup_period < 300) )
      { pwrmode.bcn_pass_cnt = 3; }
      else
        if ( (max_wakeup_period >= 20) && (max_wakeup_period < 200) )
