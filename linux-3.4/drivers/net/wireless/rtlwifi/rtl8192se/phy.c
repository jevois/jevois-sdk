  ...
  if (configtype == BASEBAND_CONFIG_PHY_REG) {
    for (i = 0; i < phy_reg_len; i = i + 2) {
      ...
      { mdelay (50); }
        ...
        if (phy_reg_table[i] == 0xfd)
        { mdelay (5); }
        else
          if (phy_reg_table[i] == 0xfc)
          { mdelay (1); }
          else
            if (phy_reg_table[i] == 0xfb)
            { udelay (50); }
            else
              if (phy_reg_table[i] == 0xfa)
              { udelay (5); }
              else
                if (phy_reg_table[i] == 0xf9)
                { udelay (1); }
  ...
  if (configtype == BASEBAND_CONFIG_PHY_REG) {
    for (i = 0; i < phy_regarray2xtxr_len; i = i + 3) {
      ...
      { mdelay (50); }
        ...
        if (phy_regarray2xtxr_table[i] == 0xfd)
        { mdelay (5); }
        else
          if (phy_regarray2xtxr_table[i] == 0xfc)
          { mdelay (1); }
          else
            if (phy_regarray2xtxr_table[i] == 0xfb)
            { udelay (50); }
            else
              if (phy_regarray2xtxr_table[i] == 0xfa)
              { udelay (5); }
              else
                if (phy_regarray2xtxr_table[i] == 0xf9)
                { udelay (1); }
  ...
  if (configtype == BASEBAND_CONFIG_PHY_REG) {
    for (i = 0; i < phy_pg_len; i = i + 3) {
      ...
      { mdelay (50); }
        ...
        if (phy_table_pg[i] == 0xfd)
        { mdelay (5); }
        else
          if (phy_table_pg[i] == 0xfc)
          { mdelay (1); }
          else
            if (phy_table_pg[i] == 0xfb)
            { udelay (50); }
            else
              if (phy_table_pg[i] == 0xfa)
              { udelay (5); }
              else
                if (phy_table_pg[i] == 0xf9)
                { udelay (1); }
  ...
  case RF90_PATH_A:
    for (i = 0; i < radio_a_tblen; i = i + 2) {
      ...
      { mdelay (50); }
        ...
        if (radio_a_table[i] == 0xfd)
        { mdelay (5); }
        else
          if (radio_a_table[i] == 0xfc)
          { mdelay (1); }
          else
            if (radio_a_table[i] == 0xfb)
            { udelay (50); }
            else
              if (radio_a_table[i] == 0xfa)
              { udelay (5); }
              else
                if (radio_a_table[i] == 0xf9)
                { udelay (1); }
                else
  ...
  case RF90_PATH_B:
    for (i = 0; i < radio_b_tblen; i = i + 2) {
      ...
      { mdelay (50); }
        ...
        if (radio_b_table[i] == 0xfd)
        { mdelay (5); }
        else
          if (radio_b_table[i] == 0xfc)
          { mdelay (1); }
          else
            if (radio_b_table[i] == 0xfb)
            { udelay (50); }
            else
              if (radio_b_table[i] == 0xfa)
              { udelay (5); }
              else
                if (radio_b_table[i] == 0xf9)
                { udelay (1); }
                else
                  rtl92s_phy_set_rf_reg (hw, rfpath,
                                         radio_b_table[i],
