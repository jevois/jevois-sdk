...
static void _rtl92ce_gen_refresh_led_state (struct ieee80211_hw * hw)
{
  ...
  { rtl92ce_sw_led_on (hw, pLed0); }
    ...
    if (ppsc->rfoff_reason == RF_CHANGE_BY_INIT)
    { rtl92ce_sw_led_on (hw, pLed0); }
    else
                                      ...
                                      enum nl80211_iftype type)
{
  ...
  if (type == NL80211_IFTYPE_UNSPECIFIED ||
    ...
    if (type == NL80211_IFTYPE_ADHOC || type == NL80211_IFTYPE_AP) {
      _rtl92ce_resume_tx_beacon (hw);
...
bool rtl92ce_gpio_radio_on_off_checking (struct ieee80211_hw * hw, u8 * valid)
{
  ...
  if ( (ppsc->hwradiooff) && (e_rfpowerstate_toset == ERFON) ) {
    ...
    if (!ppsc->hwradiooff && (e_rfpowerstate_toset == ERFOFF) ) {
      RT_TRACE (rtlpriv, COMP_RF, DBG_DMESG,
...
static void rtl8192ce_bt_var_init (struct ieee80211_hw * hw)
{
  ...
  if (rtlpcipriv->bt_coexist.bt_coexistence) {
    ...
    { rtlpcipriv->bt_coexist.bt_service = BT_OTHER_ACTION; }
      ...
      if (rtlpcipriv->bt_coexist.reg_bt_sco == 2)
      { rtlpcipriv->bt_coexist.bt_service = BT_SCO; }
      else
        if (rtlpcipriv->bt_coexist.reg_bt_sco == 4)
        { rtlpcipriv->bt_coexist.bt_service = BT_BUSY; }
        else
          if (rtlpcipriv->bt_coexist.reg_bt_sco == 5)
