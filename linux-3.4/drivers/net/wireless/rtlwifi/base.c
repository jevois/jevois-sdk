  ...
  if (rtlpriv->dm.useramask) {
    ...
    if (mac->opmode == NL80211_IFTYPE_STATION) {
      ...
      { tcb_desc->ratr_index = RATR_INX_WIRELESS_NGB; }
        ...
        if (mac->mode == WIRELESS_MODE_N_5G)
                                   ...
                                   struct octet_string vendor_ie)
{
  ...
  if (memcmp (vendor_ie.octet, athcap_1, 3) == 0 ||
    ...
    if (memcmp (vendor_ie.octet, broadcap_1, 3) == 0 ||
      ...
      if (memcmp (vendor_ie.octet, racap, 3) == 0) {
        rtlpriv->mac80211.vendor = PEER_RAL;
        matched = true;
      }
      else
        if (memcmp (vendor_ie.octet, ciscocap, 3) == 0) {
          rtlpriv->mac80211.vendor = PEER_CISCO;
          matched = true;
        }
        else
          if (memcmp (vendor_ie.octet, marvcap, 3) == 0) {
...
void rtl_recognize_peer (struct ieee80211_hw * hw, u8 * data, unsigned int len)
{
  ...
  if ( (memcmp (mac->bssid, ap5_1, 3) == 0) ||
    ...
    if ( (memcmp (mac->bssid, ap4_4, 3) == 0) ||
      ...
      if (memcmp (mac->bssid, ap6_1, 3) == 0 ||
          vendor == PEER_CISCO) {
        vendor = PEER_CISCO;
        RT_TRACE (rtlpriv, COMP_MAC80211, DBG_LOUD, "=>cisco find\n");
      }
      else
        if ( (memcmp (mac->bssid, ap3_1, 3) == 0) ||
