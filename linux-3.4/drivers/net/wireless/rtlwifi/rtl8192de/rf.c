...
static u8 _rtl92d_phy_get_chnlgroup_bypg (u8 chnlindex)
{
  ...
  { group = 0; }
    ...
    if (channel_info[chnlindex] <= 9)  /* Channel 4-9 */
    { group = 1; }
    else
      if (channel_info[chnlindex] <= 14) /* Channel 10-14 */
      { group = 2; }
      else
        if (channel_info[chnlindex] <= 64)
        { group = 6; }
        else
          if (channel_info[chnlindex] <= 140)
          { group = 7; }
          else
          { group = 8; }
  return group;
