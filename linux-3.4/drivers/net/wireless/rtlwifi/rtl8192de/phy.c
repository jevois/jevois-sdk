  ...
  if (configtype == BASEBAND_CONFIG_PHY_REG) {
    for (i = 0; i < phy_reg_arraylen; i = i + 2) {
      ...
      { mdelay (50); }
        ...
        if (phy_regarray_table[i] == 0xfd)
        { mdelay (5); }
        else
          if (phy_regarray_table[i] == 0xfc)
          { mdelay (1); }
          else
            if (phy_regarray_table[i] == 0xfb)
            { udelay (50); }
            else
              if (phy_regarray_table[i] == 0xfa)
              { udelay (5); }
              else
                if (phy_regarray_table[i] == 0xf9)
                { udelay (1); }
    ...
    if (configtype == BASEBAND_CONFIG_AGC_TAB) {
      if (rtlhal->interfaceindex == 0) {
  ...
  if (configtype == BASEBAND_CONFIG_PHY_REG) {
    for (i = 0; i < phy_regarray_pg_len; i = i + 3) {
      ...
      { mdelay (50); }
        ...
        if (phy_regarray_table_pg[i] == 0xfd)
        { mdelay (5); }
        else
          if (phy_regarray_table_pg[i] == 0xfc)
          { mdelay (1); }
          else
            if (phy_regarray_table_pg[i] == 0xfb)
            { udelay (50); }
            else
              if (phy_regarray_table_pg[i] == 0xfa)
              { udelay (5); }
              else
                if (phy_regarray_table_pg[i] == 0xf9)
                { udelay (1); }
  ...
  case RF90_PATH_A:
    for (i = 0; i < radioa_arraylen; i = i + 2) {
      if (radioa_array_table[i] == 0xfe) {
        ...
        if (radioa_array_table[i] == 0xfd) {
          /* delay_ms(5); */
          mdelay (5);
        }
        else
          if (radioa_array_table[i] == 0xfc) {
            /* delay_ms(1); */
            mdelay (1);
          }
          else
            if (radioa_array_table[i] == 0xfb) {
              udelay (50);
            }
            else
              if (radioa_array_table[i] == 0xfa) {
                udelay (5);
              }
              else
                if (radioa_array_table[i] == 0xf9) {
                  udelay (1);
                }
                else {
  ...
  case RF90_PATH_B:
    for (i = 0; i < radiob_arraylen; i = i + 2) {
      if (radiob_array_table[i] == 0xfe) {
        ...
        if (radiob_array_table[i] == 0xfd) {
          /* delay_ms(5); */
          mdelay (5);
        }
        else
          if (radiob_array_table[i] == 0xfc) {
            /* delay_ms(1); */
            mdelay (1);
          }
          else
            if (radiob_array_table[i] == 0xfb) {
              udelay (50);
            }
            else
              if (radiob_array_table[i] == 0xfa) {
                udelay (5);
              }
              else
                if (radiob_array_table[i] == 0xf9) {
                  udelay (1);
                }
                else {
...
static void _rtl92d_phy_switch_rf_setting (struct ieee80211_hw * hw, u8 channel)
{
  ...
  if (rtlhal->current_bandtype == BAND_ON_5G) {
    ...
    { index = 2; }
      ...
      if (channel == 151 || channel == 153 || channel == 163
          || channel == 165)
      { index = 3; }
      else
        if (channel == 157 || channel == 159)
        { index = 4; }
    ...
    for (i = 0; i < RF_REG_NUM_FOR_C_CUT_5G; i++) {
      if (i == 0 && (rtlhal->macphymode == DUALMAC_DUALPHY) ) {
        ...
        if (rf_reg_for_c_cut_5g[i] == RF_SYN_G4) {
    ...
    if (rtlhal->current_bandtype == BAND_ON_2_4G) {
      ...
      { index = 0; }
        ...
        if (channel == 3 || channel == 13 || channel == 14)
        { index = 1; }
        else
                                      ...
                                      u8 t, bool is2t)
{
  ...
  for (i = 0; i < retrycount; i++) {
    ...
    if (patha_ok == 0x03) {
      ...
      if (i == (retrycount - 1) && patha_ok == 0x01) {
        /* Tx IQK OK */
        RTPRINT (rtlpriv, FINIT, INIT_IQK,
  ...
  if (is2t) {
    ...
    for (i = 0; i < retrycount; i++) {
      ...
      if (pathb_ok == 0x03) {
        ...
        if (i == (retrycount - 1) && pathb_ok == 0x01) {
          /* Tx IQK OK */
          RTPRINT (rtlpriv, FINIT, INIT_IQK,
  ...
  case ERFSLEEP:
    ...
    for (queue_id = 0, i = 0;
      ...
      if (skb_queue_len (&ring->queue) == 0 ||
        ...
        if (rtlpci->pdev->current_state != PCI_D0) {
          RT_TRACE (rtlpriv, COMP_POWER, DBG_LOUD,
...
u8 rtl92d_get_chnlgroup_fromarray (u8 chnl)
{
  ...
  { group = 0; }
    ...
    if (channel_info[chnl] <= 9)
    { group = 1; }
    else
      if (channel_info[chnl] <= 14)
      { group = 2; }
      else
        if (channel_info[chnl] <= 44)
        { group = 3; }
        else
          if (channel_info[chnl] <= 54)
          { group = 4; }
          else
            if (channel_info[chnl] <= 64)
            { group = 5; }
            else
              if (channel_info[chnl] <= 112)
              { group = 6; }
              else
                if (channel_info[chnl] <= 126)
                { group = 7; }
                else
                  if (channel_info[chnl] <= 140)
                  { group = 8; }
                  else
                    if (channel_info[chnl] <= 153)
                    { group = 9; }
                    else
                      if (channel_info[chnl] <= 159)
                      { group = 10; }
                      else
                      { group = 11; }
  return group;
