...
static void rtl92d_early_mode_enabled (struct rtl_priv * rtlpriv)
{
  if ( (rtlpriv->mac80211.link_state >= MAC80211_LINKED) &&
    ...
    if (de_digtable.last_min_undecorated_pwdb_for_dm >= 50
      ...
      if (de_digtable.last_min_undecorated_pwdb_for_dm <= 55 &&
    ...
    if (! (rtl_read_byte (rtlpriv, REG_EARLY_MODE_CONTROL) & 0xf) ) {
      rtl_write_byte (rtlpriv, REG_EARLY_MODE_CONTROL, 0x0f);
...
static void rtl92d_dm_dig (struct ieee80211_hw * hw)
{
  ...
  { value_igi--; }
    ...
    if (falsealm_cnt->cnt_all < DM_DIG_FA_TH1)
    { value_igi += 0; }
    else
  ...
  { value_igi = DM_DIG_MAX; }
    ...
    if (value_igi < de_digtable.rx_gain_range_min)
  ...
  if (rtlhal->current_bandtype == BAND_ON_5G) {
    if (undecorated_smoothed_pwdb >= 0x33) {
      ...
      if ( (undecorated_smoothed_pwdb < 0x33)
        ...
        if (undecorated_smoothed_pwdb < 0x2b) {
          rtlpriv->dm.dynamic_txhighpower_lvl =
