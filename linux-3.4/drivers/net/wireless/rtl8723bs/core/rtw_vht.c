...
void  rtw_vht_use_default_setting (_adapter * padapter)
{
  ...
  if (rf_type == RF_1T1R)
  { pvhtpriv->vht_mcs_map[0] = 0xfe; } 
  else
  { pvhtpriv->vht_mcs_map[0] = 0xfa; } 
  pvhtpriv->vht_mcs_map[1] = 0xff;
  
  if (pregistrypriv->vht_rate_sel == 1)
  {
    pvhtpriv->vht_mcs_map[0] = 0xfc; 
  }
  else
    if (pregistrypriv->vht_rate_sel == 2)
    {
      pvhtpriv->vht_mcs_map[0] = 0xfd; 
    }
    else
      if (pregistrypriv->vht_rate_sel == 3)
      {
        pvhtpriv->vht_mcs_map[0] = 0xfe; 
      }
      else
        if (pregistrypriv->vht_rate_sel == 4)
        {
          pvhtpriv->vht_mcs_map[0] = 0xf0; 
        }
        else
          if (pregistrypriv->vht_rate_sel == 5)
          {
            pvhtpriv->vht_mcs_map[0] = 0xf5; 
          }
          else
            if (pregistrypriv->vht_rate_sel == 6)
            {
              pvhtpriv->vht_mcs_map[0] = 0xfa; 
            }
            else
              if (pregistrypriv->vht_rate_sel == 7)
              {
                pvhtpriv->vht_mcs_map[0] = 0xf8; 
              }
              else
                if (pregistrypriv->vht_rate_sel == 8)
                {
                  pvhtpriv->vht_mcs_map[0] = 0xf9; 
                }
                else
                  if (pregistrypriv->vht_rate_sel == 9)
                  {
                    pvhtpriv->vht_mcs_map[0] = 0xf4; 
                  }
...
void  update_sta_vht_info_apmode (_adapter * padapter, PVOID sta)
{
  ...
  if (psta->bw_mode == CHANNEL_WIDTH_80) {
    ...
    if (psta->bw_mode >= CHANNEL_WIDTH_160) {
...
u32 rtw_build_vht_cap_ie (_adapter * padapter, u8 * pbuf)
{
  ...
  SET_VHT_CAPABILITY_ELE_MCS_RX_HIGHEST_RATE (pcap, HighestRate);
  SET_VHT_CAPABILITY_ELE_MCS_TX_HIGHEST_RATE (pcap, HighestRate);
