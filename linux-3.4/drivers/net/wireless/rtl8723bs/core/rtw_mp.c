...
void mpt_InitHWConfig (PADAPTER Adapter)
{
  if (IS_HARDWARE_TYPE_8723B (Adapter) )
  {
    ...
    PHY_SetMacReg (Adapter, 0x64, BIT20, 0x0);    
    PHY_SetMacReg (Adapter, 0x64, BIT24, 0x0);    
    PHY_SetMacReg (Adapter, 0x40, BIT4, 0x0);     
    PHY_SetMacReg (Adapter, 0x40, BIT3, 0x1);     
    PHY_SetMacReg (Adapter, 0x4C, BIT24, 0x1);    
    PHY_SetMacReg (Adapter, 0x4C, BIT23, 0x0);    
    PHY_SetBBReg (Adapter, 0x944, BIT1 | BIT0, 0x3);
    PHY_SetBBReg (Adapter, 0x930, bMaskByte0, 0x77); 
    PHY_SetMacReg (Adapter, 0x38, BIT11, 0x1);    
    
    PHY_SetMacReg (Adapter, 0x778, 0x3, 0x3);        
    PHY_SetMacReg (Adapter, 0x64, bMaskDWord, 0x36000000);
    PHY_SetMacReg (Adapter, 0x948, bMaskDWord, 0x0);      
    
    PHY_SetBBReg (Adapter, 0xA00, BIT8, 0x0);    
  }
}
...
)
{
  ...
  PHY_SetBBReg (pAdapter, 0xA01, BIT0, 1);
  #endif
  #if 0
...
static VOID mpt_AdjustRFRegByRateByChan92CU (PADAPTER pAdapter, u8 RateIdx, u8 Channel, u8 BandWidthID)
{
  ...
  if (RateIdx < MPT_RATE_6M) { 
    ...
    if ( (RateIdx >= MPT_RATE_6M) && (RateIdx <= MPT_RATE_54M) ) {
      if ( (4 == Channel) || (8 == Channel) || (12 == Channel) )
      { rfReg0x26 = 0xf000; }
      else
        if ( (5 == Channel) || (7 == Channel) || (13 == Channel) || (14 == Channel) )
        { rfReg0x26 = 0xf400; }
        else
        { rfReg0x26 = 0x4f200; }
    }
    else
      if ( (RateIdx >= MPT_RATE_MCS0) && (RateIdx <= MPT_RATE_MCS15) ) {
...
void fill_tx_desc_8812a (PADAPTER padapter)
{
  ...
  if (!pattrib->qos_en) {
    SET_TX_DESC_HWSEQ_EN_8812 (pDesc, 1);
  }
  else {
...
void fill_tx_desc_8192e (PADAPTER padapter)
{
  ...
  if (!pattrib->qos_en) {
    SET_TX_DESC_HWSEQ_SEL_92E (pDesc, 1);
  }
  else {
...
static void Rtw_MPSetMacTxEDCA (PADAPTER padapter)
{
  ...
  rtw_write32 (padapter, 0x508 , 0x00a43f);
  PHY_SetMacReg (padapter, 0x458 , bMaskDWord , 0x0);
  PHY_SetMacReg (padapter, 0x460 , bMaskLWord , 0x0);
...
void SetPacketRx (PADAPTER pAdapter, u8 bStartRx)
{
  ...
  {
    #ifdef CONFIG_RTL8723B
    PHY_SetMacReg (pAdapter, 0xe70, BIT23 | BIT22, 0x3);
    write_bbreg (pAdapter, 0xa01, BIT0, bDisable);
    #endif
  ...
  {
    #ifdef CONFIG_RTL8723B
    PHY_SetMacReg (pAdapter, 0xe70, BIT23 | BIT22, 0x00);
    write_bbreg (pAdapter, 0xa01, BIT0, bEnable);
    #endif
    rtw_write32 (pAdapter, REG_RCR, 0);
  }
}
  ...
  case 1:
    {
      ...
      if (pHalData->pwrGroupCnt >= 3)
      {
        ...
        { PwrGroup = 0; }
          ...
          if (CurrChannel >= 4 && CurrChannel <= 9)
          { PwrGroup = 1; }
          else
            if (CurrChannel > 9)
            { PwrGroup = 2; }
  ...
  if (IS_HARDWARE_TYPE_8192D (pAdapter) )
  {
    ...
    if (IS_HARDWARE_TYPE_8188E (pAdapter) )
    {
      ...
      if (IS_HARDWARE_TYPE_8812 (pAdapter) )
      {
        ...
        if (IS_HARDWARE_TYPE_8821 (pAdapter) || IS_HARDWARE_TYPE_8192E (pAdapter) ||
