...
void free_tdls_sta (_adapter * padapter, struct sta_info * ptdls_sta)
{
  ...
  if ( ptdlsinfo->sta_cnt < (NUM_STA - 2 - 4) )
  {
    ptdlsinfo->sta_maximum = _FALSE;
...
{
  u8 sup_ch[ 30 * 2 ] = { 0x00 }, ch_set_idx = 0;
  ...
  do {
         ...
         pmlmeext->channel_set[ch_set_idx].ChannelNum <= 14 )
    {
      ...
      if ( pmlmeext->channel_set[ch_set_idx].ChannelNum >= 36 &&
           pmlmeext->channel_set[ch_set_idx].ChannelNum <= 48)
      {
        b1 = 1;
      }
      else
        if ( pmlmeext->channel_set[ch_set_idx].ChannelNum >= 149 &&
  ...
  if (ptdls_sta)
  {
    ...
    if (ptdls_sta->stat_code == 0 )
    {
      if (rsnie_included && (prx_pkt_attrib->encrypt == 0) ) {
        ...
        if (rsnie_included == 0 && (prx_pkt_attrib->encrypt) ) {
  ...
  else
  {
    if (prx_pkt_attrib->encrypt)
    {
      if (verify_ccmp == 1)
      {
        ...
        ptdls_sta->stat_code = 0;
        if (tdls_verify_mic (ptdls_sta->tpk.kck, 2, plinkid_ie, prsnie, ptimeout_ie, pftie) == 0)
        {
          free_tdls_sta (padapter, ptdls_sta);
          ret = _FAIL;
          goto exit;
...
int On_TDLS_Setup_Cfm (_adapter * padapter, union recv_frame * precv_frame)
{
  ...
  if (prx_pkt_attrib->encrypt) {
    ...
    if (tdls_verify_mic (ptdls_sta->tpk.kck, 3, plinkid_ie, prsnie, ptimeout_ie, pftie) == 0) {
      free_tdls_sta (padapter, ptdls_sta);
      ret = _FAIL;
      goto exit;
    }
    ...
    if (wmmps_ac && 1)
    {
      ...
      if (ptdls_sta->sleepq_len == 0)
      {
        ...
        if (state == 2) {
          ...
          if (ptdls_sta->tdls_sta_state & TDLS_SW_OFF_STATE) {
...
void rtw_build_tdls_setup_req_ies (_adapter * padapter, struct xmit_frame * pxmitframe, u8 * pframe, struct tdls_txmgmt * ptxmgmt)
{
  ...
  if (pattrib->encrypt) {
    _rtw_memset (pframe, 0, 84);
    _rtw_memset (pframe, _FTIE_, 1);
    _rtw_memset ( (pframe + 1), 82, 1);
