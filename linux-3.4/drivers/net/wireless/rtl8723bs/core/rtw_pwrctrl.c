...
void  traffic_check_for_leave_lps (PADAPTER padapter, u8 tx, u32 tx_packets)
{
  ...
  if (tx)
  {
    ...
    if (rtw_get_passing_time_ms (start_time) > 2000)
    {
      if (xmit_cnt > 8)
      {
...
u8 PS_RDY_CHECK (_adapter * padapter)
{
  ...
  { return _TRUE; }
    ...
    if (_TRUE == pwrpriv->bInSuspend && pwrpriv->wowlan_ap_mode)
    { return _TRUE; }
    else
  ...
  else
  {
    if (pwrpriv->rf_pwrstate == rf_off)
    {
      ...
      if (Adapter->registrypriv.usbss_enable)
      {
        ...
        adapter_to_dvobj (Adapter)->pusbdev->autosuspend_disabled = Adapter->bDisableAutosuspend;
        #endif
      }
      else
      #endif
