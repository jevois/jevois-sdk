  ...
  while ( (rtw_end_of_queue_search (phead, plist) ) == _FALSE)
  {
    ...
    if ( (psta->flags & WLAN_STA_HT) && (psta->htpriv.agg_enable_bitmap || psta->under_exist_checking) ) {
      ...
      if ( psta->expire_to <= (pstapriv->expire_to - 50 ) ) {
        ...
        if ( psta->expire_to <= (pstapriv->expire_to - 3) && (psta->under_exist_checking == 0) ) {
...
#define KEEP_ALIVE_TRYCNT (3)
      ...
      if (psta->keep_alive_trycnt > 0 && psta->keep_alive_trycnt <= KEEP_ALIVE_TRYCNT)
      {
        ...
        if ( (psta->keep_alive_trycnt > KEEP_ALIVE_TRYCNT) && ! (psta->state & WIFI_STA_ALIVE_CHK_STATE) )
        {
          psta->keep_alive_trycnt = 0;
        }
...
void add_RATid (_adapter * padapter, struct sta_info * psta, u8 rssi_level)
{
  ...
  if ( (pstat->rssi_level < 1) || (pstat->rssi_level > 3) )
  {
    ...
    { pstat->rssi_level = 1; }
      ...
      if ( (pstat->rssi >= priv->pshare->rf_ft_var.raGoDown20MLower) ||
...
void update_bmc_sta (_adapter * padapter)
{
  ...
  if (psta)
  {
    ...
    if (IsSupportedTxCCK (network_type) ) {
      ...
      if (network_type == WIRELESS_INVALID) {
        if ( pcur_network->Configuration.DSConfig > 14 )
...
int rtw_check_beacon_data (_adapter * padapter, u8 * pbuf,  int len)
{
  ...
  if (p && ie_len > 0)
  {
    ...
    pht_cap->ampdu_params_info |= (IEEE80211_HT_CAP_AMPDU_FACTOR & max_rx_ampdu_factor);
...
static int rtw_ht_operation_update (_adapter * padapter)
{
  ...
  { new_op_mode = OP_MODE_MIXED; }
    ...
    if ( (phtpriv_ap->ht_cap.cap_info & IEEE80211_HT_CAP_SUP_WIDTH)
         && pmlmepriv->num_sta_ht_20mhz)
    { new_op_mode = OP_MODE_20MHZ_HT_STA_ASSOCED; }
    else
      if (pmlmepriv->olbc_ht)
