  ...
  while ( p2p_ie )
  {
    ...
    if (rtw_get_p2p_attr_content (p2p_ie, p2p_ielen, P2P_ATTR_DEVICE_INFO, NULL, (uint *) &attr_contentlen) )
    {
      ...
      if (pattr_content)
      {
        ...
        _rtw_memcpy (psta->dev_addr,   pattr_content, ETH_ALEN);
        
        pattr_content += ETH_ALEN;
        
        _rtw_memcpy (&psta->config_methods, pattr_content, 2);
               ...
               rtw_p2p_chk_state (pwdinfo, P2P_STATE_RX_PROVISION_DIS_REQ) )
          {
            ...
            if (rtw_p2p_chk_state (pwdinfo, P2P_STATE_GONEGO_OK) )
            {
              ...
              if (rtw_p2p_chk_state (pwdinfo, P2P_STATE_TX_PROVISION_DIS_REQ) )
              {
                ...
                if (rtw_p2p_chk_state (pwdinfo, P2P_STATE_GONEGO_ING) && pwdinfo->nego_req_info.benable == _TRUE)
                {
                  val8 = 1;
                  set_channel_bwmode (padapter, pwdinfo->nego_req_info.peer_channel_num[0], HAL_PRIME_CHNL_OFFSET_DONT_CARE, CHANNEL_WIDTH_20);
                  rtw_hal_set_hwreg (padapter, HW_VAR_MLME_SITESURVEY, (u8 *) (&val8) );
                  ...
                  if (rtw_p2p_chk_state (pwdinfo, P2P_STATE_TX_INVITE_REQ ) && pwdinfo->invitereq_info.benable == _TRUE)
                  {
                    /*
                    val8 = 1;
...
static void ro_ch_handler (_adapter * padapter)
{
  ...
  if (rtw_get_ch_setting_union (padapter, &ch, &bw, &offset) != 0) {
    ...
    if (adapter_wdev_data (padapter)->p2p_enabled && pwdinfo->listen_channel) {
  ...
  while ( p2p_ie )
  {
    ...
    if ( (pattr = rtw_get_p2p_attr_content (p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST, NULL, (uint *) &attr_contentlen) ) != NULL)
    {
      ...
      while (attr_contentlen > 0)
      {
        ...
        for (i = 0; i < num_of_ch; i++)
        { * (pattr_temp + 2 + i) = pbuddy_mlmeext->cur_channel; }
    ...
    if ( (pattr = rtw_get_p2p_attr_content (p2p_ie, p2p_ielen, P2P_ATTR_OPERATING_CH, NULL, (uint *) &attr_contentlen) ) != NULL)
    {
      * (pattr + 4) = pbuddy_mlmeext->cur_channel;
    }
  ...
  if ( (pattr = rtw_get_p2p_attr_content (p2p_ie, p2p_ielen, P2P_ATTR_CH_LIST, NULL, &attr_contentlen) ) != NULL) {
    ...
    for (j = 0; j < ch_cnt; j++) {
      if (j == 0) {
        ...
        if (ch_list[j] - ch_list[j - 1] != 1) {
          w_sz += snprintf (buf + w_sz, buf_len - w_sz, ", %u", ch_list[j]);
        }
        else
          if (j != ch_cnt - 1 && ch_list[j + 1] - ch_list[j] == 1) {
            /* empty */
          }
          else {
            w_sz += snprintf (buf + w_sz, buf_len - w_sz, "-%u", ch_list[j]);
