  ...
  if (levelNum == 2)
  {
    ...
    if (levelNum == 3)
    {
           ...
           (pCoexSta->preBtRssiState == BTC_RSSI_STATE_STAY_LOW) )
      {
        ...
        if ( (pCoexSta->preBtRssiState == BTC_RSSI_STATE_MEDIUM) ||
  ...
  if (levelNum == 2)
  {
    ...
    if (levelNum == 3)
    {
           ...
           (pCoexSta->preWifiRssiState[index] == BTC_RSSI_STATE_STAY_LOW) )
      {
        ...
        if ( (pCoexSta->preWifiRssiState[index] == BTC_RSSI_STATE_MEDIUM) ||
...
)
{
  ...
  {
    pCoexDm->curRaMask |= rateMask;  
  }
  else
    if (BTC_RATE_ENABLE == type)
    {
      pCoexDm->curRaMask &= ~rateMask; 
    }
...
)
{
  ...
  if (ssType == 1)
  {
    halbtc8192e1ant_UpdateRaMask (pBtCoexist, FORCE_EXEC, BTC_RATE_DISABLE, 0xfff00000);
    ...
    if (ssType == 2)
    {
      halbtc8192e1ant_UpdateRaMask (pBtCoexist, FORCE_EXEC, BTC_RATE_ENABLE, 0xfff00000);
  ...
  pBtCoexist->fBtcSet (pBtCoexist, BTC_SET_ACT_SEND_MIMO_PS, &mimoPs);
}
  ...
  else
  {
    ...
    else
    {
           ...
           (btRssiState == BTC_RSSI_STATE_STAY_LOW) )
      {
        ...
        if ( (btRssiState == BTC_RSSI_STATE_MEDIUM) ||
             (btRssiState == BTC_RSSI_STATE_STAY_MEDIUM) )
        {
...
)
{
  ...
  if (BT_8192E_1ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus)
  {
    ...
    if (BT_8192E_1ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus)
    {
      halbtc8192e1ant_DecBtPwr (pBtCoexist, NORMAL_EXEC, 0);
...
)
{
  ...
  if (BT_8192E_1ANT_BT_STATUS_ACL_BUSY == pCoexDm->btStatus)
  {
    ...
    if ( (BT_8192E_1ANT_BT_STATUS_SCO_BUSY == pCoexDm->btStatus) ||
...
)
{
  ...
  if (! (btInfo & BT_INFO_8192E_1ANT_B_CONNECTION) )
  {
    ...
    if (btInfo == BT_INFO_8192E_1ANT_B_CONNECTION)
    {
           ...
           (btInfo & BT_INFO_8192E_1ANT_B_SCO_BUSY) )
      {
        ...
        if ( (btInfo & BT_INFO_8192E_1ANT_B_ACL_BUSY) ||
  ...
  {
    halbtc8192e1ant_UpdateRaMask (pBtCoexist, NORMAL_EXEC, BTC_RATE_DISABLE, 0x00000003);
  }
  else
  {
    halbtc8192e1ant_UpdateRaMask (pBtCoexist, NORMAL_EXEC, BTC_RATE_ENABLE, 0x00000003);
  }
...
)
{
  ...
  if (BTC_WIFI_PNP_SLEEP == pnpState)
  {
    ...
    if (BTC_WIFI_PNP_WAKE_UP == pnpState)
    {
