...
)
{
  ...
  if ( (!bWifiConnected) && (!pCoexSta->bWiFiIsHighPriTask) )
  {
         ...
         (pBtLinkInfo->bHidExist) )
    {
      ...
      if ( (pBtLinkInfo->bA2dpExist) ||  (pBtLinkInfo->bPanExist) ||  (bWifiBusy) )
      {
  ...
  if (pBtLinkInfo->bHidOnly)
  {
    ...
    if (pBtLinkInfo->bA2dpOnly)
    {
      ...
      if (pBtLinkInfo->bHidExist && pBtLinkInfo->bA2dpExist)
      {
        ...
        if ( (pBtLinkInfo->bPanOnly) || (pBtLinkInfo->bHidExist && pBtLinkInfo->bPanExist) )
        {
...
)
{
  ...
  {
    if (!bWifiBusy && pBtCoexist->btLinkInfo.bA2dpOnly)
    { halbtc8723b1ant_PowerSaveState (pBtCoexist, BTC_PS_WIFI_NATIVE, 0x0, 0x0); }
    else
  ...
  if (! (btInfo & BT_INFO_8723B_1ANT_B_CONNECTION) )
  {
    ...
    if (btInfo == BT_INFO_8723B_1ANT_B_CONNECTION)
    {
           ...
           (btInfo & BT_INFO_8723B_1ANT_B_SCO_BUSY) )
      {
        ...
        if (btInfo & BT_INFO_8723B_1ANT_B_ACL_BUSY)
        {
          if (BT_8723B_1ANT_BT_STATUS_ACL_BUSY != pCoexDm->btStatus)
          { pCoexDm->bAutoTdmaAdjust = FALSE; }
