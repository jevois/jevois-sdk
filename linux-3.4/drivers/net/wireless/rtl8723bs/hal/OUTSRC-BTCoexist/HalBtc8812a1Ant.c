...
)
{
  ...
  if (BT_8812A_1ANT_BT_STATUS_ACL_BUSY == pCoexDm->btStatus)
  {
    ...
    if ( (BT_8812A_1ANT_BT_STATUS_SCO_BUSY == pCoexDm->btStatus) ||
...
)
{
  ...
  else
  {
    if (BT_8812A_1ANT_BT_STATUS_NON_CONNECTED_IDLE == pCoexDm->btStatus)
    {
      ...
      if (BT_8812A_1ANT_BT_STATUS_CONNECTED_IDLE == pCoexDm->btStatus)
      {
        halbtc8812a1ant_PsTdma (pBtCoexist, NORMAL_EXEC, TRUE, 5);
        halbtc8812a1ant_CoexTableWithType (pBtCoexist, NORMAL_EXEC, 2);
      }
...
)
{
  ...
  if (! (btInfo & BT_INFO_8812A_1ANT_B_CONNECTION) )
  {
    ...
    if (btInfo == BT_INFO_8812A_1ANT_B_CONNECTION)
    {
           ...
           (btInfo & BT_INFO_8812A_1ANT_B_SCO_BUSY) )
      {
        ...
        if (btInfo & BT_INFO_8812A_1ANT_B_ACL_BUSY)
        {
          if (BT_8812A_1ANT_BT_STATUS_ACL_BUSY != pCoexDm->btStatus)
          { pCoexDm->bResetTdmaAdjust = TRUE; }
  ...
  {
    bRejApAggPkt = TRUE;
    halbtc8812a1ant_UpdateRaMask (pBtCoexist, NORMAL_EXEC, BTC_RATE_DISABLE, 0x00000003);
  }
  else
  {
    halbtc8812a1ant_UpdateRaMask (pBtCoexist, NORMAL_EXEC, BTC_RATE_ENABLE, 0x00000003);
  }
