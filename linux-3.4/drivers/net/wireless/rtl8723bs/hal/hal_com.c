...
void dump_chip_info (HAL_VERSION ChipVersion)
{
  ...
  { cnt += sprintf ( (buf + cnt), "%s_", "TSMC"); }
    ...
    { cnt += sprintf ( (buf + cnt), "%s_", "UMC"); }
      ...
      if (IS_CHIP_VENDOR_SMIC (ChipVersion) )
      { cnt += sprintf ( (buf + cnt), "%s_", "SMIC"); }
      
  if (IS_A_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "A_CUT_"); }
  else
    if (IS_B_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "B_CUT_"); }
    else
      if (IS_C_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "C_CUT_"); }
      else
        if (IS_D_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "D_CUT_"); }
        else
          if (IS_E_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "E_CUT_"); }
          else
            if (IS_I_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "I_CUT_"); }
            else
              if (IS_J_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "J_CUT_"); }
              else
                if (IS_K_CUT (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "K_CUT_"); }
                else { cnt += sprintf ( (buf + cnt), "UNKNOWN_CUT(%d)_", ChipVersion.CUTVersion); }
                
  if (IS_1T1R (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "1T1R_"); }
  else
    if (IS_1T2R (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "1T2R_"); }
    else
      if (IS_2T2R (ChipVersion) ) { cnt += sprintf ( (buf + cnt), "2T2R_"); }
...
void rtw_hal_check_rxfifo_full (_adapter * adapter)
{
      ...
      || IS_8812_SERIES (pHalData->VersionID) || IS_8821_SERIES (pHalData->VersionID) )
  {
    ...
    if (IS_8723B_SERIES (pHalData->VersionID) || IS_8192E (pHalData->VersionID) )
    {
