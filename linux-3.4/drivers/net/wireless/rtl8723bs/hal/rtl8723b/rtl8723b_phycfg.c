  ...
  if (ConfigType == BaseBand_Config_PHY_REG)
  {
    for (i = 0; i < PHY_REGArrayMPLen; i = i + 2)
    {
      if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xfe) {
        ...
        if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xfd)
        { rtw_mdelay_os (5); }
        else
          if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xfc)
          { rtw_mdelay_os (1); }
          else
            if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xfb) {
              #ifdef CONFIG_LONG_DELAY_ISSUE
              rtw_msleep_os (50);
              #else
              rtw_mdelay_os (50);
              ...
              if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xfa)
              { rtw_mdelay_os (5); }
              else
                if (Rtl8192CPHY_REGArray_Table_MP[i] == 0xf9)
                { rtw_mdelay_os (1); }
...
#define   rVhtlen_Use_Lsig_Jaguar 0x8c3
VOID
...
{
  u4Byte    freq[6] = {0xFCCD, 0xFC4D, 0xFFCD, 0xFF4D, 0xFCCD, 0xFF9A};
  u1Byte    idx = 0;
  ...
  if ( (bHW_Ctrl_S1 || bSW_Ctrl_S1) && (idx <= 5) )
  {
    ...
    PHY_SetBBReg (pAdapter, rFPGA0_PSDFunction, bMaskDWord, freq[idx]);      
    PHY_SetBBReg (pAdapter, rFPGA0_PSDFunction, bMaskDWord, 0x400000 | freq[idx]);
    ...
    PHY_SetBBReg (pAdapter, rFPGA0_PSDFunction, bMaskDWord, freq[idx]);
    PHY_SetBBReg (pAdapter, rFPGA0_AnalogParameter4, bMaskDWord, 0xccc000c0);
...
)
{
  ...
  if (bSetBandWidth)
  {
    ...
    if (bInitialzed == _FALSE)
    {
      ...
      if ( (pHalData->CurrentChannelBW != ChnlWidth) || (pHalData->nCur40MhzPrimeSC != ExtChnlOffsetOf40MHz) || (pHalData->CurrentCenterFrequencyIndex1 != CenterFrequencyIndex1) )
      {
        pHalData->bSetChnlBW = _TRUE;
      }
    #else
  ...
  if (pHalData->bSetChnlBW)
  {
    ...
    { pHalData->nCur40MhzPrimeSC = HAL_PRIME_CHNL_OFFSET_UPPER; }
      ...
      if (ExtChnlOffsetOf40MHz == EXTCHNL_OFFSET_UPPER)
    ...
    { pHalData->nCur80MhzPrimeSC = HAL_PRIME_CHNL_OFFSET_UPPER; }
      ...
      if (ExtChnlOffsetOf80MHz == EXTCHNL_OFFSET_UPPER)
