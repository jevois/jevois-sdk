...
static void _InitTxBufferBoundary (PADAPTER padapter)
{
  ...
  rtw_write8 (padapter, REG_DWBCN1_CTRL_8723B + 1, val8);
...
static u32 rtl8723bs_hal_init (PADAPTER padapter)
{
  ...
  */
  ...
  #else
  u4Tmp = rtw_read32 (padapter, REG_BT_WIFI_ANTENNA_SWITCH_8723B);
  u4Tmp |= BIT11;
...
)
{
  ...
  {
    _rtw_memcpy (pEEPROM->efuse_eeprom_data, hwinfo, HWSET_MAX_SIZE_8723B);
  }
  else
  {
