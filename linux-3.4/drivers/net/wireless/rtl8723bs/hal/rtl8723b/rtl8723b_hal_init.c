...
#define EFUSE_ACCESS_OFF_8723     0x00 
...
)
{
  ...
  /* 
    if( IS_B_CUT(ChipVersion) || IS_C_CUT(ChipVersion))
    {
      ...
      PHY_SetMacReg(padapter, 0x14, BIT23|BIT22|BIT21|BIT20, 0x5);
...
void _ResetDigitalProcedure1_92C (PADAPTER padapter, BOOLEAN bWithoutHWSM)
{
  ...
  if (IS_FW_81xxC (padapter) && (pHalData->FirmwareVersion <= 0x20) )
  {
    ...
    ******************************/
    ...
    rtw_write16 (padapter, REG_SYS_FUNC_EN, (valu16 | (FEN_HWPDN | FEN_ELDR) ) );
    ...
    
    {
      if (rtw_read8 (padapter, REG_MCUFWDL) & BIT1)
      {
        ...
        if (padapter->bFWReady)
        {
          ...
          if (retry_cnts >= 100)
          {
            rtw_write8 (padapter, REG_SYS_FUNC_EN + 1, 0x50);
            rtw_mdelay_os (10);
          }
        }
      }
...
)
{
  ...
  if (Channel <= 14)
  {
    ...
    if      (1  <= Channel && Channel <= 2 )   { *pGroup = 0; }
      ...
      if (3  <= Channel && Channel <= 5 )   { *pGroup = 1; }
      else
        if (6  <= Channel && Channel <= 8 )   { *pGroup = 2; }
        else
          if (9  <= Channel && Channel <= 11)   { *pGroup = 3; }
          else
            if (12 <= Channel && Channel <= 14)   { *pGroup = 4; }
            else
            {
  ...
  else
  {
    ...
    if      (36   <= Channel && Channel <=  42)   { *pGroup = 0; }
      ...
      if (44   <= Channel && Channel <=  48)   { *pGroup = 1; }
      else
        if (50   <= Channel && Channel <=  58)   { *pGroup = 2; }
        else
          if (60   <= Channel && Channel <=  64)   { *pGroup = 3; }
          else
            if (100  <= Channel && Channel <= 106)   { *pGroup = 4; }
            else
              if (108  <= Channel && Channel <= 114)   { *pGroup = 5; }
              else
                if (116  <= Channel && Channel <= 122)   { *pGroup = 6; }
                else
                  if (124  <= Channel && Channel <= 130)   { *pGroup = 7; }
                  else
                    if (132  <= Channel && Channel <= 138)   { *pGroup = 8; }
                    else
                      if (140  <= Channel && Channel <= 144)   { *pGroup = 9; }
                      else
                        if (149  <= Channel && Channel <= 155)   { *pGroup = 10; }
                        else
                          if (157  <= Channel && Channel <= 161)   { *pGroup = 11; }
                          else
                            if (165  <= Channel && Channel <= 171)   { *pGroup = 12; }
                            else
                              if (173  <= Channel && Channel <= 177)   { *pGroup = 13; }
                              else
                              {
  ...
  if (pHalData->CurrentChannelBW == CHANNEL_WIDTH_80)
  {
    if (pattrib->bwmode == CHANNEL_WIDTH_80)
    {
      ...
      if (pattrib->bwmode == CHANNEL_WIDTH_40)
      {
        ...
        { SCSettingOfDesc = VHT_DATA_SC_40_LOWER_OF_80MHZ; }
          ...
          if (pHalData->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER)
      ...
      else
      {
        ...
        { SCSettingOfDesc = VHT_DATA_SC_20_LOWEST_OF_80MHZ; }
          ...
          if ( (pHalData->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER) && (pHalData->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER) )
          { SCSettingOfDesc = VHT_DATA_SC_20_LOWER_OF_80MHZ; }
          else
            if ( (pHalData->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_LOWER) && (pHalData->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER) )
            { SCSettingOfDesc = VHT_DATA_SC_20_UPPER_OF_80MHZ; }
            else
              if ( (pHalData->nCur40MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER) && (pHalData->nCur80MhzPrimeSC == HAL_PRIME_CHNL_OFFSET_UPPER) )
    ...
    if (pHalData->CurrentChannelBW == CHANNEL_WIDTH_40)
    {
      ...
      if (pattrib->bwmode == CHANNEL_WIDTH_40)
      {
        ...
        if (pattrib->bwmode == CHANNEL_WIDTH_20)
        {
  ...
  if (pxmitframe->frame_tag == DATA_FRAMETAG)
  {
    ...
    if (pxmitframe->frame_tag == MGNT_FRAMETAG)
    {
      ...
      if (pxmitframe->frame_tag == TXAGG_FRAMETAG)
      {
        ...
        if (pxmitframe->frame_tag == MP_FRAMETAG)
        {
    ...
    if ( (mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_) )
    {
      ...
      if ( (mode == _HW_STATE_ADHOC_) /*|| (mode == _HW_STATE_AP_)*/)
      {
        ...
        if (mode == _HW_STATE_AP_)
        {
          ...
          rtw_write8 (padapter, REG_ATIMWND_1, 0x0a);
...
void SetHwReg8723B (PADAPTER padapter, u8 variable, u8 * val)
{
  ...
  case HW_VAR_SEC_CFG:
    #ifdef CONFIG_CONCURRENT_MODE
    rtw_write8 (padapter, REG_SECCFG, 0x0c | BIT (5) );
    #else
