...
)
{
  ...
  #if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
  {
    if (CurrSig >= 54 && CurrSig <= 100)
    {
      ...
      if (CurrSig >= 42 && CurrSig <= 53 )
      {
        RetSig = 95;
      }
      else
        if (CurrSig >= 36 && CurrSig <= 41 )
        {
          RetSig = 74 + ( (CurrSig - 36) * 20) / 6;
        }
        else
          if (CurrSig >= 33 && CurrSig <= 35 )
          {
            RetSig = 65 + ( (CurrSig - 33) * 8) / 2;
          }
          else
            if (CurrSig >= 18 && CurrSig <= 32 )
            {
              RetSig = 62 + ( (CurrSig - 18) * 2) / 15;
            }
            else
              if (CurrSig >= 15 && CurrSig <= 17 )
              {
                RetSig = 33 + ( (CurrSig - 15) * 28) / 2;
              }
              else
                if (CurrSig >= 10 && CurrSig <= 14 )
                {
                  RetSig = 39;
                }
                else
                  if (CurrSig >= 8 && CurrSig <= 9 )
                  {
                    RetSig = 33;
                  }
                  else
...
)
{
  ...
  #if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
  {
    if (CurrSig >= 31 && CurrSig <= 100)
    {
      ...
      if (CurrSig >= 21 && CurrSig <= 30)
      {
        RetSig = 90 + ( (CurrSig - 20) / 1);
      }
      else
        if (CurrSig >= 11 && CurrSig <= 20)
        {
          RetSig = 80 + ( (CurrSig - 10) / 1);
        }
        else
          if (CurrSig >= 7 && CurrSig <= 10)
          {
            RetSig = 69 + (CurrSig - 7);
          }
          else
...
)
{
  ...
  if (pDM_Odm->SupportInterface  == ODM_ITRF_PCIE)
  {
    if (CurrSig >= 61 && CurrSig <= 100)
    {
      ...
      if (CurrSig >= 41 && CurrSig <= 60)
      {
        RetSig = 78 + ( (CurrSig - 40) / 2);
      }
      else
        if (CurrSig >= 31 && CurrSig <= 40)
        {
      ...
      if (CurrSig >= 41 && CurrSig <= 50)
      {
        ...
        if (CurrSig >= 31 && CurrSig <= 40)
        {
          ...
          if (CurrSig >= 21 && CurrSig <= 30)
          {
            ...
            if (CurrSig >= 10 && CurrSig <= 20)
            {
              RetSig = 42 + ( ( (CurrSig - 10) * 2) / 3);
            }
            else
              if (CurrSig >= 5 && CurrSig <= 9)
              {
                RetSig = 22 + ( ( (CurrSig - 5) * 3) / 2);
              }
              else
                if (CurrSig >= 1 && CurrSig <= 4)
                {
  ...
  if (isCCKrate) {
    ...
    if (IS_HARDWARE_TYPE_8723AE (pDM_Odm->Adapter) )
    {
      ...
      { SQ = 100; }
        ...
        if (PWDB_ALL >= 23 && PWDB_ALL < 50)
        { SQ = 80; }
        else
          if (PWDB_ALL >= 18 && PWDB_ALL < 23)
          { SQ = 60; }
          else
            if (PWDB_ALL >= 8 && PWDB_ALL < 18)
            { SQ = 40; }
            else
            { SQ = 10; }
      #else
      ...
      if (IS_HARDWARE_TYPE_8192E (pDM_Odm->Adapter) ) {
        ...
        { SQ = 100; }
          ...
          if (PWDB_ALL >= 35 && PWDB_ALL < 50)
          { SQ = 80; }
          else
            if (PWDB_ALL >= 31 && PWDB_ALL < 35)
            { SQ = 60; }
            else
              if (PWDB_ALL >= 22 && PWDB_ALL < 31)
              { SQ = 40; }
              else
                if (PWDB_ALL >= 18 && PWDB_ALL < 22)
                { SQ = 20; }
                else
                { SQ = 10; }
      }
  ...
  if (isCCKrate) {
    ...
    { SQ = 100; }
      ...
      { SQ = 80; }
        ...
        if (PWDB_ALL >= 30 && PWDB_ALL < 35)
        { SQ = 60; }
        else
          if (PWDB_ALL >= 25 && PWDB_ALL < 30)
          { SQ = 40; }
          else
            if (PWDB_ALL >= 20 && PWDB_ALL < 25)
            { SQ = 20; }
            else
            { SQ = 10; }
    #else
...
)
{
  ...
  else
  {
    ...
    if ( (pDM_Odm->SupportPlatform == ODM_WIN) && (pDM_Odm->PatchID == 19) ) {
      ...
      if ( (pDM_Odm->SupportPlatform == ODM_WIN) && (pDM_Odm->PatchID == 25) ) {
      }
      else {
  ...
  {
    #if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
    pPhyInfo->SignalStrength = (u1Byte) (SignalScaleMapping (pDM_Odm->Adapter, PWDB_ALL) );
    #else
    pPhyInfo->SignalStrength = (u1Byte) (odm_SignalScaleMapping (pDM_Odm, PWDB_ALL) );
    #endif
  }
  else
  {
    ...
    {
      #if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
      pPhyInfo->SignalStrength = (u1Byte) (SignalScaleMapping (pDM_Odm->Adapter, total_rssi /= rf_rx_num) );
      #else
  ...
  if (pPktinfo->DataRate <= DESC_RATE54M)
  {
    ...
    case 2:
      ...
      { pPhyInfo->BandWidth = 2; }
        ...
        if (pPhyStaRpt->sub_chnl == 9 || pPhyStaRpt->sub_chnl == 10)
        { pPhyInfo->BandWidth = 1; }
        else
        { pPhyInfo->BandWidth = 0; }
      break;
  ...
  else
  {
    ...
    if ( (pDM_Odm->SupportPlatform == ODM_WIN) && (pDM_Odm->PatchID == 19) ) {
      ...
      { Max_spatial_stream = 2; }
        ...
        if (  (pPktinfo->DataRate >= DESC_RATEVHTSS2MCS0) &&
...
)
{
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8723A)
  {
    if (ConfigType == CONFIG_BB_PHY_REG)
    {
      ...
      if (ConfigType == CONFIG_BB_AGC_TAB)
      {
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8188E)
  {
    if (ConfigType == CONFIG_BB_PHY_REG)
    {
      ...
      if (ConfigType == CONFIG_BB_AGC_TAB)
      {
        ...
        if (ConfigType == CONFIG_BB_PHY_REG_PG)
        {
      ...
      if (ConfigType == CONFIG_BB_AGC_TAB)
      {
        ...
        if (ConfigType == CONFIG_BB_PHY_REG_PG)
        {
          ...
          { READ_AND_CONFIG_MP (8812A, _PHY_REG_PG_ASUS); }
            ...
            if (pMgntInfo->CustomerID == RT_CID_WNC_NEC && pDM_Odm->bIsMPChip)
...
)
{
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8188E)
  {
    if (ConfigType == CONFIG_FW_NIC)
    {
      ...
      if (ConfigType == CONFIG_FW_WoWLAN)
      {
        READ_FIRMWARE (8188E, _FW_WoWLAN_T);
      }
      else
        if (ConfigType == CONFIG_FW_NIC_2)
        {
          READ_FIRMWARE (8188E, _FW_NIC_S);
        }
        else
          if (ConfigType == CONFIG_FW_WoWLAN_2)
          {
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8723B)
  {
    if (ConfigType == CONFIG_FW_NIC)
    {
      ...
      if (ConfigType == CONFIG_FW_WoWLAN)
      {
        ...
        if (ConfigType == CONFIG_FW_AP_WoWLAN)
        {
          READ_FIRMWARE (8723B, _FW_AP_WoWLAN);
        }
    #endif
        else
          if (ConfigType == CONFIG_FW_BT)
          {
            READ_FIRMWARE_MP (8723B, _FW_BT);
          }
          else
            if (ConfigType == CONFIG_FW_MP)
            {
