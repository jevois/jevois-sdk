...
)
{
  ...
  if (DM_Type == DIG_TYPE_THRESH_HIGH)
  {
    ...
    if (DM_Type == DIG_TYPE_THRESH_LOW)
    {
      pDM_DigTable->RssiLowThresh = DM_Value;
    }
    else
      if (DM_Type == DIG_TYPE_ENABLE)
      {
        pDM_DigTable->Dig_Enable_Flag = TRUE;
      }
      else
        if (DM_Type == DIG_TYPE_DISABLE)
        {
          pDM_DigTable->Dig_Enable_Flag = FALSE;
        }
        else
          if (DM_Type == DIG_TYPE_BACKOFF)
          {
            if (DM_Value > 30)
            { DM_Value = 30; }
            ...
            if (DM_Type == DIG_TYPE_RX_GAIN_MIN)
            {
              if (DM_Value == 0)
              { DM_Value = 0x1; }
              ...
              if (DM_Type == DIG_TYPE_RX_GAIN_MAX)
              {
                if (DM_Value > 0x50)
                { DM_Value = 0x50; }
...
)
{
       ...
       (pDM_Odm->SupportICType == ODM_RTL8188E && RegC50 > 0x28) )
  {
    ...
    if ( (RegC50 < 0x25 && RegC58 < 0x25) || (pDM_Odm->SupportICType == ODM_RTL8188E && RegC50 < 0x25) )
    {
      if (pHalData->bPreEdccaEnable)
      {
...
)
{
  ...
  if (!pDM_Odm->ForceEDCCA)
  {
    ...
    { EDCCA_State = 1; }
      ...
      if (pDM_Odm->RSSI_Min < (pDM_Odm->AdapEn_RSSI - 5) )
      { EDCCA_State = 0; }
  }
  else
  { EDCCA_State = 1; }
  ...
  else
  {
    ...
    if (pDM_Odm->bBtDisabled)
    {
      ...
      { Adjust_Xtal =  ( (CFO_ave - CFO_Threshold_Xtal) >> 2) + 1; }
        ...
        if (CFO_ave < (-pDM_Odm->CFOThreshold) && pDM_Odm->CrystalCap > 0)
        { Adjust_Xtal =  ( (CFO_ave + CFO_Threshold_Xtal) >> 2) - 1; }
...
)
{
  ...
  { CurrentIGI = CurrentIGI + 2; }
    ...
    if (pFalseAlmCnt->Cnt_all > DM_DIG_FA_TH1_LPS)
    { CurrentIGI = CurrentIGI + 1; }
    else
      if (pFalseAlmCnt->Cnt_all < DM_DIG_FA_TH0_LPS)
      { CurrentIGI = CurrentIGI - 1; }
...
)
{
  ...
  */
    ...
    if (pDM_Odm->SupportPlatform & (ODM_AP | ODM_ADSL) )
    {
      ...
      { dm_dig_max = 0x24; }
        ...
        if (priv->pmib->dot11RFEntry.tx2path) {
          if (* (pDM_Odm->pWirelessMode) == ODM_WM_B)
          { dm_dig_max = 0x2A; }
          else
          { dm_dig_max = 0x32; }
        }
    ...
    if (pDM_Odm->SupportICType & (ODM_RTL8723A/*|ODM_RTL8821*/) )
    {
      ...
      else
      {
        ...
        { pDM_DigTable->rx_gain_range_max = dm_dig_max; }
          ...
          if ( (pDM_Odm->RSSI_Min + 20) < dm_dig_min )
    ...
    else
    {
      if ( (pDM_Odm->SupportICType & (ODM_RTL8192E | ODM_RTL8723B | ODM_RTL8812 | ODM_RTL8821) ) && (pDM_Odm->bBtLimitedDig == 1) ) {
        ...
        { pDM_DigTable->rx_gain_range_max = dm_dig_max; }
          ...
          if ( (pDM_Odm->RSSI_Min + 10) < dm_dig_min )
          { pDM_DigTable->rx_gain_range_max = dm_dig_min; }
          else
          { pDM_DigTable->rx_gain_range_max = pDM_Odm->RSSI_Min + 10; }
      }
      ...
      */
      if (pDM_Odm->bOneEntryOnly)
      {
        ...
        { DIG_Dynamic_MIN = dm_dig_min; }
          ...
          if (pDM_Odm->RSSI_Min - offset > DIG_MaxOfMin)
  ...
  if (pFalseAlmCnt->Cnt_all > 10000)
  {
    ...
    {
      pDM_DigTable->ForbiddenIGI = (u1Byte) CurrentIGI;
    ...
    else
    {
      if (pDM_Odm->SupportICType == ODM_RTL8192D)
      {
        ...
        { CurrentIGI = CurrentIGI + 4; }//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+2;
          ...
          if (pFalseAlmCnt->Cnt_all > DM_DIG_FA_TH1_92D)
          { CurrentIGI = CurrentIGI + 2; }
          else
            if (pFalseAlmCnt->Cnt_all < DM_DIG_FA_TH0_92D)
        ...
        if (pDM_Odm->SupportICType == ODM_RTL8723A)
        {
          if (pDM_Odm->bBtLimitedDig)
          {
            ...
            { CurrentIGI = CurrentIGI + 4; }
              ...
              if (pFalseAlmCnt->Cnt_all > 0x250)
              { CurrentIGI = CurrentIGI + 2; }
              else
                if (pFalseAlmCnt->Cnt_all < DM_DIG_FA_TH0)
                { CurrentIGI = CurrentIGI - 2; }
          }
          else
          {
            ...
            { CurrentIGI = CurrentIGI + 4; }//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+2;
              ...
              if (pFalseAlmCnt->Cnt_all > DM_DIG_FA_TH1)
              { CurrentIGI = CurrentIGI + 2; }//pDM_DigTable->CurIGValue = pDM_DigTable->PreIGValue+1;
              else
                if (pFalseAlmCnt->Cnt_all < DM_DIG_FA_TH0)
                { CurrentIGI = CurrentIGI - 2; }//pDM_DigTable->CurIGValue =pDM_DigTable->PreIGValue-1;
  ...
  else
  {
    ...
    else
    {
      ...
      { CurrentIGI = CurrentIGI + 4; }
        ...
        if (pFalseAlmCnt->Cnt_all > 8000)
        { CurrentIGI = CurrentIGI + 2; }
        else
          if (pFalseAlmCnt->Cnt_all < 500)
          { CurrentIGI = CurrentIGI - 2; }
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8192D)
  {
    if (* (pDM_Odm->pMacPhyMode) == ODM_DMSP)
    {
      ODM_Write_DIG_DMSP (pDM_Odm, (u1Byte) CurrentIGI);
      if (* (pDM_Odm->pbMasterOfDMSP) )
      {
...
)
{
  ...
  if (pDM_Odm->bLinked)
  {
    ...
    { CurCCK_CCAThres = 0xcd; }
      ...
      if ( (pDM_Odm->RSSI_Min <= 25) && (pDM_Odm->RSSI_Min > 10) )
  ...
  IN  u1Byte    rssi_level)
{
  ...
  case (ODM_WM_B|ODM_WM_G) :
    ...
    { rate_bitmap = 0x00000f00; }
      ...
      if (rssi_level == DM_RATR_STA_MIDDLE)
      { rate_bitmap = 0x00000ff0; }
      else
...
)
{
  ...
  if (pMgntInfo->mAssoc && (!ACTING_AS_AP (pAdapter) ) )
  {
    ...
    if (pHalData->UndecoratedSmoothedPWDB < pRA->LdpcThres)
    {
      ...
      if (pHalData->UndecoratedSmoothedPWDB > (pRA->LdpcThres - 5) )
      {
        pRA->bUseLdpc = FALSE;
    ...
    if (IS_STA_VALID (pstat) ) {
      ...
      if ( (pDM_Odm->SupportICType == ODM_RTL8812) || (pDM_Odm->SupportICType == ODM_RTL8821) )
      {
        if (pstat->rssi_stat.UndecoratedSmoothedPWDB < pRA->LdpcThres)
        {
          ...
          if (pstat->rssi_stat.UndecoratedSmoothedPWDB > (pRA->LdpcThres - 5) )
          {
            pRA->bUseLdpc = FALSE;
            pRA->bLowerRtsRate = FALSE;
            if ( (pDM_Odm->SupportICType == ODM_RTL8821) && (pDM_Odm->CutVersion == ODM_CUT_A) )
    ...
    for (i = 0; i < ODM_ASSOCIATE_ENTRY_NUM; i++) {
      ...
      if (IS_STA_VALID (pstat) ) {
        ...
        { pstat->hp_level = 1; }
          ...
          if ( (pstat->hp_level == 1) && (pstat->rssi < TX_POWER_NEAR_FIELD_THRESH_AP) )
          { pstat->hp_level = 0; }
      }
    }
  }
  ...
  #endif
  {
    ...
    else
    {
      ...
      if (UndecoratedSmoothedPWDB >= TX_POWER_NEAR_FIELD_THRESH_LVL2)
      {
        ...
        if ( (UndecoratedSmoothedPWDB < (TX_POWER_NEAR_FIELD_THRESH_LVL2 - 3) ) &&
             (UndecoratedSmoothedPWDB >= TX_POWER_NEAR_FIELD_THRESH_LVL1) )
        {
          ...
          if (UndecoratedSmoothedPWDB < (TX_POWER_NEAR_FIELD_THRESH_LVL1 - 5) )
          {
            pHalData->DynamicTxHighPowerLvl = TxHighPwrLevel_Normal;
  ...
  if ( pHalData->DynamicTxHighPowerLvl != pHalData->LastDTPLvl )
  {
    ...
    { odm_DynamicTxPowerRestorePowerIndex (pDM_Odm); }
      ...
      { odm_DynamicTxPowerWritePowerIndex (pDM_Odm, 0x14); }
        ...
        if (pHalData->DynamicTxHighPowerLvl == TxHighPwrLevel_Level2)
  ...
  if ( (pdmpriv->DynamicTxHighPowerLvl != pdmpriv->LastDTPLvl) )
  {
    ...
    { odm_DynamicTxPowerRestorePowerIndex (pDM_Odm); }
      ...
      { odm_DynamicTxPowerWritePowerIndex (pDM_Odm, 0x14); }
        ...
        if (pdmpriv->DynamicTxHighPowerLvl == TxHighPwrLevel_Level2)
        { odm_DynamicTxPowerWritePowerIndex (pDM_Odm, 0x10); }
  }
  ...
  if (IS_HARDWARE_TYPE_8192D (Adapter) && GET_HAL_DATA (Adapter)->CurrentBandType == 1) {
    if (UndecoratedSmoothedPWDB >= 0x33)
    {
           ...
           (UndecoratedSmoothedPWDB >= 0x2b) )
      {
        ...
        if (UndecoratedSmoothedPWDB < 0x2b)
        {
          pHalData->DynamicTxHighPowerLvl = TxHighPwrLevel_Normal;
    ...
    if (pDM_SWAT_Table->try_flag == 1)
    {
      ...
      if (pDM_SWAT_Table->RSSI_Trying == 0)
      {
        ...
        { PreByteCnt = PreByteCnt * 9; }
          ...
          if (pDM_SWAT_Table->TrafficLoad == TRAFFIC_LOW)
...
)
{
  ...
  if (pDM_SWAT_Table->SWAS_NoLink_State == 0)
  {
    ...
    if (pDM_Odm->SupportICType & (ODM_RTL8188E | ODM_RTL8821) )
    {
      ...
      if (pDM_Odm->SupportICType & (ODM_RTL8192C | ODM_RTL8723B) )
      {
        pDM_SWAT_Table->PreAntenna = pDM_SWAT_Table->CurAntenna;
        ...
        if (pTmpBssDesc->ChannelNumber == ScanChannel)
        {
          if (pTmpBssDesc->RecvSignalPower > pTestBssDesc->RecvSignalPower)
          {
            ...
            if (pTestBssDesc->RecvSignalPower > pTmpBssDesc->RecvSignalPower)
            {
              ...
              if (pTestBssDesc->bdTstamp > pTmpBssDesc->bdTstamp)
              {
...
)
{
  ...
  if (OFDM_Ant1_Cnt || OFDM_Ant2_Cnt ) {
    ...
    if ( (CCK_Ant1_Cnt + CCK_Ant2_Cnt) >= 10 )
    {
      if (CCK_Ant1_Cnt > (5 * CCK_Ant2_Cnt) )
      { (*pDefAnt) = 1; }
      else
        if (CCK_Ant2_Cnt > (5 * CCK_Ant1_Cnt) )
        { (*pDefAnt) = 0; }
        else
          if (CCK_Ant1_Cnt > CCK_Ant2_Cnt)
          { (*pDefAnt) = 0; }
          else
          { (*pDefAnt) = 1; }
    ...
    
    {
      ...
      if ( (ICType != ODM_RTL8192D) && (IOTPeer == HT_IOT_PEER_CISCO) && (WirelessMode == ODM_WM_N24G) )
      {
        ...
        if ( (IOTPeer == HT_IOT_PEER_CISCO) && ( (WirelessMode == ODM_WM_G) || (WirelessMode == (ODM_WM_B | ODM_WM_G) ) || (WirelessMode == ODM_WM_A) || (WirelessMode == ODM_WM_B) ) )
        {
          ...
          if ( (IOTPeer == HT_IOT_PEER_AIRGO ) && ( (WirelessMode == ODM_WM_G) || (WirelessMode == ODM_WM_A) ) )
          {
            EDCA_BE_DL = 0xa630;
          }
          else
            if (IOTPeer == HT_IOT_PEER_MARVELL)
            {
              EDCA_BE_DL = edca_setting_DL[IOTPeer];
              EDCA_BE_UL = edca_setting_UL[IOTPeer];
            }
            else
              if (IOTPeer == HT_IOT_PEER_ATHEROS)
              {
                EDCA_BE_DL = edca_setting_DL[IOTPeer];
              }
      ...
      else
      {
           ...
           )
        {
          ...
          if (priv->pshare->txop_enlarge == 0xe) {
            ...
            if (priv->pshare->txop_enlarge == 0xd) {
...
)
{
  ...
  if (QOS_ENABLE) {
    ...
    #endif
    {
      ...
      if (!pstat || pstat->rssi < priv->pshare->rf_ft_var.txop_enlarge_lower) {
  ...
  if ( (priv->assoc_num > 1) && (AMPDU_ENABLE) )
  {
    if (priv->swq_txmac_chg >= priv->pshare->rf_ft_var.swq_en_highthd) {
      ...
      if (priv->swq_txmac_chg <= priv->pshare->rf_ft_var.swq_dis_lowthd) {
        priv->swq_en = 0;
      }
      else
        if ( (priv->swq_en == 1) && (switch_turbo > 0) && (priv->pshare->txop_enlarge == 0) && (priv->pshare->iot_mode_enable != 0) )        {
          priv->pshare->txop_enlarge = 2;
          switch_turbo--;
        }
  }
    ...
    if ( (priv->assoc_num == 1) && (AMPDU_ENABLE) ) {
      if (pstat) {
        ...
        if ( (priv->swq_en == 0) && (pstat->current_tx_bytes > en_thd) && (pstat->current_rx_bytes > en_thd) )  {
          ...
          if ( (priv->swq_en == 1) && ( (pstat->tx_avarage < 4587520) || (pstat->rx_avarage < 4587520) ) ) {
            priv->swq_en = 0;
  ...
  if ( (!priv->pmib->dot11OperationEntry.wifi_specific || (priv->pmib->dot11OperationEntry.wifi_specific == 2) )
    ...
    if (switch_turbo || priv->pshare->rf_ft_var.low_tp_txop) {
      ...
      if (switch_turbo)
      {
        ...
        if (priv->pshare->iot_mode_enable && (priv->pshare->rf_ft_var.low_tp_txop_close != under_thd) ) {
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8723A)
  {
    ...
    { TH1 = 113; }
      ...
      { TH1 = 105; }                             
        ...
        if ( (RSSI_BT >= 33) & (RSSI_BT < 38) )
        { TH1 = 99 + (RSSI_BT - 33); }    
        else
          if ( (RSSI_BT >= 26) & (RSSI_BT < 33) )
          { TH1 = 99 - (33 - RSSI_BT) + 2; }
          else
            if ( (RSSI_BT >= 24) & (RSSI_BT < 26) )
            { TH1 = 88 - ( (RSSI_BT - 24) * 3); }
            else
              if ( (RSSI_BT >= 18) & (RSSI_BT < 24) )
              { TH1 = 77 + ( (RSSI_BT - 18) * 2); }
              else
                if ( (RSSI_BT >= 14) & (RSSI_BT < 18) )
                { TH1 = 63 + ( (RSSI_BT - 14) * 2); }
                else
                  if ( (RSSI_BT >= 8) & (RSSI_BT < 14) )
                  { TH1 = 58 + ( (RSSI_BT - 8) * 2); }
                  else
                    if ( (RSSI_BT >= 3) & (RSSI_BT < 8) )
                    { TH1 = 52 + (RSSI_BT - 3); }
                    else
                    { TH1 = 51; }
  }
  ...
  */
  ...
  if (pDM_Odm->SupportICType == ODM_RTL8723A)
  {
    ...
    { initial_gain_psd = RSSI_BT * 2; }
      ...
      if ( (RSSI_BT >= 33) & (RSSI_BT < 35) )
      { initial_gain_psd = RSSI_BT * 2 + 6; }
      else
        if ( (RSSI_BT >= 24) & (RSSI_BT < 33) )
        { initial_gain_psd = 70 - (33 - RSSI_BT); }
        else
          if ( (RSSI_BT >= 19) & (RSSI_BT < 24) )
          { initial_gain_psd = 64 - ( (24 - RSSI_BT) * 4); }
          else
            if ( (RSSI_BT >= 14) & (RSSI_BT < 19) )
...
)
{
  ...
  if (pHalData->RFEType == 3) {
    ...
    if (pDM_Odm->RSSI_TRSW >= pDM_Odm->RSSI_TRSW_H)
    {
      ...
      if (pDM_Odm->RSSI_TRSW <= pDM_Odm->RSSI_TRSW_L)
      {
        TRSW_HighPwr = 0;
        PHY_SetBBReg (Adapter, r_ANTSEL_SW_Jaguar, BIT1 | BIT0, 0x1);
  ...
  if (1) {
    ...
    else
    {
      if (pFalseAlmCnt->Cnt_all > 1000) {
        ...
        if (pFalseAlmCnt->Cnt_all > 200) {
          CurrentIGI = CurrentIGI + 4;
        }
        else
          if (pFalseAlmCnt->Cnt_all > 50) {
            CurrentIGI = CurrentIGI + 2;
          }
          else
            if (pFalseAlmCnt->Cnt_all < 2) {
              CurrentIGI = CurrentIGI - 2;
            }
  ...
  IN    PDM_ODM_T   pDM_Odm)
{
  ...
  { pRX_HP_Table->TP_Mode = Idle_Mode; }
    ...
    if ( (1000 < TP_Acc5) && (TP_Acc5 < 3750000) )
      ...
      if (pRX_HP_Table->TP_Mode == High_TP_Mode)
      {
        ...
        if (pre_state_flag == 1)
        {
          if ( ( (TP_Acc3 - curTPOkCnt) * 5) > (TP_Acc3) )
          {
            ...
            if ( ( (TP_Buff[2] - curTPOkCnt) * 5) > TP_Buff[2])
            {
              odm_PSD_RXHP (pDM_Odm);
              pRX_HP_Table->PSD_func_trigger = 1;
              TP_Degrade_flag = 1;
              ...
              if ( ( (TP_Buff[3] - curTPOkCnt) * 5) > TP_Buff[3])
              {
                odm_PSD_RXHP (pDM_Odm);
                pRX_HP_Table->PSD_func_trigger = 1;
                TP_Degrade_flag = 1;
  ...
  if (pDM_PDTable->TrainingState == 0)
  {
    ...
    if ( (pDM_PDTable->CCKPathDivEnable == TRUE) && (pDM_PDTable->OFDM_Pkt_Cnt < 100) )
    {
      ...
      { pDM_PDTable->Timer = 20; }
        ...
        if (pDM_PDTable->CCK_Pkt_Cnt > 100)
...
)
{
  ...
  { bCount = TRUE; }
    ...
    if (pHalData->RSSI_target != NULL && pEntry != NULL && pHalData->RSSI_target == pEntry)
    { bCount = TRUE; }
  ...
  if (bCount && isCCKrate)
  {
    if (pDM_PDTable->TrainingState == 1 )
    {
      ...
      if (pDM_PDTable->TrainingState == 2 )
      {
        if (pEntry)
        {
        ...
        if (mode == ANTTESTB)
        {
          if (AntA_report >= 100 && AntA_report <= 116)
          {
            if (AntB_report >= (AntA_report + 4) && AntB_report > 116)
            {
              ...
              if (AntB_report >= 100 && AntB_report <= 116)
              {
                pDM_SWAT_Table->ANTB_ON = TRUE;
                ODM_RT_TRACE (pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("ODM_SingleDualAntennaDetection(): Dual Antenna is A and B\n") );
              }
          ...
          if (mode == ANTTESTALL)
          {
            if ( (AntA_report >= 100) && (AntB_report >= 100) && (AntA_report <= 120) && (AntB_report <= 120) )
            {
              if ( (AntA_report - AntB_report < 2) || (AntB_report - AntA_report < 2) )
              {
                ...
                if ( ( (AntA_report - AntB_report >= 2) && (AntA_report - AntB_report <= 4) ) ||
...
#define DTC_DWN_BASE       (DTC_BASE-5) /* RSSI lower than this value, start to increase TX power */
  ...
  if (DTC_BASE < pDM_Odm->RSSI_Min) {
    ...
    if (DTC_DWN_BASE > pDM_Odm->RSSI_Min)
    {
