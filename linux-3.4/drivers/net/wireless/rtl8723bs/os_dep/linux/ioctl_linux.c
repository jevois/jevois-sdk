                              ...
                              char * start, char * stop)
{
  ...
  if ( SCAN_RESULT_ALL == pwdinfo->wfd_info->scan_result_type )
  {
    ...
    if ( ( SCAN_RESULT_P2P_ONLY == pwdinfo->wfd_info->scan_result_type ) ||
    ...
    if ( rtw_get_wfd_ie_from_scan_queue ( &pnetwork->network.IEs[0], pnetwork->network.IELength,  wfd_ie, &wfd_ielen, pnetwork->network.Reserved[0]) )
    {
      ...
      if ( rtw_get_wfd_attr_content ( wfd_ie, wfd_ielen, WFD_ATTR_DEVICE_INFO, wfd_devinfo, &wfd_devlen) )
      {
        if ( pwdinfo->wfd_info->wfd_device_type == WFD_DEVINFO_PSINK )
        {
          ...
          if ( pwdinfo->wfd_info->wfd_device_type == WFD_DEVINFO_SOURCE )
          {
  ...
  if ( (rtw_is_cckratesonly_included ( (u8 *) &pnetwork->network.SupportedRates) ) == _TRUE)
  {
    ...
    if ( (rtw_is_cckrates_included ( (u8 *) &pnetwork->network.SupportedRates) ) == _TRUE)
    {
      if (ht_cap == _TRUE)
  ...
  
  {
    ...
    #ifdef CONFIG_SIGNAL_DISPLAY_DBM
    iwe.u.qual.level = (u8) translate_percentage_to_dbm (ss);
    #else
      ...
      else
      {
        ...
        else//group key
        {
          if (strcmp (param->u.crypt.alg, "TKIP") == 0 || strcmp (param->u.crypt.alg, "CCMP") == 0)
          {
            ...
            if (strcmp (param->u.crypt.alg, "BIP") == 0)
            {
              int no;
                            ...
                            union iwreq_data * wrqu, char * extra)
{
  ...
  if (check_fwstate (pmlmepriv, _FW_LINKED | WIFI_ADHOC_MASTER_STATE) == _TRUE)
  {
    ...
    if (rtw_is_cckratesonly_included ( (u8 *) prates) == _TRUE)
    {
      ...
      if ( (rtw_is_cckrates_included ( (u8 *) prates) ) == _TRUE)
      {
        if (ht_cap == _TRUE)
  ...
  case Ndis802_11Encryption1Enabled:
    ...
    if (erq->length)
    {
      ...
      if (padapter->securitypriv.ndisauthtype == Ndis802_11AuthModeOpen)
      {
        ...
        if (padapter->securitypriv.ndisauthtype == Ndis802_11AuthModeShared)
        {
                            ...
                            union iwreq_data * wrqu, char * extra)
{
  ...
  while (1)
  {
    ...
    if (_rtw_memcmp (bssid, pnetwork->network.MacAddress, ETH_ALEN) == _TRUE)
    {
    ...
    union iwreq_data * wrqu, char * extra)
{
  ...
  wrqu->data.length = strlen ( extra );
  pwdinfo->wfd_info->peer_session_avail = _TRUE; 
  return ret;
      ...
      if ( (p2pie = rtw_get_p2p_ie_from_scan_queue ( &pnetwork->network.IEs[0], pnetwork->network.IELength, NULL, &p2pielen, pnetwork->network.Reserved[0]) ) )
      {
        while (p2pie)
        {
          ...
          if (rtw_get_p2p_attr_content (p2pie, p2pielen, P2P_ATTR_DEVICE_ID, attr_content, &attr_contentlen) )
          {
            ...
            if (rtw_get_p2p_attr_content (p2pie, p2pielen, P2P_ATTR_DEVICE_INFO, attr_content, &attr_contentlen) )
            {
              blnMatch = 1;
              break;
            }
                                    ...
                                    union iwreq_data * wrqu, char * extra, char * subcmd)
{
  ...
  u8 dev_type[8] = { 0x00 };
  uint dev_type_len = 0;
  u8 dev_type_str[P2P_PRIVATE_IOCTL_SET_LEN] = { 0x00 };   
                              ...
                              union iwreq_data * wrqu, char * extra)
{
  ...
  if ( _rtw_memcmp ( &extra[ 18 ], "display", 7 ) )
  {
    ...
    if ( _rtw_memcmp ( &extra[ 18 ], "keypad", 7 ) )
    {
      pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_KEYPAD;
    }
    else
      if ( _rtw_memcmp ( &extra[ 18 ], "pbc", 3 ) )
      {
        pwdinfo->tx_prov_disc_info.wps_config_method_request = WPS_CM_PUSH_BUTTON;
      }
      else
        if ( _rtw_memcmp ( &extra[ 18 ], "label", 5 ) )
        {
                        ...
                        union iwreq_data * wrqu, char * extra)
{
  ...
  if ( _rtw_memcmp ( extra, "enable=", 7 ) )
  {
    ...
    if ( _rtw_memcmp ( extra, "setDN=", 6 ) )
    {
      wrqu->data.length -= 6;
      rtw_p2p_setDN ( dev, info, wrqu, &extra[6] );
    }
    else
      if ( _rtw_memcmp ( extra, "profilefound=", 13 ) )
      {
        wrqu->data.length -= 13;
        rtw_p2p_profilefound ( dev, info, wrqu, &extra[13] );
      }
      else
        if ( _rtw_memcmp ( extra, "prov_disc=", 10 ) )
        {
          wrqu->data.length -= 10;
          rtw_p2p_prov_disc ( dev, info, wrqu, &extra[10] );
        }
        else
          if ( _rtw_memcmp ( extra, "nego=", 5 ) )
          {
            wrqu->data.length -= 5;
            rtw_p2p_connect ( dev, info, wrqu, &extra[5] );
          }
          else
            if ( _rtw_memcmp ( extra, "intent=", 7 ) )
            {
              wrqu->data.length -= 8;
              rtw_p2p_set_intent ( dev, info, wrqu, &extra[7] );
            }
            else
              if ( _rtw_memcmp ( extra, "ssid=", 5 ) )
              {
                wrqu->data.length -= 5;
                rtw_p2p_set_go_nego_ssid ( dev, info, wrqu, &extra[5] );
              }
              else
                if ( _rtw_memcmp ( extra, "got_wpsinfo=", 12 ) )
                {
                  wrqu->data.length -= 12;
                  rtw_p2p_got_wpsinfo ( dev, info, wrqu, &extra[12] );
                }
                else
                  if ( _rtw_memcmp ( extra, "listen_ch=", 10 ) )
                  {
                    wrqu->data.length -= 11;
                    rtw_p2p_set_listen_ch ( dev, info, wrqu, &extra[10] );
                  }
                  else
                    if ( _rtw_memcmp ( extra, "op_ch=", 6 ) )
                    {
                      wrqu->data.length -= 7;
                      rtw_p2p_set_op_ch ( dev, info, wrqu, &extra[6] );
                    }
                    else
                      if ( _rtw_memcmp ( extra, "invite=", 7 ) )
                      {
                        wrqu->data.length -= 8;
                        rtw_p2p_invite_req ( dev, info, wrqu, &extra[7] );
                      }
                      else
                        if ( _rtw_memcmp ( extra, "persistent=", 11 ) )
                        {
                          wrqu->data.length -= 11;
                          rtw_p2p_set_persistent ( dev, info, wrqu, &extra[11] );
                        }
                        else
                          if ( _rtw_memcmp ( extra, "uuid=", 5) )
                          {
                            wrqu->data.length -= 5;
                            ...
                            if ( _rtw_memcmp ( extra, "sa=", 3 ) )
                            {
                              ...
                              if ( _rtw_memcmp ( extra, "pc=", 3 ) )
                              {
                                wrqu->data.length -= 3;
                                rtw_p2p_set_pc ( dev, info, wrqu, &extra[3] );
                              }
                              else
                                if ( _rtw_memcmp ( extra, "wfd_type=", 9 ) )
                                {
                                  wrqu->data.length -= 9;
                                  rtw_p2p_set_wfd_device_type ( dev, info, wrqu, &extra[9] );
                                }
                                else
                                  if ( _rtw_memcmp ( extra, "wfd_enable=", 11 ) )
                                  {
                                    wrqu->data.length -= 11;
                                    rtw_p2p_set_wfd_enable ( dev, info, wrqu, &extra[11] );
                                  }
                                  else
                                    if ( _rtw_memcmp ( extra, "driver_iface=", 13 ) )
                                    {
                                      wrqu->data.length -= 13;
                        ...
                        union iwreq_data * wrqu, char * extra)
{
  ...
  if ( _rtw_memcmp ( wrqu->data.pointer, "status", 6 ) )
  {
    ...
    if ( _rtw_memcmp ( wrqu->data.pointer, "role", 4 ) )
    {
      rtw_p2p_get_role ( dev, info, wrqu, extra);
    }
    else
      if ( _rtw_memcmp ( wrqu->data.pointer, "peer_ifa", 8 ) )
      {
        rtw_p2p_get_peer_ifaddr ( dev, info, wrqu, extra);
      }
      else
        if ( _rtw_memcmp ( wrqu->data.pointer, "req_cm", 6 ) )
        {
          rtw_p2p_get_req_cm ( dev, info, wrqu, extra);
        }
        else
          if ( _rtw_memcmp ( wrqu->data.pointer, "peer_deva", 9 ) )
          {
            rtw_p2p_get_peer_devaddr ( dev, info, wrqu, extra);
          }
          else
            if ( _rtw_memcmp ( wrqu->data.pointer, "group_id", 8 ) )
            {
              rtw_p2p_get_groupid ( dev, info, wrqu, extra);
            }
            else
              if ( _rtw_memcmp ( wrqu->data.pointer, "inv_peer_deva", 13 ) )
              {
                rtw_p2p_get_peer_devaddr_by_invitation ( dev, info, wrqu, extra);
              }
              else
                if ( _rtw_memcmp ( wrqu->data.pointer, "op_ch", 5 ) )
                {
                  ...
                  if ( _rtw_memcmp ( wrqu->data.pointer, "peer_port", 9 ) )
                  {
                    ...
                    if ( _rtw_memcmp ( wrqu->data.pointer, "wfd_sa", 6 ) )
                    {
                      rtw_p2p_get_peer_wfd_session_available ( dev, info, wrqu, extra );
                    }
                    else
                      if ( _rtw_memcmp ( wrqu->data.pointer, "wfd_pc", 6 ) )
                      {
                               ...
                               union iwreq_data * wrqu, char * extra)
{
  ...
  if (_rtw_memcmp ( extra, "enable", 6 ) ) {
    ...
    if (_rtw_memcmp ( extra, "disable", 6 ) ) {
      #ifdef CONFIG_LPS
      LeaveAllPowerSaveModeDirect (padapter);
      #endif
                       ...
                       struct iw_point * wrqu, char * extra)
{
  ...
  bQueryMac = (strncmp (input, "mac", 3) == 0) ? 1 : 0;
  bSetBssid = (strncmp (input, "setbssid=", 8) == 0) ? 1 : 0;
  ...
  if (bStartRx)
  {
    ...
    if (bStopRx)
    {
      ...
      if (bQueryPhy)
      {
        ...
        {
          cckok      = PHY_QueryBBReg (padapter, 0xF04, 0x3FFF);    
          ofdmok     = PHY_QueryBBReg (padapter, 0xF14, 0x3FFF);    
          htok       = PHY_QueryBBReg (padapter, 0xF10, 0x3FFF);   
          vht_ok      = PHY_QueryBBReg (padapter, 0xF0C, 0x3FFF);   
          
          cckcrc     = PHY_QueryBBReg (padapter, 0xF04, 0x3FFF0000);
          ofdmcrc    = PHY_QueryBBReg (padapter, 0xF14, 0x3FFF0000);
          htcrc      = PHY_QueryBBReg (padapter, 0xF10, 0x3FFF0000);
          vht_err     = PHY_QueryBBReg (padapter, 0xF0C, 0x3FFF0000);
        }
        #else
        {
        ...
        {
          {
            ...
            mac_cck_ok      = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);    
            PHY_SetMacReg (padapter, 0x664, BIT28 | BIT29 | BIT30 | BIT31, 0x0);
            mac_ofdm_ok     = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);    
            PHY_SetMacReg (padapter, 0x664, BIT28 | BIT29 | BIT30 | BIT31, 0x6);
            mac_ht_ok       = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);   
            mac_vht_ok      = 0;
            
            PHY_SetMacReg (padapter, 0x664, BIT28 | BIT29 | BIT30 | BIT31, 0x4);
            mac_cck_err     = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);
            PHY_SetMacReg (padapter, 0x664, BIT28 | BIT29 | BIT30 | BIT31, 0x1);
            mac_ofdm_err    = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);
            PHY_SetMacReg (padapter, 0x664, BIT28 | BIT29 | BIT30 | BIT31, 0x7);
            mac_ht_err      = PHY_QueryMacReg (padapter, 0x664, bMaskLWord);
            mac_vht_err     = 0;
                          ...
                          struct iw_point * wrqu, char * extra)
{
  ...
  if (wrqu->length > 1) {
    if (strncmp (input, "stop", 4) == 0)
    {
      ...
      if (sscanf (input, "ther=%d", &thermal) ) {
                         ...
                         union iwreq_data * wrqu, char * extra)
{
  ...
  testmode = strncmp (extra, "testmode", 8);
                     ...
                     union iwreq_data * wrqu, char * extra)
{
  ...
  if ( _rtw_memcmp ( extra, "wfdenable=", 10 ) )
  {
    ...
    if ( _rtw_memcmp ( extra, "weaksec=", 8 ) )
    {
      ...
      if ( _rtw_memcmp ( extra, "tdlsenable=", 11 ) )
      {
        wrqu->data.length -= 11;
  ...
  if ( _rtw_memcmp ( extra, "setup=", 6 ) )
  {
    ...
    if (_rtw_memcmp ( extra, "tear=", 5 ) )
    {
      wrqu->data.length -= 5;
      rtw_tdls_teardown ( dev, info, wrqu, &extra[5] );
    }
    else
      if (_rtw_memcmp ( extra, "dis=", 4 ) )
      {
        wrqu->data.length -= 4;
        rtw_tdls_discovery ( dev, info, wrqu, &extra[4] );
      }
      else
        if (_rtw_memcmp ( extra, "sw=", 3 ) )
        {
          wrqu->data.length -= 3;
          rtw_tdls_ch_switch ( dev, info, wrqu, &extra[3] );
        }
        else
          if (_rtw_memcmp ( extra, "swoff=", 6 ) )
          {
            wrqu->data.length -= 6;
            rtw_tdls_ch_switch_off ( dev, info, wrqu, &extra[6] );
          }
          else
            if (_rtw_memcmp ( extra, "pson=", 5 ) )
            {
              wrqu->data.length -= 5;
              rtw_tdls_pson ( dev, info, wrqu, &extra[5] );
            }
            else
              if (_rtw_memcmp ( extra, "psoff=", 6 ) )
              {
                wrqu->data.length -= 6;
...
{
  { SIOCIWFIRSTPRIV + 0x00, IW_PRIV_TYPE_CHAR | 1024, 0 , ""}, 
  ...
  { MP_NULL, IW_PRIV_TYPE_CHAR | 128, 0, "NULL"},
  ...
  { WRITE_REG, IW_PRIV_TYPE_CHAR | 1024, 0, "write_reg"},
  { MP_NULL, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "NULL" },
  { WRITE_RF, IW_PRIV_TYPE_CHAR | 1024, 0, "write_rf"},
  { MP_NULL, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "NULL" },
  { MP_NULL, IW_PRIV_TYPE_CHAR | 128, 0, "NULL"},
...
static const struct iw_priv_args rtw_private_args[] = {
  ...
  #ifdef CONFIG_MP_INCLUDED
  { SIOCIWFIRSTPRIV + 0x0E, IW_PRIV_TYPE_CHAR | 1024, 0 , ""}, 
  ...
  #endif
  #ifdef CONFIG_AP_WOWLAN
  { MP_AP_WOW_ENABLE , IW_PRIV_TYPE_CHAR | 1024, 0, "ap_wow_enable" },
  #endif
};
