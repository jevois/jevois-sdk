...
#define RTW_MAX_REMAIN_ON_CHANNEL_DURATION 3000
...
void rtw_spt_band_free (struct ieee80211_supported_band * spt_band)
{
  ...
  if (spt_band->band == IEEE80211_BAND_2GHZ)
  {
    ...
    if (spt_band->band == IEEE80211_BAND_5GHZ)
    {
      size = sizeof (struct ieee80211_supported_band)
             + sizeof (struct ieee80211_channel) * RTW_5G_CHANNELS_NUM
             + sizeof (struct ieee80211_rate) * RTW_A_RATES_NUM;
...
void rtw_cfg80211_indicate_disconnect (_adapter * padapter)
{
  ...
  if (!padapter->mlmepriv.not_indic_disco) {
    ...
    if (pwdev->sme_state == CFG80211_SME_CONNECTING)
      ...
      if (pwdev->sme_state == CFG80211_SME_CONNECTED)
...
static int rtw_cfg80211_set_key_mgt (struct security_priv * psecuritypriv, u32 key_mgt)
{
  ...
  { psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_8021X; }
    ...
    if (key_mgt == WLAN_AKM_SUITE_PSK) {
      psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
    }
  #ifdef CONFIG_WAPI_SUPPORT
    else
      if (key_mgt == WLAN_AKM_SUITE_WAPI_PSK) {
        psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_WAPI;
      }
      else
        if (key_mgt == WLAN_AKM_SUITE_WAPI_CERT) {
          psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_WAPI;
        }
  #endif
                                 ...
                                 struct cfg80211_connect_params * sme)
{
  ...
  if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_WAPI_PSK) {
    ...
    if (sme->crypto.akm_suites[0] == WLAN_AKM_SUITE_WAPI_CERT) {
...
static int rtw_cfg80211_monitor_if_xmit_entry (struct sk_buff * skb, struct net_device * ndev)
{
  ...
  if ( (frame_ctl & RTW_IEEE80211_FCTL_FTYPE) == RTW_IEEE80211_FTYPE_DATA) {
    ...
    if ( (frame_ctl & (RTW_IEEE80211_FCTL_FTYPE | RTW_IEEE80211_FCTL_STYPE) )
...
static int rtw_add_beacon (_adapter * adapter, const u8 * head, size_t head_len, const u8 * tail, size_t tail_len)
{
  ...
  _rtw_memcpy (pbuf, (void *) head + 24, head_len - 24);
