  ...
  for (;;) {
    ...
    case IL_LEGACY_SWITCH_MIMO2_BC:
      ...
      { search_tbl->ant_type = ANT_AB; }
        ...
        if (tbl->action == IL_LEGACY_SWITCH_MIMO2_AC)
  ...
  for (;;) {
    ...
    case IL_SISO_SWITCH_MIMO2_BC:
      ...
      { search_tbl->ant_type = ANT_AB; }
        ...
        if (tbl->action == IL_SISO_SWITCH_MIMO2_AC)
        { search_tbl->ant_type = ANT_AC; }
        else
        { search_tbl->ant_type = ANT_BC; }
  ...
  for (;;) {
    ...
    case IL_MIMO2_SWITCH_SISO_C:
      ...
      { search_tbl->ant_type = ANT_A; }
        ...
        if (tbl->action == IL_MIMO2_SWITCH_SISO_B)
                              ...
                              struct il_lq_sta * lq_sta)
{
  ...
  if (sr <= RATE_DECREASE_TH || current_tpt == 0) {
    ...
    if (low_tpt == IL_INVALID_VALUE && high_tpt == IL_INVALID_VALUE) {
...
il4965_rs_rate_init (struct il_priv * il, struct ieee80211_sta * sta, u8 sta_id)
{
  ...
  if (!lq_sta->lq.general_params.dual_stream_ant_msk) {
    ...
    if (il4965_num_of_ant (il->hw_params.valid_tx_ant) == 2) {
      lq_sta->lq.general_params.dual_stream_ant_msk =
                         ...
                         u32 new_rate)
{
  ...
  if (il4965_num_of_ant (tbl_type.ant_type) == 1) {
    ...
    if (il4965_num_of_ant (tbl_type.ant_type) == 2) {
