  ...
  if (wpa_version & NL80211_WPA_VERSION_2) {
    ...
    if (wpa_version & NL80211_WPA_VERSION_1) {
      ...
      { auth_mode = NDIS_80211_AUTH_WPA; }
        ...
        if (keymgmt & RNDIS_WLAN_KEY_MGMT_PSK)
...
static int set_encr_mode (struct usbnet * usbdev, int pairwise, int groupwise)
{
  ...
  { encr_mode = NDIS_80211_ENCR_CCMP_ENABLED; }
    ...
    if (pairwise & RNDIS_WLAN_ALG_TKIP)
    { encr_mode = NDIS_80211_ENCR_TKIP_ENABLED; }
    else
      if (pairwise & RNDIS_WLAN_ALG_WEP)
      { encr_mode = NDIS_80211_ENCR_WEP_ENABLED; }
      else
        if (groupwise & RNDIS_WLAN_ALG_CCMP)
        { encr_mode = NDIS_80211_ENCR_CCMP_ENABLED; }
        else
          if (groupwise & RNDIS_WLAN_ALG_TKIP)
          { encr_mode = NDIS_80211_ENCR_TKIP_ENABLED; }
          else
...
static void set_multicast_list (struct usbnet * usbdev)
{
  ...
  if (usbdev->net->flags & IFF_PROMISC) {
    ...
    if (usbdev->net->flags & IFF_ALLMULTI) {
...
static void rndis_wlan_do_link_up_work (struct usbnet * usbdev)
{
  ...
  if (priv->infra_mode == NDIS_80211_INFRA_INFRA) {
    ...
    if (WARN_ON (priv->infra_mode != NDIS_80211_INFRA_ADHOC) )
    { return; }
