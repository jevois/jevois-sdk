...
static int mwl8k_load_firmware (struct ieee80211_hw * hw)
{
  ...
  do {
    ...
    if (ready_code == MWL8K_FWAP_READY) {
      ...
      if (ready_code == MWL8K_FWSTA_READY) {
        priv->ap_fw = false;
        break;
      }
                               ...
                               unsigned short pwr)
{
  ...
  if (conf->channel_type == NL80211_CHAN_NO_HT ||
    ...
    { cmd->sub_ch = cpu_to_le16 (0x3); }
      ...
      if (conf->channel_type == NL80211_CHAN_HT40PLUS)
                                  ...
                                  const struct pci_device_id * id)
{
  ...
  if (ap_mode_default && di->fw_image_ap) {
    ...
    if (!ap_mode_default && di->fw_image_sta) {
      priv->fw_pref = di->fw_image_sta;
      priv->fw_alt = di->fw_image_ap;
    }
    else
      if (ap_mode_default && !di->fw_image_ap && di->fw_image_sta) {
        printk (KERN_WARNING "AP fw is unavailable.  Using STA fw.");
        priv->fw_pref = di->fw_image_sta;
      }
      else
        if (!ap_mode_default && !di->fw_image_sta && di->fw_image_ap) {
