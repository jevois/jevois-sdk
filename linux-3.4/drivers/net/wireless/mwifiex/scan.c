                               ...
                               struct mwifiex_bssdescriptor * bss_desc, u32 mode)
{
  ...
  if (bss_desc->bss_mode == mode) {
    if (mwifiex_is_bss_no_sec (priv, bss_desc) ) {
      ...
      if (mwifiex_is_bss_static_wep (priv, bss_desc) ) {
        /* Static WEP enabled */
        ...
        if (mwifiex_is_bss_wpa (priv, bss_desc) ) {
          /* WPA enabled */
          ...
          if (mwifiex_is_bss_wpa2 (priv, bss_desc) ) {
            /* WPA2 enabled */
            if ( ( (priv->adapter->config_bands & BAND_GN ||
                    priv->adapter->config_bands & BAND_AN) &&
                   bss_desc->bcn_ht_cap) &&
            ...
            if (mwifiex_is_bss_adhoc_aes (priv, bss_desc) ) {
              /* Ad-hoc AES enabled */
              return 0;
            }
            else
              if (mwifiex_is_bss_dynamic_wep (priv, bss_desc) ) {
                /* Dynamic WEP enabled */
                return 0;
              }
  ...
  while (bytes_left >= 2) {
    ...
    case WLAN_EID_VENDOR_SPECIFIC:
      ...
      if (!memcmp
        ...
        if (!memcmp (vendor_ie->vend_hdr.oui, wmm_oui,
                     sizeof (wmm_oui) ) ) {
          if (total_ie_len ==
