    ...
    if (subelt_id == WPS_ID_VERSION) {
      ...
      if (subelt_id == WPS_ID_REQ_TYPE) {
        ...
        if (subelt_id == WPS_ID_CONFIG_METHODS) {
          ...
          if (subelt_id == WPS_ID_DEVICE_NAME) {
            char devname[100];
            ...
            if (subelt_id == WPS_ID_DEVICE_PWD_ID) {
              valptr[0] = *subel;
              valptr[1] = * (subel + 1);
              ...
              *pbc = (HTON16 (val) == DEV_PW_PUSHBUTTON) ? true : false;
              ...
              if (subelt_id == WPS_ID_PRIM_DEV_TYPE) {
                valptr[0] = *subel;
                valptr[1] = * (subel + 1);
                ...
                if (subelt_id == WPS_ID_REQ_DEV_TYPE) {
                  valptr[0] = *subel;
                  valptr[1] = * (subel + 1);
                  ...
                  if (subelt_id == WPS_ID_SELECTED_REGISTRAR_CONFIG_METHODS) {
                    valptr[0] = *subel;
                    valptr[1] = * (subel + 1);
        ...
        if (p2p_is_on (wl) && p2p_scan (wl) ) {
          ...
          if (request && request->n_channels) {
            ...
            if (num_chans == SOCIAL_CHAN_CNT && (
              ...
              if ( (dev = wl_to_p2p_bss_ndev (wl, P2PAPI_BSSCFG_CONNECTION) ) &&
                   (wl_get_mode_by_netdev (wl, dev) == WL_MODE_AP) ) {
                ...
                if (num_chans == 1) {
                  ...
                  if (num_chans == SOCIAL_CHAN_CNT + 1) {
                       ...
                       void (*callback) (void * cookie, struct key_params * params) )
  {
    ...
    case WEP_ENABLED:
      ...
      if (sec->cipher_pairwise & WLAN_CIPHER_SUITE_WEP40) {
        ...
        if (sec->cipher_pairwise & WLAN_CIPHER_SUITE_WEP104) {
          params.cipher = WLAN_CIPHER_SUITE_WEP104;
          WL_DBG ( ("WLAN_CIPHER_SUITE_WEP104\n") );
        }
      break;
                                     ...
                                     wl_action_frame_t * action_frame, u16 action_frame_len, s32 bssidx)
  {
    ...
    if (category == DOT11_ACTION_CAT_PUBLIC) {
      if ( (action == P2P_PUB_AF_ACTION) &&
        ...
        if (action_frame_len >= sizeof (wifi_p2psd_gas_pub_act_frame_t) ) {
          ...
          if (action == P2PSD_ACTION_ID_GAS_IREQ ||
            ...
            if (action == P2PSD_ACTION_ID_GAS_IRESP ||
      ...
      if (chan->band == IEEE80211_BAND_5GHZ) {
        ...
        if (err) {
          ...
          { bw = WL_CHANSPEC_BW_80; }
            ...
            if (WL_BW_CAP_40MHZ (wl->ioctl_buf[0]) )
            { bw = WL_CHANSPEC_BW_40; }
            else
            { bw = WL_CHANSPEC_BW_20; }
                                   ...
                                   const wl_event_msg_t * e, void * data)
  {
    ...
    if (event == WLC_E_ASSOC_IND && reason == DOT11_SC_SUCCESS) {
      ...
      if (event == WLC_E_DISASSOC_IND) {
        ...
        if ( (event == WLC_E_DEAUTH_IND) || (event == WLC_E_DEAUTH) ) {
          #if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 4, 0)) || defined(WL_COMPAT_WIRELESS)
          cfg80211_rx_mgmt (ndev, freq, 0, mgmt_frame, len, GFP_ATOMIC);
          #else
          cfg80211_rx_mgmt (ndev, freq, mgmt_frame, len, GFP_ATOMIC);
                                     ...
                                     const wl_event_msg_t * e, void * data)
  {
    ...
    if (event == WLC_E_JOIN || event == WLC_E_START ||
      ...
      if ( (event == WLC_E_LINK && ! (flags & WLC_EVENT_MSG_LINK) ) ||
        ...
        if (event == WLC_E_SET_SSID && status == WLC_E_STATUS_NO_NETWORKS) {
                              ...
                              const wl_event_msg_t * e, void * data)
{
  ...
  if (wl_get_mode_by_netdev (wl, ndev) == WL_MODE_AP) {
    ...
    if (wl_get_mode_by_netdev (wl, ndev) == WL_MODE_IBSS) {
                             ...
                             const wl_event_msg_t * e, void * data)
{
  ...
  if (event == WLC_E_ACTION_FRAME_RX) {
    ...
    if (wl_cfgp2p_is_pub_action (&mgmt_frame[DOT11_MGMT_HDR_LEN],
      ...
      if (wl_cfgp2p_is_p2p_action (&mgmt_frame[DOT11_MGMT_HDR_LEN],
                                   mgmt_frame_len - DOT11_MGMT_HDR_LEN) ) {
        ...
        if (wl_cfgp2p_is_gas_action (&mgmt_frame[DOT11_MGMT_HDR_LEN],
                                     mgmt_frame_len - DOT11_MGMT_HDR_LEN) ) {
          #ifdef WL_SDO
          if (wl_get_p2p_status (wl, DISC_IN_PROGRESS) ) {
  ...
  if (status == WLC_E_STATUS_PARTIAL) {
    ...
    if (status == WLC_E_STATUS_SUCCESS) {
      ...
      if (wl_get_drv_status_all (wl, FINDING_COMMON_CHANNEL) ) {
        ...
        if ( (likely (wl->scan_request) ) || (wl->sched_scan_running) ) {
    ...
    if (wl_get_drv_status (wl, CONNECTED, iter->ndev) ) {
      ...
      if (!wl->vsdb_mode) {
        ...
        { pre_ctl_chan = ctl_chan; }
          ...
          if (pre_ctl_chan && (pre_ctl_chan != ctl_chan) ) {
            wl->vsdb_mode = true;
          }
      }
    }
...
#undef LOCAL_BUF_LEN
  ...
  for (i = 0; i < dtoh32 (list->count); i++) {
    ...
    if (CHSPEC_IS2G (c) && (channel >= CH_MIN_2G_CHANNEL) &&
      ...
      if (CHSPEC_IS5G (c) && channel >= CH_MIN_5G_CHANNEL) {
...
wl_cfg80211_resume_sdo (struct net_device * dev, struct wl_priv * wl)
  {
    ...
    if (wl->sdo->dd_state == WL_DD_STATE_SEARCH) {
      ...
      if (wl->sdo->dd_state == WL_DD_STATE_LISTEN) {
      ...
      int len)
{
  ...
  if (len) {
    ...
    if (ptr == NULL) {
      ...
      if (strncmp (ptr, "chan=", strlen ("chan=") ) == 0) {
...
s32 wl_cfg80211_sd_offload (struct net_device * dev, char * cmd, char * buf, int len)
{
  ...
  if (strncmp (cmd, "P2P_SD_REQ", strlen ("P2P_SD_REQ") ) == 0) {
    ...
    if (strncmp (cmd, "P2P_SD_CANCEL_REQ", strlen ("P2P_SD_CANCEL_REQ") ) == 0) {
      ret = wl_sd_handle_sd_cancel_req (dev, buf);
    }
    else
      if (strncmp (cmd, "P2P_SD_SVC_ADD", strlen ("P2P_SD_SVC_ADD") ) == 0) {
        ret = wl_sd_handle_sd_add_svc (dev, buf, len);
      }
      else
        if (strncmp (cmd, "P2P_SD_SVC_DEL", strlen ("P2P_SD_SVC_DEL") ) == 0) {
          ret = wl_sd_handle_sd_del_svc (dev, buf, len);
        }
        else
          if (strncmp (cmd, "P2P_SD_FIND", strlen ("P2P_SD_FIND") ) == 0) {
            ret = wl_sd_handle_sd_find (dev, buf, len);
          }
          else
            if (strncmp (cmd, "P2P_SD_LISTEN", strlen ("P2P_SD_LISTEN") ) == 0) {
              ret = wl_sd_handle_sd_listen (dev, buf, len);
            }
            else
              if (strncmp (cmd, "P2P_SD_STOP", strlen ("P2P_STOP") ) == 0) {
                                    ...
                                    enum wl_management_type type)
{
  ...
  if (wl_get_drv_status (wl, AP_CREATING, net) ) {
    ...
    if (wl_get_drv_status (wl, AP_CREATED, net) ) {
      ndev = net;
      bssidx = 0;
    }
    else
...
wl_cfg80211_valid_chanspec_p2p (chanspec_t chanspec)
{
  ...
  /* channel 36 to 48 */
    ...
    if ( (chanspec >= 0x1b24) && (chanspec <= 0x1b30) ) {
      valid = true;
    }
    /* channel 149 to 161 */
    else
      if ( (chanspec >= 0x1b95) && (chanspec <= 0x1ba1) ) {
        valid = true;
      }
      else {
        valid = false;
