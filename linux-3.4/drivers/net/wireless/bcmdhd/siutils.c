  ...
  for (i = 0; i < sii->numcores; i++) {
    ...
    if (BUSTYPE (bustype) == PCI_BUS) {
      if (cid == PCI_CORE_ID) {
        ...
        if ( (cid == PCIE_CORE_ID) || (cid == PCIE2_CORE_ID) ) {
          pcieidx = i;
          pcierev = crev;
          pcie = TRUE;
...
si_flag (si_t * sih)
{
  ...
  { return sb_flag (sih); }
    ...
    { return ai_flag (sih); }
      ...
      if (CHIPTYPE (sih->socitype) == SOCI_UBUS)
      { return ub_flag (sih); }
      else {
        ASSERT (0);
        return 0;
...
si_clkctl_init (si_t * sih)
{
  ...
  if (!fast) {
    ...
    if ( (cc = (chipcregs_t *) CCREGS_FAST (sii) ) == NULL)
    { return; }
  ASSERT (cc != NULL);
...
si_gpioevent (si_t * sih, uint regtype, uint32 mask, uint32 val)
{
  ...
  { offs = OFFSETOF (chipcregs_t, gpioevent); }
    ...
    if (regtype == GPIO_REGEVT_INTMSK)
    { offs = OFFSETOF (chipcregs_t, gpioeventintmask); }
    else
      if (regtype == GPIO_REGEVT_INTPOL)
