...
rssi_to_qual (int rssi)
{
  ...
  { return 0; }
    ...
    if (rssi <= WL_IW_RSSI_VERY_LOW)
    { return 1; }
    else
      if (rssi <= WL_IW_RSSI_LOW)
      { return 2; }
      else
        if (rssi <= WL_IW_RSSI_GOOD)
        { return 3; }
        else
...
)
{
  ...
  { rts = DOT11_DEFAULT_RTS_LEN; }
    ...
    if (vwrq->value < 0 || vwrq->value > DOT11_DEFAULT_RTS_LEN)
    { return -EINVAL; }
    else
    { rts = vwrq->value; }
...
)
{
  ...
  { frag = DOT11_DEFAULT_FRAG_LEN; }
    ...
    if (vwrq->value < 0 || vwrq->value > DOT11_DEFAULT_FRAG_LEN)
    { return -EINVAL; }
    else
    { frag = vwrq->value; }
...
)
{
  ...
  case IW_AUTH_WPA_VERSION:
    ...
    { val = WPA_AUTH_DISABLED; }
      ...
      if (paramval & (IW_AUTH_WPA_VERSION_WPA) )
      { val = WPA_AUTH_PSK | WPA_AUTH_UNSPECIFIED; }
      else
        if (paramval & IW_AUTH_WPA_VERSION_WPA2)
        { val = WPA2_AUTH_PSK | WPA2_AUTH_UNSPECIFIED; }
    #ifdef BCMWAPI_WPI
        else
          if (paramval & IW_AUTH_WAPI_VERSION_1)
  ...
  case IW_AUTH_80211_AUTH_ALG:
    ...
    { val = 0; }
      ...
      if (paramval & IW_AUTH_ALG_SHARED_KEY)
      { val = 1; }
      else
      { error = 1; }
