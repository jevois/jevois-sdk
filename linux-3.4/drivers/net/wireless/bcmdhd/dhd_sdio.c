  ...
  if (bus->glom_enable) {
    ...
    if (queue_only) {
      ...
      if (bus->roundup && bus->blocksize &&
        ...
        if ( (bus->glom_total_len + len) % DHD_SDALIGN) {
          len += DHD_SDALIGN
                 ...
                 void * params, int plen, void * arg, int len, int val_size)
{
  ...
  if ( (vi->varid == IOV_KSO) && (IOV_ISSET (actionid) ) ) {
    ...
    if ( (vi->varid == IOV_DEVSLEEP) && (IOV_ISSET (actionid) ) ) {
      {
      ...
      if (si_setcore (bus->sih, ARMCR4_CORE_ID, 0) ) {
             ...
             ( (address > bus->orig_ramsize) || (address + size > bus->orig_ramsize) ) )
        {
          ...
          if (!REMAP_ENAB (bus) && (address >= SOCDEVRAM_ARM_ADDR) ) {
            ...
            if (REMAP_ENAB (bus) && REMAP_ISADDR (bus, address) && remap) {
...
dhdsdio_rxglom (dhd_bus_t * bus, uint8 rxseq)
{
  ...
  if (bus->glom) {
    ...
    if ( (uint16) ~ (sublen ^ check) ) {
      ...
      if (ROUNDUP (sublen, bus->blocksize) != dlen) {
        DHD_ERROR ( ("%s (superframe): len 0x%04x, rounded 0x%04x, expect 0x%04x\n",
                     __FUNCTION__, sublen, ROUNDUP (sublen, bus->blocksize), dlen) );
        errcode = -1;
      }
      else
        if (SDPCM_PACKET_CHANNEL (&dptr[SDPCM_FRAMETAG_LEN]) != SDPCM_GLOM_CHANNEL) {
          DHD_ERROR ( ("%s (superframe): bad channel %d\n", __FUNCTION__,
                       SDPCM_PACKET_CHANNEL (&dptr[SDPCM_FRAMETAG_LEN]) ) );
          errcode = -1;
        }
        else
          if (SDPCM_GLOMDESC (&dptr[SDPCM_FRAMETAG_LEN]) ) {
            DHD_ERROR ( ("%s (superframe): got second descriptor?\n", __FUNCTION__) );
            errcode = -1;
          }
          else
            if ( (doff < SDPCM_HDRLEN_RX) ||
    ...
    for (num = 0, pnext = pfirst; pnext && !errcode;
      ...
      if ( (uint16) ~ (sublen ^ check) ) {
        ...
        if ( (sublen > dlen) || (sublen < SDPCM_HDRLEN_RX) ) {
          ...
          if ( (chan != SDPCM_DATA_CHANNEL) &&
            ...
            if ( (doff < SDPCM_HDRLEN_RX) || (doff > sublen) ) {
    ...
    for (num = 0; pfirst; rxseq++, pfirst = pnext) {
      ...
      if (PKTLEN (osh, pfirst) == 0) {
        ...
        if (dhd_prot_hdrpull (bus->dhd, &ifidx, pfirst, reorder_info_buf,
                              &reorder_info_len) != 0) {
...
deliver:
    ...
    if (PKTLEN (osh, pkt) == 0) {
      ...
      if (dhd_prot_hdrpull (bus->dhd, &ifidx, pkt, reorder_info_buf,
                            &reorder_info_len) != 0) {
        DHD_ERROR ( ("%s: rx protocol error\n", __FUNCTION__) );
        dhd_os_sdlock_rxq (bus->dhd);
        PKTFREE (bus->dhd->osh, pkt, FALSE);
    ...
    if (!tdata--) {
      ...
      { break; }
        ...
        { tdata = 0; }
          ...
          if ( (byte + 1) < SBSDIO_CIS_SIZE_LIMIT)
          { tdata = cis[byte + 1] + 1; }
          else
          { DHD_INFO ( ("]") ); }
    }
...
dhd_bus_devreset (dhd_pub_t * dhdp, uint8 flag)
{
  ...
  if (flag == TRUE) {
    ...
    if (bus->dhd->dongle_reset) {
      ...
      sdioh_start (NULL, 1);
      #if defined(HW_OOB)
      dhd_conf_set_hw_oob_intr (bus->sdh, bus->sih->chip);
      #endif
      dhd_conf_set_fw_name_by_chip (dhdp, fw_path, firmware_path);
