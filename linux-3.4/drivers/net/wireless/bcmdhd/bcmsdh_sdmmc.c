                    ...
                    uint32 * word, uint nbytes)
{
  ...
  int err_ret2 = SDIOH_API_RC_SUCCESS;
  #if defined(MMC_SDIO_ABORT)
  int sdio_abort_retry = MMC_SDIO_ABORT_RETRY_LIMIT;
  #endif
  int ret = 0;
...
static INLINE int sdioh_request_packet_align (uint pkt_len, uint write, uint func, int blk_size)
{
  ...
  { pkt_len = (pkt_len + 3) & 0xFFFFFFFC; }
    ...
    if ( (pkt_len > blk_size) && (pkt_len % blk_size) ) {
      if (func == SDIO_FUNC_2) {
                      ...
                      uint addr, uint reg_width, uint buflen_u, uint8 * buffer, void * pkt)
{
  ...
  if (pkt == NULL) {
    ...
    if ( (ulong) PKTDATA (sd->osh, pkt) & DMA_ALIGN_MASK) {
