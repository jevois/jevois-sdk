...
handle_tkip_countermeasure:
  ...
  if (padapter->securitypriv.btkip_countermeasure == _TRUE)
  {
    ...
    cur_time = (u32) (sys_time.QuadPart / 10);
    ...
    diff_time = cur_time - padapter->securitypriv.btkip_countermeasure_time;
...
u8 rtw_set_802_11_add_key (_adapter * padapter, NDIS_802_11_KEY * key) {
  ...
  else
  {
    ...
    if ( (encryptionalgo == _TKIP_) && (key->KeyLength != 32) ) {
      ...
      if (encryptionalgo == _AES_ && (key->KeyLength != 16 && key->KeyLength != 32) ) {
...
int rtw_set_country (_adapter * adapter, const char * country_code)
{
  ...
  { channel_plan = RT_CHANNEL_DOMAIN_FCC; }
    ...
    if (0 == strcmp (country_code, "EU") )
    { channel_plan = RT_CHANNEL_DOMAIN_ETSI; }
    else
      if (0 == strcmp (country_code, "JP") )
      { channel_plan = RT_CHANNEL_DOMAIN_MKK; }
      else
        if (0 == strcmp (country_code, "CN") )
