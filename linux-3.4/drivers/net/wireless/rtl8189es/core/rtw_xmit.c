  ...
  else
  {
    while (_TRUE)
    {
      ...
      if (pHTInfo->IOTAction & HT_IOT_ACT_FORCED_CTS2SELF)
      {
        ...
        if (pHTInfo->IOTAction & (HT_IOT_ACT_FORCED_RTS | HT_IOT_ACT_PURE_N_MODE) )
        {
          pattrib->vcs_mode = RTS_CTS;
          pattrib->rts_rate = MGN_24M;
          break;
...
static s32 update_attrib (_adapter * padapter, _pkt * pkt, struct pkt_attrib * pattrib)
{
  ...
  if (bmcast) {
    ...
    if (psta == NULL) {
      ...
      if ( (check_fwstate (pmlmepriv, WIFI_AP_STATE) == _TRUE) && (! (psta->state & _FW_LINKED) ) )
      {
        res = _FAIL;
        goto exit;
      }
      ...
      if (psta) {
        ...
        if (pattrib->ampdu_en == _TRUE)
        {
          ...
          if (SN_LESS (pattrib->seqnum, tx_seq) )
          {
            ...
            if (SN_EQUAL (pattrib->seqnum, tx_seq) )
            {
