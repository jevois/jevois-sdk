...
void _rtw_free_network (struct mlme_priv * pmlmepriv , struct wlan_network * pnetwork, u8 isfreeall)
{
  ...
  if (!isfreeall)
  {
    ...
    if (delta_time  < lifetime * 1000000)
    {
      goto exit;
    }
...
int rtw_init_mlme_priv (_adapter * padapter)
{
  int res;
  _func_enter_;
        ...
        if (rtw_to_roaming (adapter) > 0) {
             ...
             )
          {
            ...
            if (pmlmepriv->assoc_by_rssi == _TRUE)
            {
              ...
              if (rtw_is_desired_network (adapter, pnetwork) == _TRUE)
              {
                if (check_fwstate (pmlmepriv, _FW_LINKED) == _TRUE)
                {
                  #if 0
...
void rtw_update_ht_cap (_adapter * padapter, u8 * pie, uint ie_len)
{
  ...
  if ( (phtpriv->ampdu_enable == _FALSE) && (pregistrypriv->ampdu_enable == 1) )
  {
    ...
    if (pregistrypriv->ampdu_enable == 2)
    {
      phtpriv->ampdu_enable = _TRUE;
    }
...
sint rtw_buddy_adapter_up (_adapter * padapter)
{
  ...
  if (padapter->pbuddy_adapter == NULL)
  {
    ...
    if ( (padapter->pbuddy_adapter->bDriverStopped) || (padapter->pbuddy_adapter->bSurpriseRemoved) ||
