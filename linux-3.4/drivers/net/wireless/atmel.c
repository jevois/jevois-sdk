  ...
  if (priv->use_wpa) {
    ...
    if (is_bcast) {
      ...
      { cipher_length = 8; }
        ...
        if (cipher_type == CIPHER_SUITE_TKIP)
        { cipher_length = 12; }
        else
          if (priv->pairwise_cipher_suite == CIPHER_SUITE_WEP_64 ||
      ...
      { cipher_length = 8; }
        ...
        { cipher_length = 12; }
          ...
          if (priv->group_cipher_suite == CIPHER_SUITE_WEP_64 ||
...
static void authenticate (struct atmel_private * priv, u16 frame_len)
{
  ...
  if (status == WLAN_STATUS_SUCCESS && priv->wep_is_on) {
    ...
    if (system == WLAN_AUTH_OPEN) {
      ...
      if (system == WLAN_AUTH_SHARED_KEY) {
        if (trans_seq_no == 0x0002 &&
