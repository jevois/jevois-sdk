...
static void ipw_led_band_on (struct ipw_priv * priv)
{
  ...
  if (priv->assoc_network->mode == IEEE_A) {
    ...
    if (priv->assoc_network->mode == IEEE_G) {
      led |= priv->led_ofdm_on;
      led |= priv->led_association_on;
                                  ...
                                  struct ipw_supported_rates * rates, int roaming)
{
  ...
  { priv->assoc_request.ieee_mode = IPW_A_MODE; }
    ...
    if (network->mode & priv->ieee->mode & IEEE_G)
    { priv->assoc_request.ieee_mode = IPW_G_MODE; }
    else
      if (network->mode & priv->ieee->mode & IEEE_B)
                                       ...
                                       struct libipw_rx_stats * stats)
{
  ...
  if (libipw_is_management (le16_to_cpu (hdr->frame_control) ) ) {
    ...
    if (libipw_is_control (le16_to_cpu (hdr->frame_control) ) ) {
      ...
      if (libipw_is_data (le16_to_cpu (hdr->frame_control) ) ) {
    ...
    * (__le16 *) skb_put (dst, sizeof (u16) ) = cpu_to_le16 (
    ...
    if (priv->channel > 14)   /* 802.11a */
      * (__le16 *) skb_put (dst, sizeof (u16) ) =
      ...
      if (priv->ieee->mode == IEEE_B) /* 802.11b */
        * (__le16 *) skb_put (dst, sizeof (u16) ) =
