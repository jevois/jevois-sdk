...
)
{
  ...
  #if (MP_DRIVER == 1)
  #if (DM_ODM_SUPPORT_TYPE == ODM_MP)
  pDM_Odm->RFCalibrateInfo.TxPowerTrackControl = pHalData->TxPowerTrackControl;
  #endif
  ...
  ThermalValue = (u1Byte) ODM_GetRFReg (pDM_Odm, RF_PATH_A, RF_T_METER_88E, 0xfc00);
  ...
  if (ThermalValue)
  {
    ...
    if (pDM_Odm->RFCalibrateInfo.bReloadtxpowerindex)
    {
      ...
      if (pDM_Odm->RFCalibrateInfo.bDoneTxpower)
      {
...
)
{
  ...
  for (i = 0 ; i < retryCount ; i++) {
    ...
    if (PathAOK == 0x01) {
      ...
      if (i == (retryCount - 1) && PathAOK == 0x01)
      {
...
)
{
  ...
  {
    if (bMain)
    { ODM_SetBBReg (pDM_Odm, rFPGA0_XB_RFInterfaceOE, BIT5 | BIT6, 0x1); }
    else
    { ODM_SetBBReg (pDM_Odm, rFPGA0_XB_RFInterfaceOE, BIT5 | BIT6, 0x2); }
  }
  else     
  {
  
    if (bMain)
    { ODM_SetBBReg (pDM_Odm, rFPGA0_XA_RFInterfaceOE, BIT8 | BIT9, 0x2); }
    else
    { ODM_SetBBReg (pDM_Odm, rFPGA0_XA_RFInterfaceOE, BIT8 | BIT9, 0x1); }
  }
}
VOID PHY_SetRFPathSwitch_8188E (
  #if (DM_ODM_SUPPORT_TYPE & ODM_AP)
