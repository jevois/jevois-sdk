    ...
    if (pattrib->ht_en)
    {
      *pdw |= (pattrib->bwmode & HT_CHANNEL_WIDTH_40) ?  cpu_to_le32 (BIT (27) ) : 0;
      ...
      { *pdw |= cpu_to_le32 ( (0x01 << 28) & 0x30000000); }
        ...
        if (pattrib->ch_offset == HAL_PRIME_CHNL_OFFSET_UPPER)
        { *pdw |= cpu_to_le32 ( (0x02 << 28) & 0x30000000); }
        else
          if (pattrib->ch_offset == HAL_PRIME_CHNL_OFFSET_DONT_CARE)
          { *pdw |= 0; }
          else
...
static s32 update_txdesc (struct xmit_frame * pxmitframe, u8 * pmem, s32 sz , u8 bagg_pkt)
{
  ...
  if ( (pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG)
  {
    ...
    if ( (pxmitframe->frame_tag & 0x0f) == MGNT_FRAMETAG)
    {
      ...
      if ( (pxmitframe->frame_tag & 0x0f) == TXAGG_FRAMETAG)
      {
        DBG_8192C ("pxmitframe->frame_tag == TXAGG_FRAMETAG\n");
      }
  #ifdef CONFIG_MP_INCLUDED
      else
        if ( ( (pxmitframe->frame_tag & 0x0f) == MP_FRAMETAG) &&
...
s32 rtl8188eu_hostap_mgnt_xmit_entry (_adapter * padapter, _pkt * pkt)
{
  ...
  ptxdesc->txdw1 |= cpu_to_le32 (0x00);
  
  ptxdesc->txdw1 |= cpu_to_le32 ( (0x12 << QSEL_SHT) & 0x00001f00);
  
  ptxdesc->txdw1 |= cpu_to_le32 ( (0x06 << 16) & 0x000f0000);
