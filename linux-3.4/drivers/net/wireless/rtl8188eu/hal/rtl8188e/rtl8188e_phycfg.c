    ...
    #ifdef CONFIG_IOL_BB_PHY_REG
    {
      ...
      for (i = 0; i < PHY_REGArrayLen; i = i + 2)
      {
        ...
        { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 50); }
          ...
          if (Rtl819XPHY_REGArray_Table[i] == 0xfd)
          { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 5); }
          else
            if (Rtl819XPHY_REGArray_Table[i] == 0xfc)
            { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 1); }
            else
              if (Rtl819XPHY_REGArray_Table[i] == 0xfb)
              { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 50); }
              else
                if (Rtl819XPHY_REGArray_Table[i] == 0xfa)
                { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 5); }
                else
                  if (Rtl819XPHY_REGArray_Table[i] == 0xf9)
                  { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 1); }
                  else
                    if (Rtl819XPHY_REGArray_Table[i] == 0xa24)
  ...
  if (ConfigType == CONFIG_BB_PHY_REG)
  {
    for (i = 0; i < PHY_REGArrayPGLen; i = i + 3)
    {
      ...
      if (Rtl819XPHY_REGArray_Table_PG[i] == 0xfe) {
        ...
        if (Rtl819XPHY_REGArray_Table_PG[i] == 0xfd)
        { rtw_mdelay_os (5); }
        else
          if (Rtl819XPHY_REGArray_Table_PG[i] == 0xfc)
          { rtw_mdelay_os (1); }
          else
            if (Rtl819XPHY_REGArray_Table_PG[i] == 0xfb)
            { rtw_udelay_os (50); }
            else
              if (Rtl819XPHY_REGArray_Table_PG[i] == 0xfa)
              { rtw_udelay_os (5); }
              else
                if (Rtl819XPHY_REGArray_Table_PG[i] == 0xf9)
                { rtw_udelay_os (1); }
      #endif
    ...
    #ifdef CONFIG_IOL_RF_RF_PATH_A
    {
      ...
      for (i = 0; i < RadioA_ArrayLen; i = i + 2)
      {
        ...
        { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 50); }
          ...
          if (Rtl819XRadioA_Array_Table[i] == 0xfd)
          { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 5); }
          else
            if (Rtl819XRadioA_Array_Table[i] == 0xfc)
            { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 1); }
            else
              if (Rtl819XRadioA_Array_Table[i] == 0xfb)
              { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 50); }
              else
                if (Rtl819XRadioA_Array_Table[i] == 0xfa)
                { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 5); }
                else
                  if (Rtl819XRadioA_Array_Table[i] == 0xf9)
                  { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 1); }
                  else
                  {
                    BB_REGISTER_DEFINITION_T * pPhyReg = &pHalData->PHYRegDef[eRFPath];
    ...
    #ifdef CONFIG_IOL_RF_RF_PATH_B
    {
      ...
      for (i = 0; i < RadioB_ArrayLen; i = i + 2)
      {
        ...
        { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 50); }
          ...
          if (Rtl819XRadioB_Array_Table[i] == 0xfd)
          { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 5); }
          else
            if (Rtl819XRadioB_Array_Table[i] == 0xfc)
            { rtw_IOL_append_DELAY_MS_cmd (xmit_frame, 1); }
            else
              if (Rtl819XRadioB_Array_Table[i] == 0xfb)
              { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 50); }
              else
                if (Rtl819XRadioB_Array_Table[i] == 0xfa)
                { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 5); }
                else
                  if (Rtl819XRadioB_Array_Table[i] == 0xf9)
                  { rtw_IOL_append_DELAY_US_cmd (xmit_frame, 1); }
                  else
                  {
                    BB_REGISTER_DEFINITION_T * pPhyReg = &pHalData->PHYRegDef[eRFPath];
...
)
{
  ...
  if (GET_RF_TYPE (Adapter) == RF_1T2R || GET_RF_TYPE (Adapter) == RF_1T1R)
  {
    ...
    if (GET_RF_TYPE (Adapter) == RF_2T2R)
    {
