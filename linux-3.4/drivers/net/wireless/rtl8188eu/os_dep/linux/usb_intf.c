...
static struct specific_device_id specific_device_id_tbl[] = {
  ...
  {.idVendor = USB_VENDER_ID_REALTEK, .idProduct = 0x8111, .flags = SPEC_DEV_ID_ASSIGN_IFNAME},
  {.idVendor = 0x2019, .idProduct = 0xAB2D, .flags = SPEC_DEV_ID_ASSIGN_IFNAME},
  ...
  for (i = 0; i < pdvobjpriv->nr_endpoint; i++)
  {
    ...
    if (phost_endp)
    {
      ...
      if (RT_usb_endpoint_is_bulk_in (pendp_desc) )
      {
        ...
        if (RT_usb_endpoint_is_int_in (pendp_desc) )
        {
  ...
  for (i = 0; i < iface_desc->bNumEndpoints; i++)
  {
    ...
    if (host_endp)
    {
      ...
      if (RT_usb_endpoint_is_bulk_in (endp_desc) )
      {
        ...
        if (RT_usb_endpoint_is_int_in (endp_desc) )
        {
          DBG_871X ("RT_usb_endpoint_is_int_in = %x, Interval = %x\n", RT_usb_endpoint_num (endp_desc), endp_desc->bInterval);
        }
        else
          if (RT_usb_endpoint_is_bulk_out (endp_desc) )
          {
            DBG_871X ("RT_usb_endpoint_is_bulk_out = %x\n", RT_usb_endpoint_num (endp_desc) );
          }
    }
...
static int __init rtw_drv_entry (void)
{
  ...
  tmp &= 0xffffff00;
  tmp |= 0x55;
  writel (tmp, (volatile unsigned int *) 0xb801a608);
  #endif
  #ifdef CONFIG_PLATFORM_ARM_SUNxI
  #ifndef CONFIG_RTL8723A
  int ret = 0;
