  ...
  
  {
    ...
    if (psta) {
      ...
      { psta->sta_stats.rx_beacon_pkts++; }
        ...
        if (GetFrameSubType (precv_frame->u.hdr.rx_data) == WIFI_PROBEREQ)
        { psta->sta_stats.rx_probereq_pkts++; }
        else
          if (GetFrameSubType (precv_frame->u.hdr.rx_data) == WIFI_PROBERSP) {
            if (_rtw_memcmp (padapter->eeprompriv.mac_addr, GetAddr1Ptr (precv_frame->u.hdr.rx_data), ETH_ALEN) == _TRUE)
            { psta->sta_stats.rx_probersp_pkts++; }
            else
              if (is_broadcast_mac_addr (GetAddr1Ptr (precv_frame->u.hdr.rx_data) )
...
sint wlanhdr_to_ethhdr ( union recv_frame * precvframe)
{
  ...
  if (psnap->dsap == 0xaa && psnap->ssap == 0xaa && psnap->ctrl == 0x03)
  {
    ...
    { bsnaphdr = _TRUE; }
      ...
      if (_rtw_memcmp (psnap->oui, SNAP_HDR_APPLETALK_DDP, WLAN_IEEE_OUI_LEN) &&
          _rtw_memcmp (psnap_type, SNAP_ETH_TYPE_APPLETALK_DDP, 2) )
      { bsnaphdr = _TRUE; }
      else
        if (_rtw_memcmp ( psnap->oui, oui_8021h, WLAN_IEEE_OUI_LEN) )
        { bsnaphdr = _TRUE; }
        else {
  ...
  if (eth_type == 0x0800)
  {
    ...
    else
    {
      ...
      VlanPriInfo.TagHeader.Reserved = 0;
...
exit:
  ...
  do {
    ...
    recvframe_put (pnrframe, type_len + ETH_HLEN);
    ...
    if (a_len > ETH_HLEN)
    {
      ...
      if (pnrframe_new)
      {
        ...
        {
          rtw_recv_indicatepkt (padapter, pnrframe_new);
        }
        else
        {
          rtw_free_recvframe (pnrframe_new, pfree_recv_queue);
        }
    ...
    else
    {
      if ( (padapter->bDriverStopped == _FALSE) && ( padapter->bSurpriseRemoved == _FALSE) )
      {
        rtw_recv_indicatepkt (padapter, pnrframe);
      }
      else
      {
        rtw_free_recvframe (pnrframe, pfree_recv_queue);
