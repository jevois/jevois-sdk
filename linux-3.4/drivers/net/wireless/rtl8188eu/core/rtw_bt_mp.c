...
)
{
  ...
  if (pExtC2h->reqNum != pH2c->reqNum)
  {
    ...
    if (pExtC2h->opCodeVer != pH2c->opCodeVer)
    {
...
)
{
  ...
  if (BT_GSET_REG == setType)
  {
    ...
    if (BT_GSET_RESET == setType)
    {
      ...
      if (BT_GSET_TARGET_BD_ADDR == setType)
      {
        ...
        if (BT_GSET_TX_PWR_FINETUNE == setType)
        {
          btOpcode = BT_LO_OP_SET_TX_POWER_CALIBRATION;
          h2cParaBuf[0] = calVal;
          h2cParaLen = 1;
          ...
          if (BT_SET_TRACKING_INTERVAL == setType)
          {
            ...
            if (BT_SET_THERMAL_METER == setType)
            {
              ...
              if (BT_ENABLE_CFO_TRACKING == setType)
              {
