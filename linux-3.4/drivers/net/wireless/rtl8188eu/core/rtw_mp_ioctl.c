  ...
  *poid_par_priv->bytes_needed = sizeof (u8);
  ...
  if (poid_par_priv->type_of_oid == SET_OID)
  {
    ...
    { encry_mode = HW_CONTROL; }
      ...
      if ( (psecuritypriv->sw_encrypt == _FALSE) && (psecuritypriv->sw_decrypt == _TRUE) )
      { encry_mode = HW_ENCRY_SW_DECRY; }
      else
        if ( (psecuritypriv->sw_encrypt == _TRUE) && (psecuritypriv->sw_decrypt == _FALSE) )
        { encry_mode = SW_ENCRY_HW_DECRY; }
        else
          if ( (psecuritypriv->sw_encrypt == _TRUE) && (psecuritypriv->sw_decrypt == _TRUE) )
          { encry_mode = SW_CONTROL; }
...
NDIS_STATUS oid_rt_set_rx_packet_type_hdl (struct oid_par_priv * poid_par_priv)
{
  ...
  _irqlevel_changed_ (&oldirql, LOWER);
  #if 0
  rcr_val8 = rtw_read8 (Adapter, 0x10250048);
  ...
  if (rx_pkt_type == RX_PKT_BROADCAST) {
    ...
    if (rx_pkt_type == RX_PKT_DEST_ADDR) {
      ...
      if (rx_pkt_type == RX_PKT_PHY_MATCH) {
  ...
  #else
  rcr_val32 = rtw_read32 (padapter, RCR);
