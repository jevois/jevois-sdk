...
u8 judge_network_type (_adapter * padapter, unsigned char * rate, int ratelen)
{
  ...
  else
  {
    ...
    if ( (cckratesonly_included (rate, ratelen) ) == _TRUE)
    {
      ...
      if ( (cckrates_included (rate, ratelen) ) == _TRUE)
      {
...
unsigned int decide_wait_for_beacon_timeout (unsigned int bcn_interval)
{
  if ( (bcn_interval << 2) < WAIT_FOR_BCN_TO_MIN)
  {
    ...
    if ( (bcn_interval << 2) > WAIT_FOR_BCN_TO_MAX)
    {
...
void update_beacon_info (_adapter * padapter, u8 * pframe, uint pkt_len, struct sta_info * psta)
{
  ...
  u8 tdls_prohibited[] = { 0x00, 0x00, 0x00, 0x00, 0x10 };
  #endif
  ...
  for (i = sizeof (NDIS_802_11_FIXED_IEs); i < len;)
  {
    ...
    case _VENDOR_SPECIFIC_IE_:
      if ( (_rtw_memcmp (pIE->data, ARTHEROS_OUI1, 3) ) || (_rtw_memcmp (pIE->data, ARTHEROS_OUI2, 3) ) )
      {
        ...
        if ( (_rtw_memcmp (pIE->data, BROADCOM_OUI1, 3) )
             ...
             || (_rtw_memcmp (pIE->data, BROADCOM_OUI2, 3) ) )
        {
          ...
          if (_rtw_memcmp (pIE->data, MARVELL_OUI, 3) )
          {
            DBG_871X ("link to Marvell AP\n");
            return HT_IOT_PEER_MARVELL;
          }
          else
            if (_rtw_memcmp (pIE->data, RALINK_OUI, 3) )
            {
              ...
              if (_rtw_memcmp (pIE->data, CISCO_OUI, 3) )
              {
                DBG_871X ("link to Cisco AP\n");
                return HT_IOT_PEER_CISCO;
              }
              else
                if (_rtw_memcmp (pIE->data, REALTEK_OUI, 3) )
                {
                  DBG_871X ("link to Realtek 96B\n");
                  return HT_IOT_PEER_REALTEK;
                }
                else
                  if (_rtw_memcmp (pIE->data, AIRGOCAP_OUI, 3) )
                  {
                    DBG_871X ("link to Airgo Cap\n");
                    return HT_IOT_PEER_AIRGO;
                  }
                  else
                    if (_rtw_memcmp (pIE->data, EPIGRAM_OUI, 3) )
                    {
...
void update_capinfo (PADAPTER Adapter, u16 updateCap)
{
  ...
  else
  {
    if ( pmlmeext->cur_wireless_mode & (WIRELESS_11G | WIRELESS_11_24N) )
    {
      ...
      if ( pmlmeext->cur_wireless_mode & (WIRELESS_11A | WIRELESS_11_5N) )
      {
        pmlmeinfo->slotTime = SHORT_SLOT_TIME;
      }
      else
