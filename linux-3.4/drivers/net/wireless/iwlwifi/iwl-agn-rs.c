  ...
  if (info->flags & IEEE80211_TX_STAT_AMPDU) {
    ...
    for (i = 0; i <= retries; ++i) {
      ...
      { tmp_tbl = curr_tbl; }
        ...
        if (table_type_matches (&tbl_type, other_tbl) )
        { tmp_tbl = other_tbl; }
        else
        { continue; }
                                       ...
                                       struct iwl_scale_tbl_info * tbl)
{
  ...
  { ht_tbl_pointer = expected_tpt_siso20MHz; }
    ...
    { ht_tbl_pointer = expected_tpt_siso40MHz; }
      ...
      if (is_mimo2 (tbl->lq_type) && (!tbl->is_ht40 || lq_sta->is_dup) )
      { ht_tbl_pointer = expected_tpt_mimo2_20MHz; }
      else
        if (is_mimo2 (tbl->lq_type) )
        { ht_tbl_pointer = expected_tpt_mimo2_40MHz; }
        else
          if (is_mimo3 (tbl->lq_type) && (!tbl->is_ht40 || lq_sta->is_dup) )
  ...
  { tbl->expected_tpt = ht_tbl_pointer[0]; }
    ...
    if (tbl->is_SGI && !lq_sta->is_agg)  /* SGI */
    { tbl->expected_tpt = ht_tbl_pointer[1]; }
    else
      if (!tbl->is_SGI && lq_sta->is_agg)  /* AGG */
  ...
  for (; ;) {
    ...
    case IWL_LEGACY_SWITCH_MIMO2_BC:
      ...
      { search_tbl->ant_type = ANT_AB; }
        ...
        if (tbl->action == IWL_LEGACY_SWITCH_MIMO2_AC)
  ...
  for (;;) {
    ...
    case IWL_SISO_SWITCH_MIMO2_BC:
      ...
      { search_tbl->ant_type = ANT_AB; }
        ...
        if (tbl->action == IWL_SISO_SWITCH_MIMO2_AC)
        { search_tbl->ant_type = ANT_AC; }
        else
        { search_tbl->ant_type = ANT_BC; }
  ...
  for (;;) {
    ...
    case IWL_MIMO3_SWITCH_MIMO2_BC:
      ...
      { search_tbl->ant_type = ANT_AB; }
        ...
        if (tbl->action == IWL_MIMO3_SWITCH_MIMO2_AC)
        { search_tbl->ant_type = ANT_AC; }
        else
        { search_tbl->ant_type = ANT_BC; }
...
void iwl_rs_rate_init (struct iwl_priv * priv, struct ieee80211_sta * sta, u8 sta_id)
{
  ...
  if (!lq_sta->lq.general_params.dual_stream_ant_msk) {
    ...
    if (num_of_ant (hw_params (priv).valid_tx_ant) == 2) {
      lq_sta->lq.general_params.dual_stream_ant_msk =
        hw_params (priv).valid_tx_ant;
    }
                              ...
                              struct iwl_lq_sta * lq_sta, u32 new_rate)
{
  ...
  if (num_of_ant (tbl_type.ant_type) == 1) {
    ...
    if (num_of_ant (tbl_type.ant_type) == 2) {
