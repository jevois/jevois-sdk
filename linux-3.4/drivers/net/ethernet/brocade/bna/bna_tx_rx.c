...
bna_rx_put (struct bna_rx_mod * rx_mod, struct bna_rx * rx)
{
  ...
  if (prev_qe == NULL) {
    ...
    if (bfa_q_next (prev_qe) == &rx_mod->rx_free_q) {
                 ...
                 void (*cbfn) (struct bnad *, struct bna_rx *) )
{
  ...
  if (is_promisc_enable (new_mode, bitmask) ) {
    ...
    if (is_promisc_disable (new_mode, bitmask) ) {
  ...
  if (is_allmulti_enable (new_mode, bitmask) ) {
    ...
    if (is_allmulti_disable (new_mode, bitmask) ) {
...
bna_rx_dim_update (struct bna_ccb * ccb)
{
  ...
  { load = BNA_LOAD_T_LOW_4; }
    ...
    if (pkt_rt < BNA_PKT_RATE_20K)
    { load = BNA_LOAD_T_LOW_3; }
    else
      if (pkt_rt < BNA_PKT_RATE_30K)
      { load = BNA_LOAD_T_LOW_2; }
      else
        if (pkt_rt < BNA_PKT_RATE_40K)
        { load = BNA_LOAD_T_LOW_1; }
        else
          if (pkt_rt < BNA_PKT_RATE_50K)
          { load = BNA_LOAD_T_HIGH_1; }
          else
            if (pkt_rt < BNA_PKT_RATE_60K)
            { load = BNA_LOAD_T_HIGH_2; }
            else
              if (pkt_rt < BNA_PKT_RATE_80K)
...
bna_tx_free (struct bna_tx * tx)
{
  ...
  if (prev_qe == NULL) {
    ...
    if (bfa_q_next (prev_qe) == &tx_mod->tx_free_q) {
      /* This is the last entry */
