                  ...
                  struct pch_gbe_tx_ring * tx_ring)
{
  ...
  while ( (tx_desc->gbec_status & DSC_INIT16) == 0x0000) {
    ...
    if ( (tx_desc->gbec_status & PCH_GBE_TXD_GMAC_STAT_ABT) ) {
      ...
      if ( (tx_desc->gbec_status & PCH_GBE_TXD_GMAC_STAT_CRSER)
         ) {
        adapter->stats.tx_carrier_errors++;
        pr_err ("Transfer Carrier Sense Error\n");
      }
      else
        if ( (tx_desc->gbec_status & PCH_GBE_TXD_GMAC_STAT_EXCOL)
           ) {
          adapter->stats.tx_aborted_errors++;
          ...
          if ( (tx_desc->gbec_status &
            ...
            if ( (tx_desc->gbec_status & PCH_GBE_TXD_GMAC_STAT_CMPLT)
               ) {
              adapter->stats.tx_packets++;
              adapter->stats.tx_bytes += skb->len;
            }
...
static int pch_gbe_change_mtu (struct net_device * netdev, int new_mtu)
{
  ...
  { adapter->rx_buffer_len = PCH_GBE_FRAME_SIZE_2048; }
    ...
    if (max_frame <= PCH_GBE_FRAME_SIZE_4096)
    { adapter->rx_buffer_len = PCH_GBE_FRAME_SIZE_4096; }
    else
      if (max_frame <= PCH_GBE_FRAME_SIZE_8192)
