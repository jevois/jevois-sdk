...
static int __devinit emac_init_phy (struct emac_instance * dev)
{
  ...
  if (dev->phy.features & SUPPORTED_Autoneg) {
    ...
    if (f & SUPPORTED_1000baseT_Full) {
      ...
      if (f & SUPPORTED_1000baseT_Half)
      { speed = SPEED_1000; }
      else
        if (f & SUPPORTED_100baseT_Full) {
          speed = SPEED_100;
          fd = DUPLEX_FULL;
        }
        else
          if (f & SUPPORTED_100baseT_Half)
          { speed = SPEED_100; }
          else
