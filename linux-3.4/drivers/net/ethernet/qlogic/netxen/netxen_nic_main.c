...
nx_update_dma_mask (struct netxen_adapter * adapter)
{
  ...
  { change = 1; }
    ...
    if ( (adapter->ahw.revision_id == NX_P2_C1) && (shift <= 4) )
    { change = 1; }
...
static void netxen_set_port_mode (struct netxen_adapter * adapter)
{
  ...
  if ( (val == NETXEN_BRDTYPE_P3_HMEZ) ||
    ...
    if (port_mode == NETXEN_PORT_MODE_802_3_AP) {
      ...
      if (port_mode == NETXEN_PORT_MODE_XG) {
        data = NETXEN_PORT_MODE_XG;
        NXWR32 (adapter, NETXEN_PORT_MODE_ADDR, data);
      }
      else
        if (port_mode == NETXEN_PORT_MODE_AUTO_NEG_1G) {
          data = NETXEN_PORT_MODE_AUTO_NEG_1G;
          NXWR32 (adapter, NETXEN_PORT_MODE_ADDR, data);
        }
        else
          if (port_mode == NETXEN_PORT_MODE_AUTO_NEG_XG) {
...
netxen_setup_pci_map (struct netxen_adapter * adapter)
{
  ...
  if (mem_len == NETXEN_PCI_128MB_SIZE) {
    ...
    if (mem_len == NETXEN_PCI_32MB_SIZE) {
      ...
      if (mem_len == NETXEN_PCI_2MB_SIZE) {
...
nx_dev_request_reset (struct netxen_adapter * adapter)
{
  ...
  { ret = 0; }
    ...
    if (state != NX_DEV_INITALIZING && state != NX_DEV_NEED_AER) {
