...
static u32 mii_check_media_mode (struct mac_regs __iomem * regs)
{
  ...
  { status |= VELOCITY_SPEED_1000 | VELOCITY_DUPLEX_FULL; }
    ...
    if (MII_REG_BITS_IS_ON (ADVERTISE_1000HALF, MII_CTRL1000, regs) )
    { status |= (VELOCITY_SPEED_1000); }
      ...
      { status |= (VELOCITY_SPEED_100 | VELOCITY_DUPLEX_FULL); }
        ...
        if (ANAR & ADVERTISE_100HALF)
        { status |= VELOCITY_SPEED_100; }
        else
          if (ANAR & ADVERTISE_10FULL)
          { status |= (VELOCITY_SPEED_10 | VELOCITY_DUPLEX_FULL); }
          else
          { status |= (VELOCITY_SPEED_10); }
    }
...
static int velocity_set_media_mode (struct velocity_info * vptr, u32 mii_status)
{
  ...
  if (mii_status & VELOCITY_AUTONEG_ENABLE) {
    ...
    if (mii_status & VELOCITY_SPEED_100) {
      ...
      if (mii_status & VELOCITY_SPEED_10) {
        if (mii_status & VELOCITY_DUPLEX_FULL)
  ...
  if (vptr->mii_status & VELOCITY_LINK_FAIL) {
    ...
    if (vptr->options.spd_dpx == SPD_DPX_AUTO) {
      ...
      { VELOCITY_PRT (MSG_LEVEL_INFO, " speed 1000M bps"); }
        ...
        if (vptr->mii_status & VELOCITY_SPEED_100)
...
static void velocity_set_multi (struct net_device * dev)
{
  ...
  if (dev->flags & IFF_PROMISC) { /* Set promiscuous. */
    ...
    if ( (netdev_mc_count (dev) > vptr->multicast_limit) ||
                                  ...
                                  struct ethtool_cmd * cmd)
{
  ...
  if ( (new_status & VELOCITY_AUTONEG_ENABLE) &&
    ...
    { spd_dpx = SPD_DPX_AUTO; }
      ...
      if ( (new_status & VELOCITY_SPEED_1000) &&
        ...
        if (new_status & VELOCITY_SPEED_100)
          ...
          if (new_status & VELOCITY_SPEED_10)
            spd_dpx = (new_status & VELOCITY_DUPLEX_FULL) ?
