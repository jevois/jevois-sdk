  ...
  efx_for_each_channel (channel, efx) {
    efx_for_each_channel_tx_queue (tx_queue, channel) {
      ...
      if (EFX_OWORD_FIELD (txd_ptr_tbl,
        ...
        if (atomic_cmpxchg (&tx_queue->flush_outstanding,
                            1, 0) ) {
...
efx_handle_tx_event (struct efx_channel * channel, efx_qword_t * event)
{
  ...
  if (likely (EFX_QWORD_FIELD (*event, FSF_AZ_TX_EV_COMP) ) ) {
    ...
    if (EFX_QWORD_FIELD (*event, FSF_AZ_TX_EV_WQ_FF_FULL) ) {
      ...
      if (EFX_QWORD_FIELD (*event, FSF_AZ_TX_EV_PKT_ERR) &&
...
efx_handle_generated_event (struct efx_channel * channel, efx_qword_t * event)
{
  ...
  if (magic == EFX_CHANNEL_MAGIC_TEST (channel) ) {
    ...
    if (rx_queue && magic == EFX_CHANNEL_MAGIC_FILL (rx_queue) ) {
      ...
      if (rx_queue && magic == EFX_CHANNEL_MAGIC_RX_DRAIN (rx_queue) ) {
