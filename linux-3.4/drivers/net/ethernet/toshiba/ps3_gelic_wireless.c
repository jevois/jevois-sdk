                              ...
                              union iwreq_data * iwreq, char * extra)
{
  ...
  case IW_AUTH_80211_AUTH_ALG:
    ...
    { param->value = IW_AUTH_ALG_SHARED_KEY; }
      ...
      if (wl->auth_method == GELIC_EURUS_AUTH_OPEN)
...
struct gelic_wl_scan_info * gelic_wl_find_best_bss (struct gelic_wl_info * wl)
{
  ...
  list_for_each_entry (scan_info, &wl->network_list, list) {
    ...
    if (wl->wpa_level == GELIC_WL_WPA_LEVEL_WPA2) {
      ...
      if (wl->wpa_level == GELIC_WL_WPA_LEVEL_WPA) {
        ...
        if (wl->wpa_level == GELIC_WL_WPA_LEVEL_NONE &&
            wl->group_cipher_method == GELIC_WL_CIPHER_WEP) {
...
static const char * eventstr (enum gelic_lv1_wl_event event)
{
  ...
  { ret = "EURUS_READY"; }
    ...
    if (event & GELIC_LV1_WL_EVENT_SCAN_COMPLETED)
    { ret = "SCAN_COMPLETED"; }
    else
      if (event & GELIC_LV1_WL_EVENT_DEAUTH)
      { ret = "DEAUTH"; }
      else
        if (event & GELIC_LV1_WL_EVENT_BEACON_LOST)
        { ret = "BEACON_LOST"; }
        else
          if (event & GELIC_LV1_WL_EVENT_CONNECTED)
          { ret = "CONNECTED"; }
          else
            if (event & GELIC_LV1_WL_EVENT_WPA_CONNECTED)
            { ret = "WPA_CONNECTED"; }
            else
              if (event & GELIC_LV1_WL_EVENT_WPA_ERROR)
              { ret = "WPA_ERROR"; }
              else {
