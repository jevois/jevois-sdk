...
static irqreturn_t at91ether_phy_interrupt (int irq, void * dev_id)
{
  ...
  if ( (lp->phy_type == MII_DM9161_ID) || (lp->phy_type == MII_DM9161A_ID) ) {
    ...
    if (lp->phy_type == MII_LXT971A_ID) {
      read_phy (lp->phy_address, MII_ISINTS_REG, &phy); /* ack interrupt in Intel PHY */
      if (! (phy & (1 << 2) ) )
      { goto done; }
    }
    else
      if (lp->phy_type == MII_BCM5221_ID) {
        read_phy (lp->phy_address, MII_BCMINTR_REG, &phy); /* ack interrupt in Broadcom PHY */
        if (! (phy & (1 << 0) ) )
        { goto done; }
      }
      else
        if (lp->phy_type == MII_KS8721_ID) {
          read_phy (lp->phy_address, MII_TPISTATUS, &phy);  /* ack interrupt in Micrel PHY */
          if (! (phy & ( (1 << 2) | 1) ) )
          { goto done; }
        }
        else
          if (lp->phy_type == MII_T78Q21x3_ID) {     /* ack interrupt in Teridian PHY */
            read_phy (lp->phy_address, MII_T78Q21INT_REG, &phy);
            if (! (phy & ( (1 << 2) | 1) ) )
            { goto done; }
          }
          else
            if (lp->phy_type == MII_DP83848_ID) {
...
static void enable_phyirq (struct net_device * dev)
{
  ...
  if ( (lp->phy_type == MII_DM9161_ID) || (lp->phy_type == MII_DM9161A_ID) ) { /* for Davicom PHY */
    ...
    if (lp->phy_type == MII_LXT971A_ID) {  /* for Intel PHY */
      read_phy (lp->phy_address, MII_ISINTE_REG, &dsintr);
      dsintr = dsintr | 0xf2;     /* set bits 1, 4..7 */
      write_phy (lp->phy_address, MII_ISINTE_REG, dsintr);
    }
    else
      if (lp->phy_type == MII_BCM5221_ID) {  /* for Broadcom PHY */
        dsintr = (1 << 15) | ( 1 << 14);
        write_phy (lp->phy_address, MII_BCMINTR_REG, dsintr);
      }
      else
        if (lp->phy_type == MII_KS8721_ID) { /* for Micrel PHY */
          dsintr = (1 << 10) | ( 1 << 8);
          write_phy (lp->phy_address, MII_TPISTATUS, dsintr);
        }
        else
          if (lp->phy_type == MII_T78Q21x3_ID) { /* for Teridian PHY */
            read_phy (lp->phy_address, MII_T78Q21INT_REG, &dsintr);
            dsintr = dsintr | 0x500;    /* set bits 8, 10 */
            write_phy (lp->phy_address, MII_T78Q21INT_REG, dsintr);
          }
          else
            if (lp->phy_type == MII_DP83848_ID) {  /* National Semiconductor DP83848 PHY */
...
static void at91ether_set_multicast_list (struct net_device * dev)
{
  ...
  { cfg |= AT91_EMAC_CAF; }
    ...
    if (dev->flags & (~IFF_PROMISC) )  /* Disable promiscuous mode */
    { cfg &= ~AT91_EMAC_CAF; }
  ...
  if (dev->flags & IFF_ALLMULTI) {    /* Enable all multicast mode */
    ...
    if (!netdev_mc_empty (dev) ) { /* Enable specific multicasts */
      at91ether_sethashtable (dev);
      cfg |= AT91_EMAC_MTI;
    }
    else
      if (dev->flags & (~IFF_ALLMULTI) ) { /* Disable all multicast mode */
                                   ...
                                   struct platform_device * pdev, struct clk * ether_clk)
{
  ...
  { printk (KERN_INFO "%s: Davicom 9161 PHY %s\n", dev->name, (lp->phy_media == PORT_FIBRE) ? "(Fiber)" : "(Copper)"); }
    ...
    { printk (KERN_INFO "%s: Intel LXT971A PHY\n", dev->name); }
      ...
      if (phy_type == MII_RTL8201_ID)
      { printk (KERN_INFO "%s: Realtek RTL8201(B)L PHY\n", dev->name); }
      else
        if (phy_type == MII_BCM5221_ID)
        { printk (KERN_INFO "%s: Broadcom BCM5221 PHY\n", dev->name); }
        else
          if (phy_type == MII_DP83847_ID)
          { printk (KERN_INFO "%s: National Semiconductor DP83847 PHY\n", dev->name); }
          else
            if (phy_type == MII_DP83848_ID)
            { printk (KERN_INFO "%s: National Semiconductor DP83848 PHY\n", dev->name); }
            else
              if (phy_type == MII_AC101L_ID)
              { printk (KERN_INFO "%s: Altima AC101L PHY\n", dev->name); }
              else
                if (phy_type == MII_KS8721_ID)
                { printk (KERN_INFO "%s: Micrel KS8721 PHY\n", dev->name); }
                else
                  if (phy_type == MII_T78Q21x3_ID)
                  { printk (KERN_INFO "%s: Teridian 78Q21x3 PHY\n", dev->name); }
                  else
                    if (phy_type == MII_LAN83C185_ID)
