...
static int mii_rw (struct net_device * dev, int addr, int miireg, int value)
{
  ...
  if (reg_delay (dev, NvRegMIIControl, NVREG_MIICTL_INUSE, 0,
    ...
    if (value != MII_READ) {
      ...
      if (readl (base + NvRegMIIStatus) & NVREG_MIISTAT_ERROR) {
        retval = -1;
      }
      else {
      ...
      if (likely (flags & NV_RX2_DESCRIPTORVALID) ) {
        ...
        if (unlikely (flags & NV_RX2_ERROR) ) {
          ...
          /* framing errors are soft errors */
            ...
            if ( (flags & NV_RX2_ERROR_MASK) == NV_RX2_FRAMINGERR) {
  ...
  if (mii_status & PHY_GIGABIT) {
    ...
    { phyreg |= NVREG_SLOTTIME_10_100_FULL; }
      ...
      { phyreg |= NVREG_SLOTTIME_10_100_FULL; }
        ...
        if ( (np->linkspeed & 0xFFF) == NVREG_LINKSPEED_1000)
...
static int nv_update_linkspeed (struct net_device * dev)
{
  ...
  if (np->autoneg == 0) {
    if (np->fixed_mode & LPA_100FULL) {
      ...
      if (np->fixed_mode & LPA_100HALF) {
        newls = NVREG_LINKSPEED_FORCE | NVREG_LINKSPEED_100;
        newdup = 0;
      }
      else
        if (np->fixed_mode & LPA_10FULL) {
          newls = NVREG_LINKSPEED_FORCE | NVREG_LINKSPEED_10;
          newdup = 1;
        }
        else {
...
static int __devinit nv_probe (struct pci_dev * pci_dev, const struct pci_device_id * id)
{
  ...
  { np->register_size = NV_PCI_REGSZ_VER3; }
    ...
    if (id->driver_data & DEV_HAS_STATISTICS_V1)
