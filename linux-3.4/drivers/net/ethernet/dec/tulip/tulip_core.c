  ...
  if (dev->if_port) {
    ...
    if (tp->chip_id == DC21142) {
      ...
      if (tp->chip_id == PNIC2) {
        ...
        if (tp->chip_id == LC82C168  &&  ! tp->medialock) {
          if (tp->mii_cnt) {
            dev->if_port = 11;
            ...
            if (ioread32 (ioaddr + CSR5) & TPLnkPass)
            { pnic_do_nway (dev); }
            else {
          ...
          if ( (tp->chip_id == MX98713 || tp->chip_id == COMPEX9881) &&
               ! tp->medialock) {
            dev->if_port = 0;
            ...
            if (tp->chip_id == MX98715 || tp->chip_id == MX98725) {
              ...
              if (tp->chip_id == COMET || tp->chip_id == CONEXANT) {
...
static void tulip_tx_timeout (struct net_device * dev)
{
  ...
  if (tulip_media_cap[dev->if_port] & MediaIsMII) {
    ...
    if (tp->chip_id == DC21140 || tp->chip_id == DC21142 ||
                                     ...
                                     const struct pci_device_id * ent)
{
  ...
  if (pdev->subsystem_vendor == PCI_VENDOR_ID_LMC) {
    ...
    if (pdev->subsystem_vendor == PCI_VENDOR_ID_SBE &&
