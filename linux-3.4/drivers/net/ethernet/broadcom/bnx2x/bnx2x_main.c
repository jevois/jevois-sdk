...
static inline void bnx2x_handle_rx_mode_eqe (struct bnx2x * bp)
{
  ...
  { bnx2x_set_storm_rx_mode (bp); }
    ...
    if (test_and_clear_bit (BNX2X_FILTER_ISCSI_ETH_START_SCHED,
                            &bp->sp_state) )
    { bnx2x_set_iscsi_eth_rx_mode (bp, true); }
    else
      if (test_and_clear_bit (BNX2X_FILTER_ISCSI_ETH_STOP_SCHED,
                              &bp->sp_state) )
...
static void bnx2x_init_internal_common (struct bnx2x * bp)
{
  ...
  if (IS_MF_SI (bp) )
    ...
    if (!CHIP_IS_E1 (bp) ) /* 57710 doesn't support MF */
      REG_WR8 (bp, BAR_TSTRORM_INTMEM +
...
static void bnx2x_setup_fan_failure_detection (struct bnx2x * bp)
{
   ...
   */
    ...
    if (val == SHARED_HW_CFG_FAN_FAILURE_PHY_TYPE)
...
static void __devinit bnx2x_get_port_hwinfo (struct bnx2x * bp)
{
  ...
  { bp->mdio.prtad = bp->port.phy_addr; }
    ...
    if ( (ext_phy_type != PORT_HW_CFG_XGXS_EXT_PHY_TYPE_FAILURE) &&
