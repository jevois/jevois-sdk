                   ...
                   struct link_vars * vars, u8 mode, u32 speed)
{
  ...
  case LED_MODE_OPER:
    /*
     * For all other phys, OPER mode is same as ON, so in case  * link is down, do nothing
     */
    if (!vars->link_up)
    { break; }
  case LED_MODE_ON:
...
int bnx2x_link_update (struct link_params * params, struct link_vars * vars)
{
  ...
  if (vars->phy_link_up) {
    if (! (SINGLE_MEDIA_DIRECT (params) ) && ext_phy_link_up &&
      ...
      if (prev_line_speed != vars->line_speed) {
                                  ...
                                  struct link_vars * vars)
{
  ...
  if ( (link_status & (1 << 2) ) && (! (link_status & (1 << 15) ) ) ) {
    ...
    if ( (link_status & (1 << 1) ) && (! (link_status & (1 << 14) ) ) ) {
      link_up = 1;
      vars->line_speed = SPEED_2500;
      ...
      if ( (link_status & (1 << 0) ) && (! (link_status & (1 << 13) ) ) ) {
        link_up = 1;
        vars->line_speed = SPEED_1000;
                                ...
                                struct link_params * params)
{
  ...
  if (bnx2x_get_edc_mode (phy, params, &edc_mode) != 0) {
    ...
    if (bnx2x_verify_sfp_module (phy, params) != 0) {
