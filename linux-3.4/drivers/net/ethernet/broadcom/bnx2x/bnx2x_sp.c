                                  ...
                                  int cam_offset)
{
  ...
  if (cmd != BNX2X_VLAN_MAC_MOVE) {
    if (test_bit (BNX2X_ISCSI_ETH_MAC, vlan_mac_flags) )
      ...
      if (test_bit (BNX2X_ETH_MAC, vlan_mac_flags) )
        bnx2x_set_mac_in_nig (bp, add, mac,
  ...
  if (pos) {
    ...
    if (!test_bit (BNX2X_DONT_CONSUME_CAM_CREDIT,
      ...
      if ( (query.cmd_data.vlan_mac.cmd ==
        ...
        if (!o->get_credit (o) ) { /* VLAN_MAC_DEL */
                            ...
                            struct bnx2x_config_rss_params * p)
{
  ...
  { rss_mode = ETH_RSS_MODE_DISABLED; }
    ...
    if (test_bit (BNX2X_RSS_MODE_REGULAR, &p->rss_flags) )
    { rss_mode = ETH_RSS_MODE_REGULAR; }
    else
      if (test_bit (BNX2X_RSS_MODE_VLAN_PRI, &p->rss_flags) )
      { rss_mode = ETH_RSS_MODE_VLAN_PRI; }
      else
        if (test_bit (BNX2X_RSS_MODE_E1HOV_PRI, &p->rss_flags) )
        { rss_mode = ETH_RSS_MODE_E1HOV_PRI; }
        else
          if (test_bit (BNX2X_RSS_MODE_IP_DSCP, &p->rss_flags) )
    ...
    { next_state = BNX2X_Q_STATE_INACTIVE; }
      ...
      { next_state = BNX2X_Q_STATE_ACTIVE; }
        ...
        if (cmd == BNX2X_Q_CMD_SETUP_TX_ONLY) {
          ...
          if (cmd == BNX2X_Q_CMD_HALT)
          { next_state = BNX2X_Q_STATE_STOPPED; }
          
          else
            if (cmd == BNX2X_Q_CMD_UPDATE) {
