  ...
  case ISCSI_UEVENT_PATH_UPDATE: {
      ...
      if (cnic_in_use (csk) &&
        ...
        if (is_valid_ether_addr (csk->ha) ) {
          ...
          if (!test_bit (SK_F_OFFLD_SCHED, &csk->flags) &&
...
static void cnic_bnx2x_kwqe_err (struct cnic_dev * dev, struct kwqe * kwqe)
{
  ...
  if (layer_code == KWQE_FLAGS_LAYER_MASK_L5_FCOE) {
    ...
    if (opcode == FCOE_KWQE_OPCODE_DISABLE_CONN) {
      ...
      if (opcode == FCOE_KWQE_OPCODE_DESTROY) {
    ...
    if (layer_code == KWQE_FLAGS_LAYER_MASK_L5_ISCSI) {
      ...
      if (layer_code == KWQE_FLAGS_LAYER_MASK_L4) {
        ...
        { kcqe_op = L4_KCQE_OPCODE_VALUE_CONNECT_COMPLETE; }
          ...
          if (opcode == L4_KWQE_OPCODE_VALUE_RESET)
          { kcqe_op = L4_KCQE_OPCODE_VALUE_RESET_COMP; }
          else
            if (opcode == L4_KWQE_OPCODE_VALUE_CLOSE)
  ...
  while (num_cqes) {
    ...
    { ulp_type = CNIC_ULP_RDMA; }
      ...
      { ulp_type = CNIC_ULP_ISCSI; }
        ...
        if (kcqe_layer == KCQE_FLAGS_LAYER_MASK_L5_FCOE)
        { ulp_type = CNIC_ULP_FCOE; }
        else
          if (kcqe_layer == KCQE_FLAGS_LAYER_MASK_L4)
          { ulp_type = CNIC_ULP_L4; }
          else
            if (kcqe_layer == KCQE_FLAGS_LAYER_MASK_L2)
            { goto end; }
            else {
    ...
    int ulp_type)
{
  ...
  { err = cnic_get_v4_route (dst_addr, &dst); }
    ...
    if (dst_addr->sin_family == AF_INET6) {
                            ...
                            u8 opcode)
{
  ...
  if (ulp_ops) {
    ...
    { ulp_ops->cm_connect_complete (csk); }
      ...
      if (opcode == L4_KCQE_OPCODE_VALUE_CLOSE_COMP)
      { ulp_ops->cm_close_complete (csk); }
      else
        if (opcode == L4_KCQE_OPCODE_VALUE_RESET_RECEIVED)
        { ulp_ops->cm_remote_abort (csk); }
        else
          if (opcode == L4_KCQE_OPCODE_VALUE_RESET_COMP)
          { ulp_ops->cm_abort_complete (csk); }
          else
            if (opcode == L4_KCQE_OPCODE_VALUE_CLOSE_RECEIVED)
