  ...
  if (local_adv & ADVERTISE_PAUSE_CAP) {
    if (local_adv & ADVERTISE_PAUSE_ASYM) {
      if (remote_adv & ADVERTISE_PAUSE_CAP) {
        ...
        if (remote_adv & ADVERTISE_PAUSE_ASYM) {
          bp->flow_ctrl = FLOW_CTRL_RX;
        }
    }
    else {
    ...
    if (common & ADVERTISE_1000FULL) {
      ...
      if (common & ADVERTISE_1000HALF) {
        ...
        if (common & ADVERTISE_100FULL) {
          ...
          if (common & ADVERTISE_100HALF) {
            bp->line_speed = SPEED_100;
            bp->duplex = DUPLEX_HALF;
          }
          else
            if (common & ADVERTISE_10FULL) {
              bp->line_speed = SPEED_10;
              bp->duplex = DUPLEX_FULL;
            }
            else
              if (common & ADVERTISE_10HALF) {
                bp->line_speed = SPEED_10;
                bp->duplex = DUPLEX_HALF;
              }
              else {
...
bnx2_phy_get_pause_adv (struct bnx2 * bp)
{
  ...
  if ( (bp->req_flow_ctrl & (FLOW_CTRL_RX | FLOW_CTRL_TX) ) ==
    ...
    if (bp->req_flow_ctrl & FLOW_CTRL_TX) {
      if (bp->phy_flags & BNX2_PHY_FLAG_SERDES) {
      ...
      if (bp->req_flow_ctrl & FLOW_CTRL_RX) {
        if (bp->phy_flags & BNX2_PHY_FLAG_SERDES) {
...
bnx2_5706_serdes_timer (struct bnx2 * bp)
{
  ...
  if (bp->serdes_an_pending) {
    ...
    if ( (bp->link_up == 0) && (bp->autoneg & AUTONEG_SPEED) ) {
      u32 bmcr;
      ...
      if ( (bp->link_up) && (bp->autoneg & AUTONEG_SPEED) &&
           (bp->phy_flags & BNX2_PHY_FLAG_PARALLEL_DETECT) ) {
        u32 phy2;
...
bnx2_get_5709_media (struct bnx2 * bp)
{
  ...
  { return; }
    ...
    if (bond_id == BNX2_MISC_DUAL_MEDIA_CTRL_BOND_ID_S) {
