...
static void tg3_switch_clocks (struct tg3 * tp)
{
  ...
  if (tg3_flag (tp, 5705_PLUS) ) {
    ...
    if ( (orig_clock_ctrl & CLOCK_CTRL_44MHZ_CORE) != 0) {
      tw32_wait_f (TG3PCI_CLOCK_CTRL,
                   clock_ctrl |
...
static u8 tg3_resolve_flowctrl_1000X (u16 lcladv, u16 rmtadv)
{
  ...
  if (lcladv & rmtadv & ADVERTISE_1000XPAUSE) {
    ...
    if (lcladv & rmtadv & ADVERTISE_1000XPSE_ASYM) {
...
out:
  ...
  if (tp->phy_flags & TG3_PHYFLG_BER_BUG) {
    ...
    if (tp->phy_flags & TG3_PHYFLG_JITTER_BUG) {
      if (!tg3_phy_toggle_auxctl_smdsp (tp, true) ) {
...
static int tg3_setup_phy (struct tg3 * tp, int force_reset)
{
  ...
  if (GET_CHIP_REV (tp->pci_chip_rev_id) == CHIPREV_5784_AX) {
    ...
    { scale = 65; }
      ...
      if (val == CPMU_CLCK_STAT_MAC_CLCK_6_25)
      { scale = 6; }
      else
      { scale = 12; }
...
static int tg3_phy_lpbk_set (struct tg3 * tp, u32 speed, bool extlpbk)
{
  ...
  if (GET_ASIC_REV (tp->pci_chip_rev_id) == ASIC_REV_5700) {
    ...
    { mac_mode &= ~MAC_MODE_LINK_POLARITY; }
      ...
      if (masked_phy_id == TG3_PHY_ID_BCM5411)
      { mac_mode |= MAC_MODE_LINK_POLARITY; }
...
static void tg3_ints_init (struct tg3 * tp)
{
  ...
  { tg3_flag_set (tp, USING_MSIX); }
    ...
    if (tg3_flag (tp, SUPPORT_MSI) && pci_enable_msi (tp->pdev) == 0)
...
static int tg3_test_nvram (struct tg3 * tp)
{
  ...
  { size = NVRAM_TEST_SIZE; }
    ...
    if ( (magic & TG3_EEPROM_MAGIC_FW_MSK) == TG3_EEPROM_MAGIC_FW) {
      ...
      if ( (magic & TG3_EEPROM_MAGIC_HW_MSK) == TG3_EEPROM_MAGIC_HW)
...
static void __devinit tg3_get_5755_nvram_info (struct tg3 * tp)
{
  ...
  case FLASH_5755VENDOR_ATMEL_FLASH_5:
    ...
    if (nvcfg1 == FLASH_5755VENDOR_ATMEL_FLASH_1 ||
      ...
      if (nvcfg1 == FLASH_5755VENDOR_ATMEL_FLASH_2)
  ...
  case FLASH_5752VENDOR_ST_M45PE40:
    ...
    if (nvcfg1 == FLASH_5752VENDOR_ST_M45PE10)
      ...
      if (nvcfg1 == FLASH_5752VENDOR_ST_M45PE20)
        tp->nvram_size = (protect ?
                          TG3_NVRAM_SIZE_64KB :
                          TG3_NVRAM_SIZE_256KB);
      else
  ...
  if (GET_ASIC_REV (tp->pci_chip_rev_id) == ASIC_REV_5717) {
    ...
    if (GET_ASIC_REV (tp->pci_chip_rev_id) == ASIC_REV_57780) {
      ...
      { strcpy (tp->board_part_number, "BCM57780"); }
        ...
        if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57760)
        { strcpy (tp->board_part_number, "BCM57760"); }
        else
          if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57790)
          { strcpy (tp->board_part_number, "BCM57790"); }
          else
            if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57788)
      ...
      if (GET_ASIC_REV (tp->pci_chip_rev_id) == ASIC_REV_57765) {
        ...
        { strcpy (tp->board_part_number, "BCM57761"); }
          ...
          if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57765)
          { strcpy (tp->board_part_number, "BCM57765"); }
          else
            if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57781)
            { strcpy (tp->board_part_number, "BCM57781"); }
            else
              if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57785)
              { strcpy (tp->board_part_number, "BCM57785"); }
              else
                if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57791)
                { strcpy (tp->board_part_number, "BCM57791"); }
                else
                  if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57795)
        ...
        if (GET_ASIC_REV (tp->pci_chip_rev_id) == ASIC_REV_57766) {
          ...
          { strcpy (tp->board_part_number, "BCM57762"); }
            ...
            if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57766)
            { strcpy (tp->board_part_number, "BCM57766"); }
            else
              if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57782)
              { strcpy (tp->board_part_number, "BCM57782"); }
              else
                if (tp->pdev->device == TG3PCI_DEVICE_TIGON3_57786)
...
static void __devinit tg3_read_mgmtfw_ver (struct tg3 * tp)
{
  ...
  { start = 0x08000000; }
    ...
    if (tg3_nvram_read (tp, offset - 4, &start) )
    { return; }
...
static void __devinit tg3_read_fw_ver (struct tg3 * tp)
{
  ...
  { tg3_read_bc_ver (tp); }
    ...
    { tg3_read_sb_ver (tp, val); }
      ...
      if ( (val & TG3_EEPROM_MAGIC_HW_MSK) == TG3_EEPROM_MAGIC_HW)
