...
ixgb_setup_rctl (struct ixgb_adapter * adapter)
{
  ...
  { rctl |= IXGB_RCTL_BSIZE_2048; }
    ...
    if (adapter->rx_buffer_len <= IXGB_RXBUFFER_4096)
    { rctl |= IXGB_RCTL_BSIZE_4096; }
    else
      if (adapter->rx_buffer_len <= IXGB_RXBUFFER_8192)
      { rctl |= IXGB_RCTL_BSIZE_8192; }
      else
        if (adapter->rx_buffer_len <= IXGB_RXBUFFER_16384)
