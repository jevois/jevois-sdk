...
static int __init media_fn (char * str)
{
  if (!strcmp (str, "rj45") ) { g_cs89x0_media__force = FORCE_RJ45; }
    ...
    if (!strcmp (str, "aui") ) { g_cs89x0_media__force = FORCE_AUI; }
    else
      if (!strcmp (str, "bnc") ) { g_cs89x0_media__force = FORCE_BNC; }
  return 1;
}
...
cs89x0_probe1 (struct net_device * dev, unsigned long ioaddr, int modular)
{
  ...
  /* allow them to force multiple transceivers.  If they force multiple, autosense */
  {
    ...
    if (count > 1)      {lp->adapter_cnf |= A_CNF_MEDIA_AUTO; }
      ...
      if (lp->force & FORCE_RJ45) {lp->adapter_cnf |= A_CNF_MEDIA_10B_T; }
      else
        if (lp->force & FORCE_AUI) {lp->adapter_cnf |= A_CNF_MEDIA_AUI; }
        else
          if (lp->force & FORCE_BNC) {lp->adapter_cnf |= A_CNF_MEDIA_10B_2; }
  }
    ...
    case ISQ_BUFFER_EVENT:
      ...
      if (status & TX_UNDERRUN) {
        ...
        if (lp->send_underrun == 3) { lp->send_cmd = TX_AFTER_381; }
          ...
          if (lp->send_underrun == 6) { lp->send_cmd = TX_AFTER_ALL; }
