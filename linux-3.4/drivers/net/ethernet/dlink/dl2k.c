    ...
    if (media[card_idx] != NULL) {
      ...
      if (strcmp (media[card_idx], "auto") == 0 ||
        ...
        if (strcmp (media[card_idx], "100mbps_fd") == 0 ||
          ...
          if (strcmp (media[card_idx], "100mbps_hd") == 0 ||
              strcmp (media[card_idx], "3") == 0) {
            np->speed = 100;
            np->full_duplex = 0;
          }
          else
            if (strcmp (media[card_idx], "10mbps_fd") == 0 ||
                strcmp (media[card_idx], "2") == 0) {
              np->speed = 10;
              np->full_duplex = 1;
            }
            else
              if (strcmp (media[card_idx], "10mbps_hd") == 0 ||
                  strcmp (media[card_idx], "1") == 0) {
                np->speed = 10;
                np->full_duplex = 0;
              }
              else
                if (strcmp (media[card_idx], "1000mbps_fd") == 0 ||
                    strcmp (media[card_idx], "6") == 0) {
                  np->speed = 1000;
                  np->full_duplex = 1;
                }
                else
                  if (strcmp (media[card_idx], "1000mbps_hd") == 0 ||
  ...
  while (1) {
    ...
    if (frame_status & RFS_Errors) {
      ...
      if (pkt_len > copy_thresh) {
        ...
        if ( (skb = netdev_alloc_skb_ip_align (dev, pkt_len) ) ) {
...
mii_get_media (struct net_device * dev)
{
  ...
  if (np->an_enable) {
    ...
    if (mscr & ADVERTISE_1000FULL && mssr & LPA_1000FULL) {
      ...
      if (mscr & ADVERTISE_1000HALF && mssr & LPA_1000HALF) {
        np->speed = 1000;
        np->full_duplex = 0;
        printk (KERN_INFO "Auto 1000 Mbps, Half duplex\n");
      }
      else
        if (negotiate & ADVERTISE_100FULL) {
          np->speed = 100;
          np->full_duplex = 1;
          printk (KERN_INFO "Auto 100 Mbps, Full duplex\n");
        }
        else
          if (negotiate & ADVERTISE_100HALF) {
            np->speed = 100;
            np->full_duplex = 0;
            printk (KERN_INFO "Auto 100 Mbps, Half duplex\n");
          }
          else
            if (negotiate & ADVERTISE_10FULL) {
              np->speed = 10;
              np->full_duplex = 1;
              printk (KERN_INFO "Auto 10 Mbps, Full duplex\n");
            }
            else
              if (negotiate & ADVERTISE_10HALF) {
                np->speed = 10;
                np->full_duplex = 0;
    ...
    if (negotiate & ADVERTISE_PAUSE_CAP) {
      ...
      if (negotiate & ADVERTISE_PAUSE_ASYM) {
        np->tx_flow = 0;
        np->rx_flow &= 1;
      }
...
mii_get_media_pcs (struct net_device * dev)
{
  ...
  if (np->an_enable) {
    ...
    if (negotiate & PCS_ANAR_PAUSE) {
      ...
      if (negotiate & PCS_ANAR_ASYMMETRIC) {
        np->tx_flow = 0;
        np->rx_flow &= 1;
      }
    /* else tx_flow, rx_flow = user select  */
