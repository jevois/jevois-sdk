...
static void ecm_fsm (struct s_smc * smc, int cmd)
{
  ...
  case EC0_OUT:
    ...
    /*EC05*/
      ...
      if (cmd == EC_CONNECT && (smc->e.path_test == PT_PASSED) &&
  ...
  case EC2_TRACE :
    ...
    /*EC23a*/
      ...
      /*EC23b*/
        ...
        if (smc->e.path_test == PT_PENDING) {
          GO_STATE (EC3_LEAVE) ;
          break ;
        }
        /*EC23c*/
        else
          if (cmd == EC_TIMEOUT_TMAX) {
...
static void prop_actions (struct s_smc * smc)
{
  ...
  if (smc->e.trace_prop & ENTITY_BIT (ENTITY_MAC) ) {
    ...
    if ( (smc->e.trace_prop & ENTITY_BIT (ENTITY_PHY (PA) ) ) &&
         port_out != PA) {
      ...
      if ( (smc->e.trace_prop & ENTITY_BIT (ENTITY_PHY (PB) ) ) &&
           port_out != PB) {
        /* trace propagate upstream */
