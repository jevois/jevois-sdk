  ...
  if ( mib->fddiPORTPCMState != oldstate ) {
    ...
    if ( mib->fddiPORTPCMState == PC0_OFF ) {
      ...
      if ( oldstate == PC0_OFF ) {
        ...
        if ( mib->fddiPORTPCMState != PC2_TRACE &&
             oldstate == PC8_ACTIVE ) {
          /* send third trap */
          snmp_fddi_trap (smc, 3, (int) mib->fddiPORTIndex );
        }
        else
          if ( mib->fddiPORTPCMState == PC8_ACTIVE ) {
            /* send fourth trap */
    ...
    if (errors <= 9)  { ber = 9 ; }
      ...
      if (errors <= 99) { ber = 8 ; }
        ...
        if (errors <= 999)  { ber = 7 ; }
          ...
          if (errors <= 9999) { ber = 6 ; }
          else
            if (errors <= 99999)  { ber = 5 ; }
            else
              if (errors <= 999999) { ber = 4 ; }
              else
                if (errors <= 9999999)  { ber = 3 ; }
                else
                  if (errors <= 99999999) { ber = 2 ; }
                  else
                    if (errors <= 999999999) { ber = 1 ; }
                    else        { ber = 0 ; }
...
static void pc_rcode_actions (struct s_smc * smc, int bit, struct s_phy * phy)
{
  ...
  case 3 :
    ...
    { mib->fddiPORTNeighborType = TA ; }
      ...
      if (phy->r_val[1] == 0 && phy->r_val[2] == 1)
      { mib->fddiPORTNeighborType = TB ; }
      else
        if (phy->r_val[1] == 1 && phy->r_val[2] == 0)
        { mib->fddiPORTNeighborType = TS ; }
        else
          if (phy->r_val[1] == 1 && phy->r_val[2] == 1)
...
/* int np;  PHY index */
{
  ...
  if (cmd & PL_PCM_CODE) { /* receive last sign.-bit | LCT complete */
    ...
    if (cmd & PL_PCM_ENABLED) { /* asserted SC_JOIN, scrub.completed*/
