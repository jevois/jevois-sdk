...
static int mca_default_procfn (char * buf, struct mca_device * mca_dev)
{
  ...
  if (slot < MCA_MAX_SLOT_NR) {
    ...
    if (slot == MCA_INTEGSCSI) {
      ...
      if (slot == MCA_INTEGVIDEO) {
        len += sprintf (buf + len, "Integrated Video Adapter\n");
      }
      else
        if (slot == MCA_MOTHERBOARD) {
...
void __init mca_do_proc_init (void)
{
  ...
  for (i = 0; i < MCA_NUMADAPTERS; i++) {
    ...
    if (i < MCA_MAX_SLOT_NR) { sprintf (mca_dev->procname, "slot%d", i + 1); }
      ...
      if (i == MCA_INTEGVIDEO) { sprintf (mca_dev->procname, "video"); }
      else
        if (i == MCA_INTEGSCSI) { sprintf (mca_dev->procname, "scsi"); }
        else
          if (i == MCA_MOTHERBOARD) { sprintf (mca_dev->procname, "planar"); }
