    ...
    if (oldn->volume_level != newn->volume_level) {
      ...
      if (oldn->volume_toggle != newn->volume_toggle) {
        ...
        { TPACPI_MAY_SEND_KEY (TP_ACPI_HOTKEYSCAN_VOLUMEDOWN); }
          ...
          if (newn->volume_level >= TP_NVRAM_LEVEL_VOLUME_MAX)
          { TPACPI_MAY_SEND_KEY (TP_ACPI_HOTKEYSCAN_VOLUMEUP); }
      }
  }
  ...
  if (oldn->brightness_level != newn->brightness_level) {
    ...
    if (oldn->brightness_toggle != newn->brightness_toggle) {
      ...
      { TPACPI_MAY_SEND_KEY (TP_ACPI_HOTKEYSCAN_FNEND); }
        ...
        if (newn->brightness_level >= bright_maxlvl
  ...
  while ( (cmd = next_cmd (&buf) ) ) {
    if (strlencmp (cmd, "enable") == 0) {
      ...
      if (strlencmp (cmd, "disable") == 0) {
        ...
        if (strlencmp (cmd, "reset") == 0) {
          mask = (hotkey_all_mask | hotkey_source_mask)
                 & ~hotkey_reserved_mask;
        }
        else
          if (sscanf (cmd, "0x%x", &mask) == 1) {
            /* mask set */
          }
          else
            if (sscanf (cmd, "%x", &mask) == 1) {
              /* mask set */
            }
            else {
              res = -EINVAL;
...
static int video_write (char * buf)
{
  ...
  while ( (cmd = next_cmd (&buf) ) ) {
    if (strlencmp (cmd, "lcd_enable") == 0) {
      ...
      if (strlencmp (cmd, "lcd_disable") == 0) {
        disable |= TP_ACPI_VIDEO_S_LCD;
      }
      else
        if (strlencmp (cmd, "crt_enable") == 0) {
          enable |= TP_ACPI_VIDEO_S_CRT;
        }
        else
          if (strlencmp (cmd, "crt_disable") == 0) {
            ...
            if (video_supported == TPACPI_VIDEO_NEW &&
              ...
              if (video_supported == TPACPI_VIDEO_NEW &&
                ...
                if (strlencmp (cmd, "auto_enable") == 0) {
                  res = video_autosw_set (1);
                  if (res)
                  { return res; }
                }
                else
                  if (strlencmp (cmd, "auto_disable") == 0) {
                    res = video_autosw_set (0);
                    if (res)
                    { return res; }
                  }
                  else
                    if (strlencmp (cmd, "video_switch") == 0) {
                      res = video_outputsw_cycle();
                      if (res)
                      { return res; }
                    }
                    else
                      if (strlencmp (cmd, "expand_toggle") == 0) {
...
static int __init brightness_init (struct ibm_init_struct * iibm)
{
  ...
  if (acpi_video_backlight_support() ) {
    ...
    if (tp_features.bright_acpimode && brightness_enable > 1) {
...
static int fan_write_cmd_level (const char * cmd, int * rc)
{
  ...
  { level = TP_EC_FAN_AUTO; }
    ...
    if ( (strlencmp (cmd, "level disengaged") == 0) |
