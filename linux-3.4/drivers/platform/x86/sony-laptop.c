...
static void sony_nc_notify (struct acpi_device * device, u32 event)
{
  ...
  if (ev >= 0x90) {
    ...
    if (key_handle) {
      ...
      if (sony_find_snc_handle (sony_rfkill_handle) == ev) {
...
static void sony_nc_backlight_setup (void)
{
  ...
  if (sony_find_snc_handle (0x12f) != -1) {
    ...
    if (sony_find_snc_handle (0x137) != -1) {
      ops = &sony_backlight_ng_ops;
      ...
      if (ACPI_SUCCESS (acpi_get_handle (sony_nc_acpi_handle, "GBRT",
                                         &unused) ) ) {
