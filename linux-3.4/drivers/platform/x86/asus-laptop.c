...
static int asus_rfkill_set (void * data, bool blocked)
{
  ...
  { return asus_wlan_set (asus, !blocked); }
    ...
    { return asus_bluetooth_set (asus, !blocked); }
      ...
      if (rfk->control_id == WM_RSTS)
      { return asus_wimax_set (asus, !blocked); }
      else
        if (rfk->control_id == WW_RSTS)
                                      ...
                                      int idx)
{
  ...
  if (asus->is_pega_lucid) {
    ...
    { supported = true; }
      ...
      if (attr == &dev_attr_ls_level.attr)
      { supported = false; }
      else
      { goto normal; }
...
normal:
  if (attr == &dev_attr_wlan.attr) {
    ...
    if (attr == &dev_attr_bluetooth.attr) {
      supported = !acpi_check_handle (handle, METHOD_BLUETOOTH, NULL);
      
    }
    else
      if (attr == &dev_attr_display.attr) {
        supported = !acpi_check_handle (handle, METHOD_SWITCH_DISPLAY, NULL);
        
      }
      else
        if (attr == &dev_attr_wimax.attr) {
          supported =
            !acpi_check_handle (asus->handle, METHOD_WIMAX, NULL);
            
        }
        else
          if (attr == &dev_attr_wwan.attr) {
            supported = !acpi_check_handle (asus->handle, METHOD_WWAN, NULL);
            
          }
          else
            if (attr == &dev_attr_ledd.attr) {
              ...
              if (attr == &dev_attr_ls_switch.attr ||
                ...
                if (attr == &dev_attr_ls_value.attr) {
                  supported = asus->is_pega_lucid;
                }
                else
                  if (attr == &dev_attr_gps.attr) {
...
static int __devinit asus_acpi_init (struct asus_laptop * asus)
{
  ...
  { asus->bled_type = TYPE_LED; }
    ...
    if (!strcmp (bled_type, "rfkill") )
    { asus->bled_type = TYPE_RFKILL; }
    
  if (!strcmp (wled_type, "led") )
  { asus->wled_type = TYPE_LED; }
  else
    if (!strcmp (wled_type, "rfkill") )
    { asus->wled_type = TYPE_RFKILL; }
