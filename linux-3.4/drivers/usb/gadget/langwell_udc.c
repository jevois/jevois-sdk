                        ...
                        u16 index, u16 length)
{
  ...
  if ( (request_type & USB_RECIP_MASK) == USB_RECIP_DEVICE) {
    ...
    if ( (request_type & USB_RECIP_MASK) == USB_RECIP_INTERFACE) {
      ...
      if ( (request_type & USB_RECIP_MASK) == USB_RECIP_ENDPOINT) {
        /* get endpoint status */
        ...
        if ( (setup->bRequestType & (USB_RECIP_MASK
          ...
          { break; }
            ...
            { dev->gadget.b_hnp_enable = 1; }
              ...
              if (setup->bRequest == USB_DEVICE_A_HNP_SUPPORT)
...
static void sram_init (struct langwell_udc * dev)
{
  ...
  if (pci_request_region (pdev, 1, kobject_name (&pdev->dev.kobj) ) ) {
    ...
    if (!dma_declare_coherent_memory (&pdev->dev, dev->sram_addr,
