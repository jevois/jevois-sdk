...
void imx_udc_reset (struct imx_udc_struct * imx_usb)
{
  ...
  do {}
  while (__raw_readl (imx_usb->base + USB_ENAB) & ENAB_RST);
  
  /* wait CFG bit to assert */
  do {}
  while (! (__raw_readl (imx_usb->base + USB_DADR) & DADR_CFG) );
...
void imx_udc_config (struct imx_udc_struct * imx_usb)
{
  ...
  for (j = 0; j < 5; j++) {
    ...
    do {}
    while (__raw_readl (imx_usb->base + USB_DADR) & DADR_BSY);
  }
  ...
  for (cfg = 1; cfg < 3; cfg++) {
    for (i = 1; i < IMX_USB_NB_EP; i++) {
      ...
      for (j = 0; j < 5; j++) {
        ...
        do {}
        while (__raw_readl (imx_usb->base
                            + USB_DADR)
               & DADR_BSY);
      }
    }
...
void imx_ep_stall (struct imx_ep_struct * imx_ep)
{
  ...
  if (!EP_NO (imx_ep) ) {
    ...
    __raw_writel (temp | CTRL_CMDOVER | CTRL_CMDERROR,
                  imx_usb->base + USB_CTRL);
    do { }
    while (__raw_readl (imx_usb->base + USB_CTRL)
           & CTRL_CMDOVER);
    temp = __raw_readl (imx_usb->base + USB_CTRL);
