  ...
  */
  ...
  if (b_requestType == (USB_DIR_OUT | USB_TYPE_VENDOR) ) {
    if (b_request == ACCESSORY_START) {
      ...
      if (b_request == ACCESSORY_SEND_STRING) {
        ...
        if (b_request == ACCESSORY_SET_AUDIO_MODE &&
            w_index == 0 && w_length == 0) {
          dev->audio_mode = w_value;
          value = 0;
        }
        else
          if (b_request == ACCESSORY_REGISTER_HID) {
            value = acc_register_hid (dev, w_value, w_index);
          }
          else
            if (b_request == ACCESSORY_UNREGISTER_HID) {
              value = acc_unregister_hid (dev, w_value);
            }
            else
              if (b_request == ACCESSORY_SET_HID_REPORT_DESC) {
                spin_lock_irqsave (&dev->lock, flags);
                ...
                if (b_request == ACCESSORY_SEND_HID_EVENT) {
                  spin_lock_irqsave (&dev->lock, flags);
    ...
    if (b_requestType == (USB_DIR_IN | USB_TYPE_VENDOR) ) {
