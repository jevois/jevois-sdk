...
uvc_function_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
  ...
  if (ctrl->bRequestType == 0x21)
  {
    ...
    struct uvc_request_data data; memset(&data, 0, sizeof(struct uvc_request_data));
    data.length = ctrl->wLength;

  
    req->length = data.length;
    req->zero = 0;///data->length < uvc->event_length;
  ...
  if (le16_to_cpu(ctrl->wLength) > UVC_MAX_REQUEST_SIZE) return -EINVAL;
  ...
  /* Tell the complete callback to generate an event for the next request that will be enqueued by uvc_event_write. */
