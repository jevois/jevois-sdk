                                    ...
                                    unsigned int delta)
{
  if (delta > 0) {
    ...
    if (queue_delayed_work (workqueue, &ring->scheduler, 0) )
    { return; }
  kref_put (&u132->kref, u132_hcd_delete);
}
      ...
      if (u132->going > 0) {
        ...
        if (!urb->unlinked) {
          ...
          if ( (condition_code == TD_CC_NOERROR) &&
            ...
            if ( (condition_code == TD_DATAUNDERRUN) &&
  ...
  if (ring->in_use) {
    ...
    if (ring->curr_endp) {
      ...
      if (last_endp->queue_next == last_endp->queue_last) {
        ...
        if ( (last_endp->delayed == 0) || time_after_eq (jiffies,
             last_endp->jiffies) ) {
          u132_endp_cancel_work (u132, last_endp);
          u132_endp_queue_work (u132, last_endp, 0);
          mutex_unlock (&u132->scheduler_lock);
    ...
    if (endp->active) {
      ...
      if (ring->in_use) {
        ...
        if (endp->queue_next == endp->queue_last) {
          ...
          if (endp->pipetype == PIPE_INTERRUPT) {
            u8 address = u132->addr[endp->usb_addr].address;
            if (ring->in_use) {
              mutex_unlock (&u132->scheduler_lock);
              u132_endp_put_kref (u132, endp);
...
extra:  {
  ...
  if (u132->flags & OHCI_QUIRK_SUPERIO) {
    ...
    if ( (u132->flags & OHCI_QUIRK_AMD756) || distrust_firmware) {
      roothub_a |= RH_A_NPS;
      retval = u132_write_pcimem (u132, roothub.a, roothub_a);
      if (retval)
      { return retval; }
    ...
    if (u132->going > 0) {
      ...
      if (hcd->self.controller) {
        ...
        if (vendor == PCI_VENDOR_ID_AMD && device == 0x740c) {
          ...
          if (vendor == PCI_VENDOR_ID_OPTI && device == 0xc861) {
            dev_err (&u132->platform_dev->dev, "WARNING: OPTi workar"
                     "ounds unavailable\n");
          }
          else
            if (vendor == PCI_VENDOR_ID_COMPAQ && device == 0xa0f8)
    ...
    if (u132->going > 0) {
      ...
      if (usb_pipetype (urb->pipe) == PIPE_INTERRUPT) {
        ...
        if (endp) {
          ...
          if (u132->num_endpoints == MAX_U132_ENDPS) {
            return -EINVAL;
          }
          else {    /*(endp == NULL) */
