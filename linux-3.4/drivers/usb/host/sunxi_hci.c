...
static int hci_clock_ctrl (struct sunxi_hci_hcd * sunxi_hci, u32 ohci, int enable)
{
  ...
  if (sunxi_hci->usbc_no == 1) {
    if (enable && usb1_open_hci_clock_cnt == 0) {
      set_hci_phy_ctrl_register (0x20002);
      for (i = 0; i < 0x10; i++);
      set_hci_sie_ctrl_register (0x20002);
      
    }
    else
      if (!enable && usb1_open_hci_clock_cnt == 1) {
        clear_hci_phy_ctrl_register (0x20002);
        for (i = 0; i < 0x10; i++);
        clear_hci_sie_ctrl_register (0x20002);
      }
      
    if (enable) {
      if (ohci) {
        set_hci_sie_ctrl_register (0x4);
      }
      usb1_open_hci_clock_cnt++;
    }
    else {
      if (ohci) {
        clear_hci_sie_ctrl_register (0x4);
      }
      usb1_open_hci_clock_cnt--;
    }
  }
  else
    if (sunxi_hci->usbc_no == 2) {
      if (enable && usb2_open_hci_clock_cnt == 0) {
        clear_hci_phy_ctrl_register (0x80008);
        set_hci_phy_ctrl_register (0x160406);
        for (i = 0; i < 0x10; i++);
        set_hci_sie_ctrl_register (0x40008);
        #ifdef CONFIG_USB_HCD_ENHANCE
        ...
        if (!enable && usb2_open_hci_clock_cnt == 1) {
          clear_hci_phy_ctrl_register (0xc040c);
          for (i = 0; i < 0x10; i++);
      ...
      if (sunxi_hci->usbc_no == 3) {
        if (enable && usb3_open_hci_clock_cnt == 0) {
        
          clear_hci_phy_ctrl_register (0x100410);
          set_hci_phy_ctrl_register (0x200020);
          for (i = 0; i < 0x10; i++);
          set_hci_sie_ctrl_register (0x80020);
          #ifdef CONFIG_USB_HCD_ENHANCE
          hci3_thread_run_flag = 1;
          #endif
        }
        else
          if (!enable && usb3_open_hci_clock_cnt == 1) {
            #ifdef CONFIG_USB_HCD_ENHANCE
            hci3_thread_run_flag = 0;
            #endif
            clear_hci_phy_ctrl_register (0x300430);
            for (i = 0; i < 0x10; i++);
            clear_hci_sie_ctrl_register (0x80020);
          }
          
        if (enable) {
          if (ohci) {
            set_hci_sie_ctrl_register (0x40);
          }
          ...
          usb3_open_hci_clock_cnt--;
          if (ohci) {
            clear_hci_sie_ctrl_register (0x40);
          }
        }
      }
      else {
...
static void hci_phy_ctrl (struct sunxi_hci_hcd * sunxi_hci, u32 enable)
{
  ...
  if (sunxi_hci->usbc_no == HCI0_USBC_NO) {
    if (enable && usb1_enable_phy_cnt == 0) {
      ...
      if (!enable && usb1_enable_phy_cnt == 1) {
    ...
    if (sunxi_hci->usbc_no == HCI1_USBC_NO) {
      if (enable && usb2_enable_phy_cnt == 0) {
        ...
        if (!enable && usb2_enable_phy_cnt == 1) {
          Hci_Phy_Set_Ctl (sunxi_hci->usb_vbase, SUNXI_HCI_PHY_CTRL_DISENABLE);
        }
      ...
      if (sunxi_hci->usbc_no == HCI2_USBC_NO) {
        if (enable && usb3_enable_phy_cnt == 0) {
          Hci_Phy_Clear_Ctl (sunxi_hci->usb_vbase, SUNXI_HCI_PHY_CTRL_DISENABLE);
        }
        else
          if (!enable && usb3_enable_phy_cnt == 1) {
            Hci_Phy_Set_Ctl (sunxi_hci->usb_vbase, SUNXI_HCI_PHY_CTRL_DISENABLE);
          }
        ...
        if (sunxi_hci->usbc_no == HCI3_USBC_NO) {
          if (enable && usb4_enable_phy_cnt == 0) {
            Hci_Phy_Clear_Ctl (sunxi_hci->usb_vbase, SUNXI_HCI_PHY_CTRL_DISENABLE);
          }
          else
            if (!enable && usb4_enable_phy_cnt == 1) {
              Hci_Phy_Set_Ctl (sunxi_hci->usb_vbase, SUNXI_HCI_PHY_CTRL_DISENABLE);
            }
...
static void usb_passby (struct sunxi_hci_hcd * sunxi_hci, u32 enable)
{
  ...
  if (sunxi_hci->usbc_no == HCI0_USBC_NO) {
    ...
    if (enable && usb1_enable_passly_cnt == 0) {
      ...
      if (!enable && usb1_enable_passly_cnt == 1) {
        if (sunxi_hci->hsic_flag) {
          reg_value = usb_get_hsic_phy_ctrl (reg_value, enable);
        }
        else {
    ...
    if (sunxi_hci->usbc_no == HCI1_USBC_NO) {
      ...
      if (enable && usb2_enable_passly_cnt == 0) {
        ...
        if (!enable && usb2_enable_passly_cnt == 1) {
          if (sunxi_hci->hsic_flag) {
            reg_value = usb_get_hsic_phy_ctrl (reg_value, enable);
      ...
      if (sunxi_hci->usbc_no == HCI2_USBC_NO) {
        ...
        if (enable && usb3_enable_passly_cnt == 0) {
          ...
          if (!enable && usb3_enable_passly_cnt == 1) {
            if (sunxi_hci->hsic_flag) {
              reg_value = usb_get_hsic_phy_ctrl (reg_value, enable);
            }
            else {
        ...
        if (sunxi_hci->usbc_no == HCI3_USBC_NO) {
          ...
          if (enable && usb4_enable_passly_cnt == 0) {
            ...
            if (!enable && usb4_enable_passly_cnt == 1) {
              if (sunxi_hci->hsic_flag) {
                reg_value = usb_get_hsic_phy_ctrl (reg_value, enable);
              }
              else {
