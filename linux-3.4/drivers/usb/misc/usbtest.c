...
static int ch9_postconfig (struct usbtest_dev * dev)
{
  ...
  if (le16_to_cpu (udev->descriptor.bcdUSB) == 0x0200) {
    ...
    if (retval == -EPIPE) {
      ...
      if (retval != sizeof (struct usb_qualifier_descriptor) ) {
  ...
  /* async unlink for cleanup? */
    ...
    if (status != -ECONNRESET) {
      ...
      /* sometimes more than one fault is allowed */
        ...
        if (subcase->number == 12 && status == -EPIPE)
        { status = 0; }
        else
  ...
  { ctx->errors += urb->error_count; }
    ...
    { ctx->errors += urb->number_of_packets; }
      ...
      { ctx->errors++; }
        ...
        if (check_guard_bytes (ctx->dev, urb) != 0)
        { ctx->errors++; }
                ...
                int pipe, struct usb_endpoint_descriptor * desc, unsigned offset)
{
  ...
  if (status != 0)
    ...
    if (context.submit_error)
    { status = -EACCES; }
    else
      if (context.errors > context.packet_count / 10)
      { status = -EIO; }
  return status;
