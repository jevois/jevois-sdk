...
extra: {
  ...
  if (quirk & OHCI_QUIRK_SUPERIO) {
    ...
    if ( (quirk & OHCI_QUIRK_AMD756) || distrust_firmware) {
      roothub_a |= RH_A_NPS;
      retval = ftdi_write_pcimem (ftdi, roothub.a, roothub_a);
      if (retval)
      { return retval; }
...
static int ftdi_elan_setupOHCI (struct usb_ftdi * ftdi)
{
  ...
  for (fn = 0; (fn < 4); fn++) {
    ...
    if ( (pciVID == PCI_VENDOR_ID_OPTI) && (pciPID == 0xc861) ) {
      ...
      if ( (pciVID == PCI_VENDOR_ID_NEC) && (pciPID == 0x0035) )
      {
        devices = ftdi_elan_found_controller (ftdi, fn, 0);
        controllers += 1;
      }
      else
        if ( (pciVID == PCI_VENDOR_ID_AL) && (pciPID == 0x5237) ) {
          devices = ftdi_elan_found_controller (ftdi, fn, 0);
          controllers += 1;
        }
        else
          if ( (pciVID == PCI_VENDOR_ID_ATT) && (pciPID == 0x5802) )
          {
            devices = ftdi_elan_found_controller (ftdi, fn, 0);
            controllers += 1;
          }
          else
            if (pciVID == PCI_VENDOR_ID_AMD && pciPID == 0x740c) {
