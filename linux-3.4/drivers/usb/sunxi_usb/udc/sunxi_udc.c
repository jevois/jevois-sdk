...
#define DRIVER_DESC "JeVois USB Device Controller"
#define DRIVER_VERSION  "20160212"
#define DRIVER_AUTHOR "JeVois"
...
#define is_sunxi_udc_dma_capable(req, ep) (is_udc_support_dma() && big_req(req, ep) && req->req.dma_flag && ep->num)

#define is_buffer_mapped(req, ep) (is_sunxi_udc_dma_capable(req, ep) && (req->map_state != UN_MAPPED))

/* Maps the buffer to dma  */
static inline void sunxi_udc_map_dma_buffer (struct sunxi_udc_request * req, struct sunxi_udc * udc,
    struct sunxi_udc_ep * ep)
{
  if (!is_sunxi_udc_dma_capable (req, ep) ) { DMSG_PANIC ("err: need not to dma map\n"); return; }
  ...
  {
    req->req.dma =
      dma_map_single (udc->controller, req->req.buf, req->req.length, (is_tx_ep (ep) ? DMA_TO_DEVICE : DMA_FROM_DEVICE) );
    req->map_state = SW_UDC_USB_MAPPED;
  }
  else
  {
    dma_sync_single_for_device (udc->controller, req->req.dma, req->req.length,
    ...
    struct sunxi_udc_ep * ep)
{
  ...
  {
    dma_unmap_single (udc->controller, req->req.dma, req->req.length,
                      (is_tx_ep (ep) ? DMA_TO_DEVICE : DMA_FROM_DEVICE) );
  ...
  {
    /* PRE_MAPPED */
    dma_sync_single_for_cpu (udc->controller, req->req.dma, req->req.length,
                             (is_tx_ep (ep) ? DMA_TO_DEVICE : DMA_FROM_DEVICE) );
  }
...
__acquires (ep->dev->lock)
{
  ...
  if (is_sunxi_udc_dma_capable (req, ep) ) { sunxi_udc_unmap_dma_buffer (req, ep->dev, ep); }
...
static inline void sunxi_udc_clear_ep_state (struct sunxi_udc * dev)
{
  ...
  for (i = 1; i < SW_UDC_ENDPOINTS; i++) { sunxi_udc_nuke (dev, &dev->ep[i], -ECONNABORTED); }
}

static inline int sunxi_udc_fifo_count_out (__hdle usb_bsp_hdle, __u8 ep_index)
{
  if (ep_index) { return USBC_ReadLenFromFifo (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX); }
  else { return USBC_ReadLenFromFifo (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_EP0); }
}

static inline int sunxi_udc_write_packet (int fifo, struct sunxi_udc_request * req, unsigned max)
{
...
static int pio_write_fifo (struct sunxi_udc_ep * ep, struct sunxi_udc_request * req)
{
  ...
  if (count != ep->ep.maxpacket) { is_last = 1; }
  else
    if (req->req.length != req->req.actual || req->req.zero) { is_last = 0; }
    else { is_last = 2; }
  ...
  if (idx)
  {
    ...
    if (ret != 0) { DMSG_PANIC ("ERR: USBC_Dev_WriteDataStatus, failed\n"); req->req.status = -EOVERFLOW; }
  }
  else
  {
    /* ep0 */
  ...
  if (is_last)
  {
    if (idx == 0) { ep->dev->ep0state = EP0_IDLE; }
...
static int sunxi_udc_write_fifo (struct sunxi_udc_ep * ep, struct sunxi_udc_request * req)
{
  ...
  if (is_sunxi_udc_dma_capable (req, ep) ) { return dma_write_fifo (ep, req); }
  else { return pio_write_fifo (ep, req); }
}

static inline int sunxi_udc_read_packet (int fifo, u8 * buf, struct sunxi_udc_request * req, unsigned avail)
{
  unsigned len = 0;
...
static int pio_read_fifo (struct sunxi_udc_ep * ep, struct sunxi_udc_request * req)
{
  ...
  if (req->req.length == 0) { DMSG_PANIC ("ERR: req->req.length == 0\n"); return 1; }
  
  buf = req->req.buf + req->req.actual;
  bufferspace = req->req.length - req->req.actual;
  if (!bufferspace) { DMSG_PANIC ("ERR: buffer full!\n"); return -1; }
  ...
  if (fifo_count > ep->ep.maxpacket) { avail = ep->ep.maxpacket; }
  else { avail = fifo_count; }
  ...
  if (idx != 0 && fifo_count < ep->ep.maxpacket)
  {
    ...
    if (fifo_count != avail) { req->req.status = -EOVERFLOW; }
  }
  else { is_last = (req->req.length <= req->req.actual) ? 1 : 0; }
  ...
  if (is_last)
  {
    if (!idx) { ep->dev->ep0state = EP0_IDLE; }
    sunxi_udc_done (ep, req, 0);
    is_last = 1;
  }
...
static int sunxi_udc_read_fifo (struct sunxi_udc_ep * ep, struct sunxi_udc_request * req)
{
  ...
  if (is_sunxi_udc_dma_capable (req, ep) ) { return dma_read_fifo (ep, req); }
  else { return pio_read_fifo (ep, req); }
}
...
static int sunxi_udc_read_fifo_crq (struct usb_ctrlrequest * crq)
{
  ...
  if (fifo_count != 8)
  {
    ...
    if (i >= 16) { DMSG_PANIC ("ERR: get ep0 fifo len failed\n"); }
  }
...
static int sunxi_udc_get_status (struct sunxi_udc * dev, struct usb_ctrlrequest * crq)
{
  ...
  case USB_RECIP_ENDPOINT:
    if (ep_num > 4 || crq->wLength > 2) { return 1; }
    ...
    {
      if (is_in) { status = USBC_Dev_IsEpStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX); }
      else  { status = USBC_Dev_IsEpStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX); }
    }
    
    if (status) { buf[0] = 0x01; buf[1] = 0x00; }
...
static void sunxi_udc_handle_ep0_idle (struct sunxi_udc * dev, struct sunxi_udc_ep * ep, struct usb_ctrlrequest * crq,
                                       u32 ep0csr)
{
  ...
  if (len != sizeof (*crq) )
  {
    DMSG_PANIC ("setup begin: fifo READ ERROR wanted %d bytes got %d. Stalling out...\n", sizeof (*crq), len);
  ...
  if (dev->req_std)
  {
    ...
    case USB_REQ_SET_CONFIGURATION:
      ...
      if (crq->bRequestType == USB_RECIP_DEVICE) { dev->req_config = 1; }
      break;
      
    case USB_REQ_SET_INTERFACE:
      DMSG_DBG_UDC ("USB_REQ_SET_INTERFACE ... \n");
      if (crq->bRequestType == USB_RECIP_INTERFACE) { dev->req_config = 1; }
      break;
    ...
    case USB_REQ_GET_STATUS:
      ...
      if (!sunxi_udc_get_status (dev, crq) ) { return; }
      break;
    ...
    case USB_REQ_CLEAR_FEATURE:
      ...
      {
        /* wValue 0-1 */
        if (crq->wValue) { dev->devstatus &= ~ (1 << USB_DEVICE_REMOTE_WAKEUP); }
        else
        {
          int k = 0;
          for (k = 0; k < SW_UDC_ENDPOINTS; k++) { sunxi_udc_set_halt_ex (&dev->ep[k].ep, 0); }
        }
        ...
        if (crq->bRequestType == USB_RECIP_INTERFACE)
        {
          ...
          if (crq->bRequestType == USB_RECIP_ENDPOINT)
          {
            ...
            /* wValue 0-1 */
            if (crq->wValue) { dev->devstatus &= ~ (1 << USB_DEVICE_REMOTE_WAKEUP); }
            else { sunxi_udc_set_halt_ex (&dev->ep[crq->wIndex & 0x7f].ep, 0); }
  ...
  if (crq->bRequestType & USB_DIR_IN) { dev->ep0state = EP0_IN_DATA_PHASE; }
  ...
  if (ret < 0)
  {
    if (dev->req_config) { DMSG_PANIC ("ERR: config change %02x fail %d?\n", crq->bRequest, ret); return; }
    
    if (ret == -EOPNOTSUPP) { DMSG_PANIC ("ERR: Operation not supported\n"); }
...
static void sunxi_udc_handle_ep0 (struct sunxi_udc * dev)
{
  ...
  if (list_empty (&ep->queue) ) { req = NULL; }
  else { req = list_entry (ep->queue.next, struct sunxi_udc_request, queue); }
  ...
  case EP0_END_XFER:
    ...
    case USB_REQ_SET_FEATURE:
      ...
      case TEST_PACKET:
        DMSG_PANIC ("Test packet not supported\n");
        break;
      }
...
static void sunxi_udc_handle_ep (struct sunxi_udc_ep * ep)
{
  ...
  /* see sunxi_udc_queue. */
  if (likely (!list_empty (&ep->queue) ) ) { req = list_entry (ep->queue.next, struct sunxi_udc_request, queue); }
  else { req = NULL; }
...
static void throw_away_all_urb (struct sunxi_udc * dev)
{
  ...
  for (k = 0; k < SW_UDC_ENDPOINTS; k++) { sunxi_udc_nuke (dev, (struct sunxi_udc_ep *) & (dev->ep[k]), -ECONNRESET); }
}
...
static void sunxi_udc_stop_dma_work (struct sunxi_udc * dev, u32 unlock)
{
  ...
  for (i = 0; i < SW_UDC_ENDPOINTS; i++)
  {
    ...
    if (sunxi_udc_dma_is_busy (ep) )
    {
      ...
      if (unlock) { spin_unlock (&ep->dev->lock); sunxi_udc_dma_stop (ep); spin_lock (&ep->dev->lock); }
      else { sunxi_udc_dma_stop (ep); }
...
void sunxi_udc_dma_completion (struct sunxi_udc * dev, struct sunxi_udc_ep * ep, struct sunxi_udc_request * req)
{
  ...
  {
    if ( (ep->bEndpointAddress) & USB_DIR_IN) { USBC_Dev_ClearEpDma (dev->sunxi_udc_io->usb_bsp_hdle, USBC_EP_TYPE_TX); }
    else { USBC_Dev_ClearEpDma (dev->sunxi_udc_io->usb_bsp_hdle, USBC_EP_TYPE_RX); }
  }
  ...
  {
    /* if DMA transfer data over, then done */
    sunxi_udc_done (ep, req, 0);
    is_complete = 1;
  }
  ...
  if (is_complete)
  {
    if (likely (!list_empty (&ep->queue) ) ) { req_next = list_entry (ep->queue.next, struct sunxi_udc_request, queue); }
    else { req_next = NULL; }
...
static irqreturn_t sunxi_udc_irq (int dummy, void * _dev)
{
  ...
  if (usb_irq & USBC_INTUSB_RESET)
  {
    ...
    if (is_udc_support_dma() ) { sunxi_udc_stop_dma_work (dev, 1); }
  ...
  if (usb_irq & USBC_INTUSB_SUSPEND)
  {
    ...
    else { DMSG_INFO_UDC ("ERR: usb speed is unkown\n"); }
  ...
  if (is_udc_support_dma() )
  {
    ...
    for (i = 0; i < DMA_CHAN_TOTAL; i++)
    {
      ...
      if (dma_irq & tmp)
      {
        ...
        else { DMSG_PANIC ("ERR: sunxi_udc_dma_callback: dma is remove, but dma irq is happened\n"); }
      }
    }
  }
  #endif
...
static int sunxi_udc_ep_enable (struct usb_ep * _ep, const struct usb_endpoint_descriptor * desc)
{
  ...
  if (_ep == NULL || desc == NULL) { DMSG_PANIC ("ERR: invalid argment\n"); return -EINVAL; }
  ...
  if (ep == NULL) { DMSG_PANIC ("ERR: usbd_ep_enable, ep = NULL\n"); return -EINVAL; }
  ...
  if ( (ep->bEndpointAddress) & USB_DIR_IN) { ep_type = USBC_EP_TYPE_TX; }
  else { ep_type = USBC_EP_TYPE_RX; }
  
  switch (desc->bmAttributes & USB_ENDPOINT_XFERTYPE_MASK)
  {
  case USB_ENDPOINT_XFER_CONTROL: ts_type   = USBC_TS_TYPE_CTRL; break;
  case USB_ENDPOINT_XFER_BULK:    ts_type   = USBC_TS_TYPE_BULK; break;
  case USB_ENDPOINT_XFER_ISOC:    ts_type   = USBC_TS_TYPE_ISO;  break;
  case USB_ENDPOINT_XFER_INT:     ts_type = USBC_TS_TYPE_INT;    break;
  default:
  ...
  DMSG_INFO_UDC ("ep enable: ep%d(0x%p, %s, %d, %d), fifo(%d, %d, %d)\n", ep->num, _ep, _ep->name,
                 (desc->bEndpointAddress & USB_DIR_IN), _ep->maxpacket, fifo_addr, fifo_size, double_fifo);
                 
  if (i >= SW_UDC_ENDPOINTS) { DMSG_PANIC ("err: usbd_ep_enable, config fifo failed\n"); goto end; }
  
  /* check fifo size */
  if ( (_ep->maxpacket & 0x7ff) > fifo_size)
  { DMSG_PANIC ("err: usbd_ep_enable, fifo size is too small for ep num %d\n", ep->num); goto end; }
  ...
  if (!is_peripheral_active() ) { DMSG_INFO_UDC ("usbd_ep_enable, usb device is not active\n"); goto end; }
  ...
  if (ts_type == USBC_TS_TYPE_ISO) { USBC_Dev_IsoUpdateEnable (g_sunxi_udc_io.usb_bsp_hdle); }
...
static int sunxi_udc_ep_disable (struct usb_ep * _ep)
{
  ...
  if (!_ep) { DMSG_PANIC ("ERR: invalid argment\n"); return -EINVAL; }
  
  ep = to_sunxi_udc_ep (_ep);
  if (ep == NULL) { DMSG_PANIC ("ERR: usbd_ep_disable: ep = NULL\n"); return -EINVAL; }
  
  if (!ep->desc) { DMSG_PANIC ("ERR: %s not enabled\n", _ep ? ep->ep.name : NULL); return -EINVAL; }
...
static struct usb_request * sunxi_udc_alloc_request (struct usb_ep * _ep, gfp_t mem_flags)
{
  ...
  if (!req) { DMSG_PANIC ("ERR: kzalloc failed\n"); return NULL; }
...
static void sunxi_udc_free_request (struct usb_ep * _ep, struct usb_request * _req)
{
  ...
  if (req == NULL) { DMSG_PANIC ("ERR: invalid argment\n"); return; }
...
static int sunxi_udc_queue (struct usb_ep * _ep, struct usb_request * _req, gfp_t gfp_flags)
{
  ...
  dev = ep->dev;
  if (!dev->driver || dev->gadget.speed == USB_SPEED_UNKNOWN)
  { DMSG_PANIC ("ERR : dev->driver=0x%p, dev->gadget.speed=%x\n", dev->driver, dev->gadget.speed); return -ESHUTDOWN; }
  
  if (!_req->complete || !_req->buf) { DMSG_PANIC ("ERR: usbd_queue: _req is invalid\n"); return -EINVAL; }
  
  req = to_sunxi_udc_req (_req);
  if (!req) { DMSG_PANIC ("ERR: req is NULL\n"); return -EINVAL; }
  
  spin_lock_irqsave (&ep->dev->lock, flags);
  _req->status = -EINPROGRESS;
  _req->actual = 0;
  
  if (is_sunxi_udc_dma_capable (req, ep) ) { sunxi_udc_map_dma_buffer (req, dev, ep); }
  
  
  list_add_tail (&req->queue, &ep->queue);
  
  if (!is_peripheral_active() ) { DMSG_PANIC ("warn: peripheral is active\n"); goto end; }
  
  
  old_ep_index = USBC_GetActiveEp (g_sunxi_udc_io.usb_bsp_hdle);
  
  if (ep->bEndpointAddress) { USBC_SelectActiveEp (g_sunxi_udc_io.usb_bsp_hdle, ep->bEndpointAddress & 0x7F); }
  else { USBC_SelectActiveEp (g_sunxi_udc_io.usb_bsp_hdle, 0); }
  ...
  if (!ep->halted && (&req->queue == ep->queue.next) )
  {
    if (ep->bEndpointAddress == 0 /* ep0 */)
    {
      ...
      case EP0_IN_DATA_PHASE:
        if (!USBC_Dev_IsWriteDataReady (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX) && sunxi_udc_write_fifo (ep, req) )
        {
          dev->ep0state = EP0_IDLE;
          req = NULL;
        }
      ...
      {
        if (sunxi_udc_write_fifo (ep, req) ) { req = NULL; }
      }
      else
        if ( (ep->bEndpointAddress & USB_DIR_IN) == 0
             && USBC_Dev_IsReadDataReady (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX) )
        {
          if (sunxi_udc_read_fifo (ep, req) ) { req = NULL; }
...
static int sunxi_udc_dequeue (struct usb_ep * _ep, struct usb_request * _req)
{
  ...
  if (!the_controller->driver) { DMSG_PANIC ("ERR: sunxi_udc_dequeue: driver is null\n"); return -ESHUTDOWN; }
  
  if (!_ep || !_req) { DMSG_PANIC ("ERR: sunxi_udc_dequeue: invalid argment\n"); return retval; }
  
  ep = to_sunxi_udc_ep (_ep);
  if (ep == NULL) { DMSG_PANIC ("ERR: ep == NULL\n"); return -EINVAL; }
  
  udc = to_sunxi_udc (ep->gadget);
  if (udc == NULL) { DMSG_PANIC ("ERR: ep == NULL\n"); return -EINVAL; }
...
static int sunxi_udc_set_halt_ex (struct usb_ep * _ep, int value)
{
  ...
  if (_ep == NULL) { DMSG_PANIC ("ERR: invalid argment\n"); return -EINVAL; }
  
  ep = to_sunxi_udc_ep (_ep);
  if (ep == NULL) { DMSG_PANIC ("ERR: invalid argment\n"); return -EINVAL; }
  ...
  else
  {
    if ( (ep->bEndpointAddress & USB_DIR_IN) != 0)
    {
      if (value) { USBC_Dev_EpSendStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX); }
      else { USBC_Dev_EpClearStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_TX); }
      
    }
    else
    {
      if (value) { USBC_Dev_EpSendStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX); }
      else { USBC_Dev_EpClearStall (g_sunxi_udc_io.usb_bsp_hdle, USBC_EP_TYPE_RX); }
    }
  }
...
static int sunxi_udc_set_halt (struct usb_ep * _ep, int value)
{
  ...
  if (!ep->desc && ep->ep.name != ep0name) { DMSG_PANIC ("ERR: !ep->desc && ep->ep.name != ep0name\n"); return -EINVAL; }
...
static int sunxi_udc_set_pullup (struct sunxi_udc * udc, int is_on)
{
  ...
  if (is_on) { sunxi_udc_enable (udc); }
  else
  {
    if (udc->gadget.speed != USB_SPEED_UNKNOWN)
    {
      if (udc->driver && udc->driver->disconnect) { udc->driver->disconnect (&udc->gadget); }
    }
...
{
  if (!is_peripheral_active() ) { DMSG_INFO ("%s_%d: usb device is not active\n", __func__, __LINE__); return 0; }
...
static void sunxi_udc_reinit (struct sunxi_udc * dev)
{
  ...
  for (i = 0; i < SW_UDC_ENDPOINTS; i++)
  {
    ...
    if (i != 0) { list_add_tail (&ep->ep.ep_list, &dev->gadget.ep_list); }
...
static s32  usbd_stop_work (void)
{
  ...
  USBC_Dev_ConectSwitch (g_sunxi_udc_io.usb_bsp_hdle, USBC_DEVICE_SWITCH_OFF);
  return 0;
}
...
static struct sunxi_udc sunxi_udc =
{
  .gadget = {
    ...
    .name   = gadget_name,
    .dev = { .init_name = "gadget", },
  },
  
  .ep[0] = {
    .num                = 0,
    .ep                 = { .name = ep0name, .ops = &sunxi_udc_ep_ops, .maxpacket = EP0_FIFO_SIZE, },
    .dev                = &sunxi_udc,
  },
  
  .ep[1] = {
    .num                = 1,
    .ep                 = { .name = ep1in_bulk_name, .ops = &sunxi_udc_ep_ops, .maxpacket = 512, },
    .dev            = &sunxi_udc,
  ...
  .ep[2] = {
    .num            = 1,
    .ep                 = { .name = ep1out_bulk_name, .ops = &sunxi_udc_ep_ops, .maxpacket = 512, },
    .dev            = &sunxi_udc,
  ...
  .ep[5] = {
    .num            = 3,
    .ep                 = { .name = ep3_int_name, .ops = &sunxi_udc_ep_ops, .maxpacket = 512, },
    .dev            = &sunxi_udc,
    .bEndpointAddress   = 3,
  ...
  .ep[6] = {
    .num            = 4,
    .ep                 = { .name = ep4_iso_name, .ops = &sunxi_udc_ep_ops, .maxpacket = 1024 | (1 << 12), },
    .dev            = &sunxi_udc,
    .bEndpointAddress   = 4,
  ...
  .ep[7] = {
    .num                = 5,
    .ep                 = { .name = ep5_int_name, .ops = &sunxi_udc_ep_ops, .maxpacket = 512, },
    .dev            = &sunxi_udc,
    .bEndpointAddress   = 5,
    .bmAttributes     = USB_ENDPOINT_XFER_INT,
  },
