...
static int pio_write_fifo(struct sunxi_udc_ep *ep, struct sunxi_udc_request *req)
{
  ...
  else if (req->req.length != req->req.actual || req->req.zero) is_last = 0;
  else is_last = 2;
...
static int sunxi_udc_get_status(struct sunxi_udc *dev, struct usb_ctrlrequest *crq)
{
  ...
  case USB_RECIP_ENDPOINT:
    ...
    if (status) { buf[0] = 0x01; buf[1] = 0x00; } else { buf[0] = 0x00; buf[1] = 0x00; }
                                      ...
                                      u32 ep0csr)
{
  ...
  if (crq->bRequestType & USB_DIR_IN) dev->ep0state = EP0_IN_DATA_PHASE; else dev->ep0state = EP0_OUT_DATA_PHASE;
...
static int sunxi_udc_ep_enable(struct usb_ep *_ep, const struct usb_endpoint_descriptor *desc)
{
  ...
  if ((ep->bEndpointAddress) & USB_DIR_IN) ep_type = USBC_EP_TYPE_TX; else ep_type = USBC_EP_TYPE_RX;
