...
static int apds990x_calc_again (struct apds990x_chip * chip)
{
  ...
  { next_again -= 2; } /* ALS saturated. Decrease gain by 2 steps */
    ...
    if (chip->lux_clear > chip->a_max_result / 2)
    { next_again--; }
    else
      if (chip->lux_clear < APDS_LUX_GAIN_LO_LIMIT_STRICT)
      { next_again += 2; } /* Too dark. Increase gain by 2 steps */
      else
        if (chip->lux_clear < APDS_LUX_GAIN_LO_LIMIT)
        { next_again++; }
  ...
  { next_again = 0; }
    ...
    if (next_again > APDS990X_MAX_AGAIN)
    ...
    const char * buf, size_t len)
{
  ...
  { chip->prox_continuous_mode = 0; }
    ...
    if (sysfs_streq (buf, reporting_modes[1]) )
