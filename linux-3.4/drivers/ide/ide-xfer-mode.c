...
const char * ide_xfer_verbose (u8 mode)
{
  ...
  { s = udma_str[i]; }
    ...
    if (mode >= XFER_MW_DMA_0 && mode <= XFER_MW_DMA_4)
    { s = mwdma_str[i]; }
    else
      if (mode >= XFER_SW_DMA_0 && mode <= XFER_SW_DMA_2)
      { s = swdma_str[i]; }
      else
        if (mode >= XFER_PIO_0 && mode <= XFER_PIO_6)
        { s = pio_str[i & 0x7]; }
        else
          if (mode == XFER_PIO_SLOW)
          { s = "PIO SLOW"; }
          else
          { s = "XFER ERROR"; }
