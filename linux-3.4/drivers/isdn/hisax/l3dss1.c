                  ...
                  void * arg)
{
  ...
  if ( (pc->para.setup.si2 >= 160) && (pc->para.setup.si2 <= 175) ) {
    ...
    *p++ = EncodeSyncParams (pc->para.setup.si2 - 160, 0x80);
    ...
    if ( (pc->para.setup.si2 >= 176) && (pc->para.setup.si2 <= 191) ) {
...
l3dss1_disconnect (struct l3_process * pc, u_char pr, void * arg)
{
  ...
  { cause = 96; }
    ...
    if ( (!cause) && (ERR_IE_UNRECOGNIZED == ret) )
    { cause = 99; }
  ret = pc->state;
  newl3state (pc, 12);
  if (cause)
...
l3dss1_release (struct l3_process * pc, u_char pr, void * arg)
{
  ...
  { cause = 96; }
    ...
    if ( (ERR_IE_UNRECOGNIZED == ret) && (!cause) )
    { cause = 99; }
  if (cause)
...
dss1up (struct PStack * st, int pr, void * arg)
{
  ...
  if (cr == -2) {  /* wrong Callref */
    ...
    if (cr == -1) {  /* Dummy Callref */
      ...
      if ( ( ( (skb->data[1] & 0x0f) == 1) && (0 == (cr & 0x7f) ) ) ||
...
dss1down (struct PStack * st, int pr, void * arg)
{
  ...
  if ( (DL_ESTABLISH | REQUEST) == pr) {
    ...
    if ( ( (CC_SETUP | REQUEST) == pr) || ( (CC_RESUME | REQUEST) == pr) ) {
      chan = arg;
      cr = newcallref();
      cr |= 0x80;
      if ( (proc = dss1_new_l3_process (st, cr) ) ) {
