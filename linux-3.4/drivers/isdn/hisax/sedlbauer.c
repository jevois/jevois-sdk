...
setup_sedlbauer_pci (struct IsdnCard * card)
{
  ...
  if (sub_vendor_id == PCI_SUBVENDOR_SPEEDFAX_PYRAMID) {
    ...
    if (sub_vendor_id == PCI_SUBVENDOR_SPEEDFAX_PCI) {
      cs->hw.sedl.chip = SEDL_CHIP_ISAC_ISAR;
      cs->subtyp = SEDL_SPEEDFAX_PCI;
    }
    else
      if (sub_vendor_id == PCI_SUBVENDOR_HST_SAPHIR3) {
        cs->hw.sedl.chip = SEDL_CHIP_IPAC;
        cs->subtyp = HST_SAPHIR3;
      }
      else
        if (sub_vendor_id == PCI_SUBVENDOR_SEDLBAUER_PCI) {
          cs->hw.sedl.chip = SEDL_CHIP_IPAC;
...
setup_sedlbauer (struct IsdnCard * card)
{
  ...
  if (cs->typ == ISDN_CTYPE_SEDLBAUER) {
    ...
    if (cs->typ == ISDN_CTYPE_SEDLBAUER_PCMCIA) {
      ...
      if (cs->typ == ISDN_CTYPE_SEDLBAUER_FAX) {
        cs->subtyp = SEDL_SPEED_FAX;
        cs->hw.sedl.bus = SEDL_BUS_ISA;
        cs->hw.sedl.chip = SEDL_CHIP_ISAC_ISAR;
      }
