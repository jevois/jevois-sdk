  ...
  while (count--) {
    ...
    if (unlikely (inputstate & INS_flag_hunt) ) {
      ...
      if (likely (seqlen < 5 && trail1 < 7) ) {
        ...
        if (likely (seqlen == 6 && inbits == 7 - lead1 &&
                    trail1 + 1 == inbits &&
                    ! (inputstate & INS_have_data) ) ) {
          /* streamlined case: flag idle - state unchanged */
        }
        else
          if (unlikely (seqlen > 6) ) {
            /* abort sequence */
            ubc->aborts++;
            hdlc_flush (bcs);
