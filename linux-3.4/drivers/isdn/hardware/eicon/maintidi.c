    ...
    if (Rc != pLib->rc_ok) {
      ...
      */
      if (pLib->general_b_ch_event == 1) {
        ...
        if (pLib->general_fax_event == 1) {
          pLib->general_fax_event = 2;
          ignore = 1;
        }
        else
          if (pLib->general_mdm_event == 1) {
            pLib->general_mdm_event = 2;
            ignore = 1;
          }
          else
            if ( (pLib->ChannelsTraceActive < pLib->Channels) && pLib->ChannelsTraceActive) {
              ...
              if (pLib->ModemTraceActive < pLib->Channels) {
                ...
                if (pLib->FaxTraceActive < pLib->Channels) {
                  ...
                  if (pLib->audio_trace_init == 2) {
                    ...
                    if (pLib->eye_pattern_pending) {
                      pLib->eye_pattern_pending =  0;
                      ignore = 1;
                    }
                    else
                      if (pLib->audio_tap_pending) {
