...
static int emu_tcxb (struct pt_regs * regs, int rx, long val) {
  ...
  case _FP_EXPMAX_Q:
    ...
    { bit = 4; }  /* infinity */
      ...
      if (_FP_FRAC_HIGH_RAW_Q (QA) & _FP_QNANBIT_Q)
      { bit = 2; }  /* quiet NAN */
      else
      { bit = 0; }  /* signaling NAN */
    break;
...
static int emu_tcdb (struct pt_regs * regs, int rx, long val) {
  ...
  case _FP_EXPMAX_D:
    ...
    { bit = 4; }  /* infinity */
      ...
      if (_FP_FRAC_HIGH_RAW_D (DA) & _FP_QNANBIT_D)
      { bit = 2; }  /* quiet NAN */
      else
      { bit = 0; }  /* signaling NAN */
    break;
...
static int emu_tceb (struct pt_regs * regs, int rx, long val) {
  ...
  case _FP_EXPMAX_S:
    ...
    { bit = 4; }  /* infinity */
      ...
      if (_FP_FRAC_HIGH_RAW_S (SA) & _FP_QNANBIT_S)
      { bit = 2; }  /* quiet NAN */
      else
      { bit = 0; }  /* signaling NAN */
    break;
...
int math_emu_ldr (__u8 * opcode) {
  ...
  if ( (opc & 0x90) == 0) {          /* test if rx in {0,2,4,6} */
    ...
    if ( (opc & 0x9) == 0) {    /* test if ry in {0,2,4,6} */
