                                 ...
                                 const char * buf, size_t len)
{
  ...
  { rc = reipl_set_type (IPL_TYPE_CCW); }
    ...
    if (strncmp (buf, IPL_FCP_STR, strlen (IPL_FCP_STR) ) == 0)
    { rc = reipl_set_type (IPL_TYPE_FCP); }
    else
      if (strncmp (buf, IPL_NSS_STR, strlen (IPL_NSS_STR) ) == 0)
...
static int __init reipl_type_init (void)
{
  ...
  if (reipl_block->hdr.pbt == DIAG308_IPL_TYPE_FCP) {
    ...
    if (reipl_block->hdr.pbt == DIAG308_IPL_TYPE_CCW) {
                                ...
                                const char * buf, size_t len)
{
  ...
  { rc = dump_set_type (DUMP_TYPE_NONE); }
    ...
    if (strncmp (buf, DUMP_CCW_STR, strlen (DUMP_CCW_STR) ) == 0)
    { rc = dump_set_type (DUMP_TYPE_CCW); }
    else
      if (strncmp (buf, DUMP_FCP_STR, strlen (DUMP_FCP_STR) ) == 0)
...
static void vmcmd_run (struct shutdown_trigger * trigger)
{
  ...
  { cmd = vmcmd_on_reboot; }
    ...
    { cmd = vmcmd_on_panic; }
      ...
      if (strcmp (trigger->name, ON_HALT_STR) == 0)
      { cmd = vmcmd_on_halt; }
      else
        if (strcmp (trigger->name, ON_POFF_STR) == 0)
        { cmd = vmcmd_on_poff; }
        else
          if (strcmp (trigger->name, ON_RESTART_STR) == 0)
