            ...
            struct module * me)
{
  ...
  case R_390_PC64:  /* PC relative 64 bit.  */
    ...
    { * (unsigned short *) loc = val; }
      ...
      if (r_type == R_390_PC16DBL)
      { * (unsigned short *) loc = val >> 1; }
      else
        if (r_type == R_390_PC32DBL)
        { * (unsigned int *) loc = val >> 1; }
        else
          if (r_type == R_390_PC32)
          { * (unsigned int *) loc = val; }
          else
            if (r_type == R_390_PC64)
            { * (unsigned long *) loc = val; }
    break;
  case R_390_GOT12: /* 12 bit GOT offset.  */
  case R_390_GOT16: /* 16 bit GOT offset.  */
  ...
  case R_390_GOTPLTENT: /* 32 bit rel. offset to jump slot >> 1. */
    ...
    if (r_type == R_390_GOT12 ||
      ...
      * (unsigned short *) loc = (val & 0xfff) |
      ...
      if (r_type == R_390_GOT16 ||
          r_type == R_390_GOTPLT16)
      { * (unsigned short *) loc = val; }
      else
        if (r_type == R_390_GOT20 ||
            r_type == R_390_GOTPLT20)
          * (unsigned int *) loc =
            (* (unsigned int *) loc & 0xf00000ff) |
            (val & 0xfff) << 16 | (val & 0xff000) >> 4;
        else
          if (r_type == R_390_GOT32 ||
    ...
    { * (unsigned short *) loc = val >> 1; }
      ...
      { * (unsigned short *) loc = val; }
        ...
        { * (unsigned int *) loc = val >> 1; }
          ...
          if (r_type == R_390_PLT32 ||
              r_type == R_390_PLTOFF32)
          { * (unsigned int *) loc = val; }
          else
            if (r_type == R_390_PLT64 ||
                r_type == R_390_PLTOFF64)
            { * (unsigned long *) loc = val; }
    break;
  ...
  case R_390_GOTOFF64:  /* 64 bit offset to GOT. */
    ...
    { * (unsigned short *) loc = val; }
      ...
      if (r_type == R_390_GOTOFF32)
      { * (unsigned int *) loc = val; }
      else
        if (r_type == R_390_GOTOFF64)
        { * (unsigned long *) loc = val; }
    break;
