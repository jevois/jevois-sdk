...
static inline void do_fp_trap (struct pt_regs * regs, int fpc)
{
  ...
  if ( (fpc & 0x00000300) == 0) {
    ...
    { si_code = FPE_FLTINV; }
      ...
      if (fpc & 0x4000) /* div by 0 */
      { si_code = FPE_FLTDIV; }
      else
        if (fpc & 0x2000) /* overflow */
        { si_code = FPE_FLTOVF; }
        else
          if (fpc & 0x1000) /* underflow */
          { si_code = FPE_FLTUND; }
          else
            if (fpc & 0x0800) /* inexact */
            { si_code = FPE_FLTRES; }
  }
    ...
    if (* ( (__u16 *) opcode) == S390_BREAKPOINT_U16) {
      ...
      if (opcode[0] == 0xb3) {
        ...
        if (opcode[0] == 0xed) {
          ...
          if (* ( (__u16 *) opcode) == 0xb299) {
            if (get_user (* ( (__u16 *) (opcode + 2) ), location + 1) )
            { return; }
            signal = math_emu_srnm (opcode, regs);
          }
          else
            if (* ( (__u16 *) opcode) == 0xb29c) {
              if (get_user (* ( (__u16 *) (opcode + 2) ), location + 1) )
              { return; }
              signal = math_emu_stfpc (opcode, regs);
            }
            else
              if (* ( (__u16 *) opcode) == 0xb29d) {
                if (get_user (* ( (__u16 *) (opcode + 2) ), location + 1) )
                { return; }
