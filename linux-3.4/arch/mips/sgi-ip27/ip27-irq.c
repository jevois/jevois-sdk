...
static void ip27_do_irq_mask0 (void)
{
  ...
  if (pend0 & (1UL << CPU_RESCHED_A_IRQ) ) {
    ...
    if (pend0 & (1UL << CPU_RESCHED_B_IRQ) ) {
      LOCAL_HUB_CLR_INTR (CPU_RESCHED_B_IRQ);
      scheduler_ipi();
    }
    else
      if (pend0 & (1UL << CPU_CALL_A_IRQ) ) {
        LOCAL_HUB_CLR_INTR (CPU_CALL_A_IRQ);
        smp_call_function_interrupt();
      }
      else
        if (pend0 & (1UL << CPU_CALL_B_IRQ) ) {
...
asmlinkage void plat_irq_dispatch (void)
{
  ...
  { do_IRQ (rt_timer_irq); }
    ...
    if (pending & CAUSEF_IP2)  /* PI_INT_PEND_0 or CC_PEND_{A|B} */
    { ip27_do_irq_mask0(); }
    else
      if (pending & CAUSEF_IP3)  /* PI_INT_PEND_1 */
      { ip27_do_irq_mask1(); }
      else
