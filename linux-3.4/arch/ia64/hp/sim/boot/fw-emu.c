              ...
              unsigned long in6, unsigned long in7)
{
  ...
  if (index == SAL_FREQ_BASE) {
    ...
    { r9 = 200000000; }
      ...
      if (in1 == SAL_FREQ_BASE_INTERVAL_TIMER) {
        /*
        ...
        if (in1 == SAL_FREQ_BASE_REALTIME_CLOCK)
        { r9 = 1; }
        else
        { status = -1; }
  }
  else
    if (index == SAL_SET_VECTORS) {
      ;
    }
    else
      if (index == SAL_GET_STATE_INFO) {
        ;
      }
      else
        if (index == SAL_GET_STATE_INFO_SIZE) {
          ;
        }
        else
          if (index == SAL_CLEAR_STATE_INFO) {
            ;
          }
          else
            if (index == SAL_MC_RENDEZ) {
              ;
            }
            else
              if (index == SAL_MC_SET_PARAMS) {
                ;
              }
              else
                if (index == SAL_CACHE_FLUSH) {
                  ;
                }
                else
                  if (index == SAL_CACHE_INIT) {
                    ;
                    #ifdef CONFIG_PCI
                  }
                  else
                    if (index == SAL_PCI_CONFIG_READ) {
                      /*
