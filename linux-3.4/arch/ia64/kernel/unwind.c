...
build_script (struct unw_frame_info * info)
{
  ...
  { i = UNW_REG_PRI_UNAT_MEM; }
    ...
    if (sr.when_target < sr.curr.reg[UNW_REG_PRI_UNAT_MEM].when)
    { i = UNW_REG_PRI_UNAT_GR; }
    else
      if (sr.curr.reg[UNW_REG_PRI_UNAT_MEM].when > sr.curr.reg[UNW_REG_PRI_UNAT_GR].when)
      { i = UNW_REG_PRI_UNAT_MEM; }
      else
      { i = UNW_REG_PRI_UNAT_GR; }
