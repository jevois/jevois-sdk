                           ...
                           struct module * mod)
{
  ...
  for (s = sechdrs; s < sechdrs_end; ++s)
    ...
    { mod->arch.core_plt = s; }
      ...
      if (strcmp (".init.plt", secstrings + s->sh_name) == 0)
      { mod->arch.init_plt = s; }
      else
        if (strcmp (".got", secstrings + s->sh_name) == 0)
        { mod->arch.got = s; }
        else
          if (strcmp (".opd", secstrings + s->sh_name) == 0)
          { mod->arch.opd = s; }
          else
            if (strcmp (".IA_64.unwind", secstrings + s->sh_name) == 0)
            { mod->arch.unwind = s; }
  #ifdef CONFIG_PARAVIRT
            else
              if (strcmp (".paravirt_bundles",
                          secstrings + s->sh_name) == 0)
              { mod->arch.paravirt_bundles = s; }
              else
                if (strcmp (".paravirt_insts",
                            secstrings + s->sh_name) == 0)
