  ...
  while (odsc) {
    ...
    while (dcfg) {
      ...
      if (digest_ctx.tcfg && (digest_ctx.tcfg->tid == dcfg->tid) ) {
        ...
        if (cipher_ctx.tcfg && (cipher_ctx.tcfg->tid == dcfg->tid) ) {
          tc = &cipher_ctx;
        }
        else
          if (csum_ctx.tcfg && (csum_ctx.tcfg->tid == dcfg->tid) ) {
            tc = &csum_ctx;
          }
      if (!tc) {
      ...
      if (tc->current_src != src_dma) {
        ...
        if (digest_ctx.unit_no == tc->current_src) {
          ...
          if (cipher_ctx.unit_no == tc->current_src) {
            tc->curr_src = &cipher_ctx;
          }
          else
            if (csum_ctx.unit_no == tc->current_src) {
