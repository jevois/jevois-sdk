...
static void __kprobes prepare_singlestep (struct kprobe * p, struct pt_regs * regs)
{
  ...
  if (p != NULL) {
    ...
    if (OPCODE_JSR (p->opcode) || OPCODE_JMP (p->opcode) ) {
      ...
      if (OPCODE_BRA (p->opcode) || OPCODE_BSR (p->opcode) ) {
        ...
        if (OPCODE_BRAF (p->opcode) || OPCODE_BSRF (p->opcode) ) {
          unsigned int reg_nr = ( (p->opcode >> 8) & 0x000F);
          op1->addr =
          ...
          if (OPCODE_RTS (p->opcode) ) {
            op1->addr = (kprobe_opcode_t *) regs->pr;
            
          }
          else
            if (OPCODE_BF (p->opcode) || OPCODE_BT (p->opcode) ) {
              ...
              if (OPCODE_BF_S (p->opcode) || OPCODE_BT_S (p->opcode) ) {
                unsigned long disp = (p->opcode & 0x00FF);
                /* case 1 */
                op1->addr = p->addr + 2;
                /* case 2 */
