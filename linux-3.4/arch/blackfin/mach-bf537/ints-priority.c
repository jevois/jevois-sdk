                                   ...
                                   struct irq_desc * inta_desc)
{
  ...
  { irq = IRQ_MAC_ERROR; }
    ...
    { irq = IRQ_SPORT0_ERROR; }
      ...
      if (bfin_read_SPORT1_STAT() & SPORT_ERR_MASK)
      { irq = IRQ_SPORT1_ERROR; }
      else
        if (bfin_read_PPI_STATUS() & PPI_ERR_MASK)
        { irq = IRQ_PPI_ERROR; }
        else
          if (bfin_read_CAN_GIF() & CAN_ERR_MASK)
          { irq = IRQ_CAN_ERROR; }
          else
            if (bfin_read_SPI_STAT() & SPI_ERR_MASK)
            { irq = IRQ_SPI_ERROR; }
            else
              if ( (bfin_read_UART0_IIR() & UART_ERR_MASK) == UART_ERR_MASK)
              { irq = IRQ_UART0_ERROR; }
              else
                if ( (bfin_read_UART1_IIR() & UART_ERR_MASK) == UART_ERR_MASK)
                { irq = IRQ_UART1_ERROR; }
