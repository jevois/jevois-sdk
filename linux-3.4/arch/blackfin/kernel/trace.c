...
void decode_address (char * buf, unsigned long address)
{
  ...
  if (address >= FIXED_CODE_START && address < FIXED_CODE_END) {
    ...
    if (address < CONFIG_BOOT_LOAD) {
      ...
      if (address >= COREMMR_BASE) {
        strcat (buf, "/* core mmrs */");
        return;
        
      }
      else
        if (address >= SYSMMR_BASE) {
          strcat (buf, "/* system mmrs */");
          return;
          
        }
        else
          if (address >= L1_ROM_START && address < L1_ROM_START + L1_ROM_LENGTH) {
            strcat (buf, "/* on-chip L1 ROM */");
            return;
            
          }
          else
            if (address >= L1_SCRATCH_START && address < L1_SCRATCH_START + L1_SCRATCH_LENGTH) {
              strcat (buf, "/* on-chip scratchpad */");
              return;
              
            }
            else
              if (address >= physical_mem_end && address < ASYNC_BANK0_BASE) {
                strcat (buf, "/* unconnected memory */");
                return;
                
              }
              else
                if (address >= ASYNC_BANK3_BASE + ASYNC_BANK3_SIZE && address < BOOT_ROM_START) {
                  strcat (buf, "/* reserved memory */");
                  return;
                  
                }
                else
                  if (address >= L1_DATA_A_START && address < L1_DATA_A_START + L1_DATA_A_LENGTH) {
                    strcat (buf, "/* on-chip Data Bank A */");
                    return;
                    
                  }
                  else
                    if (address >= L1_DATA_B_START && address < L1_DATA_B_START + L1_DATA_B_LENGTH) {
...
static void decode_ProgCtrl_0 (unsigned int opcode)
{
  ...
  { pr_cont ("NOP"); }
    ...
    if (prgfunc == 1 && poprnd == 0)
    { pr_cont ("RTS"); }
    else
      if (prgfunc == 1 && poprnd == 1)
      { pr_cont ("RTI"); }
      else
        if (prgfunc == 1 && poprnd == 2)
        { pr_cont ("RTX"); }
        else
          if (prgfunc == 1 && poprnd == 3)
          { pr_cont ("RTN"); }
          else
            if (prgfunc == 1 && poprnd == 4)
            { pr_cont ("RTE"); }
            else
              if (prgfunc == 2 && poprnd == 0)
              { pr_cont ("IDLE"); }
              else
                if (prgfunc == 2 && poprnd == 3)
                { pr_cont ("CSYNC"); }
                else
                  if (prgfunc == 2 && poprnd == 4)
                  { pr_cont ("SSYNC"); }
                  else
                    if (prgfunc == 2 && poprnd == 5)
                    { pr_cont ("EMUEXCPT"); }
                    else
                      if (prgfunc == 3)
...
static void decode_opcode (unsigned int opcode)
{
  ...
  { pr_cont ("BUG"); }
    ...
    { decode_ProgCtrl_0 (opcode); }
      ...
      if ( (opcode & 0xfffff000) == BRCC_opcode)
      { decode_BRCC_0 (opcode); }
      else
        if ( (opcode & 0xfffff000) == 0x2000)
        { pr_cont ("JUMP.S"); }
        else
          if ( (opcode & 0xfe000000) == CALLa_opcode)
          { decode_CALLa_0 (opcode); }
          else
            if ( (opcode & 0xff8000C0) == LoopSetup_opcode)
            { decode_LoopSetup_0 (opcode); }
            else
              if ( (opcode & 0xfffffc00) == DspLDST_opcode)
              { decode_dspLDST_0 (opcode); }
              else
                if ( (opcode & 0xfffff000) == LDST_opcode)
                { decode_LDST_0 (opcode); }
                else
                  if ( (opcode & 0xffffe000) == LDSTii_opcode)
                  { decode_LDSTii_0 (opcode); }
                  else
                    if ( (opcode & 0xfc000000) == LDSTidxI_opcode)
