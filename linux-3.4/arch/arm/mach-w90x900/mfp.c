...
void mfp_set_groupc (struct device * dev)
{
  ...
  { mfpen |= GPSELC; }/*enable lcd*/
    ...
    if (strcmp (dev_id, "nuc900-kpi") == 0) {
      mfpen &= (~GPSELC);/*enable kpi*/
      mfpen |= ENKPI;
    }
    else
      if (strcmp (dev_id, "nuc900-nand") == 0) {
...
void mfp_set_groupi (struct device * dev)
{
  ...
  { mfpen |= GPSELEI1; }/*enable wdog*/
    ...
    if (strcmp (dev_id, "nuc900-atapi") == 0)
    { mfpen |= GPSELEI0; }/*enable atapi*/
    else
      if (strcmp (dev_id, "nuc900-keypad") == 0)
...
void mfp_set_groupg (struct device * dev, const char * subname)
{
  ...
  if (strcmp (dev_id, "nuc900-spi") == 0) {
    ...
    if (strcmp (dev_id, "nuc900-i2c0") == 0) {
      mfpen &= ~ (GPIOG0TO1);
      mfpen |= ENI2C0;/*enable i2c0*/
    }
    else
      if (strcmp (dev_id, "nuc900-i2c1") == 0) {
        mfpen &= ~ (GPIOG2TO3);
        mfpen |= ENI2C1;/*enable i2c1*/
      }
      else
        if (strcmp (dev_id, "nuc900-ac97") == 0) {
          mfpen &= ~ (GPIOG22TO23);
          mfpen |= ENAC97;/*enable AC97*/
        }
        else
          if (strcmp (dev_id, "nuc900-mmc-port1") == 0) {
            mfpen &= ~ (GPIOG18TO20);
