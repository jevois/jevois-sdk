...
read_failed:
  ...
  for (raw = first_raw; raw != jeb->last_node; raw = ref_next (raw) ) {
    ...
    if (ic && ic->class == RAWNODE_CLASS_XATTR_DATUM) {
      ...
      if (ic && ic->class == RAWNODE_CLASS_XATTR_REF) {
        ...
        if (ic && ic->class == RAWNODE_CLASS_INODE_CACHE) {
          ...
          if (ic->state == INO_STATE_PRESENT && !ref_obsolete (raw) ) {
            ...
            if (unlikely (ic->state != INO_STATE_PRESENT &&
  ...
  */
  ...
  if (ret) {
    ...
    if (retlen != c->wbuf_pagesize) {
      ...
      if ( (ret = jffs2_verify_write (c, c->wbuf, c->wbuf_ofs) ) ) {
wfail:
