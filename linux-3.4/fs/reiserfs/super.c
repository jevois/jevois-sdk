...
static int reiserfs_show_options (struct seq_file * seq, struct dentry * root)
{
  ...
  { seq_puts (seq, ",errors=continue"); }
    ...
    if (opts & (1 << REISERFS_ERROR_PANIC) )
  ...
  { seq_puts (seq, ",data=journal"); }
    ...
    if (opts & (1 << REISERFS_DATA_WRITEBACK) )
...
static void handle_data_mode (struct super_block * s, unsigned long mount_options)
{
  if (mount_options & (1 << REISERFS_DATA_LOG) ) {
    ...
    if (mount_options & (1 << REISERFS_DATA_ORDERED) ) {
      ...
      if (mount_options & (1 << REISERFS_DATA_WRITEBACK) ) {
...
static int read_super_block (struct super_block * s, int offset)
{
  ...
  if (is_reiserfs_jr (rs) ) {
    ...
    if (sb_version (rs) == REISERFS_VERSION_2)
      ...
      if (sb_version (rs) == REISERFS_VERSION_1)
...
static int what_hash (struct super_block * s)
{
  ...
  if (code != UNSET_HASH && reiserfs_hash_detect (s) ) {
    ...
    if (reiserfs_rupasov_hash (s) && code != YURA_HASH) {
      ...
      if (reiserfs_tea_hash (s) && code != TEA_HASH) {
        ...
        if (reiserfs_r5_hash (s) && code != R5_HASH) {
    ...
    if (reiserfs_rupasov_hash (s) ) {
      ...
      if (reiserfs_tea_hash (s) ) {
        code = TEA_HASH;
      }
      else
        if (reiserfs_r5_hash (s) ) {
          code = R5_HASH;
        }
  }
