...
static int yaffs_statfs (struct dentry * dentry, struct kstatfs * buf)
{
  ...
  if (dev->data_bytes_per_chunk & (dev->data_bytes_per_chunk - 1) ) {
    ...
    if (sb->s_blocksize > dev->data_bytes_per_chunk) {
...
static unsigned yaffs_bg_gc_urgency (struct yaffs_dev * dev)
{
  ...
  { return 0; }
    ...
    if (scattered < (dev->param.chunks_per_block * 2) )
    { return 0; }
    else
      if (erased_chunks > dev->n_free_chunks / 2)
      { return 0; }
      else
        if (erased_chunks > dev->n_free_chunks / 4)
        { return 1; }
        else
        { return 2; }
}
                                ...
                                const char * options_str)
{
  ...
  while (options_str && *options_str && !error) {
    ...
    if (!strcmp (cur_opt, "inband-tags") ) {
      ...
      if (!strcmp (cur_opt, "tags-ecc-off") ) {
        options->tags_ecc_on = 0;
        options->tags_ecc_overridden = 1;
      }
      else
        if (!strcmp (cur_opt, "tags-ecc-on") ) {
          options->tags_ecc_on = 1;
          options->tags_ecc_overridden = 1;
        }
        else
          if (!strcmp (cur_opt, "lazy-loading-off") ) {
            options->lazy_loading_enabled = 0;
            options->lazy_loading_overridden = 1;
          }
          else
            if (!strcmp (cur_opt, "lazy-loading-on") ) {
              options->lazy_loading_enabled = 1;
              options->lazy_loading_overridden = 1;
            }
            else
              if (!strcmp (cur_opt, "empty-lost-and-found-off") ) {
                options->empty_lost_and_found = 0;
                options->empty_lost_and_found_overridden = 1;
              }
              else
                if (!strcmp (cur_opt, "empty-lost-and-found-on") ) {
                  options->empty_lost_and_found = 1;
                  options->empty_lost_and_found_overridden = 1;
                }
                else
                  if (!strcmp (cur_opt, "no-cache") ) {
                    options->no_cache = 1;
                  }
                  else
                    if (!strcmp (cur_opt, "no-checkpoint-read") ) {
                      options->skip_checkpoint_read = 1;
                    }
                    else
                      if (!strcmp (cur_opt, "no-checkpoint-write") ) {
                        options->skip_checkpoint_write = 1;
                      }
                      else
                        if (!strcmp (cur_opt, "no-checkpoint") ) {
                          options->skip_checkpoint_read = 1;
                          options->skip_checkpoint_write = 1;
                        }
                        else {
    ...
    void * data, int silent)
{
  ...
  { printk (KERN_INFO "yaffs: sb is NULL\n"); }
    ...
    { printk (KERN_INFO "yaffs: sb->s_dev is NULL\n"); }
      ...
      if (!yaffs_devname (sb, devname_buf) )
