                                  ...
                                  struct buffer_head * group_bh)
{
  ...
  if (le16_to_cpu (gd->bg_chain) != input->chain)
    ...
    if (le16_to_cpu (gd->bg_bits) != input->clusters * cl_bpc)
      mlog (ML_ERROR, "Group descriptor # %llu has bit count %u but "
            "input has %u clusters set\n",
            (unsigned long long) le64_to_cpu (gd->bg_blkno),
            le16_to_cpu (gd->bg_bits), input->clusters);
    else
      if (le16_to_cpu (gd->bg_free_bits_count) != input->frees * cl_bpc)
    ...
    struct buffer_head * group_bh)
{
  ...
  { mlog (ML_ERROR, "add a group which is in the current volume.\n"); }
    ...
    if (input->chain >= cl_count)
    { mlog (ML_ERROR, "input chain exceeds the limit.\n"); }
    else
      if (next_free != cl_count && next_free != input->chain)
        mlog (ML_ERROR,
              "the add group should be in chain %u\n", next_free);
      else
        if (total_clusters + input->clusters < total_clusters)
        { mlog (ML_ERROR, "add group's clusters overflow.\n"); }
        else
          if (input->clusters > cl_cpg)
          { mlog (ML_ERROR, "the cluster exceeds the maximum of a group\n"); }
          else
            if (input->frees > input->clusters)
            { mlog (ML_ERROR, "the free cluster exceeds the total clusters\n"); }
            else
              if (total_clusters % cl_cpg != 0)
                mlog (ML_ERROR,
                      "the last group isn't full. Use group extend first.\n");
              else
                if (input->group != ocfs2_which_cluster_group (inode, cluster) )
                { mlog (ML_ERROR, "group blkno is invalid\n"); }
                else
                  if ( (ret = ocfs2_check_new_group (inode, di, input, group_bh) ) )
