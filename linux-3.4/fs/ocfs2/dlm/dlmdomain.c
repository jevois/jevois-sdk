...
static int dlm_wait_on_domain_helper (const char * domain)
{
  ...
  { ret = 1; }
    ...
    if (tmp->dlm_state == DLM_CTXT_JOINED)
    { ret = 1; }
    ...
    if (dlm->dlm_state == DLM_CTXT_NEW &&
      ...
      if (dlm->joining_node != DLM_LOCK_RES_OWNER_UNKNOWN) {
        ...
        if (dlm->reco.state & DLM_RECO_STATE_ACTIVE) {
          ...
          if (test_bit (bit, dlm->recovery_map) ) {
            mlog (0, "node %u trying to join, but it "
                  "still needs recovery.\n", bit);
            packet.code = JOIN_DISALLOW;
          }
          else
            if (test_bit (bit, dlm->domain_map) ) {
              mlog (0, "node %u trying to join, but it "
              ...
              if (dlm_query_join_proto_check ("DLM", bit,
                ...
                if (dlm_query_join_proto_check ("fs", bit,
                             ...
                             enum dlm_query_join_response_code * response)
{
  ...
  if (status == -ENOPROTOOPT) {
    ...
    *response = JOIN_OK_NO_MAP;
    ...
    if (packet.code == JOIN_DISALLOW ||
        packet.code == JOIN_OK_NO_MAP) {
      *response = packet.code;
    }
    else
      if (packet.code == JOIN_PROTOCOL_MISMATCH) {
        mlog (ML_NOTICE,
