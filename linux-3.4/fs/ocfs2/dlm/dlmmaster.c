...
way_up_top:
  ...
  if (res) {
    ...
    if (tmpmle->type == DLM_MLE_BLOCK) {
      ...
      if (tmpmle->type == DLM_MLE_MIGRATION) {
        ...
        if (tmpmle->master != DLM_LOCK_RES_OWNER_UNKNOWN) {
          set_maybe = 0;
          if (tmpmle->master == dlm->node_num) {
            response = DLM_MASTER_RESP_YES;
  ...
  while ( (nodenum = dlm_node_iter_next (iter) ) >= 0) {
    ...
    if (ret < 0) {
      ...
      if (status < 0) {
        ...
        if (status == DLM_MIGRATE_RESPONSE_MASTERY_REF) {
