                                ...
                                struct ocfs2_blockcheck_stats * stats)
{
  ...
  if (memcmp (di->i_signature, OCFS2_SUPER_BLOCK_SIGNATURE,
    ...
    if ( (1 << le32_to_cpu (di->id2.i_super.s_blocksize_bits) ) != blksz) {
      ...
      if (le16_to_cpu (di->id2.i_super.s_major_rev_level) !=
        ...
        if (bh->b_blocknr != le64_to_cpu (di->i_blkno) ) {
          ...
          if (le32_to_cpu (di->id2.i_super.s_clustersize_bits) < 12 ||
              le32_to_cpu (di->id2.i_super.s_clustersize_bits) > 20) {
            mlog (ML_ERROR, "bad cluster size found: %u\n",
                  1 << le32_to_cpu (di->id2.i_super.s_clustersize_bits) );
          }
          else
            if (!le64_to_cpu (di->id2.i_super.s_root_blkno) ) {
              mlog (ML_ERROR, "bad root_blkno: 0\n");
            }
            else
              if (!le64_to_cpu (di->id2.i_super.s_system_dir_blkno) ) {
                mlog (ML_ERROR, "bad system_dir_blkno: 0\n");
              }
              else
                if (le16_to_cpu (di->id2.i_super.s_max_slots) > OCFS2_MAX_SLOTS) {
                  mlog (ML_ERROR,
