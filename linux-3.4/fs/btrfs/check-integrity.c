      ...
      if (!l->block_ref_to->is_iodone) {
        ...
        if (l->parent_generation !=
          ...
          if (l->block_ref_to->flush_gen >
            ...
            if (-1 == btrfsic_check_all_ref_blocks (state,
                                                    l->block_ref_to,
                                                    recursion_level +
                                                    1) ) {
              ret = -1;
  ...
  int recursion_level)
{
  ...
  list_for_each (elem_ref_from, &block->ref_from_list) {
    ...
    { return 1; }
      ...
      if (btrfsic_is_block_ref_by_superblock (state,
                                              l->block_ref_from,
                                              recursion_level +
                                              1) )
      { return 1; }
...
int btrfsic_submit_bh (int rw, struct buffer_head * bh)
{
  ...
  if (NULL != dev_state &&
    ...
    if (NULL != dev_state && (rw & REQ_FLUSH) ) {
      if (dev_state->state->print_mask &
          BTRFSIC_PRINT_MASK_SUBMIT_BIO_BH)
        printk (KERN_INFO
