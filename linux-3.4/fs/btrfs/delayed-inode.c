  ...
  struct btrfs_delayed_node * node)
{
  ...
  if (!node->in_list) { /* not in the list */
    ...
    if (list_is_last (&node->n_list, &delayed_root->node_list) )
    { goto out; }
    else
    { p = node->n_list.next; }
  ...
  if (prev) {
    ...
    { *prev = NULL; }
      ...
      { *prev = delayed_item; }
        ...
        if ( (node = rb_prev (prev_node) ) != NULL) {
  ...
  if (next) {
    ...
    { *next = NULL; }
      ...
      { *next = delayed_item; }
        ...
        if ( (node = rb_next (prev_node) ) != NULL) {
                                     ...
                                     int action)
{
  ...
  { root = &delayed_node->ins_root; }
    ...
    if (action == BTRFS_DELAYED_DELETION_ITEM)
  ...
  struct btrfs_delayed_node * node)
{
  ...
  if (!src_rsv || (!trans->bytes_reserved &&
    ...
    if (src_rsv == &root->fs_info->delalloc_block_rsv) {
