                                ...
                                struct p9_wstat * stat, dev_t * rdev)
{
  ...
  *rdev = 0;
  ...
  { res |= S_IFDIR; }
    ...
    if ( (mode & P9_DMSYMLINK) && (v9fs_proto_dotu (v9ses) ) )
    { res |= S_IFLNK; }
    else
      if ( (mode & P9_DMSOCKET) && (v9fs_proto_dotu (v9ses) )
           && (v9ses->nodev == 0) )
      { res |= S_IFSOCK; }
      else
        if ( (mode & P9_DMNAMEDPIPE) && (v9fs_proto_dotu (v9ses) )
             && (v9ses->nodev == 0) )
        { res |= S_IFIFO; }
        else
          if ( (mode & P9_DMDEVICE) && (v9fs_proto_dotu (v9ses) )
               && (v9ses->nodev == 0) ) {
            char type = 0, ext[32];
            int major = -1, minor = -1;
