  ...
  xfs_extlen_t * lenp)    /* in/out: aligned length */
{
  ...
  if (rt && (temp = (align_alen % mp->m_sb.sb_rextsize) ) ) {
     ...
     */
      ...
      if (align_off + align_alen - temp >= orig_end)
      { align_alen -= temp; }
      /*
    ...
    if (!ap->eof) {
      ...
      if (!isnullstartblock (ap->got.br_startblock) ) {
        ...
        { gotbno -= adjust; }
          ...
          if (ISVALID (gotbno - ap->length, gotbno) ) {
            gotbno -= ap->length;
  ...
  xfs_bmalloca_t * ap)    /* bmap alloc argument struct */
{
  ...
  if (unlikely (align) ) {
    ...
    if (mp->m_sb.sb_blocksize >= PAGE_CACHE_SIZE) {
      args.prod = 1;
      args.mod = 0;
    }
    else {
...
error0:
  ...
  { bma.logflags &= ~xfs_ilog_fext (whichfork); }
    ...
    if ( (bma.logflags & xfs_ilog_fbroot (whichfork) ) &&
         XFS_IFORK_FORMAT (ip, whichfork) != XFS_DINODE_FMT_BTREE)
...
error0:
  ...
  { logflags &= ~xfs_ilog_fext (whichfork); }
    ...
    if ( (logflags & xfs_ilog_fbroot (whichfork) ) &&
         XFS_IFORK_FORMAT (ip, whichfork) != XFS_DINODE_FMT_BTREE)
