...
CIFSSMBNegotiate (unsigned int xid, struct cifs_ses * ses)
{
  ...
  { pSMB->hdr.Flags2 |= SMBFLG2_EXT_SEC; }
    ...
    if ( (secFlags & CIFSSEC_AUTH_MASK) == CIFSSEC_MAY_KRB5) {
      cFYI (1, "Kerberos only mechanism, enable extended security");
      pSMB->hdr.Flags2 |= SMBFLG2_EXT_SEC;
    }
    else
      if ( (secFlags & CIFSSEC_MUST_NTLMSSP) == CIFSSEC_MUST_NTLMSSP)
      { pSMB->hdr.Flags2 |= SMBFLG2_EXT_SEC; }
      else
        if ( (secFlags & CIFSSEC_AUTH_MASK) == CIFSSEC_MAY_NTLMSSP) {
  ...
  if ( (pSMBr->hdr.WordCount < 13) || (server->dialect == BAD_PROT) ) {
    ...
    if ( (pSMBr->hdr.WordCount == 13)
      ...
      if (rsp->EncryptionKeyLength ==
        ...
        if (server->sec_mode & SECMODE_PW_ENCRYPT) {
...
signing_check:
  ...
  if ( (secFlags & CIFSSEC_MAY_SIGN) == 0) {
    ...
    if ( (secFlags & CIFSSEC_MUST_SIGN) == CIFSSEC_MUST_SIGN) {
      /* signing required */
  ...
  *nbytes = 0;
  ...
  if (*buf) {
    ...
    { cifs_small_buf_release (iov[0].iov_base); }
      ...
      if (resp_buf_type == CIFS_LARGE_BUFFER)
      { cifs_buf_release (iov[0].iov_base); }
  }
  else
    if (resp_buf_type != CIFS_NO_BUFFER) {
...
cifs_writev_complete (struct work_struct * work)
{
  ...
  if (wdata->result == 0) {
    ...
    if (wdata->sync_mode == WB_SYNC_ALL && wdata->result == -EAGAIN)
                    ...
                    __u32 * pparmlen, __u32 * pdatalen)
{
  ...
  *ppdata = (char *) &pSMBr->hdr.Protocol + data_offset;
  ...
  if (*ppparm > end_of_smb) {
    ...
    if (parm_count + *ppparm > end_of_smb) {
      cFYI (1, "parm end after end of smb");
      return -EINVAL;
    }
    else
      if (*ppdata > end_of_smb) {
        cFYI (1, "data starts after end of smb");
        return -EINVAL;
      }
      else
        if (data_count + *ppdata > end_of_smb) {
  ...
  if (rc) {
    ...
    { rc = -EIO; }
      ...
      { rc = -EIO; }  /* bad smb */
        ...
        if (legacy && get_bcc (&pSMBr->hdr) < 24)
