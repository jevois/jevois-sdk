...
static int power_cut_emulated (struct ubifs_info * c, int lnum, int write)
{
  ...
  if (lnum == UBIFS_SB_LNUM) {
    ...
    if (lnum == UBIFS_MST_LNUM || lnum == UBIFS_MST_LNUM + 1) {
      if (chance (19, 20) )
      { return 0; }
      ubifs_warn ("failing in master LEB %d", lnum);
    }
    else
      if (lnum >= UBIFS_LOG_LNUM && lnum <= c->log_last) {
        ...
        if (lnum >= c->lpt_first && lnum <= c->lpt_last) {
          ...
          if (lnum >= c->orph_first && lnum <= c->orph_last) {
            if (write && chance (1, 2) )
            { return 0; }
            if (chance (9, 10) )
            { return 0; }
            ...
            if (lnum == c->ihead_lnum) {
              if (chance (99, 100) )
              { return 0; }
              ubifs_warn ("failing in index head LEB %d", lnum);
            }
            else
              if (c->jheads && lnum == c->jheads[GCHD].wbuf.lnum) {
                if (chance (9, 10) )
                { return 0; }
                ubifs_warn ("failing in GC head LEB %d", lnum);
              }
              else
                if (write && !RB_EMPTY_ROOT (&c->buds) &&
  ...
  { val = d->chk_gen; }
    ...
    { val = d->chk_index; }
      ...
      { val = d->chk_orph; }
        ...
        if (dent == d->dfs_chk_lprops)
        { val = d->chk_lprops; }
        else
          if (dent == d->dfs_chk_fs)
          { val = d->chk_fs; }
          else
            if (dent == d->dfs_tst_rcvry)
            { val = d->tst_rcvry; }
            else
            { return -EINVAL; }
                                     ...
                                     size_t count, loff_t * ppos)
{
  ...
  { val = ubifs_dbg.chk_gen; }
    ...
    if (dent == dfs_chk_index)
    { val = ubifs_dbg.chk_index; }
    else
      if (dent == dfs_chk_orph)
      { val = ubifs_dbg.chk_orph; }
      else
        if (dent == dfs_chk_lprops)
        { val = ubifs_dbg.chk_lprops; }
        else
          if (dent == dfs_chk_fs)
          { val = ubifs_dbg.chk_fs; }
          else
            if (dent == dfs_tst_rcvry)
