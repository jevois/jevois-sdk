...
__u32 _get_data_from_r_cache (__u32 blk, __u32 nblk, void * buf)
{
  ...
  if ( (blk / SECTOR_CNT_OF_LOGIC_PAGE) != ( (blk + nblk - 1) / SECTOR_CNT_OF_LOGIC_PAGE) )
  {
  ...
  if ( (blk / SECTOR_CNT_OF_LOGIC_PAGE) != nand_r_cache.hit_page)
  {
    return -1;
  }
  ...
  for (sec = blk; sec < blk + nblk; sec++)
  {
    ...
    {
      MEMCPY ( (__u8 *) buf + (sec - blk) * 512, nand_r_cache.data + SecWithinPage * 512, 512);
    }
...
__s32 _get_all_data_from_cache (__u32 blk, __u32 nblk, void * buf)
{
  ...
  for (sec = blk; sec < blk + nblk; sec++)
  {
    ...
    if (w_cache_hit_page_index >= 0)
    {
      if ( (nand_w_cache[w_cache_hit_page_index].hit_page == page) && (nand_w_cache[w_cache_hit_page_index].secbitmap & SecBitmap) )
      {
        MEMCPY ( (__u8 *) buf + (sec - blk) * 512, nand_w_cache[w_cache_hit_page_index].data + SecWithinPage * 512, 512);
        continue;
      }
    }
    else
...
__s32 NAND_CacheReadSecs (__u32 blk, __u32 nblk, void * buf)
{
  ...
  if (_get_all_data_from_cache (blk, nblk, buf) == 0)
  {
    return 0;
  }
      ...
      if (SecBitmap == FULL_BITMAP_OF_LOGIC_PAGE)
      {
        ...
        for (i = 0; i < N_NAND_W_CACHE; i++)
        {
          if (nand_w_cache[i].hit_page == page)
          {
            ...
            if ( ( (nand_w_cache[i].hit_page / PAGE_CNT_OF_LOGIC_BLK) == (page / PAGE_CNT_OF_LOGIC_BLK) ) && (page > 0) )
            {
              _flush_w_cache_simple (i);
            }
