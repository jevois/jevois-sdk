  ...
  if ( NdfcVersion == NDFC_VERSION_V1 ) {
    ...
    if ( NdfcVersion == NDFC_VERSION_V2 ) {
      ...
      if (NdfcDmaMode == 1) {
        ...
        ndfc_dma_desc_cpu[0].buff = nand_dma_addr[NandIndex][0];
...
void _dma_config_start_v2 (__u8 rw, __u32 buf_cnt, __u32 buf_addr[], __u32 buf_size[])
{
  ...
  if ( NdfcVersion == NDFC_VERSION_V2)
  {
    ...
    for (i = 0; i < buf_cnt; i++)
    {
      ...
      ndfc_dma_desc_cpu[i].buff = nand_dma_addr[NandIndex][i];
...
__s32 _read_secs_in_page_mode (NFC_CMD_LIST * rcmd, void * mainbuf, void * cachebuf, void * sparebuf, __u32 secbitmap)
{
  ...
  __u32 random_read_cmd0, random_read_cmd1;
  ...
  __u8  addr[2] = {0, 0};
  __u32 col_addr;
  __u32 eccblk_zone_list[4][3] = {{0}, {0}, {0}, {0}};
...
{
  __s32 i, ret;
  __u32 * tmpsparebuf, *tmpbuf1, *tmpbuf3;
  __u32 fir_sect;
  __u32 sect_cnt;
  __u32 last_sect;
  __u32 ind, last_ind;
  ...
  */
  if (last_sect <= last_spare_sect)
  {
    ...
    if ( (last_sect % 0x2) == 0)
    {
      ...
      dst_buf_cnt++;
      dst_buf_size[dst_buf_cnt - 1] = 1024 * (spare_ecc_blk_cnt - pos);
    ...
    else
    {
      ...
      {
        dst_buf_cnt++;
        dst_buf_size[dst_buf_cnt - 1] = 1024 * (spare_ecc_blk_cnt - pos - 1);
        dst_buf_addr[dst_buf_cnt - 1] = (__u32) tmpbuf3;
      }
    }
    ...
    if (fir_sect > last_spare_sect)
    {
      mask = spare_ecc_blk_mask;
      dst_buf_cnt = 1;
      dst_buf_size[dst_buf_cnt - 1] = spare_ecc_blk_cnt * 1024;
...
__s32 NFC_GetFeature (NFC_CMD_LIST * get_feature_cmd, __u8 * feature)
{
  ...
  cfg |= NDFC_WAIT_FLAG | NDFC_DATA_TRANS | NDFC_SEND_CMD1;
...
__s32 NFC_ChangMode (NFC_INIT_INFO * nand_info)
{
  ...
  { cfg |= ( 0x0 << 8 ); }
    ...
    if (nand_info->pagesize == 4 )      /*  2K  */
    { cfg |= ( 0x1 << 8 ); }
    else
      if (nand_info->pagesize == 8 )      /*  4K  */
      { cfg |= ( 0x2 << 8 ); }
      else
        if (nand_info->pagesize == 16 )      /*  8K  */
        { cfg |= ( 0x3 << 8 ); }
        else
          if (nand_info->pagesize == 32 )      /*  16K  */
          { cfg |= ( 0x4 << 8 ); }
          else                                      /* default 4K */
          { cfg |= ( 0x2 << 8 ); }
