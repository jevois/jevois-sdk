...
__s32 NFC_Write_CFG( NFC_CMD_LIST  *wcmd, void *mainbuf, void *sparebuf,  __u8 dma_wait_mode, __u8 rb_wait_mode, __u8 page_mode, struct boot_ndfc_cfg *ndfc_cfg)
{
    ...
    /*page_size_temp = (NDFC_READ_REG_CTL() & 0xf00)>>8;
	if(ndfc_cfg->page_size_kb  ==  1) 	    page_size_mode = 0x0;  
	else if (ndfc_cfg->page_size_kb  ==  2) page_size_mode = 0x1;  
	else if (ndfc_cfg->page_size_kb  ==  4) page_size_mode = 0x2;  
	else if (ndfc_cfg->page_size_kb  ==  8) page_size_mode = 0x3;  
	else if (ndfc_cfg->page_size_kb  == 16) page_size_mode = 0x4;  
	else if (ndfc_cfg->page_size_kb  == 32) page_size_mode = 0x5;  
	else
		return -1;
...
__s32 _read_in_page_mode_1st_1K_normal_ecc(NFC_CMD_LIST  *rcmd,void *mainbuf,void *sparebuf,__u8 dma_wait_mode)
{
	...
	_dma_config_start(0, (__u32)mainbuf, data_size);
	...
	if (NdfcVersion == NDFC_VERSION_V1) {
		/*set NFC_REG_SECTOR_NUM*/
		NDFC_WRITE_REG_SECTOR_NUM(data_size/1024);
	} else if (NdfcVersion == NDFC_VERSION_V2) {
		/*set NFC_REG_BLOCK_MASK*/
    ...
    NAND_WaitDmaFinish();
    ret = _wait_dma_end(0, (__u32)mainbuf, data_size);
	if (ret)
		return ret;
...
__s32 _read_in_page_mode_spare(NFC_CMD_LIST  *rcmd,void *mainbuf,void *sparebuf,__u8 dma_wait_mode)
{
	...
	if (NdfcVersion == NDFC_VERSION_V1) {
		/*set NFC_REG_SECTOR_NUM*/
		NDFC_WRITE_REG_SECTOR_NUM(4096/1024);
	} else if (NdfcVersion == NDFC_VERSION_V2) {
		/*set NFC_REG_BLOCK_MASK*/
