...
__s32 _read_single_page_1K (struct boot_physical_param * readop, __u8 dma_wait_mode)
{
  ...
  if (readop->oobbuf) {
    MEMCPY (readop->oobbuf, sparebuf, 1 * 4);
  }
...
__s32  PHY_PageRead (struct __PhysicOpPara_t * pPageAdr)
{
  ...
  if ( (ret[0][0] == -ERR_TIMEOUT) || (ret[0][1] == -ERR_TIMEOUT) || (ret[1][0] == -ERR_TIMEOUT) || (ret[1][1] == -ERR_TIMEOUT) )
  {
    ...
    if ( (ret[0][0] == -ERR_ECC) || (ret[0][1] == -ERR_ECC) || (ret[1][0] == -ERR_ECC) || (ret[1][1] == -ERR_ECC) )
    {
      ...
      if ( (ret[0][0] == ECC_LIMIT) || (ret[0][1] == ECC_LIMIT) || (ret[1][0] == ECC_LIMIT) || (ret[1][1] == ECC_LIMIT) )
      {
...
void _cal_addr_in_chip_for_spare (__u32 block, __u32 page, __u32 sector, __u8 * addr, __u8 cycle)
{
  ...
  { ecc_size = 32; }
    ...
    if (NandStorageInfo.EccMode == 1)
    { ecc_size = 53; }
    else
      if (NandStorageInfo.EccMode == 2)
      { ecc_size = 60; }
      else
      { ecc_size = 32; }
  #endif
