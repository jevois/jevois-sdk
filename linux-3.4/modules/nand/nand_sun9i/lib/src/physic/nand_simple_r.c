...
__s32 _check_scan_data (__u32 first_check, __u32 chip, __u32 * scan_good_blk_no)
{
  ...
  if (first_check)
  {
    for (b = start_blk; b < start_blk + blk_cnt; b++)
    {
      ...
      ret = PHY_SimpleRead_1st1K (&readop);
      if (ret == 0) {
        if ( (oob_buf[0] == 0xff) && (oob_buf[1] == 0xff) && (oob_buf[2] == 0xff) && (oob_buf[3] == 0xff) ) {
          ...
          if ( (oob_buf[0] == 0x0) && (oob_buf[1] == 0x0) && (oob_buf[2] == 0x0) && (oob_buf[3] == 0x0) ) {
            ret = -1;
                              ...
                              NFC_DDR_INFO * pddr_info, __u32 dclk)
{
  ...
  /*check parameter*/
  if  ( ( (ddr_type == ONFI_DDR) || (ddr_type == ONFI_DDR2) ) && (pre_ddr_type == SDR) )
  { ddr_change_mode = 1; }
  else
    if ( (ddr_type == SDR) && ( (pre_ddr_type == ONFI_DDR) || (pre_ddr_type == ONFI_DDR2) ) )
    { ddr_change_mode = 2; }
    else
      if ( ( (ddr_type == TOG_DDR) || (ddr_type == TOG_DDR2) ) && (pre_ddr_type == SDR) )
      { ddr_change_mode = 3; }
      else
        if ( (ddr_type == SDR) && ( (pre_ddr_type == TOG_DDR) || (pre_ddr_type == TOG_DDR2) ) )
        { ddr_change_mode = 4; }
        else
          if ( ( (ddr_type == TOG_DDR) && (pre_ddr_type == TOG_DDR2) )
               || ( (ddr_type == TOG_DDR2) && (pre_ddr_type == TOG_DDR) ) )
          { ddr_change_mode = 5; }
          else
            if (ddr_type == pre_ddr_type)
            { ddr_change_mode = 6; }
            else {
  ...
  for (bank = 0; bank < NandStorageInfo.ChipCnt; bank++)
  {
    ...
    if (ddr_change_mode == 1) {
      ...
      if (ddr_change_mode == 2) {
      
        /* ONFI DDR/DDR2 => Async */
        if (SUPPORT_DDR2_SPECIFIC_CFG) {
          if (pre_ddr_type == ONFI_DDR2) {
            if ( _setup_nand_onfi_ddr2_para (ddr_info) ) {
              PHY_ERR ("_setup_nand_onfi_ddr2_para() failed! %d\n", ddr_change_mode);
              return -1;
            }
    ...
    for (edo = 0; edo < edo_cnt; edo++)
    {
             ...
             && (NandStorageInfo.NandChipId[2] == tmpChipID[2] ) && (NandStorageInfo.NandChipId[3] == tmpChipID[3]) )
        {
          ...
          if (err_flag == 0)
          {
            ...
            if (store_index == 0)
            {
              ...
              {
                edo_delay[index][0] = edo;
                edo_delay[index][1] = delay;
                edo_delay[index][2] = delay;
              }
              else
              {
                edo_delay[index][2] = delay;
              ...
              if (store_index == 1)
              {
                ...
                {
                  tmp_edo_delay[0] = edo;
                  tmp_edo_delay[1] = delay;
                  tmp_edo_delay[2] = delay;
                }
                else
                {
                  tmp_edo_delay[2] = delay;
      ...
      if (good_flag)
      {
        ...
        if (index == 0) {
          ...
          if (good_cnt >= GOOD_DDR_EDO_DELAY_CHAIN_TH)
          { break; }
          ...
          if ( (READ_RETRY_MODE >= 0x10) && (READ_RETRY_MODE < 0x20) )
          {
            ...
            if ( (READ_RETRY_MODE >= 0x30) && (READ_RETRY_MODE < 0x40) )
            {
              ...
              if ( (READ_RETRY_MODE >= 0x20) && (READ_RETRY_MODE < 0x30) )
              {
                ...
                if ( (READ_RETRY_MODE >= 0x40) && (READ_RETRY_MODE < 0x50) )
                {
                  NFC_SetDefaultParam (readop->chip, default_value, READ_RETRY_TYPE);
                }
                else
                  if ( (READ_RETRY_MODE >= 0x50) && (READ_RETRY_MODE < 0x60) )
                  {
                    NFC_SetDefaultParam (readop->chip, default_value, READ_RETRY_TYPE);
                  }
    ...
    if (timing_change) {
      ...
      if (is_blank_page) {
        ...
        if ( (nand_info.ddr_type == ONFI_DDR2) || (nand_info.ddr_type == TOG_DDR2) ) {
          ...
          reg_val = NDFC_READ_REG_INT_DEBUG();
          reg_val |= (0x1U << 8);
...
__s32 PHY_SimpleRead_2CH (struct boot_physical_param * readop)
{
  ...
  if (readop->oobbuf)
  { MEMCPY (readop->oobbuf, &sparebuf[0][0], 16); }
