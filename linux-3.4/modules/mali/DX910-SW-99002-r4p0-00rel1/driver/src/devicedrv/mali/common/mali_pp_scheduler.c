...
void mali_pp_scheduler_job_done (struct mali_group * group, struct mali_pp_job * job, u32 sub_job, mali_bool success, mali_bool in_upper_half)
{
  ...
  if (mali_group_is_virtual (group) ) {
    ...
    if (0 < job_queue.depth && (!mali_scheduler_mask_is_set (schedule_mask, MALI_SCHEDULER_MASK_PP) || _mali_osk_list_empty (&group_list_idle) ) ) {
      ...
      if ( (0 < virtual_group_job_queue.depth) && (!mali_scheduler_mask_is_set (schedule_mask, MALI_SCHEDULER_MASK_PP) ) ) {
